#!/usr/bin/env bash
# Build + roll the preloaded airis-kata runner image onto the self-hosted CI host,
...
#
# Usage:
#   CI_HOST=my-ci-host ./infra/reload-runner.sh                # tag: airis-kata-runner:YYYY-MM-DD-HHMMSS
#   CI_HOST=my-ci-host ./infra/reload-runner.sh 2026-06-20     # tag: airis-kata-runner:2026-06-20
...
#   REMOTE_CTX               remote build dir for the Dockerfile           [/root/airis-kata-runner-image]
#   ARC_VALUES               remote ARC scale-set helm values file         [/root/arc-airis-values.yaml]
#   ARC_RELEASE              helm release name of the runner scale set     [airis-kata]
...
REMOTE_CTX="${REMOTE_CTX:-/root/airis-kata-runner-image}"
ARC_VALUES="${ARC_VALUES:-/root/arc-airis-values.yaml}"
...
build_with_containerd() {
  ...
  k3s ctr -n k8s.io images tag "$IMAGE" airis-kata-runner:preloaded >/dev/null 2>&1 || true
...
sed -i "s#image: airis-kata-runner:.*#image: $IMAGE#" "$ARC_VALUES"
