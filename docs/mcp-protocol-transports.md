...
- timeout from `resolveMCPTimeoutMs`: `AIRIS_MCP_TIMEOUT_MS` env override, else `config.timeout ?? 30000`; `0` disables
...
- timeout uses `AbortController` via `createMCPTimeout` (`AIRIS_MCP_TIMEOUT_MS` override, else `config.timeout ?? 30000`; `0` disables)
...
2. **Background SSE listener** (`startSSEListener()`)
   ...
   - listener startup waits up to one second, or less for very small request timeouts; `timeout: 0` / `AIRIS_MCP_TIMEOUT_MS=0` disables that startup deadline
