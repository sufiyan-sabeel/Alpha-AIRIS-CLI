# airis-native tool-call format

The **airis-native** format is the tool-call serialization used by the airis / pi coding agent. Unlike the JSON-in-a-tag conventions (Hermes/Qwen, Harmony) and unlike the fully separate JSON content-block channel (Anthropic Messages API), airis-native serializes each call as an **XML-flavored block** whose tag carries the tool name — `<call:NAME>…</call:NAME>` — and whose arguments are child elements named after the parameters. It is **schema-driven**: the tool's JSON Schema decides how each value is typed (string vs number vs object vs array) and which compact spellings are legal.
...
# TODO
- ship airis-native parser
</call:write>
```

→ `write({ "path": "notes/todo.md", "content": "# TODO\n- ship airis-native parser" })` (here `path` is given by attribute, so the body fills the next string parameter, `content`).
...
airis-native does not prescribe how tools are advertised; a host typically lists them as JSON Schema, exactly as the OpenAI / Anthropic / Hermes families do. What airis-native **requires** is that the parser have access to each tool's parameter schema, because the schema is what disambiguates:
...
airis-native calls carry **no per-call wire id** (like GLM and Qwen, unlike Anthropic's `toolu_…`). Results are therefore correlated to calls **positionally, by emission order**: the host delivers tool outputs in the same order the `<call:…>` blocks appeared, using whatever its envelope provides for tool output (a `tool`/`user` turn, a Harmony tool message, an Anthropic `tool_result` block, …). When a transport requires an id (e.g. an OpenAI-compatible bridge), the host synthesizes one and maintains the call↔result mapping itself; the id never appears in the airis-native text.
...
- **Lenient, not strict XML.** Do not feed airis-native to an XML parser: tag names contain a colon (`call:read`), attribute values may be unquoted, bodies are not entity-encoded, and the stream need not be balanced beyond each call's own open/close. Match the tags as literals (regex / streaming state machine).
...
airis-native is specified here; it is not derived from a published model template. Its two direct influences are documented in this folder:
