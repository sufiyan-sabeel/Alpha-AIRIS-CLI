...
Providers are the model backends `airis` can route requests to: Anthropic, OpenAI, Google Gemini, Groq, OpenRouter, Mistral, xAI, local engines like Ollama, hosted gateways, custom `models.yml` providers, and providers registered by extensions.
...
## How `airis` decides a provider is available
...
2. Custom provider and model entries from `~/.airis/agent/models.yml`.
...
When a provider needs an API key, `airis` resolves it in this order (first match wins):
...
Stored credentials live in the auth store at `~/.airis/agent/agent.db` for local auth, or in the configured auth-broker snapshot when running in broker mode. (`PI_CODING_AGENT_DIR` relocates the `~/.airis/agent` base, and the auth store moves with it.)
...
For headless or remote setups backed by a shared auth broker, the CLI exposes `airis auth-broker login <provider>` / `airis auth-broker logout` (and `status`, `list`, `import`, `migrate`). See [Secrets and credentials](./secrets.md) for the broker model.

When a model has no credentials, `airis` tells you to run `/login` or set the provider's environment variable.
...
`airis` eagerly loads `.env` files into the process environment before any provider lookup. It reads four files and, for each variable, the **first** source that defines it wins. Effective precedence, high to low:

1. The process environment inherited by `airis` (already-set variables always win).
2. `<cwd>/.env`
3. `~/.airis/agent/.env`
4. `~/.airis/.env`
5. `~/.env`

A variable already present in the process environment is never overwritten by a `.env` file. Among the files, a value set in `<cwd>/.env` wins over `~/.airis/agent/.env`, which wins over `~/.airis/.env`, which wins over `~/.env`. So a shell-exported `OPENAI_API_KEY` beats every `.env` file, and a project's `<cwd>/.env` beats your home `~/.env`.
...
- an `AIRIS_`-prefixed key is also mirrored to the matching `PI_`-prefixed name.
...
```yaml
# ~/.airis/agent/config.yml or <project>/.airis/config.yml
disabledProviders:
  - anthropic
  - openai
  - google
...
Project settings live in `<project>/.airis/config.yml`. Use them when one repository must allow or hide a different provider set than your global default:

```yaml
# <project>/.airis/config.yml
disabledProviders:
  - openai
  - openrouter
```
...
Custom providers live in `~/.airis/agent/models.yml` under `providers:`. A provider ID defined there participates in the same selection, credential resolution, and `disabledProviders` rules as built-in providers.
...
**A provider still appears even though I disabled it.** `disabledProviders` arrays are replaced, not merged: a project `<project>/.airis/config.yml` array fully overrides the global one. Verify the *effective* list for the directory you are in (path-scoped entries only apply at or under their configured path), and confirm the ID is spelled exactly. Use `airis config get disabledProviders` to inspect the merged value (see [Settings](./settings.md)).
