...
description: Use when creating a new airis extension. Covers ExtensionAPI, factory signature, tool/command/event registration, and local-dev testing.
---

# Authoring Extensions

Extensions are the primary way to add capabilities to `alpha-airis-cli`. A single extension module can register tools the LLM can call, slash commands users can invoke, and event handlers that run throughout the session lifecycle — all from one TypeScript file.
...
That is a working extension. Drop it into `~/.airis/agent/extensions/hello.ts` and restart airis to see the notification.
...
airis loads extension modules from these sources:

1. Native `.airis` locations discovered through the capability system:
   - `<cwd>/.airis/extensions/`
   - `~/.airis/agent/extensions/`
   - legacy extension paths listed in `.airis/settings.json#extensions` or `~/.airis/agent/settings.json#extensions`
2. Installed plugins under `~/.airis/plugins/node_modules` (`airis plugin install` npm/git specs, or `airis plugin link`) via their `airis.extensions`/`pi.extensions` manifests. Marketplace cache installs do not feed extension modules — they surface skills/commands/hooks/tools/MCP only.
3. Explicit configured paths passed by the CLI (`airis --extension ./my-ext.ts`, also `-e`; `--hook` is treated as an alias) and by the `extensions:` setting in config.

The runtime de-duplicates by resolved absolute path — first seen wins.

When a path points to a directory, airis resolves the entry point in this order:

1. `package.json` with `airis.extensions` (or legacy `pi.extensions`) field
2. `index.ts`
3. `index.js`

When scanning an `extensions/` directory, airis also loads direct `*.ts`/`*.js` files and one-level subdirectories that have `index.ts`, `index.js`, or a manifest.

Extension packages can also bundle sibling capability directories. When a package is loaded through `extensions:` or `--extension`/`-e`, the `airis-plugins` provider discovers its `skills/`, `hooks/pre|post/`, `tools/`, `commands/`, `rules/`, `prompts/`, and `.mcp.json`.

## package.json manifest

To package an extension as an installable plugin, add an `airis` field to `package.json`:

```json
{
  "name": "my-airis-extension",
  "airis": {
...
| Legacy hook module already exists | **Hook** (`HookAPI` from `@airis/airis-coding-agent/extensibility/hooks`) |
...
airis writes structured logs to a rotating file under `~/.airis/logs/` (debug level is always on; nothing is written to the console, which would corrupt the TUI). Tail today's log to see extension load diagnostics:

```
tail -f ~/.airis/logs/airis.$(date +%F).log
```
