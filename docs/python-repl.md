...
Each Python kernel is a single subprocess: `<resolved-python> -u <runner.py>`. The runner is bundled with the host binary (Bun text import), written to an `airis-python-runner` cache under the OS temp directory once per script hash, and reused by subsequent spawns.
...
Status events the prelude emits (e.g. `_emit_status("find", count=…)`) ship inside display bundles under `application/x-airis-status` so the existing TUI status renderer keeps working.
...
2. Managed venv at `~/.airis/python-env`
...
- `application/x-airis-status` MIME inside `display` → structured status events
...
- `application/x-airis-status` → status events
...
- **No Python on PATH** — Install a system Python 3.8+ or place a venv at `~/.airis/python-env`. `airis setup python --check` reports the resolved interpreter.
