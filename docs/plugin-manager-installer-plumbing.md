...
This document describes how `airis plugin` npm/git/link and marketplace operations mutate plugin state on disk and become runtime capabilities. Marketplace installs keep their own registries and cache, then register the cached plugin through the same `node_modules` and `airis-plugins.lock.json` runtime surfaces used by npm/git/link installs; see `docs/marketplace.md`.
...
```text
airis plugin <npm/link action> ...
  -> src/commands/plugin.ts
  -> runPluginCommand(...) in src/cli/plugin-cli.ts
  -> PluginManager method (install/list/uninstall/link/...)
  -> mutate ~/.airis/plugins/{package.json,node_modules,airis-plugins.lock.json}
...
airis plugin install name@marketplace / airis install name@marketplace
  -> MarketplaceManager
  -> mutate marketplace registries and cache
  -> symlink the cached package into the scope's node_modules and update airis-plugins.lock.json
...
Global plugin state lives under `~/.airis/plugins`:
...
- `airis-plugins.lock.json` — runtime state for npm/link/marketplace plugins:
...
- `<cwd>/.airis/plugin-overrides.json`
...
- `~/.airis/marketplaces.json` — configured marketplace catalogs
- `~/.airis/plugins/installed_plugins.json` — user-scoped marketplace installs
- `<cwd>/.airis/plugins/installed_plugins.json` — project-scoped marketplace installs when available
- `~/.airis/plugins/cache/{marketplaces,plugins}/` — cached catalogs and plugin directories
- `<scope>/plugins/node_modules/<package>` — symlink to the cached plugin, allowing its `package.json` `airis.extensions` and tools to load
- `<scope>/plugins/airis-plugins.lock.json` — enablement and feature state shared with the runtime plugin loader
...
- A package missing `airis`/`pi` is still installable and listable.
- Runtime plugin loading (`getEnabledPlugins`) skips packages without `airis`/`pi` manifest.
...
3. Ensure plugin `package.json` exists (`airis-plugins`, private dependencies map).
4. Run `bun install <packageSpec>` in `~/.airis/plugins`.
...
- `airis plugin install pkg@newVersion` updates dependency and lockfile version.
...
1. Read plugin dependency map from `~/.airis/plugins/package.json`.
2. Load lockfile runtime config (missing file -> empty defaults).
3. Load project overrides (`<cwd>/.airis/plugin-overrides.json`, parse/read errors -> empty object with warning).
...
`link` supports local plugin development by symlinking a local package into `~/.airis/plugins/node_modules/<pkg.name>`.
...
- skip if manifest (`airis`/`pi`) absent
