...
```text
Config sources (.airis/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.airis/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.airis/mcp.json` (project) or `~/.airis/agent/mcp.json` (user) for explicit control.
...
OMP-native MCP config (`.airis/mcp.json`, `~/.airis/agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`.
