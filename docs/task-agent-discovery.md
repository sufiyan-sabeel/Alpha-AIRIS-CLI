...
`discoverAgents(cwd, home)` (`src/task/discovery.ts`) merges agents from OMP-native roots and Claude plugin roots before appending bundled definitions. Cross-harness roots such as `.claude/agents`, `.codex/agents`, and `.gemini/agents` are intentionally skipped — their frontmatter schema is not the OMP task-agent contract (`TASK_AGENT_CONFIG_SOURCE = ".airis"` filters both dir lists).

### Discovery inputs

1. Nearest project `.airis` agents dir from `findAllNearestProjectConfigDirs("agents", cwd)` (filtered to `.airis`; first hit only)
2. User `.airis` agents dir from `getConfigDirs("agents", { project: false })` (filtered to `.airis`; first hit only)
...
1. project `.airis/agents`
2. user `~/.airis/agent/agents`
...
- Project `.airis` overrides user `.airis`.
