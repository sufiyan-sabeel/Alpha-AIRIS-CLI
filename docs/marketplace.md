...
A **marketplace** is a Git repository (or local directory) containing a catalog file at `.airis-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). The catalog lists available plugins with their sources, descriptions, and metadata.

A **plugin** is a directory containing Claude/OMP plugin content such as skills, commands, agents, hooks, tools, MCP servers, or LSP servers. Marketplace installs also load extension modules declared by `package.json` `airis.extensions`: installation symlinks the cached plugin into the scope's `node_modules` tree and records it in `airis-plugins.lock.json`, the same runtime surfaces used by npm-installed and `airis plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in `~/.airis/plugins/installed_plugins.json`
- **project** -- available only in the active project, stored in the nearest project `.airis/plugins/installed_plugins.json`
...
```
airis plugin marketplace add <source>
airis plugin marketplace remove <name>
airis plugin marketplace update [name]
airis plugin marketplace list
airis plugin discover [marketplace]
airis plugin install [--force] [--scope user|project] name@marketplace
airis plugin uninstall [--scope user|project] name@marketplace
airis plugin upgrade [--scope user|project] [name@marketplace]
airis plugin enable [--scope user|project] name@marketplace
airis plugin disable [--scope user|project] name@marketplace
```
...
Git and local sources must contain a catalog at `.airis-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). Direct catalog URLs cache only the JSON catalog; plugins in URL-sourced catalogs cannot use relative string sources like `"./plugins/foo"`.

## Catalog format (marketplace.json)

A marketplace catalog lives at `.airis-plugin/marketplace.json` in the repository root. When airis is the only intended consumer, prefer this path. To remain Claude Code-compatible (airis loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — airis uses it as a fallback when `.airis-plugin/marketplace.json` is absent. A repository may ship both: airis reads the `.airis-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
...
~/.airis/
  ...
  plugins/
    ...
    airis-plugins.lock.json         # Runtime enable/feature state
...
<project>/.airis/
  plugins/
    ...
    airis-plugins.lock.json         # Project runtime enable/feature state
    node_modules/<package>        # Symlink to the cached plugin
```
