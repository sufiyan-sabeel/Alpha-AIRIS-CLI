...
- `description` is required for:
  - native `.airis` provider skill discovery (`requireDescription: true`)
  - `airis-plugins` extension-package skills and the `github` provider (`.github/skills/`), which also pass `requireDescription: true`
...
3. **Managed (auto-learn) skills** (`airis-managed` provider) resolved dead-last with first-wins, so any same-named authored skill from any provider or custom directory takes precedence
...
1. `native` (priority 100) — `.airis` user/project skills via `src/discovery/builtin.ts`
2. `airis-plugins` (priority 90) — `skills/` bundled next to extension packages loaded through `extensions:`, `--extension`/`-e`, or installed plugins under `~/.airis/plugins/node_modules`
...
7. `airis-managed` (priority 5) — auto-learn skills under `~/.airis/agent/managed-skills`, registered in `src/discovery/builtin.ts` and discovered unconditionally (only writing/nudging is gated by `autolearn.enabled`); always defers to a same-named authored skill
