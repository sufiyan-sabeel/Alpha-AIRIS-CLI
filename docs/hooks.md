...
- JS/TS hook factories discovered through `hookCapability` (for example `.airis/hooks/pre/*.ts`) are loaded as extension modules so their `pi.on(...)` handlers bind to the runtime event bus
...
`src/extensibility/hooks/index.ts` and the package subpath `@airis/airis-coding-agent/extensibility/hooks` export:
...
The package root (`@airis/airis-coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
