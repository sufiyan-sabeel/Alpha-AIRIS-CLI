...
Unchanged from the former `launch` tool: the first process op starts a detached broker over a private socket under `~/.airis/run/daemons/<project-hash>/`; every airis instance in the project shares names, logs, and state. After the last airis process exits, the broker stops non-persistent processes and exits. `persist: true` opts out of last-client teardown; restart policies (`no`/`on-failure`/`always`) use bounded exponential backoff up to 30 s.
