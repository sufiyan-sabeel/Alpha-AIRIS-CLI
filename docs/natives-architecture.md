...
`@airis/airis-natives` is a two-layer package around an ESM loader:
...
There is no current `packages/natives/src` TypeScript wrapper layer. Consumers import functions/classes/enums directly from `@airis/airis-natives`; the type contract is the generated `native/index.d.ts` plus the explicit named exports generated into `native/index.js` by `scripts/gen-enums.ts`.
...
- Default: `airis_natives.<platform>-<arch>.node`
- x64 variant: `airis_natives.<platform>-<arch>-modern.node` or `...-baseline.node`
...
2. Check legacy compiled-binary location:
   - Windows: `%LOCALAPPDATA%/airis` (fallback `%USERPROFILE%/AppData/Local/airis`)
   - non-Windows: `~/.local/bin`
3. Fall back to packaged `native/` and executable directory candidates.

`getNativesDir()` uses `$XDG_DATA_HOME/airis/natives` when `$XDG_DATA_HOME/airis` exists; otherwise it uses `~/.airis/natives`.
...
`crates/airis-natives/src/lib.rs` declares exported module ownership:
...
- `iso`
- `keys`
- `language` (re-exported from `airis_ast`)
- `power`
- `prof`
- `ps`
- `pty`
...
- **Rust ownership (`crates/airis-natives/src`)**
...
- **Platform leaf package**: Per-platform npm package `@airis/airis-natives-<tag>` that carries one platform's prebuilt `.node`. The core depends on every leaf via `optionalDependencies`; the package manager installs only the host-matching one (`os`/`cpu`).
...
- **Version sentinel**: Rust export named from the package version (for example `__airisNativesV16_0_3`) that lets the loader reject a `.node` from a different release.
