...
- [`packages/coding-agent/src/discovery/airis-plugins.ts`](../packages/coding-agent/src/discovery/airis-plugins.ts)
...
- `native` (priority `100`)
- `airis-plugins` (priority `90`) — `rules/*.{md,mdc}` inside configured extension package roots, normalized via the shared `buildRuleFromMarkdown` path
- `agents` (priority `70`)
- `cursor` (priority `50`)
...
Loads `.airis` rules from:

- project: `<cwd>/.airis/rules/*.{md,mdc}` when the cwd `.airis` directory exists
- user: `~/.airis/agent/rules/*.{md,mdc}`
- sticky user rule: `~/.airis/agent/RULES.md`
- sticky project rule: nearest ancestor `.airis/RULES.md` while walking from cwd toward the repository root
...
- `native` appends project `.airis/rules`, user `~/.airis/agent/rules`, user `RULES.md`, then nearest project `RULES.md`.
- `airis-plugins` appends `rules/` results per configured extension package root.
...
1. The rule providers currently loaded for `rules` are `native`, `airis-plugins`, `agents`, `cursor`, `windsurf`, `cline`, and embedded `builtin-defaults`; provider files for other tools may parse other config formats but do not register rule loaders.
