...
def _terminate_process_group(process: subprocess.Popen[Any], pgid: int | None) -> None:
    ...
    airis is spawned with `start_new_session=True`, so it leads a session/group
...
class RpcClient:
    ...
    def start(self) -> RpcClient:
        ...
        self._stdout_thread = threading.Thread(
            target=self._read_stdout_loop, name="airis-rpc-stdout", daemon=True
        )
        self._stderr_thread = threading.Thread(
            target=self._read_stderr_loop, name="airis-rpc-stderr", daemon=True
        )
    ...
    def _normalize_host_tool_event(self, payload: JsonObject) -> None:
        ...
        With `tools.xdev` enabled, airis mounts custom tools as `xd://` devices
    ...
    def _handle_host_tool_call(self, payload: JsonObject) -> None:
        ...
        threading.Thread(
            target=run_tool, name=f"airis-rpc-host-tool:{tool_name}", daemon=True
        ).start()
    ...
    def _handle_host_uri_request(self, payload: JsonObject) -> None:
        ...
        threading.Thread(
            target=run, name=f"airis-rpc-host-uri:{scheme}:{operation}", daemon=True
        ).start()
