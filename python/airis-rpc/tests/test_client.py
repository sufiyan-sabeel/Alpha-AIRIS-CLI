...
from airis_rpc import RpcClient, RpcCommandError, RpcConcurrencyError, RpcError, host_tool
from airis_rpc.client import _RpcFrameDecoder
...
class RpcClientTests(unittest.TestCase):
    ...
    def test_xd_dispatched_custom_tool_events_carry_host_tool_name(self) -> None:
        ...
        With `tools.xdev` on, airis invokes a custom tool through `write
        xd://<name>` and the wire events carry the transport tool (`write`).
        Consumers must observe the host-tool name on update/end events
        regardless of transport — roboairis's terminal-action detection
        triple-posted PR reviews when end events only said `write`
        (alpha-airis-cli#6696). `tool_execution_start` precedes the `host_tool_call`
...
class TerminatesProcessGroupTests(unittest.TestCase):
    ...
    the airis process. Before the fix, stop() signalled only the leader pid, so
    such grandchildren reparented to the container init and kept running —
    once ballooning to tens of GB of RAM. airis is now spawned in its own
    ...
    def test_stop_kills_grandchild_spawned_by_server(self) -> None:
        ...
        # Fake airis server: spawn the long-lived grandchild, signal ready, then
