...
def _start_and_capture(**kwargs):
    ...
    with patch(
        "airis_rpc.client.subprocess.Popen", side_effect=_Sentinel("aborted")
    ) as mock_popen:
...
def test_no_user_group_defaults_to_none():
    call = _start_and_capture(executable="airis")
