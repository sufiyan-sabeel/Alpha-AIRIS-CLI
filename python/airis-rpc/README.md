...
Typed Python bindings for the `airis --mode rpc` protocol used by the coding agent.
...
- typed startup options for common `airis --mode rpc` flags such as thinking level,
...
```python
from airis_rpc import RpcClient
...
```python
from airis_rpc import MessageUpdateEvent, RpcClient
...
from airis_rpc import RpcClient, host_tool
...
```python
from airis_rpc import RpcClient, host_uri
...
```python
from airis_rpc import assistant_text, assistant_text_with_thinking
