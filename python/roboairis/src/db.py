...
from roboairis.github_client import IssueIndexEntry
...
class Database:
    ...
    def list_running_events(self) -> list[dict[str, Any]]:
        ...
        - `model`: the airis model the worker picked for this run, set after
    ...
    def has_authorized_impl_event(self, issue_key: str) -> bool:
        ...
        for row in rows:
            payload = json.loads(row["payload_json"])
            directive = payload.get("_roboairis_directive")
    ...
    def processed_issue_keys(self, keys: Iterable[str]) -> set[str]:
        ...
        Membership in `issues` means roboairis has at minimum upserted state for the
