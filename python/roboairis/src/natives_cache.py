...
The napi-rs build of ``airis_natives.<platform>-<arch>[-variant].node`` takes
...
Ownership: cache root is provisioned ``root:airis 02770`` by ``entrypoint.sh``
so slot subprocesses (group ``airis``) can capture under setgid inheritance.
...
# napi build reads: all workspace crates (airis-natives transitively depends on
# airis-ast/airis-iso/airis-shell), the workspace Cargo manifest + lock, the rust
...
_CACHED_NODE_GLOB = "airis_natives.*.node"
...
def _git_safe_directory_env(repo_dir: Path) -> dict[str, str]:
    ...
    ``roboairis.sandbox._safe_directory_env`` but kept local to avoid a circular
...
class NativesCache:
    """Per-repo content-addressed cache of airis-natives build outputs."""
        ...
        *,
        ...
        with _flock(self.lockfile(repo)):
            ...
            try:
                ...
                # the setgid `airis` model that lets other slots read it.
                # A copy creates a fresh inode owned by the orchestrator
                # (root) and inherits gid `airis` from the setgid 2770
                # cache root.
