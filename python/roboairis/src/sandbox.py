...
PAT injection via `--config-env` (see `roboairis.git_ops`); the `ProxyGitTransport`
in `roboairis.proxy_client` forwards the same set of operations over HMAC RPC.
...
1. **Workspace tree** (`/data/workspaces/<key>/`, including `repo/`,
   `.airis-session/`, `context/`, `artifacts/`, `.airis-tmp/`, `.airis-xdg`):
   single-owner. Owned by the active slot UID/GID (`airis-N`) when slot
...
2. **Clone pool** (`/data/workspaces/_pool/<owner>__<repo>/`): genuinely
   multi-slot. Owned by `root:airis` (gid 2000) with setgid `02770`; cross-slot
...
3. **Language tool caches** (`/data/cache/{cargo,cargo-target,rustup,bun-cache}`):
   multi-slot. Owned by `root:airis` with setgid `02770`; provisioned by
   `entrypoint.sh`.
...
from roboairis.natives_cache import CacheHit, NativesCache
...
class GitTransport(Protocol):
    ...
    - `roboairis.proxy_client.ProxyGitTransport`: forwards over HMAC RPC.
    """
...
def _prepare_slot_tmpdir(workspace: Workspace, slot_uid: int | None) -> Path:
    ...
    - replaces any non-directory at ``.airis-tmp`` (symlink-protection: a user
    ...
    tmpdir = workspace.root / ".airis-tmp"
    try:
        st = tmpdir.lstat()
...
def _slot_subprocess_kwargs(slot_uid: int | None) -> dict[str, Any]:
    ...
    if not _slot_permissions_active(slot_uid):
        return {}
    assert slot_uid is not None
    return {"user": slot_uid, "group": slot_uid, "extra_groups": [_SHARED_AIRIS_GID], "umask": 0o002}
...
def _prepare_slot_runtime_env(workspace: Workspace, slot_uid: int | None) -> dict[str, str]:
    ...
    and are group-shared via ``airis``. Bun's install cache is explicitly
    ...
    xdg_root = workspace.root / ".airis-xdg"
    ...
    for base in (xdg_data, xdg_state, xdg_cache):
        ...
        (base / "airis").mkdir(parents=True, exist_ok=True)
...
def _provision_runtime_dirs(ws_root: Path) -> None:
    ...
    picks up ``.airis-tmp`` and the per-workspace XDG tree. Without this,
    ...
    Symlink-safe on ``.airis-tmp`` (replaces a planted non-directory in place).
    """
    tmpdir = ws_root / ".airis-tmp"
    try:
        st = tmpdir.lstat()
    except FileNotFoundError:
        pass
...
def _share_git_metadata_with_slots(repo_dir: Path, slot_uid: int | None) -> None:
    ...
    `airis-N` user, so the pool-side worktree gitdir, refs, reflogs, and object
    directories must stay writable through the shared `airis` group.
    """
    if not _slot_permissions_active(slot_uid):
        return
...
class SandboxManager:
    ...
    def ensure_workspace(
        ...
        *,
        ...
        with self._repo_lock(repo):
            ...
            session_dir = ws_root / ".airis-session"
            ...
            # keeps its `root:airis` ownership (the slot reads through group `airis`);
    ...
    def _populate_natives_cache(self, workspace: Workspace, *, slot_uid: int | None = None) -> None:
        """Try to hardlink cached airis-natives artifacts into the worktree.
        ...
        `.node` files are LEFT at `root:airis` ownership — chowning them
        would chown the cache file too (shared inode), breaking the
        cross-slot sharing model. The slot reads them via group `airis`.
        """
    ...
    def _chown_natives_for_slot(native_dir: Path, hit: CacheHit, *, slot_uid: int) -> None:
        ...
        hardlinks back into the root:airis cache and the slot reads them via
        group `airis`. Everything else (the directory itself, copied
