"""`python -m roboairis.proxy serve` — run the gh-proxy FastAPI app."""
...
from roboairis.config import Settings, load_proxy_settings
...
def _settings_or_die() -> Settings:
    ...
    `ROBAIRIS_GH_PROXY_HMAC_KEY` — the orchestrator's webhook secret,
...
def serve() -> None:
    ...
    if cfg.gh_proxy_hmac_key is None:
        click.echo("gh-proxy: ROBAIRIS_GH_PROXY_HMAC_KEY is required in proxy mode", err=True)
        sys.exit(2)
