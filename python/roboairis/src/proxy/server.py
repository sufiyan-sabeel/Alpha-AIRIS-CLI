...
Roboairis calls every endpoint with HMAC headers (see `roboairis.proxy_hmac`).
Authenticated requests dispatch to a single `GitHubClient` instance holding
the PAT, or to `roboairis.git_ops` for git transport. The PAT never leaves
this process.

Endpoint payloads are deliberately typed (no generic GitHub passthrough):
each one names exactly one operation roboairis performs.
"""
...
from roboairis.github_client import GitHubClient, GitHubError
from roboairis.proxy_hmac import HEADER_SIGNATURE, HEADER_TIMESTAMP, verify
from roboairis.sandbox import _safe_directory_env, _slot_subprocess_kwargs
from roboairis.sandbox import workspace_key as compute_workspace_key
...
def _resolve_hmac_key(cfg: Settings) -> bytes:
    if cfg.gh_proxy_hmac_key is None:
        raise HTTPException(500, "gh-proxy: ROBAIRIS_GH_PROXY_HMAC_KEY not configured")
...
def create_proxy_app(settings: Settings) -> FastAPI:
    ...
    app = FastAPI(title="roboairis-gh-proxy", version="0.1.0", lifespan=lifespan)
    ...
    # ---- git transport ----
    #
    # The underlying `roboairis.git_ops` primitives are blocking `subprocess.run`
