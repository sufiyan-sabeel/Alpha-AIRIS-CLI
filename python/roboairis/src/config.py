"""Env-driven configuration for roboairis."""
...
class Settings(BaseSettings):
    ...
    bot_login: str = Field(..., alias="ROBAIRIS_BOT_LOGIN")
    git_author_name: str | None = Field(None, alias="ROBAIRIS_GIT_AUTHOR_NAME")
    git_author_email: str = Field(..., alias="ROBAIRIS_GIT_AUTHOR_EMAIL")
    repo_allowlist_raw: str = Field("", alias="ROBAIRIS_REPO_ALLOWLIST")
    pr_review_enabled: bool = Field(True, alias="ROBAIRIS_PR_REVIEW_ENABLED")
    ...
    gh_proxy_url: str | None = Field(None, alias="ROBAIRIS_GH_PROXY_URL")
    gh_proxy_hmac_key: SecretStr | None = Field(None, alias="ROBAIRIS_GH_PROXY_HMAC_KEY")
    # Bind address for `python -m roboairis.proxy serve`. Internal-only by
    # default; gh-proxy never exposes a host port.
    gh_proxy_bind_host: str = Field("0.0.0.0", alias="ROBAIRIS_GH_PROXY_BIND_HOST")
    gh_proxy_bind_port: int = Field(8081, alias="ROBAIRIS_GH_PROXY_BIND_PORT")
    ...
    gh_proxy_max_body_bytes: int = Field(1 << 20, alias="ROBAIRIS_GH_PROXY_MAX_BODY_BYTES")
    ...
    gh_proxy_git_timeout_seconds: float = Field(60.0, alias="ROBAIRIS_GH_PROXY_GIT_TIMEOUT_SECONDS")

    # Model selection
    model: str = Field("anthropic/claude-sonnet-4-6", alias="ROBAIRIS_MODEL")
    provider: str | None = Field(None, alias="ROBAIRIS_PROVIDER")
    thinking_level: ThinkingLevel = Field("high", alias="ROBAIRIS_THINKING")

    # Runtime
    max_concurrency: int = Field(8, alias="ROBAIRIS_MAX_CONCURRENCY")
    task_timeout_seconds: float = Field(2400.0, alias="ROBAIRIS_TASK_TIMEOUT_SECONDS")
    task_timeout_hard_grace_seconds: float = Field(60.0, alias="ROBAIRIS_TASK_TIMEOUT_HARD_GRACE_SECONDS")
    request_timeout_seconds: float = Field(120.0, alias="ROBAIRIS_REQUEST_TIMEOUT_SECONDS")
    ...
    event_max_retries: int = Field(3, alias="ROBAIRIS_EVENT_MAX_RETRIES")
    event_retry_delays_raw: str = Field("30,120,600", alias="ROBAIRIS_EVENT_RETRY_DELAYS_SECONDS")
    ...
    # opening a PR — continue" reminder prompts into the same airis session.
    # Set to 0 to disable.
    task_completion_max_reminders: int = Field(2, alias="ROBAIRIS_TASK_COMPLETION_MAX_REMINDERS")
    omp_command: str = Field("airis", alias="ROBAIRIS_AIRIS_COMMAND")
    ...
    # complete cleanly; any still running after that get their airis
    ...
    shutdown_drain_timeout_seconds: float = Field(25.0, alias="ROBAIRIS_SHUTDOWN_DRAIN_TIMEOUT_SECONDS")
    shutdown_kill_timeout_seconds: float = Field(5.0, alias="ROBAIRIS_SHUTDOWN_KILL_TIMEOUT_SECONDS")

    # Paths
    workspace_root: Path = Field(Path("./data/workspaces"), alias="ROBAIRIS_WORKSPACE_ROOT")
    sqlite_path: Path = Field(Path("./data/roboairis.sqlite"), alias="ROBAIRIS_SQLITE_PATH")
    log_dir: Path = Field(Path("./data/logs"), alias="ROBAIRIS_LOG_DIR")

    # Server
    bind_host: str = Field("0.0.0.0", alias="ROBAIRIS_BIND_HOST")
    bind_port: int = Field(8080, alias="ROBAIRIS_BIND_PORT")

    # Dev-only replay header value; if empty, /replay is disabled
    replay_token: SecretStr | None = Field(None, alias="ROBAIRIS_REPLAY_TOKEN")
    ...
    rate_limit_window_seconds: float = Field(3600.0, alias="ROBAIRIS_RATE_LIMIT_WINDOW_SECONDS")
    rate_limit_default: int = Field(3, alias="ROBAIRIS_RATE_LIMIT_DEFAULT")
    rate_limit_contributor: int = Field(10, alias="ROBAIRIS_RATE_LIMIT_CONTRIBUTOR")
    rate_limit_unlimited_raw: str = Field("", alias="ROBAIRIS_RATE_LIMIT_UNLIMITED")
    ...
    maintainer_logins_raw: str = Field("", alias="ROBAIRIS_MAINTAINER_LOGINS")
    ...
    reviewer_bots_raw: str = Field("", alias="ROBAIRIS_REVIEWER_BOTS")
    ...
    question_autoclose_enabled: bool = Field(True, alias="ROBAIRIS_QUESTION_AUTOCLOSE_ENABLED")
    question_autoclose_hours: float = Field(4.0, alias="ROBAIRIS_QUESTION_AUTOCLOSE_HOURS")
    question_autoclose_scan_seconds: float = Field(60.0, alias="ROBAIRIS_QUESTION_AUTOCLOSE_SCAN_SECONDS")
    ...
    issue_index_sync_seconds: float = Field(900.0, alias="ROBAIRIS_ISSUE_INDEX_SYNC_SECONDS")

    # airis-natives build-output cache. Hardlinks pre-built
    ...
    natives_cache_enabled: bool = Field(True, alias="ROBAIRIS_NATIVES_CACHE_ENABLED")
    natives_cache_root: Path = Field(Path("/data/cache/airis-natives"), alias="ROBAIRIS_NATIVES_CACHE_ROOT")
    natives_cache_max_entries_per_repo: int = Field(8, alias="ROBAIRIS_NATIVES_CACHE_MAX_ENTRIES_PER_REPO")
    natives_cache_max_bytes: int = Field(4 * 1024**3, alias="ROBAIRIS_NATIVES_CACHE_MAX_BYTES")
    natives_cache_gc_interval_seconds: float = Field(3600.0, alias="ROBAIRIS_NATIVES_CACHE_GC_INTERVAL_SECONDS")
    ...
    reclaim_workspace_caches: bool = Field(True, alias="ROBAIRIS_RECLAIM_WORKSPACE_CACHES")
    ...
    def _require_bot_login(cls, value: str) -> str:
        ...
        if cleaned.endswith("[bot]"):
            cleaned = cleaned[:-5]
        if not cleaned:
            raise ValueError("ROBAIRIS_BOT_LOGIN must be a non-empty GitHub login")
        return cleaned
    ...
    def _blank_replay_disables(cls, value: object) -> object:
        ...
        # ROBAIRIS_REPLAY_TOKEN becomes SecretStr("") which the server would
        # happily compare against an empty X-Roboairis-Replay-Token header.
    ...
    def _validate_proxy_or_pat(self) -> Settings:
        ...
        if has_token and has_url:
            raise ValueError(
                "GITHUB_TOKEN and ROBAIRIS_GH_PROXY_URL are mutually exclusive — "
                "set ONE to choose between direct-PAT and gh-proxy modes."
            )
        if has_url != has_key:
            raise ValueError(
                "ROBAIRIS_GH_PROXY_URL and ROBAIRIS_GH_PROXY_HMAC_KEY must both be set together (or both empty)."
            )
        if not has_token and not has_url:
            raise ValueError(
                "no GitHub access configured: set GITHUB_TOKEN, or set "
                "ROBAIRIS_GH_PROXY_URL + ROBAIRIS_GH_PROXY_HMAC_KEY to use gh-proxy."
            )
        return self
    ...
    def model_pool(self) -> tuple[str, ...]:
        """ROBAIRIS_MODEL may be a single id or a comma-separated list; this
    ...
    def resolved_author_name(self) -> str:
        """Falls back to bot_login if ROBAIRIS_GIT_AUTHOR_NAME isn't set."""
...
class _ProxyEnvLoader(BaseSettings):
    """Minimal env loader for `python -m roboairis.proxy serve`.
    ...
    gh_proxy_hmac_key: SecretStr = Field(..., alias="ROBAIRIS_GH_PROXY_HMAC_KEY")
    gh_proxy_bind_host: str = Field("0.0.0.0", alias="ROBAIRIS_GH_PROXY_BIND_HOST")
    gh_proxy_bind_port: int = Field(8081, alias="ROBAIRIS_GH_PROXY_BIND_PORT")
    workspace_root: Path = Field(Path("./data/workspaces"), alias="ROBAIRIS_WORKSPACE_ROOT")
    log_dir: Path = Field(Path("./data/logs"), alias="ROBAIRIS_LOG_DIR")
