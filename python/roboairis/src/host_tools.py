"""Host tools exposed to the agent through `airis_rpc.host_tool`.
...
from airis_rpc import HostTool, HostToolContext, RpcCommandError, host_tool

from roboairis import persona
from roboairis.config import Settings
from roboairis.db import Database, IssueState, issue_key
from roboairis.git_ops import GitCommandError, HeadDriftError
from roboairis.github_backend import GitHubBackend
from roboairis.github_client import GitHubError, IssueInfo, PullRequestFileInfo, RepoInfo
from roboairis.issue_index import parse_search_query
...
class AbortController:
    ...
    teardown of the airis subprocess. The worker pre-populates `stop` with a
...
class ToolBindings:
    ...
    # Set by the worker before launching airis. Carries the abort-task signal
...
def ensure_workspace_dependencies(bindings: ToolBindings) -> None:
    ...
    (``@airis/airis-*``) only exist after an install, so without one any
    ...
    orchestrator's read-only ``/work/airis``), so tests exercise the PR's edited
    source.
...
def _repair_commit_message_escapes(bindings: ToolBindings, args: Mapping[str, Any], *, tool_name: str) -> None:
    ...
    update = _run_repo_command(
        bindings,
        ["git", "update-ref", "-m", "roboairis: repaired commit message escapes", "HEAD", new_head, old_head],
    )
...
def _build_abort_task(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        ...
        # races against the imminent airis teardown.
