...
from roboairis.sandbox import Workspace
...
def _load(name: str) -> str:
    return resources.files("roboairis.prompts").joinpath(name).read_text(encoding="utf-8")
...
def resume_triage(*, repo: RepoInfo, issue: IssueInfo, workspace: Workspace) -> str:
    """Resume prompt for a `triage_issue` task whose airis session already exists."""
...
def dirty_state_reminder(
    *,
    ...
    leaves changes behind that roboairis would otherwise discard. The summary
