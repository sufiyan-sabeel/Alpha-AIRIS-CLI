...
```
@roboairis-bot /model gpt /thinking low
...
- `/model <alias>` — pick the first id in `ROBAIRIS_MODEL` whose model id
  contains `<alias>` (case-insensitive). Falls back to the normal random
  pool selection if no member matches.
- `/thinking <level>` — override `ROBAIRIS_THINKING` for this run. Accepts
