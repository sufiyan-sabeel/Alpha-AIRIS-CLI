...
from roboairis.db import INACTIVE_EVENT_STATES, get_database
from roboairis.logging_config import configure_logging
...
from roboairis.proxy_client import GitHubProxyClient
...
def triage(issue_ref: str, wait_timeout: float | None) -> None:
    ...
    if not cfg.allows(repo_full):
        click.echo(f"refusing: {repo_full} not in ROBAIRIS_REPO_ALLOWLIST", err=True)
        sys.exit(2)
