...
from roboairis import github_events, issue_index
from roboairis.autoclose import AutocloseScheduler
from roboairis.config import Settings, get_settings
from roboairis.dashboard import render_index, static_dir, tail_jsonl
from roboairis.db import (
...
from roboairis.github_client import GitHubError, IssueSummary
from roboairis.issue_index import IssueIndexSync
from roboairis.manual_triage import (
    InvalidIssueRef,
...
from roboairis.proxy_client import GitHubProxyClient, ProxyGitTransport
from roboairis.queue import WorkerPool
...
def _require_proxy_mode(cfg: Settings) -> tuple[str, bytes]:
    if cfg.github_token is not None:
        raise SystemExit(
            "roboairis orchestrator refuses to start with GITHUB_TOKEN set in env. "
            "The PAT must live only in the gh-proxy container."
        )
    if cfg.gh_proxy_url is None or cfg.gh_proxy_hmac_key is None:
        raise SystemExit(
            "roboairis orchestrator requires ROBAIRIS_GH_PROXY_URL and "
            "ROBAIRIS_GH_PROXY_HMAC_KEY (run gh-proxy in a sibling container)."
        )
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="roboairis", version="0.1.0", lifespan=lifespan)
    ...
    async def replay(
        request: Request,
        x_roboairis_token: str | None = Header(None, alias="X-Roboairis-Replay-Token"),
        delivery_id: str = "",
    ) -> JSONResponse:
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        ...
        if x_roboairis_token != cfg.replay_token.get_secret_value():
    ...
    def _require_trigger_token(cfg: Settings, token: str | None) -> None:
        if cfg.replay_token is None:
            raise HTTPException(404, "trigger disabled (set ROBAIRIS_REPLAY_TOKEN to enable)")
    ...
    async def api_github_issues(
        ...
        """Browse issues across `ROBAIRIS_REPO_ALLOWLIST` for the trigger picker.
        ...
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        _require_trigger_token(cfg, x_roboairis_token)
    ...
    async def api_trigger(
        ...
        if mode == "triage":
            ...
            if not cfg.allows(repo_full):
                raise HTTPException(403, f"{repo_full} not in ROBAIRIS_REPO_ALLOWLIST")
            try:
    ...
    async def api_cancel(
        ...
        """Stop a running event. The airis subprocess is killed; the row lands in
    ...
    async def api_logs(request: Request, limit: int = 400) -> dict[str, Any]:
        ...
        entries = tail_jsonl(cfg.log_dir / "roboairis.log.jsonl", limit=capped)
