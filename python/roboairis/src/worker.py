...
from roboairis import host_tools, persona, pragmas
from roboairis.cancellation import register_cancel_hook, unregister_cancel_hook
from roboairis.config import Settings
from roboairis.db import Database, issue_key
from roboairis.git_ops import DirtyState, inspect_dirty_state
from roboairis.github_backend import GitHubBackend
from roboairis.github_client import CommentInfo, IssueInfo, PullRequestInfo, RepoInfo
from roboairis.host_tools import AbortController, ToolBindings, _git_identity_env
from roboairis.natives_cache import NativesCache
from roboairis.natives_cache import compute_key as natives_compute_key
from roboairis.sandbox import GitTransport, Workspace, _prepare_slot_runtime_env, _safe_directory_env
...
_SCRUBBED_ENV_KEYS: tuple[str, ...] = (
    ...
    # `bash` tool could otherwise `printenv` them out of roboairis's env.
    "GITHUB_TOKEN",
    "GITHUB_WEBHOOK_SECRET",
    "ROBAIRIS_REPLAY_TOKEN",
...
def _stage_agent_home() -> None:
    ...
    for rel in (Path(".agent"), Path(".airis/agent")):
    ...
    for root, dirs, files in os.walk(_AGENT_HOME):
        root_path = Path(root)
        if root_path == _AGENT_HOME / ".airis":
            # ~/.airis/run is slot-writable daemon presence state, not template
...
def _ensure_agent_run_dir() -> None:
    """Keep ``~/.airis/run`` writable by every sandbox slot.

    airis registers daemon project presence under ``~/.airis/run`` at startup,
    ...
    The tree stays group ``airis``, setgid, group-writable; slot subprocesses
    spawn with umask 0002 so their entries inherit group write.
    """
    if os.geteuid() != 0:
        return
    run_dir = _AGENT_HOME / ".airis" / "run"
    try:
        gid = grp.getgrnam("airis").gr_gid
    except KeyError:
        return
    try:
...
def _build_extra_env(settings: Settings) -> dict[str, str]:
    """Build the env overlay passed to the airis subprocess.

    `airis_rpc` merges this dict on top of `os.environ`, so overlaying empty
...
def _has_prior_session(session_dir: Path) -> bool:
    """Return True iff `session_dir` already contains an airis JSONL transcript.
...
def _run_rpc_blocking(
    ...
    *,
    ...
    def _on_tool_end(event: ToolExecutionEndEvent) -> None:
        ...
        # `tool_name` is transport-normalized by airis_rpc: an xd:// device
    ...
    # can resolve workspace packages (@airis/airis-*) and actually run tests.
    ...
    with RpcClient(
        ...
        extra_groups=["airis"] if inputs.slot_uid is not None else None,
    ) as client:
        # Arm cancellation: from this point the API can kill the airis subprocess
        ...
        # it's idempotent.
        #
        # NOTE: airis_rpc.RpcClient.stop() has a bug where it sets `_stopping=True`
        ...
        try:
            ...
            if hard_timeout_fired.is_set():
                raise TimeoutError("airis task exceeded hard timeout")
            if turn is not None and turn.assistant_message is not None:
                ...
                if stop_reason == "error":
                    ...
                    raise RuntimeError(f"airis agent error (stopReason=error): {error_msg}")
            log.info(
                "rpc_done",
                extra={
                    "issue": bindings.issue_key,
