...
from roboairis.db import Database, IssueRow, IssueState, issue_key
...
from roboairis.sandbox import GitTransport, SandboxManager
from roboairis.worker import DirectiveInfo, TaskInputs, ThreadMessage, run_task
...
def _directive_from_payload(payload: Mapping[str, Any]) -> DirectiveInfo | None:
    ...
    raw = payload.get("_roboairis_directive")
...
async def triage_issue(
    *,
    ...
    if existing is None:
        ...
        # resources (workspace, airis session) to this issue.
        try:
...
async def handle_pr_conversation(
    *,
    ...
    # _roboairis_directive; _directive_from_payload rejects it but the key
    # being present tells us a mention happened. Reply cheaply without airis.
    if directive is None and payload.get("_roboairis_directive") is not None:
        comment = _comment_from_payload(payload)
        log.info(
