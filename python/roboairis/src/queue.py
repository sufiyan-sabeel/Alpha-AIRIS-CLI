...
from roboairis.cancellation import clear_current_event, set_current_event
from roboairis.config import Settings
from roboairis.db import Database, EventRow
from roboairis.github_backend import GitHubBackend
from roboairis.sandbox import GitTransport, SandboxManager, _reap_slot
from roboairis.slot_pool import SlotPool
...
class WorkerPool:
    ...
    async def start(self) -> None:
        ...
        self._workers.append(asyncio.create_task(self._dispatch_loop(), name="roboairis-dispatch"))
        ...
        if self.sandbox.natives_cache is not None and self.settings.natives_cache_gc_interval_seconds > 0:
            self._workers.append(asyncio.create_task(self._natives_cache_gc_loop(), name="roboairis-natives-gc"))
    ...
    async def stop(self, *, drain_timeout: float = 25.0, kill_timeout: float = 5.0) -> None:
        ...
        `reset_stuck_running()`. The resumed airis session then picks up via
        ...
        #    if one was registered (kills the airis subprocess); otherwise
        ...
        for task in still_running:
            ...
            if hook is not None:
                ...
                continue
            # No hook armed yet — the worker hasn't reached the airis spawn
    ...
    async def _dispatch_loop(self) -> None:
        ...
        try:
            while not self._stop.is_set():
                ...
                task = asyncio.create_task(self._run_event(row), name=f"roboairis-event-{row.delivery_id[:8]}")
    ...
    async def cancel_event(self, delivery_id: str) -> bool:
        ...
        # doesn't stall the event loop for up to the airis.shutdown grace period.
        try:
            await asyncio.to_thread(hook)
        except Exception:
    ...
    async def _run_event(self, row: EventRow) -> None:
        ...
        try:
            ...
            if row.delivery_id in self._shutdown_cancelled:
                ...
                # resumed airis session picks up via `--continue`.
