...
// `__ROBAIRIS_CONFIG__` sentinel in `static/index.html` with a JSON blob so the
...
function readConfig(): AppConfig {
  const node = document.getElementById("roboairis-config");
  const text = node?.textContent?.trim();
  if (!text || text === "__ROBAIRIS_CONFIG__") {
...
export const AUTH_HEADERS: Readonly<Record<string, string>> = CONFIG.replayEnabled
  ? Object.freeze({ "X-Roboairis-Replay-Token": CONFIG.replayToken })
  : Object.freeze({});
