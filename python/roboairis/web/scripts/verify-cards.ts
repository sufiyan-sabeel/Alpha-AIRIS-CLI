#!/usr/bin/env bun
/**
 * Roboairis Lifecycle Cards Verification Harness.
...
async function main(): Promise<void> {
  const outDir = path.resolve(import.meta.dir, "../verify-out/roboairis-lifecycle-cards");
  ...
  statusMock = {
    ...
    runtime: {
      ...baseStatus.runtime,
      repo_allowlist: ["octo/widget", "octo/roboairis"],
    },
  };
  ...
  results.push({
    ...
    ok: emptyText.includes("No active work") && emptyText.includes("watching octo/widget, octo/roboairis"),
  ...
  const applyTheme = async (themeName: "dark" | "light") => {
    await page.evaluate((theme) => {
      localStorage.setItem("airis-roboairis-theme", theme);
  ...
  // 5. Read-only pass
  // Fetch the real index.html from server, and replace the roboairis-config script tag
  const resp = await fetch(BASE_URL);
  let html = await resp.text();
  html = html.replace(
    /<script id="roboairis-config" type="application\/json">[^<]*<\/script>/,
    '<script id="roboairis-config" type="application/json">{"replayEnabled":false,"replayToken":""}</script>'
  );
  ...
  // request carries an auditable `X-Roboairis-Replay-Token: trigger-secret`
  ...
  const replayHtml = (await replayResp.text()).replace(
    /<script id="roboairis-config" type="application\/json">[^<]*<\/script>/,
    `<script id="roboairis-config" type="application/json">{"replayEnabled":true,"replayToken":"${REPLAY_TOKEN}"}</script>`,
  );
  ...
  results.push({
    ...
    ok: retryReqs.length === 1 && retryReqs[0].headers["x-roboairis-replay-token"] === REPLAY_TOKEN && statusTextA.includes("queued retry: fail-del-1"),
    detail: `Matching retry requests: ${retryReqs.length}, token: "${retryReqs[0]?.headers["x-roboairis-replay-token"] ?? ""}". Status text: "${statusTextA}"`,
  });
  ...
  results.push({
    ...
    ok: cancelReqs.length === 1 && cancelReqs[0].headers["x-roboairis-replay-token"] === REPLAY_TOKEN && statusTextB.includes("cancel signaled: run-del- (fired=false)"),
    detail: `Matching cancel requests: ${cancelReqs.length}, token: "${cancelReqs[0]?.headers["x-roboairis-replay-token"] ?? ""}". Status text: "${statusTextB}"`,
  });
  ...
  results.push({
    ...
    ok: triageReqs.length === 1 && triageReqs[0].headers["x-roboairis-replay-token"] === REPLAY_TOKEN,
    detail: `Matching triage requests: ${triageReqs.length}, token: "${triageReqs[0]?.headers["x-roboairis-replay-token"] ?? ""}"`,
  });
  ...
  results.push({
    ...
    ok: retryLatestReqs.length === 1 && retryLatestReqs[0].headers["x-roboairis-replay-token"] === REPLAY_TOKEN,
    detail: `Matching retry-latest requests: ${retryLatestReqs.length}, token: "${retryLatestReqs[0]?.headers["x-roboairis-replay-token"] ?? ""}"`,
  });
  ...
  results.push({
    ...
    ok: enterReqs.length === 1 && enterReqs[0].headers["x-roboairis-replay-token"] === REPLAY_TOKEN,
    detail: `Matching enter requests: ${enterReqs.length}, token: "${enterReqs[0]?.headers["x-roboairis-replay-token"] ?? ""}"`,
  });
  ...
  results.push({
    ...
    ok:
      ...
      activityRetryReqs[0].headers["x-roboairis-replay-token"] === REPLAY_TOKEN,
    detail: `Activity status: ${JSON.stringify(activityStatus)}. Retry requests: ${activityRetryReqs.length}, token: "${activityRetryReqs[0]?.headers["x-roboairis-replay-token"] ?? ""}"`,
  });
