...
Wire roboairis to **review pull requests opened by contributors** (and other bots), in two
phases: (1) classify + rank, (2) a real line-by-line review posted as one GitHub review.
roboairis **never merges, closes, approves, or pushes** — the rank label is the verdict; the
       ...
       └─ WorkerPool._dispatch(row)   re-derives handler from (event_type, action)
            └─ tasks.<entry>(settings, db, github, sandbox, git_transport, payload, delivery_id, …)
                 ...
                 └─ worker.run_task(task_kind=..., inputs=TaskInputs, …)
                      ...
                      ├─ RpcClient(airis --mode rpc, cwd=worktree, custom_tools=host_tools.build(bindings))
...
Add `pr_review_enabled: bool = True` (`ROBAIRIS_PR_REVIEW_ENABLED`) so the whole flow can be
...
Do **not** enable the integration smoke (`ROBOAIRIS_INTEGRATION=1`) in the default suite.

---

## 14. Open questions for @sufiyan-sabeel
