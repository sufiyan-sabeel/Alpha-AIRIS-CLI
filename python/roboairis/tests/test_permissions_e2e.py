...
from roboairis.sandbox import LocalGitTransport, SandboxManager, Workspace

pytestmark = pytest.mark.skipif(
    os.environ.get("ROBAIRIS_PERMISSION_E2E") != "1",
    reason="set ROBAIRIS_PERMISSION_E2E=1 to run slot-permission e2e tests",
)

_SLOT_ONE = 2001
_SLOT_TWO = 2002
_SHARED_AIRIS_GID = 2000
_AUTHOR_NAME = "roboairis-bot"
_AUTHOR_EMAIL = "roboairis-bot@example.invalid"
_REPO = "octo/permission-e2e"


def _require_linux_root_toolchain() -> None:
    if platform.system() != "Linux" or os.geteuid() != 0:
        pytest.skip("slot permission e2e tests require Linux root so subprocesses can drop to airis-N UIDs")
...
def slot_tmp_path() -> Iterator[Path]:
    root = Path(tempfile.mkdtemp(prefix="roboairis-permission-e2e-", dir="/tmp"))
    root.chmod(0o755)
    try:
        yield root
    finally:
...
def _share_tree_with_slots(path: Path) -> None:
    for root, dirs, files in os.walk(path):
        root_path = Path(root)
        os.chown(root_path, 0, _SHARED_AIRIS_GID)
        root_path.chmod(0o2770)
        for dirname in dirs:
            child = root_path / dirname
            os.chown(child, 0, _SHARED_AIRIS_GID)
            child.chmod(0o2770)
...
def _prepare_shared_cargo_cache(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    for path in (cargo_home, cargo_target):
        ...
        os.chown(path, 0, _SHARED_AIRIS_GID)
        path.chmod(0o2770)
...
def test_slot_workspace_runs_bun_biome_cargo_and_git_after_root_reentry(
    ...
    stale_bun_cache = first.root / ".airis-xdg" / "cache" / "bun-install" / "root-owned-stale"
    ...
    assert (cargo_target / "debug").stat().st_gid == _SHARED_AIRIS_GID
...
def _prepare_shared_natives_cache(slot_tmp_path: Path) -> NativesCache:
    """Provision `/data/cache/airis-natives` shape (root:airis, setgid 2770)."""
    cache_root = slot_tmp_path / "cache" / "airis-natives"
    cache_root.mkdir(parents=True)
    os.chown(cache_root, 0, _SHARED_AIRIS_GID)
...
def _stage_built_natives(bindings: host_tools.ToolBindings, *, body: str = "ELFx") -> None:
    ...
    """
    _write_as_slot(bindings, "packages/natives/native/airis_natives.linux-arm64.node", body)
...
def test_natives_cache_shares_artifacts_across_slot_workspaces(
    ...
    1. A capture from a slot-owned workspace lands in the shared cache with
       group `airis` setgid inheritance so any other slot can read it.
    ...
    cached_node = stored / "airis_natives.linux-arm64.node"
    cached_companion = stored / "index.d.ts"
    # Cache root is setgid `airis`; new files inherit gid `airis` so any slot
    # with `extra_groups=[airis]` can read them.
    assert cached_node.stat().st_gid == _SHARED_AIRIS_GID
    assert cached_companion.stat().st_gid == _SHARED_AIRIS_GID
    ...
    ws2_node = native_dir2 / "airis_natives.linux-arm64.node"
    ...
    # Slot 2 must be able to read the populated artifacts (group airis + 0660
    # via setgid inheritance from the cache root).
    _run_ok(bindings2, ["test", "-r", "packages/natives/native/airis_natives.linux-arm64.node"])
    ...
    _run_ok(
        ...
        [
            ...
            "packages/natives/native/airis_natives.linux-arm64.node",
        ],
    )
