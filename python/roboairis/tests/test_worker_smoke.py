...
Runs only when ROBOAIRIS_INTEGRATION=1 and `airis` is available on PATH (or via
ROBAIRIS_AIRIS_COMMAND). Spins up:

- a local bare git repo with a trivial failing test,
- a fake GitHub API via httpx.MockTransport that records comments + PRs,
- a real `airis --mode rpc` subprocess driven by `worker.run_task`.
...
INTEGRATION = os.environ.get("ROBOAIRIS_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="ROBOAIRIS_INTEGRATION=1 required to run the airis-backed smoke test",
)
...
def test_triage_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from roboairis.sandbox import LocalGitTransport, SandboxManager
    from roboairis.tasks import triage_issue

    bare = _seed_failing_repo(tmp_path)

    monkeypatch.setenv("ROBAIRIS_GH_PROXY_URL", "http://gh-proxy.invalid:8081")
    monkeypatch.setenv("ROBAIRIS_GH_PROXY_HMAC_KEY", "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    monkeypatch.setenv("GITHUB_TOKEN", "")
    monkeypatch.setenv("GITHUB_WEBHOOK_SECRET", "secret")
    monkeypatch.setenv("ROBAIRIS_BOT_LOGIN", "roboairis-bot")
    monkeypatch.setenv("ROBAIRIS_REPO_ALLOWLIST", "octo/widget")
    monkeypatch.setenv("ROBAIRIS_WORKSPACE_ROOT", str(tmp_path / "workspaces"))
    monkeypatch.setenv("ROBAIRIS_SQLITE_PATH", str(tmp_path / "roboairis.sqlite"))
    monkeypatch.setenv("ROBAIRIS_LOG_DIR", str(tmp_path / "logs"))
    monkeypatch.setenv("ROBAIRIS_TASK_TIMEOUT_SECONDS", "300")
