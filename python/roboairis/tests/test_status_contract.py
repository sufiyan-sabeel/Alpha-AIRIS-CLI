"""Contract test for the roboairis server API response vs status-contract.json.

Regenerate the committed fixture with:
    ROBAIRIS_UPDATE_STATUS_CONTRACT=1 pytest tests/test_status_contract.py
"""
...
from roboairis.db import get_database
...
_UPDATE_ENV = "ROBAIRIS_UPDATE_STATUS_CONTRACT"
...
def test_status_contract(settings: Settings) -> None:
    ...
    with TestClient(app) as client:
        ...
        # pytest runs assert equality; regenerate only when ROBAIRIS_UPDATE_STATUS_CONTRACT=1.
...
def test_cancel_errors_and_gating(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        resp = client.post(
            "/api/cancel",
            json={"delivery_id": "run-cancel-2"},
            headers={"X-Roboairis-Replay-Token": "bad-token"},
        )
