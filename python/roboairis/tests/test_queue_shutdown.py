...
or airis subprocess. The contract under test is `stop()`'s drain-then-kill
...
async def test_non_root_fallback_semaphore_caps_dispatch_concurrency(
    ...
    monkeypatch.setattr("roboairis.queue.os.geteuid", lambda: 501)
...
async def test_stop_cancels_hookless_inflight_task(settings: Settings, db: Database) -> None:
    """A task claimed but stuck pre-hook MUST be cancelled by stop(), not allowed to spawn airis.
