"""Unit tests for `roboairis.natives_cache`.
...
from roboairis.natives_cache import (
    CACHE_KEY_PATHS,
    NativesCache,
    _atomic_link,
    compute_key,
...
def _seed_repo(root: Path, *, with_all_inputs: bool = True) -> Path:
    ...
    if with_all_inputs:
        ...
        crates = root / "crates" / "airis-natives"
        ...
        (natives / "package.json").write_text('{"name":"@airis/airis-natives"}\n')
...
def _populate_built_artifacts(repo_dir: Path, *, body: bytes = b"\x7fELF...native") -> Path:
    ...
    (native_dir / "airis_natives.linux-arm64.node").write_bytes(body)
...
def test_compute_key_changes_when_each_input_changes(tmp_path: Path) -> None:
    ...
    mutations: dict[str, tuple[str, str]] = {
        "crates": ("crates/airis-natives/src.rs", "// new comment\n"),
...
def test_compute_key_handles_missing_inputs(tmp_path: Path) -> None:
    ...
    crates = repo / "crates" / "airis-natives"
    crates.mkdir(parents=True)
...
def test_capture_then_populate_shares_node_inode_but_copies_companions(tmp_path: Path) -> None:
    ...
    assert "airis_natives.linux-arm64.node" in manifest["node_files"]
    ...
    workspace_node = dst_native / "airis_natives.linux-arm64.node"
...
def test_capture_skips_when_artifacts_incomplete(tmp_path: Path) -> None:
    ...
    (native_dir / "airis_natives.linux-arm64.node").write_bytes(b"x")
...
def test_populate_cross_device_falls_back_to_copy(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cached_node = cache.entry_dir(REPO, key) / "airis_natives.linux-arm64.node"
    copied_node = dst_native / "airis_natives.linux-arm64.node"
...
def test_populate_replaces_existing_file_atomically(tmp_path: Path) -> None:
    ...
    target = dst_native / "airis_natives.linux-arm64.node"
...
def _stamp_entry(cache: NativesCache, repo: str, key: str, captured_at: float) -> Path:
    ...
    (entry / "airis_natives.linux-arm64.node").write_bytes(b"x" * 1024)
    (entry / "index.d.ts").write_text("")
    (entry / "index.js").write_text("")
    (entry / "embedded-addon.js").write_text("")
    (entry / "manifest.json").write_text(
        json.dumps({"key": key, "captured_at": captured_at, "node_files": ["airis_natives.linux-arm64.node"]})
    )
    return entry
...
def test_gc_preserves_workspace_hardlinks(tmp_path: Path) -> None:
    ...
    ws_node = tmp_path / "ws" / "airis_natives.linux-arm64.node"
    ws_node.parent.mkdir(parents=True)
    os.link(entry / "airis_natives.linux-arm64.node", ws_node)
    cache.gc(REPO)
...
def test_gc_drops_entry_with_missing_manifest(tmp_path: Path) -> None:
    ...
    (incomplete / "airis_natives.linux-arm64.node").write_bytes(b"x")
    cache.gc(REPO)
