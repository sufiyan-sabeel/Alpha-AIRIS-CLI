...
from roboairis.proxy.server import create_proxy_app
from roboairis.proxy_hmac import HEADER_SIGNATURE, HEADER_TIMESTAMP, sign
from roboairis.sandbox import workspace_key
...
def _build_settings(tmp_path: Path) -> Settings:
    ...
    cfg = Settings.model_construct(
        ...
        sqlite_path=tmp_path / "roboairis.sqlite",
...
def test_read_remote_urls_uses_safe_directory_and_slot_identity(
    ...
    from roboairis.proxy import server as proxy_server

    captured: dict[str, object] = {}
    repo_dir = tmp_path / "repo"
    monkeypatch.setenv("GITHUB_TOKEN", "parent-token")
    monkeypatch.setenv("ROBAIRIS_GIT_HTTP_AUTH", "parent-auth")
    ...
    monkeypatch.setattr("roboairis.proxy.server.subprocess.run", fake_run)
    monkeypatch.setattr(
        "roboairis.proxy.server._slot_subprocess_kwargs",
    ...
    assert "ROBAIRIS_GIT_HTTP_AUTH" not in env
...
async def test_hmac_accept_post_comment_round_trip(proxy_settings: Settings) -> None:
    ...
    assert resp.json() == {"id": 42, "author": "roboairis-bot", "body": "hello", "created_at": "2026-01-01T00:00:00Z"}
...
async def test_authenticated_login(proxy_settings: Settings) -> None:
    def gh(req: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json={"login": "roboairis-bot"})
    ...
    assert resp.status_code == 200
    assert resp.json() == {"login": "roboairis-bot"}
...
async def test_git_clone_github_url_passes_scoped_token(
    ...
    monkeypatch.setattr("roboairis.proxy.server.git_clone", fake_git_clone)
    app = _build_app(proxy_settings)
...
async def test_git_fetch_github_origin_uses_explicit_scoped_remote(
    ...
    monkeypatch.setattr("roboairis.proxy.server.git_fetch_prune", fake_fetch_prune)
    app = _build_app(proxy_settings)
    body = b'{"repo":"octo/widget"}'
    async with await _async_client(app) as client:
        resp = await client.post(
...
async def test_git_push_passes_slot_uid_to_git_push(
    ...
    monkeypatch.setattr("roboairis.proxy.server.git_push", fake_git_push)
    app = _build_app(proxy_settings)
    body = (
        b'{"repo":"octo/widget","workspace_key":"octo__widget__1","branch":"'
        + branch.encode()
