...
from roboairis.tasks import _attach_thread, _directive_from_payload
from roboairis.worker import DirectiveInfo
...
def test_directive_from_payload_missing_pragmas_is_empty_tuple() -> None:
    directive = _directive_from_payload({"_roboairis_directive": {"body": "x", "author": "can1357"}})
    assert directive is not None
...
def test_directive_from_payload_returns_none_for_missing_directive() -> None:
    ...
    assert _directive_from_payload({"_roboairis_directive": "not-a-mapping"}) is None
...
async def test_attach_thread_preserves_authorizes_impl(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboairis.tasks._fetch_thread", fake_fetch_thread)
...
def _payload_with_directive(*, issue_number: int, body: str = "@roboairis-bot ship it") -> dict[str, object]:
    return {
        "repository": {
...
def _workspace(tmp_path):
    ...
    return SimpleNamespace(root=tmp_path, repo_dir=repo_dir, session_dir=session_dir, branch="roboairis/issue-42")
