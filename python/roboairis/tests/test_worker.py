...
These tests swap `roboairis.worker.RpcClient` for a recording fake so we can
observe the `extra_args` and `set_todos` decisions the driver takes based on
whether the workspace's airis session directory already holds a JSONL transcript.
"""
...
from roboairis import worker
from roboairis.config import Settings
from roboairis.git_ops import DirtyState
...
def _patch_worker(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr("roboairis.worker.RpcClient", _FakeRpcClient)
    monkeypatch.setattr("roboairis.worker._AGENT_HOME_STAGE", tmp_path / "missing-agent-home-stage")
    monkeypatch.setattr("roboairis.worker.host_tools.build", lambda _b: ())
    monkeypatch.setattr(
        "roboairis.worker.persona.system_append",
        lambda *, repo, issue, workspace, bot_login: "SYS",
    )
    monkeypatch.setattr(
        "roboairis.worker.persona.seed_phases",
...
async def test_run_rpc_omits_continue_when_session_empty(
    ...
    assert client_kwargs["env"]["ROBAIRIS_REPLAY_TOKEN"] == ""
    assert client_kwargs["env"]["ROBAIRIS_GH_PROXY_HMAC_KEY"] == ""
...
def test_build_extra_env_stages_agent_home(tmp_path: Path, settings: Settings, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    omp_agent_dir = stage_home / ".airis" / "agent"
    ...
    assert (agent_home / ".airis" / "agent" / "models.yml").is_file()
    ...
    assert (agent_home / ".airis" / "agent").stat().st_mode & 0o777 == 0o755
    assert (agent_home / ".airis" / "agent" / "models.yml").stat().st_mode & 0o777 == 0o644
...
async def test_run_rpc_uses_workspace_xdg_dirs_without_slot(tmp_path: Path, settings: Settings) -> None:
    ...
    xdg_root = inputs.workspace.root / ".airis-xdg"
    for key in ("XDG_DATA_HOME", "XDG_STATE_HOME", "XDG_CACHE_HOME"):
        path = Path(env[key])
        assert path.is_relative_to(xdg_root)
        assert (path / "airis").is_dir()
    tmpdir = inputs.workspace.root / ".airis-tmp"
...
async def test_run_rpc_uses_workspace_xdg_dirs_for_slot_without_chown(
    ...
    monkeypatch.setattr("roboairis.sandbox.os.chown", lambda path, uid, gid: chown_calls.append((Path(path), uid, gid)))
...
@pytest.mark.asyncio
async def test_run_rpc_passes_slot_uid_user_slot_group_and_airis_extra_group(tmp_path: Path, settings: Settings) -> None:
    inputs, bindings = _make_inputs(tmp_path, settings, session_has_jsonl=False, slot_uid=2001)
    loop = asyncio.new_event_loop()
    try:
        worker._run_rpc_blocking(
    ...
    assert client_kwargs["extra_groups"] == ["airis"]
...
async def test_run_rpc_arms_hard_timeout_timer(
    ...
    monkeypatch.setattr("roboairis.worker.threading.Timer", FakeTimer)
...
async def test_run_rpc_hard_timeout_stops_client_and_fails(
    ...
    monkeypatch.setattr("roboairis.worker.threading.Timer", FiringTimer)
    inputs, bindings = _make_inputs(tmp_path, settings, session_has_jsonl=False)
    loop = asyncio.new_event_loop()
    try:
    ...
    # `_closed_error` unset (airis_rpc bug), so the worker would hang otherwise.
    assert len(fake.mark_closed_calls) == 1
    from airis_rpc import RpcProcessExitError
...
async def test_run_rpc_cancel_hook_stops_and_marks_closed(
    ...
    an upstream airis_rpc bug where `stop()` does not set `_closed_error`, leaving
    `_wait_for_agent_end` blocked until timeout."""
    captured: list = []
    monkeypatch.setattr("roboairis.worker.register_cancel_hook", lambda hook: captured.append(hook))
    monkeypatch.setattr("roboairis.worker.unregister_cancel_hook", lambda: None)
...
async def test_run_rpc_sends_reminder_when_pr_class_quits_early(tmp_path: Path, settings: Settings) -> None:
    ...
    try:
        ...
        loop.close()
    fake = _FakeRpcClient.instances[0]
    # kickoff + 2 reminders (default ROBAIRIS_TASK_COMPLETION_MAX_REMINDERS=2)
...
async def test_run_rpc_review_pr_still_reminds_when_submit_fails(tmp_path: Path, settings: Settings) -> None:
    ...
    airis_rpc normalizes xd:// device dispatches to the host-tool name, so a
...
def _make_capture_inputs(
    ...
    *,
    ...
    if with_native_artifacts:
        ...
        (native_dir / "airis_natives.linux-arm64.node").write_bytes(b"ELFx")
