...
real airis subprocess; that's covered by the integration smoke test.
"""
...
from roboairis.cancellation import (
    clear_current_event,
...
async def test_dispatch_marks_cancelled_event_failed_with_marker(
    ...
    async def fake_dispatch(self: WorkerPool, r: EventRow, *, slot_uid: int | None = None) -> None:
        # Simulate cancellation hitting mid-task and the airis subprocess raising.
...
async def test_start_reaps_configured_slot_uids(
    ...
    calls: list[int] = []
    monkeypatch.setattr("roboairis.queue._reap_slot", lambda uid: calls.append(uid))
    pool = WorkerPool(
        settings=settings,
        db=db,
        github=_StubGitHub(),  # type: ignore[arg-type]
...
async def test_run_event_reaps_slot_before_release(
    ...
    monkeypatch.setattr("roboairis.queue._reap_slot", lambda uid: order.append(("reap", uid)))
