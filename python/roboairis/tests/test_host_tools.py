...
import httpx
import pytest
from airis_rpc import HostToolContext, RpcCommandError

from roboairis import host_tools
from roboairis.db import Database
from roboairis.github_client import GitHubClient, IssueIndexEntry, IssueInfo, RepoInfo
from roboairis.host_tools import AbortController, ToolBindings, build
from roboairis.sandbox import LocalGitTransport, Workspace
...
def test_repo_command_env_scrubs_secrets_and_uses_workspace_cache(
    ...
    monkeypatch.setenv("ROBAIRIS_GH_PROXY_HMAC_KEY", "secret-proxy")
    ...
    assert env["ROBAIRIS_GH_PROXY_HMAC_KEY"] == ""
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bindings.workspace.root / ".airis-xdg" / "cache" / "bun-install")
    assert env["XDG_CACHE_HOME"] == str(bindings.workspace.root / ".airis-xdg" / "cache")
    assert env["TMPDIR"] == str(bindings.workspace.root / ".airis-tmp")
    ...
    assert (bindings.workspace.root / ".airis-tmp").is_dir()
...
def test_run_repo_command_uses_slot_identity_kwargs(
    ...
    assert kwargs["env"]["BUN_INSTALL_CACHE_DIR"].endswith("/.airis-xdg/cache/bun-install")
...
def test_guarded_push_branch_rev_parse_runs_via_repo_command_and_passes_slot_uid(
    ...
    from roboairis.git_ops import PushResult
    ...
    def fake_run_repo_command(
        ...
        if command[:3] == ["git", "log", "--format=%H%x09%ae%x09%an"]:
            return subprocess.CompletedProcess(
                command,
                0,
                "abc123\troboairis-bot@example.invalid\troboairis-bot\n",
                "",
            )
...
def test_gh_post_comment_happy_path(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(201, json={"id": 999, "user": {"login": "roboairis-bot"}, "body": "hi", "created_at": "t"})
...
def test_gh_post_comment_defaults_to_inbound_pr_thread(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        captured["url"] = str(request.url)
        return httpx.Response(201, json={"id": 7, "user": {"login": "roboairis-bot"}, "body": "hi", "created_at": "t"})
...
def test_mark_unable_posts_comment_marks_needs_info_and_labels_issue(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(201, json={"id": 321, "user": {"login": "roboairis-bot"}, "body": "x", "created_at": "t"})
...
def test_submit_pr_review_posts_summary_only_when_no_staged_comments(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"id": 45, "user": {"login": "roboairis-bot"}, "body": "ok", "state": "COMMENTED", "submitted_at": "t"},
        )
...
def test_gh_push_branch_rejects_wrong_identity(db: Database, tmp_path: Path) -> None:
    ...
    from roboairis.sandbox import SandboxManager
    ...
    try:
        ...
        assert "roboairis-bot <roboairis-bot@example.invalid>" in msg
...
def test_gh_push_branch_rejects_invalid_identity_scan_range(db: Database, tmp_path: Path) -> None:
    ...
    seed.mkdir()
    env = os.environ | {
        "GIT_AUTHOR_NAME": "roboairis-bot",
        "GIT_AUTHOR_EMAIL": "roboairis-bot@example.invalid",
        "GIT_COMMITTER_NAME": "roboairis-bot",
        "GIT_COMMITTER_EMAIL": "roboairis-bot@example.invalid",
    }
    subprocess.run(["git", "init", "--initial-branch=main", str(seed)], check=True, capture_output=True)
    (seed / "README.md").write_text("init\n")
    for cmd in (
        ...
        [
            ...
            str(seed),
            "-c",
            "user.email=roboairis-bot@example.invalid",
            "-c",
            "user.name=roboairis-bot",
            "commit",
            "-m",
            "init",
        ],
...
def test_gh_push_branch_runs_fix_and_check_before_pushing(
    ...
    assert lines[0] == "roboairis-bot <roboairis-bot@example.invalid> feat: follow-up", lines
    assert lines[1] == "roboairis-bot <roboairis-bot@example.invalid> init", lines
...
def test_gh_open_pr_runs_fix_then_check_and_amends_formatter_diff(
    ...
    lines = log.stdout.strip().splitlines()
    assert lines[0] == "roboairis-bot|roboairis-bot@example.invalid|feat: initial change"
...
def _question_handler(captured: dict[str, Any]):
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            201,
            json={"id": 4242, "user": {"login": "roboairis-bot"}, "body": "x", "created_at": "t"},
        )
