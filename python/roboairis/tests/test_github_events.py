...
from roboairis.github_events import (
    extract_mention,
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@roboairis-bot please re-review",
            },
            "issue": {"number": 9, "user": {"login": "contributor"}, "pull_request": {"url": "x"}},
            "repository": {"full_name": "octo/widget"},
        },
...
def test_extract_mention_returns_body_minus_mention() -> None:
    assert extract_mention("hey @roboairis-bot please look", "roboairis-bot") == "hey please look"
    assert extract_mention("@roboairis-bot do X", "roboairis-bot") == "do X"


@pytest.mark.parametrize("configured_login", ["@roboairis", "roboairis[bot]", "@roboairis[bot]"])
def test_extract_mention_accepts_prefixed_or_app_bot_login(configured_login: str) -> None:
    assert extract_mention("@roboairis go ahead", configured_login) == "go ahead"


def test_extract_mention_strips_literal_app_suffix_from_body() -> None:
    assert extract_mention("@roboairis[bot] go ahead", "roboairis[bot]") == "go ahead"


def test_extract_mention_rejects_extended_literal_app_suffix() -> None:
    assert extract_mention("@roboairis[bot]-helper go ahead", "roboairis[bot]") is None


def test_extract_mention_returns_none_without_mention() -> None:
    assert extract_mention("hello there", "roboairis-bot") is None
    assert extract_mention(None, "roboairis-bot") is None
    assert extract_mention("", "roboairis-bot") is None


def test_extract_mention_is_case_insensitive() -> None:
    assert extract_mention("yo @ROBOAIRIS-BOT", "roboairis-bot") == "yo"


def test_extract_mention_respects_hyphen_word_boundary() -> None:
    # @roboairis-bot-helper must NOT match @roboairis-bot.
    assert extract_mention("@roboairis-bot-helper hi", "roboairis-bot") is None


def test_extract_mention_handles_multiple_occurrences() -> None:
    assert extract_mention("@roboairis-bot one, then @roboairis-bot two", "roboairis-bot") == "one, then two"
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@roboairis-bot go ahead and push",
            },
            "issue": {"number": 9},
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@roboairis-bot go ahead with the plan",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@roboairis-bot change the indentation in foo.py",
            },
    ...
    decision = route(
        "pull_request_review_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@roboairis-bot use a generator here",
            },
    ...
    decision = route(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@roboairis-bot /model gpt /thinking low\nrefactor X",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
