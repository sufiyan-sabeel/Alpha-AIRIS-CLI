...
from roboairis.dashboard import reset_index_cache, static_dir
from roboairis.db import Database, close_database

# Minimum HTML the dashboard handler needs to render: `<title>` plus a script
# block carrying the `__ROBAIRIS_CONFIG__` sentinel. The real Vite-built bundle
# adds JS/CSS asset links; tests only care about the rendering contract.
_PLACEHOLDER_INDEX_HTML = (
    "<!doctype html>\n"
    '<html lang="en">\n'
    '  <head><meta charset="utf-8"><title>roboairis</title></head>\n'
    "  <body>\n"
    '    <div id="app"></div>\n'
    '    <script id="roboairis-config" type="application/json">__ROBAIRIS_CONFIG__</script>\n'
    "  </body>\n"
    "</html>\n"
)
...
def _baseline_env(tmp_path: Path) -> dict[str, str]:
    return {
        ...
        "ROBAIRIS_GH_PROXY_URL": "http://gh-proxy.invalid:8081",
        "ROBAIRIS_GH_PROXY_HMAC_KEY": "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
        "GITHUB_WEBHOOK_SECRET": "test-webhook-secret",
        "ROBAIRIS_BOT_LOGIN": "roboairis-bot",
        "ROBAIRIS_GIT_AUTHOR_NAME": "roboairis-bot",
        "ROBAIRIS_GIT_AUTHOR_EMAIL": "roboairis-bot@example.invalid",
        "ROBAIRIS_REPO_ALLOWLIST": "octo/widget",
        "ROBAIRIS_MODEL": "anthropic/claude-sonnet-4-5",
        "ROBAIRIS_THINKING": "high",
        "ROBAIRIS_WORKSPACE_ROOT": str(tmp_path / "workspaces"),
        "ROBAIRIS_SQLITE_PATH": str(tmp_path / "roboairis.sqlite"),
        "ROBAIRIS_LOG_DIR": str(tmp_path / "logs"),
        # Production default is `/data/cache/airis-natives` (provisioned by the
        ...
        # cache flip `ROBAIRIS_NATIVES_CACHE_ENABLED=true` explicitly.
        "ROBAIRIS_NATIVES_CACHE_ROOT": str(tmp_path / "natives-cache"),
        "ROBAIRIS_NATIVES_CACHE_ENABLED": "false",
        ...
        "ROBAIRIS_ISSUE_INDEX_SYNC_SECONDS": "0",
    }
...
def env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    monkeypatch.delenv("ROBAIRIS_PROVIDER", raising=False)
    monkeypatch.setenv("ROBAIRIS_REPLAY_TOKEN", "")
...
def proxy_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> dict[str, str]:
    ...
    baseline.pop("ROBAIRIS_GH_PROXY_URL", None)
    baseline.pop("ROBAIRIS_GH_PROXY_HMAC_KEY", None)
    ...
    monkeypatch.setenv("ROBAIRIS_GH_PROXY_URL", "")
    monkeypatch.setenv("ROBAIRIS_GH_PROXY_HMAC_KEY", "")
    monkeypatch.delenv("ROBAIRIS_PROVIDER", raising=False)
    monkeypatch.setenv("ROBAIRIS_REPLAY_TOKEN", "")
    reset_settings_cache()
    yield baseline
