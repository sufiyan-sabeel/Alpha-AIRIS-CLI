...
def _workspace(root: Path) -> Workspace:
    return Workspace(
        root=root,
        repo_dir=root / "repo",
        session_dir=root / ".airis-session",
...
def test_rename_workspace_branch_refreshes_shared_metadata(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "roboairis.sandbox._share_git_metadata_with_slots",
...
def test_rename_workspace_branch_runs_git_as_slot_when_permissions_active(
    ...
    monkeypatch.setattr("roboairis.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboairis.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboairis.sandbox.subprocess.run", fake_run)
    monkeypatch.setattr("roboairis.sandbox._share_git_metadata_with_slots", lambda _repo_dir, _slot_uid: None)
...
def test_delete_bad_refs_removes_worktree_holding_the_ref(tmp_path: Path) -> None:
    ...
    from roboairis.git_ops import _delete_bad_refs
...
def test_ensure_workspace_creates_worktree(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    ws = mgr.ensure_workspace(
        ...
        default_branch="main",
        author_name="roboairis-bot",
        author_email="roboairis-bot@example.invalid",
    )
...
def test_chown_workspace_noops_when_not_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboairis.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboairis.sandbox.os.geteuid", lambda: 1000)
    monkeypatch.setattr(
        "roboairis.sandbox.subprocess.run",
...
def test_chown_workspace_noops_off_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboairis.sandbox.platform.system", lambda: "Darwin")
    monkeypatch.setattr("roboairis.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr(
        "roboairis.sandbox.subprocess.run",
        lambda cmd, *, check: calls.append((cmd, check)),
...
def test_chown_workspace_runs_chown_and_chmod_as_root_on_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # 2001 is the slot-private GID matching the slot UID, not the shared airis group.
    assert calls == [
...
def test_reap_slot_noops_when_permissions_inactive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboairis.sandbox.platform.system", lambda: "Darwin")
    monkeypatch.setattr("roboairis.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboairis.sandbox.os.kill", lambda pid, sig: calls.append((pid, sig)))
...
def test_reap_slot_kills_slot_uid_on_linux_root(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboairis.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboairis.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboairis.sandbox._slot_pids", lambda _uid: (111, 222))
    monkeypatch.setattr("roboairis.sandbox.os.kill", lambda pid, sig: calls.append((pid, sig)))
...
def test_prepare_slot_tmpdir_mkdirs_without_chown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboairis.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboairis.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboairis.sandbox.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))

    tmpdir = _prepare_slot_tmpdir(_workspace(tmp_path), 2001)

    assert tmpdir == tmp_path / ".airis-tmp"
...
def test_provision_runtime_dirs_replaces_tmpdir_symlink_and_creates_xdg_tree(tmp_path: Path) -> None:
    ...
    assert target.is_dir()
    assert stat.S_IMODE(tmpdir.stat().st_mode) == 0o700
    for base in (tmp_path / ".airis-xdg" / "data", tmp_path / ".airis-xdg" / "state", tmp_path / ".airis-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "airis").is_dir()
    assert (tmp_path / ".airis-xdg" / "cache" / "bun-install").is_dir()
...
def test_chown_workspace_normalizes_to_root_when_slots_disabled(
    ...
    monkeypatch.setattr("roboairis.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("roboairis.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboairis.sandbox.os.getegid", lambda: 0)
    monkeypatch.setattr("roboairis.sandbox.subprocess.run", fake_run)
...
def test_prepare_slot_runtime_env_returns_workspace_private_paths_without_chown(
    ...
    monkeypatch.setattr("roboairis.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("roboairis.sandbox.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))
    monkeypatch.setattr("roboairis.sandbox.subprocess.run", lambda cmd, **_kwargs: calls.append(cmd))

    ws = _workspace(tmp_path)
    bun_cache = ws.root / ".airis-xdg" / "cache" / "bun-install"

    env = _prepare_slot_runtime_env(ws, 2001)

    assert env["TMPDIR"] == str(ws.root / ".airis-tmp")
    assert env["XDG_CACHE_HOME"] == str(ws.root / ".airis-xdg" / "cache")
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bun_cache)
    for base in (ws.root / ".airis-xdg" / "data", ws.root / ".airis-xdg" / "state", ws.root / ".airis-xdg" / "cache"):
        assert base.is_dir()
        assert (base / "airis").is_dir()
...
def test_ensure_workspace_refreshes_permissions_for_retry_slot_and_session(
    ...
    monkeypatch.setattr("roboairis.sandbox._chown_workspace", record_chown)
    monkeypatch.setattr("roboairis.sandbox._share_git_metadata_with_slots", record_share)
...
def test_ensure_workspace_provisions_and_slot_owns_runtime_dirs(
    ...
    def record_chown(ws_root: Path, slot_uid: int | None) -> None:
        ...
        paths = [
            ws_root / ".airis-tmp",
            ws_root / ".airis-xdg" / "data",
            ws_root / ".airis-xdg" / "data" / "airis",
            ws_root / ".airis-xdg" / "state",
            ws_root / ".airis-xdg" / "state" / "airis",
            ws_root / ".airis-xdg" / "cache",
            ws_root / ".airis-xdg" / "cache" / "airis",
            ws_root / ".airis-xdg" / "cache" / "bun-install",
        ]
    ...
    assert set(runtime_paths) == {
        ws.root / ".airis-tmp",
        ws.root / ".airis-xdg" / "data",
        ws.root / ".airis-xdg" / "data" / "airis",
        ws.root / ".airis-xdg" / "state",
        ws.root / ".airis-xdg" / "state" / "airis",
        ws.root / ".airis-xdg" / "cache",
        ws.root / ".airis-xdg" / "cache" / "airis",
        ws.root / ".airis-xdg" / "cache" / "bun-install",
    }
...
def test_remove_workspace_prunes_pool_after_failed_worktree_remove(
    ...
    monkeypatch.setattr("roboairis.sandbox._safe_run", fake_safe_run)
...
def test_redact_credentials_strips_userinfo() -> None:
    from roboairis.sandbox import redact_credentials
...
def test_git_command_error_redacts_url_in_args_and_stderr(tmp_path: Path) -> None:
    ...
    from roboairis.sandbox import _run
...
def test_push_force_with_lease_succeeds_after_local_amend(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    from roboairis.git_ops import push as git_push
...
def test_push_force_with_lease_refuses_when_origin_moved(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    force-pushing."""
    from roboairis.git_ops import GitCommandError
    from roboairis.git_ops import push as git_push
...
def test_run_git_injects_safe_directory_and_subprocess_identity(
    ...
    from roboairis.git_ops import _run_git
    ...
    monkeypatch.setattr("roboairis.git_ops.subprocess.run", fake_run)
...
def test_run_git_scopes_token_and_scrubs_parent_auth_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from roboairis.git_ops import AUTH_ENV_VAR, _run_git
...
def test_run_git_kills_hung_child(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    timeout."""
    from roboairis.git_ops import GitCommandError, _run_git
...
# ---------------------------------------------------------------------------
# Partial-clone blob backfill (alpha-airis-cli#1818)
# ---------------------------------------------------------------------------
...
def test_fetch_ref_backfills_missing_blobs_into_partial_clone(tmp_path: Path) -> None:
    """Regression for alpha-airis-cli#1818: ``fetch_ref`` is called immediately before
...
def test_ensure_workspace_populates_from_natives_cache(tmp_path: Path, upstream_repo: Path) -> None:
    from roboairis.natives_cache import NativesCache, compute_key, target_triple
    ...
    (native_dir1 / f"airis_natives.{triple}.node").write_bytes(b"ELFx")
    ...
    node_name = f"airis_natives.{triple}.node"
...
def test_ensure_workspace_cache_miss_is_silent_noop(tmp_path: Path, upstream_repo: Path) -> None:
    from roboairis.natives_cache import NativesCache
...
def test_ensure_workspace_acquires_repo_lock(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboairis.sandbox._run", lambda *a, **k: ok)
    monkeypatch.setattr("roboairis.sandbox._safe_run", lambda *a, **k: ok)
    monkeypatch.setattr("roboairis.sandbox._chown_workspace", lambda *a, **k: None)
    monkeypatch.setattr("roboairis.sandbox._share_git_metadata_with_slots", lambda *a, **k: None)
    monkeypatch.setattr("roboairis.sandbox._provision_runtime_dirs", lambda *a, **k: None)
...
def test_safe_run_timeout_returns_124(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboairis.sandbox.subprocess.run", boom)
...
def test_ensure_workspace_raises_when_local_branch_probe_times_out(
    ...
    monkeypatch.setattr("roboairis.sandbox._safe_run", fake_safe_run)
    monkeypatch.setattr("roboairis.sandbox._run", lambda *a, **k: subprocess.CompletedProcess(["x"], 0, "", ""))
    monkeypatch.setattr("roboairis.sandbox._chown_workspace", lambda *a, **k: None)
    monkeypatch.setattr("roboairis.sandbox._share_git_metadata_with_slots", lambda *a, **k: None)
    monkeypatch.setattr("roboairis.sandbox._provision_runtime_dirs", lambda *a, **k: None)
...
def test_ensure_workspace_raises_when_symbolic_ref_probe_times_out(
    ...
    monkeypatch.setattr("roboairis.sandbox._share_git_metadata_with_slots", lambda *a, **k: None)
    monkeypatch.setattr("roboairis.sandbox._provision_runtime_dirs", lambda *a, **k: None)
    monkeypatch.setattr("roboairis.sandbox._git_env_for_repo", lambda *a, **k: {})
...
def test_worktree_add_cleans_partial_state_on_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("roboairis.sandbox._run", boom_run)
    monkeypatch.setattr("roboairis.sandbox._safe_run", fake_safe_run)
...
def _seed_reclaimable_workspace(mgr: SandboxManager, repo: str, number: int) -> Path:
    ...
    for rel in (
        ...
        "repo/src",
        ".airis-session",
        ".airis-xdg/cache/bun-install",
        ".airis-xdg/state/airis",
        ".airis-tmp",
        "artifacts",
    ):
    ...
    (ws_root / ".airis-session/session.jsonl").write_text("{}", encoding="utf-8")
    (ws_root / ".airis-xdg/cache/bun-install/pkg.tgz").write_text("x", encoding="utf-8")
    (ws_root / ".airis-xdg/state/airis/state.json").write_text("{}", encoding="utf-8")
    (ws_root / ".airis-tmp/scratch").write_text("x", encoding="utf-8")
...
def test_reclaim_workspace_caches_strips_dep_caches_and_preserves_state(tmp_path: Path) -> None:
    ...
    assert not (ws_root / ".airis-xdg/cache").exists()
    assert not (ws_root / ".airis-tmp").exists()
    assert (ws_root / "repo/src/keep.ts").read_text(encoding="utf-8") == "keep"
    assert (ws_root / ".airis-session/session.jsonl").exists()
    assert (ws_root / ".airis-xdg/state/airis/state.json").exists()
