...
from roboairis import config as config_mod
...
def test_event_retry_delays_parsing_skips_garbage(env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ROBAIRIS_EVENT_RETRY_DELAYS_SECONDS", "30, 120 ,600,,abc,-5")
...
def test_event_retry_delays_defaults_when_empty(env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ROBAIRIS_EVENT_RETRY_DELAYS_SECONDS", "   ")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
...
def test_retry_delay_escalates_and_clamps(env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ROBAIRIS_EVENT_RETRY_DELAYS_SECONDS", "1,2,3")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
...
def test_retry_delay_jitter_stays_in_band(env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ROBAIRIS_EVENT_RETRY_DELAYS_SECONDS", "100")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    for _ in range(200):
...
def _retry_settings(monkeypatch: pytest.MonkeyPatch, *, max_retries: int) -> Settings:
    monkeypatch.setenv("ROBAIRIS_EVENT_MAX_RETRIES", str(max_retries))
    monkeypatch.setenv("ROBAIRIS_EVENT_RETRY_DELAYS_SECONDS", "0")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    return cfg
...
async def test_run_event_retries_then_marks_failed(
    ...
    monkeypatch.setattr("roboairis.queue._reap_slot", lambda uid: None)
    pool = WorkerPool(
        settings=cfg,
        db=db,
