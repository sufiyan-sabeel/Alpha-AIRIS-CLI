...
Self-hosted GitHub triage bot. Drives [`airis --mode rpc`](https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI)
...
- `question` → one comment, suffixed with a 👎-to-keep-open prompt; if the
  issue author doesn't react 👎 within `ROBAIRIS_QUESTION_AUTOCLOSE_HOURS`
...
Follow-up issue comments and PR review comments resume the same airis session
...
- **roboairis** — FastAPI + sqlite event queue + `WorkerPool` running `airis` in
...
- **gh-proxy** — sibling on an `internal: true` network. Holds `GITHUB_TOKEN`,
  verifies HMAC-signed requests from roboairis, executes REST + `git push`.
...
→ `worker.run_task` spawns `airis --mode rpc` with `cwd=worktree`,
persistent `session_dir`, model randomly drawn from `ROBAIRIS_MODEL` (CSV).

The agent uses airis's built-in tools (`read`/`edit`/`bash`/`lsp`, scoped to
...
`~/.airis/agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host airis doesn't route through the gateway). roboairis lives inside the alpha-airis-cli
monorepo at `python/roboairis/`; both the docker build context and the
`/work/airis` bind mount default to the parent monorepo (`../..`). Override
`AIRIS_ROOT` only if you want a different alpha-airis-cli checkout backing the build
and runtime.

Bot account needs **Write** on every repo in `ROBAIRIS_REPO_ALLOWLIST`. A
...
cp .env.example .env
$EDITOR .env
openssl rand -hex 32              # ROBAIRIS_GH_PROXY_HMAC_KEY
openssl rand -hex 32              # GITHUB_WEBHOOK_SECRET

bun run airis:image                  # build airis/airis:dev (one-time / on pi change)
bun run roboairis:build && bun run roboairis:up
...
comment out `ROBAIRIS_GH_PROXY_URL` / `ROBAIRIS_GH_PROXY_HMAC_KEY` and set
`GITHUB_TOKEN`. The two modes are mutually exclusive (`config.py`
rejects a `.env` setting both).

Build invalidation is bounded: editing roboairis Python touches only the
runtime layer; editing pi source rebuilds `airis/airis:dev`, which
roboairis's `Dockerfile.roboairis` extends via `FROM ${AIRIS_BASE}`.

### Public URL

roboairis does not ship a tunnel. Cloudflare, smee, ngrok are all fine. The
...
The container entrypoint is `python -m roboairis serve`. Other commands run
inside the running container:

```bash
docker compose exec roboairis roboairis triage  owner/repo#123   # synthesize an issues.opened and wait
docker compose exec roboairis roboairis replay  <delivery_id>    # re-enqueue a stored event and wait
docker compose exec roboairis roboairis status                   # dump issues table
docker compose exec roboairis roboairis cleanup owner/repo#123   # force workspace removal, state=abandoned
```

`bun run roboairis:…` shortcuts in the root `package.json` cover the common
lifecycle commands (`roboairis:dev`, `roboairis:build`, `roboairis:up`, `roboairis:down`,
`roboairis:logs`, `roboairis:restart`, `roboairis:reset`).

## Tests

```bash
pytest -x tests/                              # unit suite, no network
ROBOAIRIS_INTEGRATION=1 pytest -x tests/test_worker_smoke.py
```

The integration test spawns a real `airis --mode rpc` against an
`httpx.MockTransport` GitHub and a local bare repo, so it needs `airis` on
...
- gh-proxy has no host port. The `roboairis_internal` network is
...
- Agent subprocess env is scrubbed of `GITHUB_TOKEN` /
  `ROBAIRIS_GH_PROXY_HMAC_KEY` / friends via `worker._SCRUBBED_ENV_KEYS`.
...
- Pre-push gates (`gh_push_branch`): branch matches the workspace
  ...
  `origin/<default>..HEAD` carries `ROBAIRIS_GIT_AUTHOR_NAME` +
  `ROBAIRIS_GIT_AUTHOR_EMAIL`. Commit messages carrying shell-literal
...
- **Crash recovery.** On startup, `db.reset_stuck_running()` flips
  ...
  `ROBAIRIS_SHUTDOWN_DRAIN_TIMEOUT_SECONDS` (25s) +
  `ROBAIRIS_SHUTDOWN_KILL_TIMEOUT_SECONDS` (5s); compose
  `stop_grace_period: 30s` covers both.
- **Logs.** Structured JSON on stdout, rotated to
  `/data/logs/roboairis.log.jsonl`.
...
| Container exits with `AIRIS_ROOT … missing` | `/work/airis` mount empty inside the container; on the host either run `docker compose` from `python/roboairis/` so `AIRIS_ROOT` defaults to `../..`, or export `AIRIS_ROOT` to a valid alpha-airis-cli checkout. |
| `git push: Authentication required` | Bot PAT lacks push, or `ROBAIRIS_BOT_LOGIN` does not identify the PAT account's mention handle (production: `roboairis`, no `@`/`[bot]`). |
| `refusing to push: commit author identity mismatch` | Some commit not authored as `ROBAIRIS_GIT_AUTHOR_*`. The error lists the offending shas; `git commit --amend --reset-author --no-edit`. |
...
| `Failed to load airis_natives` | Wrong arch / missing native. `bun run airis:image` then `bun run roboairis:build`. |
| `No API key found for <provider>` | `~/.airis/agent/models.container.yml` mount missing or provider id mismatch with `ROBAIRIS_MODEL`. |
...
src/
  ...
  worker.py          synchronous airis RPC driver, prompt assembly, env scrubbing
...
tests/               pytest unit suite + one ROBOAIRIS_INTEGRATION=1 smoke test
