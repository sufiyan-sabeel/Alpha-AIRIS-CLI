<p align="center">
  <img src="https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI/blob/main/assets/hero.svg?raw=true" alt="Alpha AIRIS-CLI TUI">
</p>

<p align="center">
  <strong>A coding agent with the IDE wired in.</strong>
  <strong><a href="https://airis.sh">airis.sh</a></strong>
</p>

<p align="center">
  <a href="https://www.npmjs.com/package/@airis/airis-coding-agent"><img src="https://img.shields.io/npm/v/@airis/airis-coding-agent?style=flat&colorA=222222&colorB=CB3837" alt="npm version"></a>
  <a href="https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI/blob/main/packages/coding-agent/CHANGELOG.md"><img src="https://img.shields.io/badge/changelog-keep-E05735?style=flat&colorA=222222" alt="Changelog"></a>
  <a href="https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI/actions"><img src="https://img.shields.io/github/actions/workflow/status/sufiyan-sabeel/Alpha-AIRIS-CLI/ci.yml?style=flat&colorA=222222&colorB=3FB950" alt="CI"></a>
  <a href="https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI/blob/main/LICENSE"><img src="https://img.shields.io/github/license/sufiyan-sabeel/Alpha-AIRIS-CLI?style=flat&colorA=222222&colorB=58A6FF" alt="License"></a>
...
<p align="center">
  Fork of <a href="https://github.com/badlogic/pi-mono">Pi</a> by <a href="https://github.com/mariozechner">@mariozechner</a>, extended by <a href="https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI">Alpha AIRIS-CLI</a> · KageOS
</p>
...
```sh
curl -fsSL https://airis.sh/install | sh
```

**Homebrew**

```sh
brew install sufiyan-sabeel/tap/airis
```

**Bun (recommended)**

```sh
bun install -g @airis/airis-coding-agent
```

**Windows (PowerShell)**

```powershell
irm https://airis.sh/install.ps1 | iex
```

**Pinned versions (mise)**

```sh
mise use -g github:sufiyan-sabeel/Alpha-AIRIS-CLI
```
...
`airis` generates its own completion scripts for **bash**, **zsh**, and **fish** from the live command/flag metadata, so they never drift from the actual CLI. Subcommands, flags, and enum values complete statically; model names (`--model`, `--smol`, `--slow`, `--plan`) resolve against the bundled model catalog and `--resume` against your on-disk sessions.

```sh
# zsh — add to ~/.zshrc (or write the output into a file on your $fpath)
eval "$(airis completions zsh)"

# bash — add to ~/.bashrc
eval "$(airis completions bash)"

# fish
airis completions fish > ~/.config/fish/completions/airis.fish
```

## Every tool, _benchmaxxed_.

Edits that land on the first attempt. Reads that summarize files instead of dumping their content. Searches that return instantly. Pick any model — airis will get it right.
...
Originally built on [Mario Zechner](https://github.com/mariozechner)'s wonderful [Pi](https://github.com/badlogic/pi-mono), airis adds everything you're missing.
...
![airis TUI: a single eval session with `[1/2] pandas describe` (Python) printing a real DataFrame.describe() table, followed by `[2/2] top scorer` (JavaScript) running a reduce. Footer: 'Both kernels ran in one session.'](https://airis.sh/captures/eval.webp)

### 02 · LSP wired into every write

Ask for a rename and you get a rename. The call goes through workspace/willRenameFiles, so re-exports, barrel files, and aliased imports update before the file moves. Everything your IDE knows, the agent knows.

![airis TUI: `LSP references` returns five hits across three files for the symbol `formatBytes`, then `LSP rename` applies the change with edits to format.ts/report.ts/cli.ts, then a `Search formatBytes 0 matches` confirmation. Final line: 'Rename complete. Five edits across three files…'.](https://airis.sh/captures/lsp.webp)
...
![airis TUI: a live lldb-dap session against a native binary at /tmp/airis-native/demo. Adapter=lldb-dap, Status=stopped, Frame=xorshift32, Instruction pointer 0x10000055C, Location demo.c:6:10. Debug scopes and Debug variables cards show locals (x = 57351) and the agent confirms the math: x went from 7 → 57351 (= 7 ^ (7<<13)).](https://airis.sh/clips/dap-poster.webp)

_[Watch the capture ↗](https://airis.sh/clips/dap.mp4)_
...
![airis TUI: /advisor status shows the advisor running on openai-codex/gpt-5.5; after the main agent scopes a catch to ENOENT instead of swallowing every error, an amber 'Advisor 1 note (concern)' card warns the fix no longer matches the user's literal acceptance criterion.](https://airis.sh/clips/advisor-poster.webp)

_[Watch the capture ↗](https://airis.sh/clips/advisor.mp4)_

### 07 · Hand someone the link, they're in.

/collab puts your live session on a relay and hands back a link — and a QR. A teammate joins from another terminal with airis join, or just opens it in a browser. Share read-write to pair on the same agent, or /collab view for a read-only link anyone can watch but no one can steer. Frames are sealed client-side; the relay never sees your keys.

![airis TUI: /collab view prints 'Collab session started!' with an airis join command, a my.airis.sh browser link, the note 'Anyone with this link can watch the session but cannot prompt the agent', and a large scannable QR code.](https://airis.sh/clips/collab-poster.webp)

_[Watch the capture ↗](https://airis.sh/clips/collab.mp4)_
...
![airis TUI: web_search returns 10 ranked Perplexity sources for inference-time compute scaling, the agent picks an arxiv paper, calls read https://arxiv.org/pdf/2604.10739v1, and summarizes the paper's headline result with real numbers.](https://airis.sh/clips/web-poster.webp)

_[Watch the capture ↗](https://airis.sh/clips/web.mp4)_

### 09 · Unapologetically native. Even on Windows.

Other agents shell out to rg, grep, find, and bash. On many machines those binaries don't exist, and on the ones where they do, every call costs a fork-exec round-trip. airis links the real implementations into the process. ripgrep, glob, find: in-process. brush is the bash, with sessions that survive across calls. The same airis binary runs on macOS, Linux, and Windows — no WSL bridge.
...
![airis TUI reading pr://sufiyan-sabeel/Alpha-AIRIS-CLI/1063 and then /diff/1, showing hunk headers, added lines, and a [MODIFIED] (+12 -0) summary.](https://airis.sh/captures/pr.webp)
...
![airis TUI: ✓ Read src/session.ts (⚠ 1 conflict), then ✓ Write conflict://1 · 1 line with content @theirs, then a confirmation 'Resolved.'](https://airis.sh/clips/conflict-poster.webp)

_[Watch the capture ↗](https://airis.sh/clips/conflict.mp4)_
...
![airis TUI: ✓ AST Edit: console.log($X) (proposed) 3 replacements · 1 file, then ✓ Accept: 3 replacements in 1 file (AST Edit), followed by 'Applied 3 replacements in src/auth.ts.'](https://airis.sh/clips/codemod-poster.webp)

_[Watch the capture ↗](https://airis.sh/clips/codemod.mp4)_
...
![airis TUI driving the browser tool against DuckDuckGo](https://airis.sh/captures/browser.webp)
...
[Full reference →](https://airis.sh/docs/tools)
...
Define custom providers in `~/.airis/agent/models.yml`:
...
Run `airis models spark` to verify discovery. Then run `airis setup` and choose the model in the default-model step, or open `/model` in a session and assign it to the `default` role.

To preconfigure the default without the picker, add the selector to `~/.airis/agent/config.yml`:
...
- **Custom providers** — Declare anything that speaks `openai-completions`, `openai-responses`, `openai-codex-responses`, `azure-openai-responses`, `anthropic-messages`, `google-generative-ai`, or `google-vertex` in `~/.airis/agent/models.yml`.
...
Full provider & routing reference at [airis.sh/docs/providers](https://airis.sh/docs/providers).
...
[`web_search` reference ↗](https://airis.sh/docs/tools#web_search)
...
- Crates: `airis-natives`, `airis-shell`, `airis-ast`, `airis-iso`
...
| iso        | Workspace isolation shim · apfs · btrfs · zfs · reflink · overlayfs · projfs · rcopy | airis-iso (PAL)                              |   245 |
...
Same engine, four wrappers. `airis` runs the TUI. `airis -p` answers a single prompt and exits. The Node SDK embeds the session in your process. `airis --mode rpc` and `airis acp` hand the wheel to another program over stdio.
...
![airis TUI: the ask tool renders an option picker with three choices, a (Recommended) badge on the first, and 'up/down navigate · enter select · esc cancel' footer.](https://airis.sh/captures/ask.webp)
...
Full reference: [airis.sh/docs/sdk](https://airis.sh/docs/sdk).

## A harness worth keeping is one you _don't_ outgrow.

Pick it up at **[airis.sh](https://airis.sh)**.

Alpha AIRIS-CLI is a coding agent derived from [Oh My Pi](https://github.com/can1357/oh-my-pi) (itself a fork of [Pi](https://github.com/badlogic/pi-mono) by [Mario Zechner](https://github.com/mariozechner)). It is maintained by [Umaiz Sufiyan](https://github.com/sufiyan-sabeel) under the KageOS brand. All original copyright notices and MIT license terms are preserved.
...
On first run airis inherits whatever is already on disk: rules, skills, and MCP servers from `.claude`, `.cursor`, `.windsurf`, `.gemini`, `.codex`, `.cline`, `.github/copilot`, and `.vscode`. No migration script.

### Extensibility

Ask airis to write the piece you're missing, then `/reload-plugins`. Keep it local, ship it in a `marketplace`, or publish it to npm.

## Philosophy

Alpha AIRIS-CLI is derived from [Oh My Pi](https://github.com/can1357/oh-my-pi) (itself a fork of [Pi](https://github.com/badlogic/pi-mono) by [Mario Zechner](https://github.com/mariozechner)), extended with a batteries-included coding workflow.
...
`bun setup` installs Bun workspaces and builds `@airis/airis-natives`. Re-run `bun run build:native` after changing Rust crates or `packages/natives`.
...
| Package                                                   | Description                                                                |
| --------------------------------------------------------- | -------------------------------------------------------------------------- |
| **[@airis/airis-collab-web](packages/collab-web)**           | Browser guest client, mock host, and local relay for collab live sessions  |
| **[@airis/airis-ai](packages/ai)**                        | Multi-provider LLM client with streaming and model/provider integration    |
| **[@airis/airis-catalog](packages/catalog)**              | Model catalog: bundled model database, provider descriptors, and identity  |
| **[@airis/airis-agent-core](packages/agent)**             | Agent runtime with tool calling and state management                       |
| **[@airis/airis-coding-agent](packages/coding-agent)**    | Interactive coding agent CLI and SDK                                       |
| **[@airis/airis-tui](packages/tui)**                      | Terminal UI library with differential rendering                            |
| **[@airis/airis-natives](packages/natives)**              | N-API bindings for grep, shell, image, text, syntax highlighting, and more |
| **[@airis/airis-stats](packages/stats)**                 | Local observability dashboard for AI usage statistics                      |
| **[@airis/airis-utils](packages/utils)**                  | Shared utilities (logging, streams, dirs/env/process helpers)              |
| **[@airis/airis-wire](packages/wire)**                    | Shared collab live-session protocol types and relay constants              |
| **[@airis/airis-hashline](packages/hashline)**               | Line-anchored patch language and applier behind the `edit` tool            |
| **[@airis/airis-mnemosyne](packages/mnemosyne)**              | Local SQLite memory engine for Alpha AIRIS-CLI agents                             |
| **[@airis/airis-snapcompact](packages/snapcompact)**         | Bitmap-frame context compression package and SQuAD eval suite              |
| **[@airis/airis-swarm-extension](packages/swarm-extension)** | Swarm orchestration extension package                                      |

### Rust Crates

| Crate                                              | Description                                                                                         |
| -------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **[airis-natives](crates/airis-natives)**                | Core Rust native addon (N-API `cdylib`) used by `@airis/airis-natives`; aggregates the crates below |
| **[airis-shell](crates/airis-shell)**                    | Embedded shell / PTY / process management split out of `airis-natives` (wraps `brush-*`)               |
| **[airis-ast](crates/airis-ast)**                        | tree-sitter-based code summarizer and AST utilities (50+ language grammars)                         |
| **[airis-iso](crates/airis-iso)**                        | Task isolation backend resolver: APFS clones, btrfs/zfs reflinks, overlayfs, projfs, rcopy          |
...
- [airis.sh](https://airis.sh)
- [GitHub](https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI)
- [Changelog](https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI/blob/main/packages/coding-agent/CHANGELOG.md)
- [npm](https://www.npmjs.com/package/@airis/airis-coding-agent)
- [Discord](https://discord.gg/4NMW9cdXZa)
- [MIT](https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI/blob/main/LICENSE)
