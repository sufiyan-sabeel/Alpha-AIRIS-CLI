...
mod tests {
	...
	fn bun_run_check_colon_compacts_workspace_success_noise() {
		...
		let out = filter(
			...
			"$ bun run check:tools && bun run --workspaces --if-present check\n$ biome check . \
			 ...
			 applied.\n@airis/airis-utils check: Checked 40 files in 11ms. No fixes \
			 applied.\n@airis/airis-utils check: $ tsgo -p tsconfig.json \
			 --noEmit\n@airis/airis-utils check: Exited with code 0\n@airis/airis-coding-agent \
			 check: Checked 1178 files in 287ms. No fixes applied.\n@airis/airis-coding-agent check: \
			 $ tsgo -p tsconfig.json --noEmit\n@airis/airis-coding-agent check: Exited with code 0\n",
			0,
		);

		assert!(out.text.contains("check:ts: passed"));
		assert!(out.text.contains("root biome: ok"));
		assert!(out.text.contains("@airis/airis-utils"));
		assert!(out.text.contains("@airis/airis-coding-agent"));
	...
	fn bun_check_failure_keeps_diagnostic_and_emits_failed_status() {
		...
		let input = concat!(
			...
			"@airis/airis-utils check: $ tsgo -p tsconfig.json --noEmit\n",
			"@airis/airis-utils check: Exited with code 0\n",
			"@airis/airis-coding-agent check: $ tsgo -p tsconfig.json --noEmit\n",
			"src/tools/bash.ts(42,7): error TS2322: Type 'string' is not assignable to type \
			 'number'.\n",
			"@airis/airis-coding-agent check: Exited with code 1\n",
		);
	...
	fn bun_check_success_emits_passed_status_and_no_noise() {
		...
		let input = concat!(
			...
			"@airis/airis-utils check: Checked 40 files in 11ms. No fixes applied.\n",
			"@airis/airis-utils check: $ tsgo -p tsconfig.json --noEmit\n",
			"@airis/airis-utils check: Exited with code 0\n",
			"@airis/airis-coding-agent check: Checked 1178 files in 287ms. No fixes applied.\n",
			"@airis/airis-coding-agent check: $ tsgo -p tsconfig.json --noEmit\n",
			"@airis/airis-coding-agent check: Exited with code 0\n",
		);
