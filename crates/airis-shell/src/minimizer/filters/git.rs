...
";
	...
	fn fetch_output_counts_new_refs_as_updates() {
		...
		let out = filter(
			&ctx,
			"From github.com:sufiyan-sabeel/Alpha-AIRIS-CLI\n * [new branch]      feature -> origin/feature\n",
			0,
		);
		assert!(out.changed);
		assert!(
	...
	fn push_output_keeps_deleted_refs() {
		...
		let out =
			filter(&ctx, "To github.com:sufiyan-sabeel/Alpha-AIRIS-CLI.git\n - [deleted]         old-branch\n", 0);
		assert!(out.changed);
	...
	fn diff_stat_is_summarized() {
		...
		let input = "\
 crates/airis-shell/src/minimizer/filters/git.rs     | 385 +++++++++++++++++------
...
";
		...
		assert!(
			out.text
				.contains("crates/airis-shell/src/minimizer/filters/git.rs")
		);
		assert!(
			out.text
