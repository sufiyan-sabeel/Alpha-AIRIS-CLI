...
pub struct MinimizerOptions {
	...
	/// `AIRIS_MINIMIZER_LEGACY_FILTERS` environment variable (truthy = "1",
...
pub struct MinimizerConfig {
	...
	/// `MinimizerOptions.legacy_filters` or the `AIRIS_MINIMIZER_LEGACY_FILTERS`
...
impl MinimizerConfig {
	...
	pub fn from_options(opts: &MinimizerOptions) -> Self {
		...
		let legacy_requested = resolve_legacy_filters(
			opts.legacy_filters,
			std::env::var("AIRIS_MINIMIZER_LEGACY_FILTERS")
				.ok()
				.as_deref(),
		);
			...
			if let Ok(contents) = fs::read_to_string(&expanded) {
					...
					&& !expected.is_empty()
				{
					...
					if !actual_hex.eq_ignore_ascii_case(expected) {
						eprintln!(
							"[airis-natives minimizer] settings_hash mismatch for {} (expected {}, got {}); \
							 ignoring file",
							expanded.display(),
							expected,
							actual_hex
				...
				match pipeline::parse_file(&contents, "user") {
					...
					Err(err) => {
						eprintln!("[airis-natives minimizer] user filters: {err}");
					},
				}
			}
		}
...
impl SettingsFile {
	fn merge_into(self, cfg: &mut MinimizerConfig) {
		...
		{
			eprintln!(
				"[airis-natives minimizer] unsupported schema_version {v} in settings file (expected \
...
/// and the raw `AIRIS_MINIMIZER_LEGACY_FILTERS` env value.
///
...
mod tests {
	...
	fn explicit_disabled_option_overrides_enabled_settings_file() {
		let path = std::env::temp_dir()
			.join(format!("airis-minimizer-config-disabled-{}.toml", std::process::id()));
	...
	fn explicit_legacy_true_overrides_disabled_settings_file() {
		let path = std::env::temp_dir()
			.join(format!("airis-minimizer-config-legacy-{}.toml", std::process::id()));
