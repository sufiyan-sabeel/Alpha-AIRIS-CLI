...
//!
//! Each builtin installs a [`airis_uutils_ctx`] scope — the command's stdio file
...
/// Drives a patched uutils utility to completion under a [`airis_uutils_ctx`]
...
fn run_caught(run: UutilRun, argv: Vec<OsString>) -> i32 {
	...
	if let Ok(code) = catch_unwind(|| run(argv)) {
		...
		let _ = writeln!(airis_uutils_ctx::stderr(), "{name}: internal error");
		1
	}
}
...
uutil_builtin!(pub fn grep_builtin => airis_uu_grep::run);
uutil_builtin!(pub fn rg_builtin => airis_uu_grep::run_rg);
...
uutil_builtin!(pub fn diff_builtin => airis_uu_diff::run);
...
mod tests {
	...
	fn scope_io(stderr: Box<dyn Write + Send>) -> airis_uutils_ctx::ScopeIo {
		airis_uutils_ctx::ScopeIo {
	...
	fn run_in_scope(run: UutilRun, argv: Vec<OsString>) -> (i32, String) {
		...
		let code = airis_uutils_ctx::scope(scope_io(Box::new(ChanWriter(tx))), || run_caught(run, argv));
		let mut err = Vec::new();
