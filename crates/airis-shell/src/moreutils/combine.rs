...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match execute(&file1, op, &file2) {
		...
		Err(Error::Msg(message)) => {
			let _ = writeln!(airis_uutils_ctx::stderr(), "combine: {message}");
			1
		},
	}
}
...
fn command() -> Command {
	Command::new("combine")
		.version(concat!("combine (airis-shell) ", env!("CARGO_PKG_VERSION")))
...
fn open_input(name: &OsStr) -> Result<Box<dyn BufRead>, Error> {
	if name == OsStr::new("-") {
		return Ok(Box::new(BufReader::new(airis_uutils_ctx::stdin())));
	}
