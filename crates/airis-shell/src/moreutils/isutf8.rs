...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	for file in &files {
		...
		let valid = match validate_file(file) {
			Err(err) => {
				let _ = writeln!(airis_uutils_ctx::stderr(), "isutf8: {display}: {err}");
				io_error = true;
				continue;
			},
		...
		if valid == invert {
			any_failed = true;
			if list && !quiet {
				let _ = writeln!(airis_uutils_ctx::stdout(), "{display}");
			}
		}
	}
...
fn command() -> Command {
	Command::new("isutf8")
		.version(concat!("isutf8 (airis-shell) ", env!("CARGO_PKG_VERSION")))
...
fn validate_file(name: &OsStr) -> io::Result<Verdict> {
	if name == "-" {
		validate(&mut airis_uutils_ctx::stdin())
	} else {
		let mut file = File::open(airis_uutils_ctx::resolve(name))?;
		validate(&mut file)
	}
}
