...
pub fn run(argv: Vec<OsString>) -> i32 {
		...
		.as_deref()
	{
		...
		Some("--help") => {
			let _ = writeln!(airis_uutils_ctx::stdout(), "{USAGE}");
			return 0;
		},
		_ => {},
	}
	...
	if command.is_empty() {
		let _ = writeln!(airis_uutils_ctx::stderr(), "{USAGE}");
		return 1;
	}
	...
	let got = loop {
		match stdin.read(&mut first) {
			...
			Err(err) => {
				let _ = writeln!(airis_uutils_ctx::stderr(), "ifne: stdin: {err}");
				return 1;
			},
		}
	};
	...
	if empty != invert {
		...
		return match copy_cancellable(&mut stdin, &mut airis_uutils_ctx::stdout(), Some(first[0])) {
			Ok(()) => 0,
			Err(CopyError::Cancelled) => 130,
			Err(CopyError::Io(err)) => {
				let _ = writeln!(airis_uutils_ctx::stderr(), "ifne: {err}");
				1
			},
		};
	}
...
fn spawn_and_pump(command: &[OsString], first: Option<u8>, stdin: &mut impl Read) -> i32 {
	let mut child = match Command::new(&command[0])
		...
		.env_clear()
		.envs(airis_uutils_ctx::env_snapshot())
		.stdin(Stdio::piped())
		.stdout(Stdio::piped())
		.stderr(Stdio::piped())
		.spawn()
	{
		Ok(child) => child,
		Err(err) => {
			let _ = writeln!(airis_uutils_ctx::stderr(), "ifne: {}: {err}", command[0].to_string_lossy());
			return 127;
		},
	};
	...
	let _ = airis_uutils_ctx::stdout().write_all(&out_buf);
	let _ = airis_uutils_ctx::stderr().write_all(&err_buf);
