...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	let buffer = match soak_stdin() {
		...
		Err(SoakError::Io(err)) => {
			let _ = writeln!(airis_uutils_ctx::stderr(), "sponge: stdin: {err}");
			return 1;
		},
	};
	...
	let Some(file) = matches.get_one::<OsString>(ARG_FILE) else {
		...
		if let Err(err) = out.write_all(&buffer).and_then(|()| out.flush()) {
			let _ = writeln!(airis_uutils_ctx::stderr(), "sponge: stdout: {err}");
			return 1;
		}
		return 0;
	};

	let target = airis_uutils_ctx::resolve(file);
	...
	match result {
		Ok(()) => 0,
		Err(err) => {
			let _ = writeln!(airis_uutils_ctx::stderr(), "sponge: {}: {err}", file.to_string_lossy());
			1
		},
	}
}
...
fn command() -> Command {
	Command::new("sponge")
		.version(concat!("sponge (airis-shell) ", env!("CARGO_PKG_VERSION")))
