...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	if args.is_empty() {
		let _ = writeln!(airis_uutils_ctx::stderr(), "errno: no errno name or number given");
		return 1;
	}
...
fn command() -> Command {
	Command::new("errno")
		.version(concat!("errno (airis-shell) ", env!("CARGO_PKG_VERSION")))
...
fn print_entry(name: &str, number: i32) {
	let _ = writeln!(airis_uutils_ctx::stdout(), "{name} {number} {}", description(number));
}
...
fn lookup(arg: &str) -> bool {
		...
		.find(|(name, _)| name.eq_ignore_ascii_case(arg))
	{
		...
		let _ = writeln!(airis_uutils_ctx::stderr(), "errno: unknown errno {arg}");
		false
	}
}
