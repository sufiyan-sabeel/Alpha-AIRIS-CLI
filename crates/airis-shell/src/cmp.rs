...
enum InputReader {
	File(File),
	Stdin(airis_uutils_ctx::CtxStdin),
	Bytes(Cursor<Vec<u8>>),
}
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match compare(&matches) {
		Ok(code) => code,
		Err(message) => {
			let _ = writeln!(airis_uutils_ctx::stderr(), "cmp: {message}");
			2
		},
	}
}
...
fn command() -> Command {
	Command::new("cmp")
		.version(concat!("cmp (airis-shell) ", env!("CARGO_PKG_VERSION")))
	...
	if name == OsStr::new("-") {
		return Ok(Input {
			reader:      BufReader::with_capacity(
				BUFFER_SIZE,
				InputReader::Stdin(airis_uutils_ctx::stdin()),
			),
			regular_len: None,
		});
	}
