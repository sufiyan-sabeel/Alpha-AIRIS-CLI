//! In-process `fd` builtin backed by `airis_walker`, `globset`, and `regex`.
...
use airis_walker::CollectedEntry;
...
fn search(
	...
	for search_path in &search_paths {
		...
		let outcome = match request.collect_with_heartbeat(cancel_heartbeat(cancelled)) {
			Ok(outcome) => outcome,
			Err(airis_walker::WalkError::Interrupted(_)) if cancelled.load(Ordering::Relaxed) => break,
...
fn fd_walk_request(
	...
	airis_walker::WalkRequest::new(root)
		...
		.order(airis_walker::WalkOrder::Path)
		.emit_root(true)
...
fn process_walker_entry<W: Write>(
	...
	path: &Path,
	depth: usize,
	file_type: airis_walker::FileType,
	out: &mut W,
	matches: &mut usize,
) -> io::Result<airis_walker::WalkDecision> {
	let is_directory = file_type == airis_walker::FileType::Dir;
...
fn matches_walker_type_filter(
	...
	let kind_matches = if filter.has_kind() {
		(filter.regular && file_type == airis_walker::FileType::File)
			|| (filter.directory && file_type == airis_walker::FileType::Dir)
			|| (filter.symlink && file_type == airis_walker::FileType::Symlink)
	} else {
		true
	};
	if !kind_matches {
...
/// closure, `airis_walker`'s per-entry heartbeat never checks the flag and a
...
fn walker_error_to_io(err: airis_walker::WalkError<io::Error>) -> io::Error {
	match err {
		airis_walker::WalkError::Interrupted(err) => err,
		airis_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
}

fn walker_collect_error_to_io(err: airis_walker::WalkError<String>) -> io::Error {
	match err {
		airis_walker::WalkError::Interrupted(err) => io::Error::other(err),
		airis_walker::WalkError::InvalidData { path, message } => {
			io::Error::other(format!("{}: {message}", path.display()))
		},
	}
...
fn process_collected_entry<W: Write>(
	...
	let is_directory = entry.file_type == airis_walker::FileType::Dir;
	if depth == 0 && is_directory {
		return Ok(());
	}
...
mod tests {
	...
	fn walk_request(tree: &std::path::Path) -> airis_walker::WalkRequest {
		airis_walker::WalkRequest::new(tree)
			.hidden(true)
			.gitignore(false)
			.emit_root(true)
	}
	...
	fn cancel_heartbeat_aborts_collect_with_heartbeat() {
		...
		assert!(
			matches!(err, airis_walker::WalkError::Interrupted(_)),
	...
	fn cancel_heartbeat_aborts_for_each_entry_with_heartbeat() {
		...
		let result = walk_request(&tree).for_each_entry_with_heartbeat(
			...
			|_entry| {
				...
				Ok::<_, std::io::Error>(airis_walker::WalkDecision::Include)
			},
			|_error| Ok::<_, std::io::Error>(airis_walker::WalkDecision::Include),
		);
