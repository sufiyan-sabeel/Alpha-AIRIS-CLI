...
//! `airis-uutils-ctx` so the builtin writes to the command's redirected file
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match diff_main(&matches) {
		Ok(code) => code,
		Err(msg) => {
			let _ = writeln!(airis_uutils_ctx::stderr(), "diff: {msg}");
			2
		},
	}
}
...
pub fn uu_app() -> Command {
	Command::new("diff")
		.version(concat!("diff (airis-uu-diff) ", env!("CARGO_PKG_VERSION")))
