...
fn collect_candidates(
	...
	let mut filter =
		airis_walker::WalkFilter::files_only().node_modules_unless_mentioned(mentions_node_modules);
	...
	let request = airis_walker::WalkRequest::new(&search_path)
		.hidden(true)
		.gitignore(true)
		.skip_git(true)
...
mod tests {
	...
	fn make_temp_tree() -> TempTree {
		let unique = SystemTime::now()
			...
			.as_nanos();
		let root = std::env::temp_dir().join(format!("airis-ast-glob-test-{unique}"));
	...
	fn make_mixed_temp_tree() -> TempTree {
		let unique = SystemTime::now()
			...
			.expect("system time should be after UNIX_EPOCH")
			.as_nanos();
		let root = std::env::temp_dir().join(format!("airis-ast-mixed-lang-test-{unique}"));
	...
	fn ast_edit_dedupes_identical_matches_across_rules() {
		let unique = SystemTime::now()
			...
			.expect("system time should be after UNIX_EPOCH")
			.as_nanos();
		let root = std::env::temp_dir().join(format!("airis-ast-dedupe-{unique}"));
	...
	fn make_apply_failure_tree() -> TempTree {
		let unique = SystemTime::now()
			...
			.expect("system time should be after UNIX_EPOCH")
			.as_nanos();
		let root = std::env::temp_dir().join(format!("airis-ast-apply-fail-{unique}"));
