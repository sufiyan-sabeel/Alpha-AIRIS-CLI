...
fn build_workspace_walk_request(config: &WorkspaceConfig) -> airis_walker::WalkRequest {
	airis_walker::WalkRequest::new(config.root.clone())
		.hidden(config.include_hidden)
		.gitignore(config.use_gitignore)
		.skip_git(true)
		.skip_node_modules(true)
		.follow_links(airis_walker::FollowLinks::Never)
		.detail(airis_walker::WalkDetail::Full)
...
fn glob_match_from_path(root: &Path, path: &Path) -> Option<GlobMatch> {
	let relative = airis_walker::normalize_relative_path(root, path);
	if relative.is_empty() {
		return None;
	}
	let (file_type, mtime, size) = airis_walker::classify_file_type(path)?;
	Some(GlobMatch {
...
pub fn list_workspace(options: ListWorkspaceOptions<'_>) -> task::Promise<ListWorkspaceResult> {
	...
	task::blocking("listWorkspace", ct, move |ct| {
		...
		run_list_workspace(
			WorkspaceConfig {
				root: airis_walker::resolve_search_path(&path).map_err(crate::iofs::map_walker_error)?,
				max_depth,
				walk_max_depth,
