...
impl From<airis_ast::block::BlockRange> for BlockRange {
	fn from(value: airis_ast::block::BlockRange) -> Self {
...
pub fn block_range_at(options: BlockRangeOptions) -> Result<Option<BlockRange>> {
	airis_ast::block::block_range_at(airis_ast::block::BlockRangeOptions {
		code: options.code,
		lang: options.lang,
		path: options.path,
		line: options.line,
...
pub fn enclosing_block_boundaries(options: EnclosingBoundaryOptions) -> Result<Option<Vec<u32>>> {
	airis_ast::block::enclosing_block_boundaries(airis_ast::block::EnclosingBoundaryOptions {
		code:   options.code,
		lang:   options.lang,
		path:   options.path,
		ranges: options
			.ranges
			.into_iter()
			.map(|range| airis_ast::block::LineRange {
