...
//! subsequence scoring. Uses `airis-walker` for directory traversal and caching.
...
fn fuzzy_find_sync(config: FuzzyFindConfig, ct: task::CancelToken) -> Result<FuzzyFindResult> {
	let root = airis_walker::resolve_search_path(&config.path).map_err(iofs::map_walker_error)?;
	...
	let outcome = airis_walker::WalkRequest::new(root)
		.hidden(include_hidden)
		.gitignore(respect_gitignore)
		.skip_git(true)
		.skip_node_modules(true)
		.follow_links(airis_walker::FollowLinks::Always)
...
mod tests {
	...
	impl TempDirGuard {
		fn new() -> Self {
			...
			let path = std::env::temp_dir().join(format!("airis-fd-test-{pid}-{nanos}-{seq}"));
	...
	fn fuzzy_find_ranks_shallow_paths_first_on_score_tie() {
		...
		fs::create_dir_all(root.path().join(".airis/skills/opt/scripts"))
