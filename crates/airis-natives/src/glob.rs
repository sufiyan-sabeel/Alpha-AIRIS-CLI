...
//!
//! # Overview
//! Resolves a search root, scans entries via `airis-walker`, applies glob matching
...
fn collect_ranked_matches(
	...
	let outcome = request
		.collect_ranked_with_heartbeat(
			airis_walker::WalkRank::MtimeDescPathAsc,
			config.max_results,
			|| ct.heartbeat(),
		)
...
fn run_glob(
	...
	let walk_glob = airis_walker::CompiledWalkGlob::new([walk_glob_pattern])
	...
	let base_request = airis_walker::WalkRequest::new(config.root.clone())
		...
		.cache(config.cache)
		.empty_recheck(airis_walker::EmptyRecheck::Configured)
		.filter(
			airis_walker::WalkFilter::all()
				.glob(walk_glob)
...
pub fn glob(
	...
	task::blocking("glob", ct, move |ct| {
		run_glob(
			GlobConfig {
				root: airis_walker::resolve_search_path(&path).map_err(iofs::map_walker_error)?,
...
mod tests {
	...
	impl TempDirGuard {
		fn new() -> Self {
			...
			let path = std::env::temp_dir().join(format!("airis-glob-test-{timestamp}-{counter}"));
