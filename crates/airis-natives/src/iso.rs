//! napi shim for the `airis-iso` PAL.
//!
//! Mirrors [`airis_iso::IsolationBackend`] across the FFI boundary:
//!
...
use airis_iso::{BackendKind, ChangeKind, Diff, FileChange, IsoError, IsolationBackend};
...
pub fn iso_probe(kind: Option<IsoBackendKind>) -> IsoProbeResult {
	...
	let backend = airis_iso::backend(resolved);
...
pub fn iso_resolve(preferred: Option<IsoBackendKind>) -> IsoResolveResult {
	let resolution = airis_iso::resolve(preferred.map(from_napi_kind));
	IsoResolveResult {
...
pub async fn iso_start(kind: Option<IsoBackendKind>, lower: String, merged: String) -> Result<()> {
	...
	tokio::task::spawn_blocking(move || airis_iso::backend(resolved).start(&lower_path, &merged_path))
		.await
...
pub async fn iso_stop(kind: Option<IsoBackendKind>, merged: String) -> Result<()> {
	let resolved = kind.map_or_else(BackendKind::native, from_napi_kind);
	let merged_path = std::path::PathBuf::from(merged);
	tokio::task::spawn_blocking(move || airis_iso::backend(resolved).stop(&merged_path))
		.await
...
pub async fn iso_diff(lower: String, merged: String) -> Result<IsoDiff> {
	...
	let backend = airis_iso::backend(BackendKind::Rcopy);
	let diff = backend
...
fn _assert_backend_object_safe() {
	...
	let backend = airis_iso::default_backend();
