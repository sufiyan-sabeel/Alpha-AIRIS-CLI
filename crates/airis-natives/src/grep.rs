...
fn build_grep_walk_request(
	...
	order: airis_walker::WalkOrder,
) -> Result<airis_walker::WalkRequest> {
	let mut filter = airis_walker::WalkFilter::files_only();
	if let Some(glob) = glob.map(str::trim).filter(|value| !value.is_empty()) {
		let pattern = glob_util::build_glob_pattern(glob, true);
		let compiled = airis_walker::CompiledWalkGlob::new([pattern])
			.map_err(|err| Error::from_reason(format!("Invalid glob pattern: {err}")))?;
		filter = filter.glob(compiled);
	}

	Ok(airis_walker::WalkRequest::new(search_path)
		.hidden(include_hidden)
		.gitignore(use_gitignore)
		.skip_git(true)
		.skip_node_modules(skip_node_modules)
		.follow_links(airis_walker::FollowLinks::Never)
		.detail(airis_walker::WalkDetail::Minimal)
		.size_hints(airis_walker::SizeHintPolicy::WhenCheap)
		.order(order)
		.emit_root(false)
		.depth(1, usize::MAX)
		.directory_errors(airis_walker::DirectoryErrorMode::SkipSkippable)
		.cache(false)
		.filter(filter))
}
...
fn collect_grep_candidates(
	...
	order: airis_walker::WalkOrder,
	ct: &task::CancelToken,
) -> Result<Option<Vec<airis_walker::FileCandidate>>> {
...
struct PassState {
	...
	deferred:          Mutex<Vec<airis_walker::FileCandidate>>,
...
fn search_one_file<M: Matcher + Sync>(
	...
	matcher: &M,
	file: &airis_walker::FileCandidate,
...
fn run_pass<M: Matcher + Sync>(
	candidates: &[airis_walker::FileCandidate],
	matcher: &M,
	file_params: SearchParams,
	policy: ReadPolicy,
	...
	if parallel_allowed && airis_walker::should_parallelize(candidates.len()) {
		airis_walker::execute_candidates_init(
			candidates,
...
fn process_candidates<M: Matcher + Sync>(
	candidates: Vec<airis_walker::FileCandidate>,
	matcher: &M,
	params: SearchParams,
	parallel_allowed: bool,
	stop_after_matches: Option<u64>,
	...
	request
		.for_each_file_candidate_parallel(
			|file| {
				...
				{
					return Ok(airis_walker::ParallelWalkControl::Continue);
				}
				...
				Ok(airis_walker::ParallelWalkControl::Continue)
			},
			|| ct.heartbeat(),
		)
...
fn flush_stream_window<M: Matcher + Sync>(
	window: &mut Vec<airis_walker::FileCandidate>,
	...
	request
		.for_each_entry_with_heartbeat(
			...
			|entry| {
				...
				window.push(airis_walker::FileCandidate {
			...
			|_| Ok(airis_walker::WalkDecision::Include),
		)
		.map_err(iofs::map_walker_error)?;
...
fn run_streaming_grep<M: Matcher + Sync>(
	...
	match stop_after_matches {
		...
		Some(stop) if stop <= ORDERED_STREAMING_STOP_MAX_COUNT || airis_walker::walk_workers() <= 1 => {
			run_sequential_grep(
				search_path,
				matcher,
				glob,
...
mod tests {
	...
	impl TempDirGuard {
		fn new() -> Self {
			...
			let path = std::env::temp_dir().join(format!("airis-grep-test-{pid}-{nanos}-{seq}"));
