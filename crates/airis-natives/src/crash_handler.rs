...
//! native module loads, so any crash inside `airis-natives` writes an actionable
...
//!   `$XDG_STATE_HOME/airis/logs/` on Linux / macOS when the user has migrated to
//!   XDG (i.e. that directory already exists and `PI_CODING_AGENT_DIR` isn't
//!   pointed somewhere custom), otherwise `<home>/<AIRIS_CONFIG_DIR>/logs/`
//!   (defaulting to `~/.airis/logs/`).
...
/// `AIRIS_CONFIG_DIR`, matching `packages/utils/src/dirs.ts`).
const DEFAULT_CONFIG_DIR: &str = ".airis";

/// App name used as the XDG-root subdirectory (`$XDG_STATE_HOME/airis/`),
/// matching `APP_NAME` in `packages/utils/src/dirs.ts`.
#[cfg(any(target_os = "linux", target_os = "macos"))]
const APP_NAME: &str = "airis";
...
fn panic_disposition() -> PanicDisposition {
	if blocking_task_panic_scope_active() || airis_uutils_ctx::is_active() {
		PanicDisposition::LoggedRecoverable
	} else {
		PanicDisposition::Fatal
	}
...
fn persist(report: &str, kind: CrashKind, echo_stderr: bool) {
	...
	if let Ok(mut f) = OpenOptions::new().create(true).append(true).open(&path) {
		...
		if echo_stderr {
			let _ =
				writeln!(std::io::stderr(), "airis-natives crash report written to {}", path.display());
		}
	}
}
...
fn logs_dir() -> Option<PathBuf> {
	let home = home_dir()?;
	let config_override = std::env::var_os("AIRIS_CONFIG_DIR");
...
fn resolve_logs_dir(
	...
	// XDG takes precedence so users who migrated to `$XDG_STATE_HOME/airis/logs/`
...
/// `$XDG_STATE_HOME` set, `$XDG_STATE_HOME/airis` exists on disk, and
...
/// `<xdg_state_home>/airis` actually lives on disk.
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn xdg_state_logs(
...
mod tests {
	...
	fn alloc_report_contains_size_alignment_and_backtrace() {
		...
		let report = format_alloc_report(layout);
		assert!(report.contains("airis-natives alloc crash"), "report missing header: {report}");
	...
	fn resolve_logs_dir_defaults_under_dot_omp() {
		let dir = resolve_logs_dir(Path::new("/tmp/airis-natives-test-home"), None, None);
		assert_eq!(dir, PathBuf::from("/tmp/airis-natives-test-home/.airis/logs"));
	}

	#[test]
	fn resolve_logs_dir_honors_relative_airis_config_dir() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/airis-natives-test-home"),
			Some(OsStr::new(".airis-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/airis-natives-test-home/.airis-dev/logs"));
	}

	#[test]
	fn resolve_logs_dir_reroots_absolute_airis_config_dir_under_home() {
		// JS resolves the config root via `path.join(os.homedir(),
		// getConfigDirName())`, which never honors an absolute AIRIS_CONFIG_DIR — it is
		// always re-rooted under `$HOME` (and `..` components are normalized away).
		let dir = resolve_logs_dir(
			Path::new("/tmp/airis-natives-test-home"),
			Some(OsStr::new("/var/tmp/airis-natives-state")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/airis-natives-test-home/var/tmp/airis-natives-state/logs"));
	}
	...
	fn resolve_logs_dir_normalizes_parent_components_like_path_join() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/airis-natives-test-home"),
			Some(OsStr::new("nested/../.airis-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/airis-natives-test-home/.airis-dev/logs"));
	}
	...
	fn xdg_state_logs_ignores_empty_agent_dir_override() {
		...
		let dir = xdg_state_logs(
			...
			Some(OsStr::new("")),
			Path::new("/tmp/airis-natives-test-home/.airis/agent"),
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/airis/logs")));
	}

	#[test]
	fn resolve_logs_dir_ignores_empty_airis_config_dir() {
		let dir =
			resolve_logs_dir(Path::new("/tmp/airis-natives-test-home"), Some(OsStr::new("")), None);
		assert_eq!(dir, PathBuf::from("/tmp/airis-natives-test-home/.airis/logs"));
	}
	...
	fn resolve_logs_dir_prefers_xdg_when_provided() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/airis-natives-test-home"),
			None,
			Some(PathBuf::from("/xdg/state/airis/logs")),
		);
		assert_eq!(dir, PathBuf::from("/xdg/state/airis/logs"));
	}
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn xdg_state_logs_skipped_when_airis_dir_missing() {
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			None,
			Path::new("/tmp/airis-natives-test-home/.airis/agent"),
	...
	fn xdg_state_logs_skipped_when_xdg_state_home_unset_or_empty() {
		let default_agent = Path::new("/tmp/airis-natives-test-home/.airis/agent");
	...
	fn xdg_state_logs_honored_when_agent_override_matches_default() {
		let default_agent = std::path::absolute(Path::new("./.airis/agent")).unwrap();
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			Some(OsStr::new("./.airis/agent")),
			&default_agent,
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/airis/logs")));
	...
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_uses_dot_airis_by_default() {
		let dir = default_agent_dir(Path::new("/tmp/airis-natives-test-home"), None);
		assert_eq!(dir, PathBuf::from("/tmp/airis-natives-test-home/.airis/agent"));
	}
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_respects_airis_config_dir() {
		let dir =
			default_agent_dir(Path::new("/tmp/airis-natives-test-home"), Some(OsStr::new(".airis-dev")));
		assert_eq!(dir, PathBuf::from("/tmp/airis-natives-test-home/.airis-dev/agent"));
	}

	#[test]
	fn build_crash_log_path_tags_kind_and_pid() {
		let dir = Path::new("/tmp/airis-natives-test-home/.airis/logs");
		let panic_log = build_crash_log_path(dir, CrashKind::Panic, 4242, 1_700_000_000_000);
		assert_eq!(
			panic_log,
			PathBuf::from("/tmp/airis-natives-test-home/.airis/logs/native-panic-4242-1700000000000.log")
		);
		let alloc_log = build_crash_log_path(dir, CrashKind::Alloc, 99, 1);
		assert_eq!(
			alloc_log,
			PathBuf::from("/tmp/airis-natives-test-home/.airis/logs/native-alloc-99-1.log")
		);
	}
}
