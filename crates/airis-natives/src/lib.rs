//! Native utilities exported via N-API for the Alpha AIRIS-CLI toolchain.
//!
//! # Overview
...
//! # Example
//! ```ignore
//! use airis_natives::text::visible_width;
//!
...
fn configure_rayon_pool() {
	...
	let result = match plan {
		RayonPoolPlan::WorkerThreads(threads) => rayon::ThreadPoolBuilder::new()
			...
			.build_global(),
		RayonPoolPlan::SkipGlobalPool => {
			airis_uutils_ctx::set_rayon_global_pool_available(false);
			return;
		},
	};
	if result.is_ok() {
		airis_uutils_ctx::set_rayon_global_pool_available(true);
	}
}
...
pub const fn airis_natives_version_sentinel() {}
...
/// calls. Idempotent.
#[napi(js_name = "__airisInstallTokioRuntime")]
