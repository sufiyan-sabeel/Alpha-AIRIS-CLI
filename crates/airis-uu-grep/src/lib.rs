...
//! with directory recursion via `airis-walker` and `--include` filtering via
//! `globset`. All I/O and path resolution is routed through `airis-uutils-ctx` so
...
#[command(
	name = "grep",
	version = concat!("grep (airis-uu-grep) ", env!("CARGO_PKG_VERSION")),
...
fn resolve_follow_links(cli: &Cli, matches: &ArgMatches) -> airis_walker::FollowLinks {
	let mut selected = (0, airis_walker::FollowLinks::Roots);
	choose_latest(&mut selected, last_index(matches, "recursive"), airis_walker::FollowLinks::Roots);
	choose_latest(
		&mut selected,
		last_index(matches, "dereference_recursive"),
		airis_walker::FollowLinks::Always,
	);
	if cli.directories == Some(DirectoryAction::Recurse) {
		choose_latest(
			&mut selected,
			last_index(matches, "directories"),
			airis_walker::FollowLinks::Roots,
		);
	}
	choose_latest(
		&mut selected,
	...
	choose_latest(&mut selected, last_index(matches, "no_follow"), airis_walker::FollowLinks::Never);
	choose_latest(&mut selected, last_index(matches, "follow_all"), airis_walker::FollowLinks::Always);
	selected.1
}
...
fn grep_walk_request(root: &Path, follow_links: airis_walker::FollowLinks) -> airis_walker::WalkRequest {
	airis_walker::WalkRequest::new(root)
		...
		.order(airis_walker::WalkOrder::Unordered)
		.emit_root(true)
		.depth(0, usize::MAX)
		...
		.cache(false)
		.filter(airis_walker::WalkFilter::all())
}
...
fn search_dir<M: Matcher, W: Write>(
	...
	opts: &Options,
	rules: &PathRules,
	follow_links: airis_walker::FollowLinks,
	out: &mut W,
	had_error: &mut bool,
) -> bool {
	...
	let walk = request.for_each_entry_with_heartbeat(
		...
		|entry: airis_walker::EntryMeta<'_>| {
			...
			if entry.file_type == airis_walker::FileType::Dir {
				if entry.depth > 0 && !rules.allows_dir(Path::new(entry.relative_path)) {
					return Ok(airis_walker::WalkDecision::SkipDescend);
				}
				return Ok(airis_walker::WalkDecision::Include);
			}
		...
		|error: airis_walker::DirectoryError<'_>| {
	...
	*had_error |= had_error_state.get();
	match walk {
		Ok(airis_walker::WalkStatus::Complete | airis_walker::WalkStatus::Stopped) => any,
		...
		Err(airis_walker::WalkError::Interrupted(error)) => {
			*had_error = true;
			if !opts.no_messages {
				let _ = writeln!(airis_uutils_ctx::stderr(), "grep: {error}");
			}
			any
		},
...
fn read_auxiliary_file(path: &OsStr) -> Result<Vec<u8>, String> {
	...
	let result = if path == OsStr::new("-") {
		airis_uutils_ctx::stdin().read_to_end(&mut bytes)
	} else {
		File::open(airis_uutils_ctx::resolve(path)).and_then(|mut file| file.read_to_end(&mut bytes))
	};
	result
		.map(|_| bytes)
	...
	for operand in files {
		...
		if operand == OsStr::new("-") {
			...
			match process_reader(
				...
				Err(error) => {
					had_error = true;
					if !opts.no_messages {
						let _ = writeln!(airis_uutils_ctx::stderr(), "grep: (standard input): {error}");
					}
				},
			}
			if airis_uutils_ctx::is_cancelled() {
		...
		match std::fs::metadata(&resolved) {
			...
			Err(error) => {
				...
				if !opts.no_messages {
					let _ =
						writeln!(airis_uutils_ctx::stderr(), "grep: {}: {error}", operand.to_string_lossy());
				}
			},
		}
		if airis_uutils_ctx::is_cancelled() {
...
mod tests {
	...
	fn version_flag_prints_and_exits_zero() {
		...
		assert!(
			stdout.contains("grep") && stdout.contains("airis-uu-grep"),
	...
	fn recursive_search_observes_scope_cancellation() {
		...
		// airis_walker, so directory walks ignored the uutils cancel flag and the
