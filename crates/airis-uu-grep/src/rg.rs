...
enum RgOutput {
	Buffered(BufWriter<airis_uutils_ctx::CtxStdout>),
	Direct(airis_uutils_ctx::CtxStdout),
}
...
struct RgWalk {
	request: airis_walker::WalkRequest,
	filters: PathFilters,
}
...
impl PathFilters {
	fn includes(&self, path: &Path, file_type: airis_walker::FileType, size: Option<f64>) -> bool {
		let is_dir = file_type == airis_walker::FileType::Dir;
		...
		if file_type != airis_walker::FileType::File {
			return true;
		}
		if !explicitly_included
			&& self
...
fn build_walk(cli: &RgCli, root: &Path) -> Result<RgWalk, String> {
	...
	let request = airis_walker::WalkRequest::new(root)
		.hidden(include_hidden)
		.gitignore(!no_ignore)
		.skip_git(!no_ignore)
		.skip_node_modules(false)
		.follow_links(airis_walker::FollowLinks::from(cli.follow && !cli.no_follow))
		.detail(if filters.max_filesize.is_some() {
			airis_walker::WalkDetail::Full
		} else {
			airis_walker::WalkDetail::Minimal
		})
		.order(order)
		.emit_root(false)
		.depth(1, cli.max_depth.unwrap_or(usize::MAX))
		.visit_order(airis_walker::VisitOrder::PreOrder)
...
fn report_path_error(
	...
	if !opts.no_messages {
		...
		let _ = writeln!(airis_uutils_ctx::stderr(), "rg: {name}: {err}");
	}
	true
}
...
fn search_collected_files<M: Matcher, W: Write>(
	...
	let mut files = match collect_filtered_files(cli, root) {
		Ok(files) => files,
		Err(_) if airis_uutils_ctx::is_cancelled() => {
			return SearchOutcome { any_match: false, had_error: true };
		},
		Err(err) => {
			if !opts.no_messages {
				let _ = writeln!(airis_uutils_ctx::stderr(), "{err}");
			}
			return SearchOutcome { any_match: false, had_error: true };
		},
	};
	...
	for path in files {
		...
		if processed_file && airis_uutils_ctx::is_cancelled() {
			had_error = true;
			break;
		}
		processed_file = true;
	...
	let streamed = match walk.request.for_each_entry_with_heartbeat(
		...
		|entry| {
			if opts.quiet && any_match.get() {
				return Ok(airis_walker::WalkDecision::Stop);
			}
			let path = entry.absolute_path.as_ref();
			if !walk.filters.includes(path, entry.file_type, entry.size) {
				return Ok(if entry.file_type == airis_walker::FileType::Dir {
					airis_walker::WalkDecision::SkipDescend
				} else {
					airis_walker::WalkDecision::Skip
				});
			}
			if entry.file_type != airis_walker::FileType::File {
				return Ok(airis_walker::WalkDecision::Skip);
			}
			let display_path = display_path(operand, root, path);
			let display_bytes = display_path.as_os_str().as_encoded_bytes().to_vec();
			let display = (show_names || opts.json).then_some(display_bytes.as_slice());
			...
			Ok(if opts.quiet && any_match.get() {
				airis_walker::WalkDecision::Stop
			} else {
				airis_walker::WalkDecision::Include
			})
		},
		|error| {
			had_error.set(true);
			if !opts.no_messages {
				let _ =
					writeln!(airis_uutils_ctx::stderr(), "rg: {}: {}", error.path.display(), error.error);
			}
		...
		Err(airis_walker::WalkError::Interrupted(_)) if airis_uutils_ctx::is_cancelled() => {
		...
		Err(err) => {
			had_error.set(true);
			if !opts.no_messages {
				let _ = writeln!(airis_uutils_ctx::stderr(), "rg: {err}");
			}
			Some(SearchOutcome { any_match: any_match.get(), had_error: had_error.get() })
		},
	};
...
fn list_files<W: Write>(cli: &RgCli, paths: &[OsString], out: &mut W) -> SearchOutcome {
	...
	for operand in paths {
		if processed_operand && airis_uutils_ctx::is_cancelled() {
			had_error = true;
			break;
		}
		processed_operand = true;
		let resolved = airis_uutils_ctx::resolve(operand);
		match std::fs::metadata(&resolved) {
			...
			Ok(_) => {},
			Err(err) => {
				let _ = writeln!(airis_uutils_ctx::stderr(), "rg: {}: {err}", operand.to_string_lossy());
				had_error = true;
			},
		}
		if airis_uutils_ctx::is_cancelled() {
...
fn execute_search<M: Matcher, W: Write>(
	...
	let mut auto_searcher = match build_searcher(cli, opts, BinaryMode::Automatic) {
		...
		Err(error) => {
			let _ = writeln!(airis_uutils_ctx::stderr(), "rg: {error}");
			return 2;
		},
	};
	...
	let recursive = paths.iter().any(|path| {
		path.as_os_str() != OsStr::new("-")
			&& std::fs::metadata(airis_uutils_ctx::resolve(path)).is_ok_and(|meta| meta.is_dir())
	});
	...
	for operand in paths {
		...
		if operand.as_os_str() == OsStr::new("-") {
			...
			match process_reader(
				...
				Err(error) => {
					had_error = true;
					if !opts.no_messages {
						let _ = writeln!(airis_uutils_ctx::stderr(), "rg: <stdin>: {error}");
					}
				},
			}
			if airis_uutils_ctx::is_cancelled() {
		...
		match std::fs::metadata(&resolved) {
			...
			Err(error) => {
				...
				if !opts.no_messages {
					let _ =
						writeln!(airis_uutils_ctx::stderr(), "rg: {}: {error}", operand.to_string_lossy());
				}
			},
		}
		if airis_uutils_ctx::is_cancelled() {
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	if opts.json
		&& (cli.files
			...
			|| cli.vimgrep)
	{
		let _ = writeln!(airis_uutils_ctx::stderr(), "rg: --json cannot be combined with summary modes");
		return 2;
	}
	let mut out = if cli.line_buffered && !cli.no_line_buffered {
		RgOutput::Direct(airis_uutils_ctx::stdout())
	} else {
		RgOutput::Buffered(BufWriter::new(airis_uutils_ctx::stdout()))
	};
	let (patterns, mut paths) = match resolve_patterns(&cli) {
		Ok(resolved) => resolved,
		Err(error) => {
			let _ = writeln!(airis_uutils_ctx::stderr(), "{error}");
			return 2;
		},
	};
	if cli.type_list {
	...
	default_paths(
		&mut paths,
		!cli.files && !pattern_stdin_consumed && airis_uutils_ctx::stdin_is_search_input(),
	);
	if cli.files {
...
mod tests {
	...
	use airis_uutils_ctx::{ScopeIo, scope};
	...
	fn recursive_search_observes_scope_cancellation() {
		...
		// heartbeat to airis_walker, so cancellation was not observed during
