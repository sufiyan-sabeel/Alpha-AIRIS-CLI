...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	let matches = match uu_app().try_get_matches_from(args) {
		...
		Err(clap_error) => {
			...
			if let Some(message) = override_clap_error(&clap_error) {
				let _ = writeln!(airis_uutils_ctx::stderr(), "uniq: {message}");
				return 1;
			}
	...
	match uniq_main(&matches, skip_fields_old, skip_chars_old) {
		...
		Err(err) => {
			let code = err.code();
			let _ = writeln!(airis_uutils_ctx::stderr(), "uniq: {err}");
...
fn open_input_file(in_file_name: Option<&OsStr>) -> UResult<Box<dyn BufRead>> {
	Ok(match in_file_name {
		Some(path) if path != "-" => {
			...
			let in_file = File::open(airis_uutils_ctx::resolve(path))
				.map_err_context(|| format!("Could not open {}", path.maybe_quote()))?;
			Box::new(BufReader::new(in_file))
		},
		// airis-uutils: stdin is a plain context reader; wrap for BufRead.
		_ => Box::new(BufReader::new(airis_uutils_ctx::stdin())),
	})
}
...
fn open_output_file(out_file_name: Option<&OsStr>) -> UResult<Box<dyn Write>> {
	Ok(match out_file_name {
		Some(path) if path != "-" => {
			// airis-uutils: resolve the operand against the shell working directory.
			let out_file = File::create(airis_uutils_ctx::resolve(path))
				.map_err_context(|| format!("Could not open {}", path.maybe_quote()))?;
			Box::new(BufWriter::with_capacity(OUTPUT_BUFFER_CAPACITY, out_file))
		},
		// airis-uutils: stdout is routed through the context.
		_ => Box::new(BufWriter::with_capacity(OUTPUT_BUFFER_CAPACITY, airis_uutils_ctx::stdout())),
	})
}
