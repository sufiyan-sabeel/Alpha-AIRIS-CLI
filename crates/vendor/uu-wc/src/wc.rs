...
impl<'a> Input<'a> {
	...
	fn try_as_files0(&self) -> UResult<Option<Vec<Input<'static>>>> {
		match self {
			Self::Path(path) => match fs::metadata(airis_uutils_ctx::resolve(path)) {
...
// airis-uutils: the context stdin is a plain streaming reader with no fd, so we
...
pub fn run(args: Vec<OsString>) -> i32 {
	...
	match run_matches(&matches) {
		...
		Err(err) => {
			let code = err.code();
			let _ = writeln!(airis_uutils_ctx::stderr(), "wc: {err}");
...
>(
	...
	let posixly_correct = airis_uutils_ctx::var("POSIXLY_CORRECT").is_some();
...
fn word_count_from_input(input: &Input<'_>, settings: &Settings) -> CountResult {
	let (total, maybe_err) = match input {
		Input::Stdin(_) => word_count_from_reader(airis_uutils_ctx::stdin(), settings),
		Input::Path(path) => match File::open(airis_uutils_ctx::resolve(path)) {
		...
		Inputs::Paths(inputs) => {
			...
			for input in inputs {
				match input {
					...
					Input::Path(path) => {
						if let Ok(meta) = fs::metadata(airis_uutils_ctx::resolve(path)) {
							if meta.is_file() {
								total += meta.len();
							} else {
...
fn files0_iter_stdin<'a>() -> impl Iterator<Item = InputIterItem<'a>> {
	files0_iter(airis_uutils_ctx::stdin(), STDIN_REPR.into()).map(|i| match i {
...
/// airis-uutils: replacement for uucore's process-global recoverable-error macro.
...
macro_rules! record_error {
	($err:expr) => {{
		...
		let _ = writeln!(airis_uutils_ctx::stderr(), "wc: {e}");
	}};
}
...
fn wc(inputs: &Inputs, settings: &Settings) -> UResult<()> {
	...
	if settings.debug {
		...
		if enabled_empty && !runtime_disabled {
			let _ =
				writeln!(airis_uutils_ctx::stderr(), "debug: hardware support unavailable on this CPU");
...
fn print_stats(
	...
	let mut stdout = airis_uutils_ctx::stdout().lock();
