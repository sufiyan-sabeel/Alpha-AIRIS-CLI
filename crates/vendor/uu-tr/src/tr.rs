...
/// airis-uutils: context-safe in-process entry point. `argv` includes the command
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match tr_main(&matches) {
		...
		Err(err) => {
			let code = err.code();
			let _ = writeln!(airis_uutils_ctx::stderr(), "tr: {err}");
...
fn tr_main(matches: &clap::ArgMatches) -> UResult<()> {
	...
	// airis-uutils: replace process-global stdin/stdout with the invocation context.
	let mut input = BufReader::new(airis_uutils_ctx::stdin());
	let mut output = airis_uutils_ctx::stdout();
