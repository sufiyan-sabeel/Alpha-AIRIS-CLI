...
// `airis_uutils_ctx`, `translate!` strings are literalized, POSIXLY_CORRECT is
...
pub fn run(argv: Vec<OsString>) -> i32 {
	...
	match readlink_main(&matches) {
		...
		Err(err) => {
			let code = err.code();
			// airis-uutils: silent failures surface as bare exit-code errors that
			// render to an empty message (upstream prints nothing for them);
			// don't emit a dangling "readlink: " prefix.
			let msg = err.to_string();
			if !msg.is_empty() {
				let _ = writeln!(airis_uutils_ctx::stderr(), "readlink: {msg}");
			}
...
fn readlink_main(matches: &ArgMatches) -> UResult<()> {
	...
	// airis-uutils: POSIXLY_CORRECT comes from the scope environment (the shell's
	// exported variables), not the host process environment.
	let verbose = matches.get_flag(OPT_VERBOSE) || airis_uutils_ctx::var("POSIXLY_CORRECT").is_some();
	...
	for p in &files {
		...
		let resolved = airis_uutils_ctx::resolve(p);
		...
		match path_result {
			...
			Err(err) => {
				...
				let _ = writeln!(airis_uutils_ctx::stderr(), "readlink: {message}");
				return Err(1.into());
			},
		}
	}
...
/// airis-uutils: replacement for upstream's `show` — writes the resolved path
...
mod tests {
	...
	fn resolves_relative_operand_against_scope_cwd() {
		...
		// call-site `airis_uutils_ctx::resolve` patch makes this find the link.
