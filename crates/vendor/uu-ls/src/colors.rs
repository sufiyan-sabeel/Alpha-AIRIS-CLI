...
impl<'a> StyleManager<'a> {
	...
	fn raw_indicator_style_for_path(&self, path: &PathData) -> Option<RawIndicatorStyle> {
		...
		let should_skip = indicator == Indicator::SymbolicLink
			...
			&& airis_uutils_ctx::resolve(path.path()).exists();
	...
	fn color_symlink_name(
		...
		let mut target = airis_uutils_ctx::resolve(path.path()).read_link().ok()?;
		...
		match fs::metadata(airis_uutils_ctx::resolve(&target)) {
			Ok(metadata) => {
				let style = self
					.colors
	...
	fn indicator_for_raw_code(&self, path: &PathData) -> Option<Indicator> {
		...
		let mut entry_exists = || -> bool {
			*existence_cache.get_or_insert_with(|| airis_uutils_ctx::resolve(path.path()).exists())
		};
...
pub(crate) fn color_name(
	...
	let md_option: Option<Metadata> = path
		.metadata()
		.cloned()
		.or_else(|| airis_uutils_ctx::resolve(&path.p_buf).symlink_metadata().ok());
...
pub(crate) fn validate_ls_colors_env() -> Result<(), LsColorsParseError> {
	let Some(ls_colors) = airis_uutils_ctx::var("LS_COLORS") else {
		return Ok(());
	};
...
fn parse_indicator_codes() -> (FxHashMap<Indicator, String>, bool) {
	...
	if let Some(ls_colors) = airis_uutils_ctx::var("LS_COLORS") {
