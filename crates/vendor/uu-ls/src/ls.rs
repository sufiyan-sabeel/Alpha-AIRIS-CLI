...
use airis_uutils_ctx::{CtxStdout, format_usage};
#[cfg(unix)]
...
enum LsError {
	...
	#[error("{}", match .1.kind() {
        ...
        ErrorKind::PermissionDenied => match .1.raw_os_error().unwrap_or(1) {
            ...
            _ => if airis_uutils_ctx::resolve(.0).is_dir() {
...
/// airis-uutils: in-process replacement for uucore's process-global error
...
pub(crate) fn show_ls_error(err: LsError) {
	...
	let _ = writeln!(airis_uutils_ctx::stderr(), "ls: {err}");
}
...
impl<'a> PathData<'a> {
	fn new(
		...
		let must_dereference = match &config.dereference {
			...
			Dereference::DirArgs => {
				if command_line {
					if let Ok(md) = airis_uutils_ctx::resolve(&p_buf).metadata() {
						md.is_dir()
					} else {
						false
					}
		...
		let de: RefCell<Option<DirEntry>> = if let Some(de) = dir_entry {
			if must_dereference && let Ok(md_pb) = airis_uutils_ctx::resolve(&p_buf).metadata() {
			...
			.get_or_init(|| {
				...
				match get_metadata_with_deref_opt(self.path(), self.must_dereference) {
					Err(err) => {
						...
						let _ = airis_uutils_ctx::stdout().flush();
						...
						if self.must_dereference
							&& errno == 9i32
							&& let Ok(file) = airis_uutils_ctx::resolve(self.path()).read_link()
						{
...
pub fn list(locs: Vec<&Path>, config: &Config) -> UResult<()> {
	...
	let mut state = ListState {
		out: BufWriter::new(airis_uutils_ctx::stdout()),
	...
	for (pos, path_data) in dirs.iter().enumerate() {
		...
		let read_dir = match fs::read_dir(airis_uutils_ctx::resolve(path_data.path())) {
			Err(err) => {
		...
		state.listed_ancestors.insert(FileInformation::from_path(
			airis_uutils_ctx::resolve(path_data.path()),
		...
		while let Some(dir_data) = state.stack.pop() {
			let read_dir = match fs::read_dir(airis_uutils_ctx::resolve(&dir_data.0)) {
				Err(err) => {
					// flush stdout buffer before the error to preserve formatting and order
					state.out.flush()?;
					show_ls_error(LsError::IOErrorContext(
...
fn depth_first_list(
	...
	if config.recursive {
			...
			.rev()
		{
			...
			if let Err(err) = fs::read_dir(airis_uutils_ctx::resolve(e.path())) {
				state.out.flush()?;
				show_ls_error(LsError::IOErrorContext(e.path().to_path_buf(), err, e.command_line));
			} else {
				let fi =
					FileInformation::from_path(airis_uutils_ctx::resolve(e.path()), e.must_dereference)?;
...
fn get_metadata_with_deref_opt(p_buf: &Path, dereference: bool) -> std::io::Result<Metadata> {
	let resolved = airis_uutils_ctx::resolve(p_buf);
	if dereference {
		resolved.metadata()
	} else {
...
fn get_security_context<'a>(
	...
	if config.selinux_supported {
		match selinux::SecurityContext::of_path(airis_uutils_ctx::resolve(path), must_dereference, false)
		{
			Err(_r) => {
	...
	if config.smack_supported {
		...
		let target_path = if must_dereference {
			fs::canonicalize(airis_uutils_ctx::resolve(path)).unwrap_or_else(|_| path.to_path_buf())
		} else {
			path.to_path_buf()
		};
