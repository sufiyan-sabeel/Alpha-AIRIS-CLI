//! Ignored deterministic timing harness for airis-walker.
//!
//! Run with:
//! cargo test --profile ci -p airis-walker --test perf -- --ignored --nocapture
//! --test-threads=1
...
use airis_walker::{WalkDetail, WalkOrder, WalkRequest};
...
#[test]
#[ignore = "run with: cargo test --profile ci -p airis-walker --test perf -- --ignored --nocapture \
            --test-threads=1"]
...
fn build_synthetic_tree() -> PathBuf {
	let root = unique_temp_root("airis-walker-perf");
