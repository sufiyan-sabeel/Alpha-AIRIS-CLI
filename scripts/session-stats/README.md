...
(`~/.airis/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.airis/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.airis/agent/sessions/ and populates ss_* tables
...
The LLM phase (default `anthropic/claude-sonnet-4-6` via `@airis/airis-ai`,
credentials resolved through airis's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.airis/stats-audit-cache.json` (keyed by session id +
