...
const linkScript = path.join(repoRoot, "scripts", "link-airis.sh");
const targetWrapper = path.join(repoRoot, "packages", "coding-agent", "scripts", "airis");
const tempDirs: string[] = [];

function makeTempDir() {
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), "airis-link-"));
	tempDirs.push(dir);
	return dir;
}
...
describe("scripts/link-airis.sh", () => {
	it("falls back to BUN_INSTALL/bin when bun cannot resolve the global bin", () => {
		...
		expect(fs.readlinkSync(path.join(bunInstall, "bin", "airis"))).toBe(targetWrapper);
	});
	...
	it("uses bun pm -g bin when it succeeds", () => {
		...
		expect(result.status).toBe(0);
		expect(result.stderr).toBe("");
		expect(fs.readlinkSync(path.join(globalBin, "airis"))).toBe(targetWrapper);
		expect(fs.existsSync(path.join(dir, "unused-bun-install", "bin", "airis"))).toBe(false);
	});
});
