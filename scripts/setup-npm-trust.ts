...
const FALLBACK_REPO = "sufiyan-sabeel/Alpha-AIRIS-CLI";
	...
	for (const pkg of packages) {
		...
		if (pkg.kind === "native") {
			for (const target of LEAF_TARGETS) {
				const leaf = `@airis/airis-natives-${target.tag}`;
				if (!seen.has(leaf)) {
					seen.add(leaf);
					names.push(leaf);
				}
...
function nativeLeafName(tag: string): string {
	return `@airis/airis-natives-${tag}`;
}
...
function placeholderManifest(name: string, target: NativeLeafTarget, repo: string): PlaceholderManifest {
	return {
		...
		description: `Placeholder for the ${target.tag} native addon of @airis/airis-natives. The real binary is published during release.`,
		license: "MIT",
		os: [target.os],
		cpu: [target.cpu],
		repository: {
...
function placeholderReadme(name: string, target: NativeLeafTarget): string {
	return [
		`# ${name}`,
		"",
		`Placeholder package reserving the npm name for the \`${target.tag}\` native addon of \`@airis/airis-natives\`.`,
		"",
...
async function publishNativeLeafPlaceholder(name: string, target: NativeLeafTarget, repo: string): Promise<boolean> {
	const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-native-placeholder-"));
	try {
		await Bun.write(
