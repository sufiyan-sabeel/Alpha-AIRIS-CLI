 ...
 *
 * The default mode publishes public JS packages and the `@airis/airis-natives`
...
export const packages: PublishPackage[] = [
	...
	{ dir: "packages/coding-agent", kind: "typescript", publishBin: { airis: "dist/cli.js" } },
];
...
function buildNativeOptionalDependencies(version: string): JsonObject {
	...
	for (const target of LEAF_TARGETS) {
		optionalDependencies[`@airis/airis-natives-${target.tag}`] = version;
	}
...
async function packAndPublish(dir: string, name: string): Promise<void> {
	...
	const packDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-pack-"));
	try {
