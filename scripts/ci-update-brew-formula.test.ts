...
const SUMS = {
	"airis-darwin-arm64": "darwin_arm64_sha",
	"airis-darwin-x64": "darwin_x64_sha",
	"airis-linux-arm64": "linux_arm64_sha",
	"airis-linux-x64": "linux_x64_sha",
};
...
describe("renderFormula", () => {
	...
	// outside the staging CWD, `Dir["airis-*"].first` returns `nil`, and
	// `bin.install nil => "airis"` raises (issue #2398).
	it("attaches `using: :nounzip` to every per-platform url stanza", () => {
		const matches = formula.match(/using: :nounzip/g) ?? [];
		expect(matches).toHaveLength(4);
		for (const arch of ["airis-darwin-arm64", "airis-darwin-x64", "airis-linux-arm64", "airis-linux-x64"]) {
	...
	// popened binary doesn't touch the real `~/.airis` (denied by Homebrew's
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"airis", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
