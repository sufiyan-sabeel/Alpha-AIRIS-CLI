...
```bash
gh_PR=<NUMBER>
# pr_checkout creates ~/.airis/wt/<encoded-repo>/pr-<N>/ and configures push remote
```
...
```bash
MAIN="<absolute path to main worktree, e.g. ~/Projects/alpha-airis-cli>"
WT="$(pwd)"
...
Worktree: ~/.airis/wt/.../pr-<N>   (or: not checked out)
