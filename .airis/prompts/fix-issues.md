...
2. `gh search prs` for the issue number to see if a fix is already in flight.
   - If a PR exists and looks reasonable → switch tracks: review that PR per `.airis/commands/review-prs.md` instead, and report back as `existing-pr`. Do **not** open a competing fix.
...
WT="$HOME/.airis/wt/${ENC}/fix-issue-<N>"

git -C "$MAIN" fetch origin main
git -C "$MAIN" worktree add -B "fix/issue-<N>" "$WT" origin/main
```

Branch naming: `fix/issue-<N>` (or `fix/issue-<N>-<slug>` if you'll open multiple). Path under `~/.airis/wt/<encoded-main-path>/...` matches the convention `pr_checkout` uses.
...
Worktree:  ~/.airis/wt/.../fix-issue-<N>            (if created)
