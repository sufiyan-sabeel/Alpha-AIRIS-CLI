...
`scripts/probe.ts` routes through `@airis/airis-ai` (`completeSimple`) so model/auth/provider behavior matches production.

```bash
bun .airis/skills/tool-prompt-optimization/scripts/probe.ts \
...
```bash
bun .airis/skills/tool-prompt-optimization/scripts/probe-builtin.ts --tool <name> [--no-summary] [--show]
```
...
```ts
import { arkToWireSchema } from "@airis/airis-ai"; // or toolWireSchema(tool)
