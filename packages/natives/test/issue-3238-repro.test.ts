/**
 * Regression for https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI/issues/3238.
 *
 * On macOS x64 (Intel), `airis stats` builds only the `modern`
 * (`airis_natives.darwin-x64-modern.node`) variant when the host has AVX2,
 ...
 * `airis_natives.darwin-x64-baseline.node` + `airis_natives.darwin-x64.node` —
 ...
 *      flakiness.
 *   2. The user-facing `AIRIS_NATIVE_VARIANT` override always wins, including
...
describe("issue 3238: variant resolution across worker contexts", () => {
	...
	it("honors AIRIS_NATIVE_VARIANT override above both cache and detection", () => {
		let detectorCalls = 0;
		const result = selectCpuVariant({
			arch: "x64",
			override: "baseline",
	...
	it("ignores garbage values in AIRIS_NATIVE_VARIANT and in the cache", () => {
		const result = selectCpuVariant({
			arch: "x64",
	...
	it("places modern ahead of baseline when the resolved verdict is modern (issue #3238 root file-search list)", () => {
		...
		expect(filenames[0]).toBe("airis_natives.darwin-x64-modern.node");
		expect(filenames).toContain("airis_natives.darwin-x64-baseline.node");
	});
});
