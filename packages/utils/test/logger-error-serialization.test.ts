...
beforeAll(() => {
	...
	// `getLogsDir()` honors AIRIS_AGENT_DIR / HOME for its base; pin to our tmp.
	...
	prevAgentDir = process.env.AIRIS_AGENT_DIR;
	process.env.AIRIS_AGENT_DIR = tempDir;
	logger.setTransports({ file: tempDir, console: false });
});

afterAll(() => {
	if (prevAgentDir === undefined) {
		delete process.env.AIRIS_AGENT_DIR;
	} else {
		process.env.AIRIS_AGENT_DIR = prevAgentDir;
	}
...
async function waitForLogEntry(targetMessage: string): Promise<Record<string, unknown>> {
	for (let i = 0; i < 40; i++) {
		const files = fs.readdirSync(tempDir).filter(f => f.startsWith("airis.") && f.endsWith(".log"));
