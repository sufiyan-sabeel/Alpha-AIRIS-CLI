 ...
 * when `AIRIS_TIMING` is set) that record an inclusive module window plus resolved
 * static child edges:
 *
 ...
 * - **Dev runs only.** In the compiled `airis` binary every module is pre-bundled
...
const MODULE_COMPLETE_KEY: symbol = Symbol.for("airis.moduleLoadComplete");
const MODULE_BODY_START_KEY: symbol = Symbol.for("airis.moduleBodyStart");
...
function bodyStartMarker(path: string): string {
	return `;globalThis[Symbol.for("airis.moduleBodyStart")]?.(${JSON.stringify(path)});\n`;
}

function completionMarker(path: string): string {
	return `\n;globalThis[Symbol.for("airis.moduleLoadComplete")]?.(${JSON.stringify(path)});\n`;
}
