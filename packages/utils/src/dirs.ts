/**
 * Centralized path helpers for airis config directories.
 *
 * Uses AIRIS_CONFIG_DIR (default ".airis") for the config root and
 * PI_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/airis/. This requires running `airis config migrate` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, airis trusts that the migration has been done.
 */
...
/** App name (e.g. "airis") */
export const APP_NAME: string = "airis";

/** Config directory name (e.g. ".airis") */
export const CONFIG_DIR_NAME: string = ".airis";
...
const PROFILE_ENV_KEYS = ["AIRIS_PROFILE", "PI_PROFILE"] as const;
 ...
 *
 * Exported so consumers of `@airis/airis-utils/dirs` (CLI bootstrap, tests,
...
export function normalizeProfileName(profile: string | undefined): string | undefined {
	...
	if (
		...
		throw new Error(
			`Invalid AIRIS profile "${profile}". Profile names must match ${PROFILE_NAME_RE.source}, ` +
...
/**
 * Resolve the active profile from the two profile env vars. `AIRIS_PROFILE` is the
 * canonical variable and takes precedence; `PI_PROFILE` is the legacy
 * compatibility fallback, consulted only when `AIRIS_PROFILE` is undefined. An
 * explicitly-empty `AIRIS_PROFILE` therefore selects the default profile rather
 ...
 */
export function resolveProfileEnv(airis: string | undefined, airs: string | undefined): string | undefined {
	return normalizeProfileName(airis !== undefined ? airis : airs);
}

function getProfileFromEnv(): string | undefined {
	return resolveProfileEnv(process.env.AIRIS_PROFILE, process.env.PI_PROFILE);
}

/**
 * Module-load profile resolution. Unlike {@link getProfileFromEnv}, an invalid
 * AIRIS_PROFILE/PI_PROFILE value does NOT throw here — a bad env var must not
 ...
 * user still gets a clean "Invalid AIRIS profile" message.
 */
...
/** Get the config directory name relative to home (e.g. ".airis" or AIRIS_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.AIRIS_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".airis/agent" or AIRIS_CONFIG_DIR + "/agent"). */
...
/**
 * Resolves and caches all airis directory paths. On Linux, when XDG environment
 * variables are set, paths are redirected under $XDG_*_HOME/airis/. A new
...
class DirResolver {
	...
	// With XDG on Linux, they point to $XDG_*_HOME/airis/.
	...
	constructor(options: { agentDirOverride?: string; profile?: string } = {}) {
		...
		// resolves under $XDG_*_HOME/airis once `airis config init-xdg` has migrated
		...
		// profile could resolve to `~/.airis/profiles/<name>` on first activation
		// (when no $XDG_*_HOME/airis exists yet) and then silently move to
		// `$XDG_*_HOME/airis/profiles/<name>` the moment the base appeared, orphaning
		...
		// XDG flattens the agent/ prefix: ~/.airis/agent/sessions → $XDG_DATA_HOME/airis/sessions
		this.#agentDirs = {
 ...
 * `PI_PROFILE` that was bypassed because `AIRIS_PROFILE` explicitly selected the
 * default profile. Returns `undefined` in those cases so reset falls back to the
 * standard `~/.airis/agent`.
 */
 ...
 * `~/.airis/agent`. Captured at module load — ignoring a profile-derived value
...
/** Get the config root directory (~/.airis). */
...
export function setProfile(profile: string | undefined): void {
	...
	if (activeProfile) {
		...
		process.env.AIRIS_PROFILE = activeProfile;
...
/** Get the agent config directory (~/.airis/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.airis). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.airis/*)
// =============================================================================

/** Get the reports directory (~/.airis/reports). */
export function getReportsDir(): string {
	return dirs.rootSubdir("reports", "state");
}

/** Get the logs directory (~/.airis/logs). */
export function getLogsDir(): string {
	return dirs.rootSubdir("logs", "state");
}

/** Get this process's dated log path (~/.airis/logs/airis.YYYY-MM-DD.PID.log). */
...
/**
 * Get the plugins directory (~/.airis/plugins or its XDG equivalent).
 *
 ...
 * (PR checkout, task isolation) and cleanup (`airis worktree`). A relative value
 * would resolve against whatever cwd happened to launch `airis`, so checkout and
 ...
 * isolation, and `airis worktree` cleanup all read the same base). Driven by the
 * `worktree.base` setting in coding-agent; pass `undefined`/empty to clear and
 * fall back to `AIRIS_WORKTREE_DIR` or the `~/.airis/wt` default.
 *
 ...
 * `AIRIS_WORKTREE_DIR` env var, then the {@link setWorktreesDir} override (the
 * `worktree.base` setting), then the `~/.airis/wt` default. The env var and the
 * override are both `~`-expanded and must be absolute; a relative value is
 * ignored and resolution falls through.
 */
export function getWorktreesDir(): string {
	return resolveWorktreeBase(process.env.AIRIS_WORKTREE_DIR) ?? worktreesDirOverride ?? dirs.rootSubdir("wt", "data");
}

/** Get the SSH control socket directory (~/.airis/ssh-control). */
export function getSshControlDir(): string {
	return dirs.rootSubdir("ssh-control", "state");
}

/** Get the remote host info directory (~/.airis/remote-host). */
export function getRemoteHostDir(): string {
	return dirs.rootSubdir("remote-host", "data");
}

/** Get the managed Python venv directory (~/.airis/python-env). */
export function getPythonEnvDir(): string {
	return dirs.rootSubdir("python-env", "data");
}

/** Get the shared Python gateway state directory (~/.airis/agent/python-gateway; XDG default: $XDG_STATE_HOME/airis/python-gateway). */
export function getPythonGatewayDir(): string {
	return dirs.agentSubdir(undefined, "python-gateway", "state");
}

/** Get the puppeteer sandbox directory (~/.airis/puppeteer). */
...
/** Get the auto-QA grievances SQLite database path (~/.airis/autoqa.db; XDG: $XDG_DATA_HOME/airis/autoqa.db). */
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.airis/wt/`).
...
/** Get the path to a single worktree directory (~/.airis/wt/<segment>). */
export function getWorktreeDir(segment: string): string {
	return path.join(getWorktreesDir(), segment);
}

/** Get the GPU cache path (~/.airis/gpu_cache.json). */
export function getGpuCachePath(): string {
	return dirs.rootSubdir("gpu_cache.json", "cache");
}

/**
 * Get the GitHub view cache database path (~/.airis/cache/github-cache.db).
 * Honors the `AIRIS_GITHUB_CACHE_DB` env var when set so tests can isolate the
 * cache file without touching the rest of the config root.
 */
export function getGithubCacheDbPath(): string {
	const override = process.env.AIRIS_GITHUB_CACHE_DB;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "github-cache.db"), "cache");
}

/**
 * Get the encrypted auth-broker snapshot cache path (~/.airis/cache/auth-broker-snapshot.enc).
 * Honors the `AIRIS_AUTH_BROKER_SNAPSHOT_CACHE` env var when set so tests and
 * operators can isolate or relocate the cache file.
 */
export function getAuthBrokerSnapshotCachePath(): string {
	const override = process.env.AIRIS_AUTH_BROKER_SNAPSHOT_CACHE;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "auth-broker-snapshot.enc"), "cache");
}

/** Get the local FastEmbed model cache directory (~/.airis/cache/fastembed). */
export function getFastembedCacheDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed"), "cache");
}

/** Get the on-demand fastembed runtime install root (~/.airis/cache/fastembed-runtime). */
export function getFastembedRuntimeDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed-runtime"), "cache");
}

/** Get the natives directory (~/.airis/natives). */
export function getNativesDir(): string {
	return dirs.rootSubdir("natives", "cache");
}

/** Get the stats database path (~/.airis/stats.db). */
export function getStatsDbPath(): string {
	return dirs.rootSubdir("stats.db", "data");
}

/** Get the autoresearch state directory (~/.airis/autoresearch). */
export function getAutoresearchDir(): string {
	return dirs.rootSubdir("autoresearch", "state");
}

/** Get the per-project autoresearch state directory (~/.airis/autoresearch/<encoded-project>). */
export function getAutoresearchProjectDir(encodedProject: string): string {
	return path.join(getAutoresearchDir(), encodedProject);
}

/** Get the per-project autoresearch SQLite database path (~/.airis/autoresearch/<encoded-project>.db). */
export function getAutoresearchDbPath(encodedProject: string): string {
	return path.join(getAutoresearchDir(), `${encodedProject}.db`);
}

/** Get the per-run artifact directory (~/.airis/autoresearch/<encoded-project>/runs/<runId>). */
...
/** Get the last-seen-changelog-version marker file (~/.airis/agent/last-changelog-version). */
...
/** Get the tiny title model cache directory (~/.airis/agent/cache/tiny-models). */
...
/** Get the document conversion cache directory (~/.airis/agent/cache/document-conversions; XDG default: $XDG_CACHE_HOME/airis/cache/document-conversions). */
...
/**
 * Persistent per-install UUID stored at `~/.airis/install-id`.
 *
 ...
 * `~/.airis/agent/` does not regenerate it. Server-side dedup for grievance
 * pushes (and similar telemetry) keys on this id.
 *
 * Anchored to the base config root (`~/.airis/install-id`) regardless of the
