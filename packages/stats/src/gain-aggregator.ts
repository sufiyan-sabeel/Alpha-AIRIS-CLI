...
import { getStatsDbPath, isEnoent, logger } from "@airis/airis-utils";
...
const TEMP_PATH_RE = /(?:^|\/)(?:T|tmp|pi-bash-exec|airis-bash-exec|pi-bash-detach)(?:\/|$)|^\/var\/folders(?:\/|$)/;
 ...
 *
 * Rules are generic: airis internal wt paths are dropped; conventional worktree
...
export function normalizeProjectPath(p: string): string | null {
	...
	if (/\/\.airis\/wt\//u.test(clean)) return null;
