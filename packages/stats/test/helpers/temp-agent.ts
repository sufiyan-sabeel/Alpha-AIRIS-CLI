 ...
 *
 * The default profile's stats.db is redirected to `$XDG_DATA_HOME/airis/stats.db`
 * by {@link DirResolver} whenever `agentDirOverride === defaultAgent`. Tests
 * that only set `AIRIS_CONFIG_DIR` + `setAgentDir(<home>/<config>/agent)` resolve
 ...
 * `AIRIS_CONFIG_DIR` for the test, points the agent directory at a fresh
...
import { closeDb } from "@airis/airis-stats/db";
import { getAgentDir, setAgentDir, TempDir } from "@airis/airis-utils";
