...
- **Session log parsing**: Reads JSONL session logs from `~/.airis/agent/sessions/`
...
```typescript
import { getDashboardStats, syncAllSessions } from "@airis/airis-stats";
...
- **Session logs**: `~/.airis/agent/sessions/` (JSONL files)
- **Stats database**: `~/.airis/stats.db` (SQLite)
