 ...
 *   bun scripts/bench-title-models.ts --db ~/.airis/agent/history.db --out bench.json
 */
...
function parseArgs(argv: string[]): BenchConfig {
	...
	return {
		dbPath: (get("--db") ?? path.join(os.homedir(), ".airis/agent/history.db")).replace(/^~/, os.homedir()),
