/**
 * Bun `--preload` shim for the airis dev launcher (`scripts/airis`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the airis CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `@airis/airis-utils/dirs`).
 */
const launchCwd = process.env.AIRIS_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.AIRIS_LAUNCH_CWD;
	try {
