...
import { isEnoent } from "@airis/airis-utils/fs-error";

/** Build-time specifier resolved to bundled legacy Pi module namespaces. */
export const LEGACY_AIRIS_MODULES_SPECIFIER = "airis-legacy-airis-modules";

const VIRTUAL_NAMESPACE = "airis-legacy-airis-modules-build";
...
const BUNDLED_PACKAGES: readonly BundledPackage[] = [
	...
	{ dir: "ai", identifier: "AirisAi", rootShim: "legacy-airis-ai-shim.ts" },
	{ dir: "coding-agent", identifier: "AirisCodingAgent", rootShim: "legacy-airis-coding-agent-shim.ts" },
	{ dir: "natives", identifier: "airisNatives", rootShim: null },
	{ dir: "tui", identifier: "AirisTui", rootShim: "legacy-airis-tui-shim.ts" },
 ...
 */
export async function createLegacyAirisVirtualModulePlugin(): Promise<Bun.BunPlugin> {
	const source = __renderLegacyPiVirtualModule(await collectBundledPiEntries());
	return {
		name: "airis:legacy-airis-modules",
		setup(build) {
			build.onResolve({ filter: /^airis-legacy-airis-modules$/ }, () => ({
				path: LEGACY_AIRIS_MODULES_SPECIFIER,
