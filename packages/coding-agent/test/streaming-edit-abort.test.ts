...
import type { AssistantMessage, StopReason, ToolCall } from "@airis/airis-ai";
...
import * as autoGeneratedGuard from "@airis/airis-coding-agent/tools/auto-generated-guard";
...
it("keeps the session alive when replace mode streams an ssh:// path", async () => {
	const checkSpy = vi.spyOn(autoGeneratedGuard, "assertEditableFile");
	const abortSignalRef: { current?: AbortSignal } = {};
	const remotePath = "ssh://test-host/tmp/airis-repro.txt";
