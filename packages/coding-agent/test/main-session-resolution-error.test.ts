...
import type { Args } from "@airis/airis-coding-agent/cli/args";
import type { Settings } from "@airis/airis-coding-agent/config/settings";
import { createSessionManager, SessionResolutionError, writeStartupNotice } from "@airis/airis-coding-agent/main";
...
describe("createSessionManager — missing session (#2084)", () => {
	it("rejects --resume with SessionResolutionError carrying a usage hint", async () => {
		...
		try {
			await expect(
				...
				hint: expect.stringContaining("airis --resume"),
			});
	...
	it("rejects --resume with unknown id instead of falling back to latest persisted session", async () => {
		const cwd = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-resume-unknown-id-"));
	...
	it("rejects --continue followed by an unknown session id instead of falling back to latest", async () => {
		const cwd = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-continue-unknown-id-"));
		const sessionDir = path.join(cwd, "sessions");
		const missingId = "019ea530-ffff-7000-8000-000000000000";
		try {
			const latest = SessionManager.create(cwd, sessionDir);
