...
// public `loadLegacyAirisModule` entry point.
...
async function writePackage(files: Record<string, string>): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-legacy-inplace-"));
	tempRoots.push(dir);
...
describe("legacy-airis in-place module loading (issue #1674)", () => {
	it("reads __dirname-relative HTML assets from the real extension directory", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { dirName: string; html: string };
	...
	it("loads CommonJS helpers required by an ES module extension", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.js"))) as { value: string };
	...
	it("loads a relative CommonJS helper imported by a TypeScript extension", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { value: number };
	...
	it("remaps legacy Pi requires in graph-owned CommonJS packages to the host shim", async () => {
		const dir = await writePackage({
			"package.json": JSON.stringify({ name: "cjs-legacy-airis-require-ext", version: "1.0.0", type: "module" }),
			...
			"index.ts": [
				'import { Type } from "@airis/airis-ai";',
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { sharesHostType: boolean };
	...
	it("loads a default import from linkedom's CommonJS canvas fallback", async () => {
		...
		const mod = await loadLegacyAirisModule(path.join(dir, "index.js"));
	...
	it("reads a lazy CommonJS helper at import time", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.js"))) as { loadValue(): Promise<string> };
	...
	it("reloads an edited CommonJS helper imported from ESM", async () => {
		...
		const first = await loadLegacyAirisModule(entry);
		...
		const second = await loadLegacyAirisModule(entry);
	...
	it("reloads an edited entry module without polluting fileURLToPath-derived paths", async () => {
		...
		const first = (await loadLegacyAirisModule(entry)) as { version: string; entryPath: string; asset: string };
		...
		const second = (await loadLegacyAirisModule(entry)) as { version: string; entryPath: string; asset: string };
	...
	it("reloads an edited relative helper module on same-process re-import", async () => {
		...
		const first = (await loadLegacyAirisModule(entry)) as { entryVersion: string; H: string; entryPath: string };
		...
		const second = (await loadLegacyAirisModule(entry)) as { entryVersion: string; H: string; entryPath: string };
	...
	it("reloads edited children of an extension-local bare dependency", async () => {
		...
		const first = (await loadLegacyAirisModule(entry)) as { entryVersion: string; depValue: string };
		expect(first.entryVersion).toBe("v1");
		...
		const second = (await loadLegacyAirisModule(entry)) as { entryVersion: string; depValue: string };
		expect(second.entryVersion).toBe("v2");
	...
	it("keeps transitive bare dependency importer paths query-free", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { nestedUrl: string };
	...
	it("preserves named re-exports from relative ESM children in a no-type dual package", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { result: string };
	...
	it("keeps a syntax-ambiguous conditional import target on its ESM branch", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { loaded: boolean };
	...
	it("reloads a hoisted CommonJS dependency required by a relative CommonJS child", async () => {
		...
		const first = (await loadLegacyAirisModule(entry)) as { entryVersion: string; dependencyValue: string };
		expect(first.entryVersion).toBe("v1");
		...
		const second = (await loadLegacyAirisModule(entry)) as { entryVersion: string; dependencyValue: string };
		expect(second.entryVersion).toBe("v2");
	...
	it("preserves named imports through CommonJS package re-exports", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { value: string };
	...
	it("preserves named imports from CommonJS defineProperty and exportStar patterns", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as {
			local: string;
			value: string;
		};
	...
	it("reloads a lazily imported CommonJS package re-export", async () => {
		...
		const first = (await loadLegacyAirisModule(entry)) as {
			entryVersion: string;
		...
		const second = (await loadLegacyAirisModule(entry)) as {
			entryVersion: string;
			loadValue(): Promise<string>;
		};
		expect(second.entryVersion).toBe("v2");
	...
	it("reloads modules added to the relative import graph after the first load", async () => {
		...
		const first = (await loadLegacyAirisModule(entry)) as { entryVersion: string; entryPath: string };
		expect(first.entryVersion).toBe("v1");
		expect(first.entryPath).toBe(expectedEntryPath);
		...
		const third = (await loadLegacyAirisModule(entry)) as {
			entryVersion: string;
			entryPath: string;
			leafValue: string;
		};
	...
	it("resolves a .css sibling of a relatively-imported submodule", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { css: string };
	...
	it("leaves JSON import-attribute targets on Bun's native loader", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { inputPrice: number };
	...
	it("loads the extension's own node_modules deps natively while remapping legacy pi imports", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { depValue: string; hasZod: boolean };
	...
	it("preserves relative paths from legacy find operations", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { lines: string[] };
	...
	it("rewrites legacy imports in ../src modules reached through relative imports", async () => {
		...
		const mod = (await loadLegacyAirisModule(path.join(dir, "dist", "extension.ts"))) as { ok: boolean };
	...
	it("does not rewrite sibling files outside the loaded extension's import graph", async () => {
		...
		const entryMod = (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as { local: string };
