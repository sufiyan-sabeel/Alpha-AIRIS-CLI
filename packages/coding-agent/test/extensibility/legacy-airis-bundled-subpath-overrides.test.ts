...
import { __buildLegacyPiPackageRootOverrides } from "@airis/airis-coding-agent/extensibility/plugins/legacy-airis-compat";
import { TempDir } from "@airis/airis-utils";
import { __renderLegacyPiVirtualModule, collectBundledPiEntries } from "../../scripts/legacy-airis-virtual-module";
...
// the same `airis-legacy-airis-bundled:` virtual namespace as package roots without
...
describe("legacy pi compat compiled-mode subpath overrides (issue #3442)", () => {
	it("does not evaluate unrelated host modules while loading the registry", async () => {
		using tempDir = TempDir.createSync("@airis-legacy-airis-loaders-");
...
`,
	...
	it("serves @airis/airis-ai/oauth through the bundled virtual namespace in compiled mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@airis/airis-ai/oauth"]).toBe("airis-legacy-airis-bundled:@airis/airis-ai/oauth");
	});
	...
	it("expands wildcard exports for concrete on-disk targets (issue #3442 follow-up)", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@airis/airis-ai/oauth/anthropic"]).toBe(
			"airis-legacy-airis-bundled:@airis/airis-ai/oauth/anthropic",
		);
		// Sanity: the wildcard expansion also reaches deeper subroots so plugins
		// pinned to e.g. `@airis/airis-ai/providers/openai` keep resolving.
		expect(bundledModuleKeys.has("@airis/airis-ai/oauth/anthropic")).toBe(true);
		expect(bundledModuleKeys.has("@airis/airis-ai/oauth/openai-codex")).toBe(true);
	});
	...
	it("actually loads the shim's shared Pi translation through the bundled registry", async () => {
		...
		const key = "@airis/airis-ai/providers/cursor-airis-args";
		...
		const registryPath = path.join(packageRoot, `.probe-legacy-airis-args-${Bun.randomUUIDv7()}.ts`);
		await Bun.write(
			registryPath,
...
`,
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides[key]).toBe(`airis-legacy-airis-bundled:${key}`);
	});

	it("expands web search provider wildcard exports for compiled plugin imports", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		const providerKeys = [
			"@airis/airis-coding-agent/web/search/providers/xai",
			"@airis/airis-coding-agent/web/search/providers/tinyfish",
			"@airis/airis-coding-agent/web/search/providers/firecrawl",
			"@airis/airis-coding-agent/web/search/providers/duckduckgo",
		] as const;
	...
	it("does not enumerate root catch-all wildcards (./* / ./*.js)", () => {
		...
		// Concrete check: `@airis/airis-coding-agent/cli` is NOT bundled.
		expect(bundledModuleKeys.has("@airis/airis-coding-agent/cli")).toBe(false);
		expect(bundledModuleKeys.has("@airis/airis-coding-agent/main")).toBe(false);
	});
	...
	it("does not bundle main-thread-unsafe worker entrypoints", () => {
		...
		expect(bundledModuleKeys.has("@airis/airis-coding-agent/eval/js/worker-entry")).toBe(false);
	});
	...
	it("maps every bundled key (minus shimmed roots + typebox) to its virtual specifier in compiled mode", () => {
		...
		for (const key of bundledModuleKeys) {
			...
			if (
				key === "@airis/airis-ai" ||
				key === "@airis/airis-coding-agent" ||
				key === "@airis/airis-tui" ||
				key === "typebox"
			)
				continue;
			if (overrides[key] !== `airis-legacy-airis-bundled:${key}`) {
				missing.push(key);
			}
		}
	...
	it("keeps pi-ai/pi-coding-agent/pi-tui roots routed to their compat shims in compiled mode", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@airis/airis-ai"]).toBeDefined();
		expect(overrides["@airis/airis-ai"]).not.toBe("airis-legacy-airis-bundled:@airis/airis-ai/oauth");
		expect(overrides["@airis/airis-coding-agent"]).toBeDefined();
		expect(overrides["@airis/airis-tui"]).toBeDefined();
	});

	it("does not register subpath overrides in dev/install mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(false);
		expect(overrides).not.toHaveProperty("@airis/airis-ai/oauth");
		expect(overrides).not.toHaveProperty("@airis/airis-coding-agent/tools");
