...
import * as configModule from "@airis/airis-coding-agent/config";
import * as shim from "@airis/airis-coding-agent/extensibility/legacy-airis-coding-agent-shim";
import * as utils from "@airis/airis-utils";
...
describe("legacy shim path helpers", () => {
	...
	it("getPackageDir resolves the coding-agent package root in source mode", () => {
		// airis's canonical helper returns the package root containing package.json
	...
	// `path.join(getPackageDir(), ...)`. airis's canonical helper returns
