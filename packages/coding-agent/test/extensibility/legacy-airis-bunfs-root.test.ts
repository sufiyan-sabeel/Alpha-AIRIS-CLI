...
import { __computeBundledSelfPackageRoot } from "@airis/airis-coding-agent/extensibility/plugins/legacy-airis-compat";
...
// bundled host packages and shims through the `airis-legacy-airis-bundled:`
// virtual namespace (see `legacy-airis-bundled-virtual.test.ts`). The bunfs
...
describe("legacy pi compat bundled-self package root computation", () => {
	it("derives the npm prebuilt bundle package root from dist import.meta.dir", () => {
		const winMetaDir = "C:\\Users\\me\\.bun\\install\\global\\node_modules\\@alpha-airis-cli\\pi-coding-agent\\dist";
		expect(__computeBundledSelfPackageRoot(winMetaDir, path.win32)).toBe(
			"C:\\Users\\me\\.bun\\install\\global\\node_modules\\@alpha-airis-cli\\pi-coding-agent",
		);

		const posixMetaDir = "/home/me/.bun/install/global/node_modules/@airis/airis-coding-agent/dist";
		expect(__computeBundledSelfPackageRoot(posixMetaDir, path.posix)).toBe(
			"/home/me/.bun/install/global/node_modules/@airis/airis-coding-agent",
		);
	});
