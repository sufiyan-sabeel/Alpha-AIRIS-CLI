...
import {
	...
	getBundledProviders,
	modelsAreEqual,
} from "@airis/airis-catalog/models";
import {
...
// pi-ai 15.1.0 removed the runtime `Type` export from `@airis/airis-ai`'s
...
// `legacy-airis-compat.ts` patches that gap by redirecting bare pi-ai root
// imports through `legacy-airis-ai-shim.ts`, which re-exports the canonical
...
async function writeFixtureExtension(source: string): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-pi-ai-type-remap-"));
	tempRoots.push(dir);
...
describe("legacy-airis @(scope)/pi-ai root `Type` remap (issue #1437)", () => {
	...
	it('redirects `import { Type } from "@airis/airis-ai"` for plugins published against the canonical scope', async () => {
		const entry = await writeFixtureExtension(
			['import { Type } from "@airis/airis-ai";', "export const probe = Type;"].join("\n"),
		);

		const loaded = (await loadLegacyAirisModule(entry)) as { probe: typeof TypeBoxShimType };
		expect(loaded.probe).toBe(TypeBoxShimType);
	});
	...
	it("does not redirect subpath imports such as @airis/airis-ai/utils/schema", async () => {
		const entry = await writeFixtureExtension(
			[
				...
				'import { zodToWireSchema } from "@airis/airis-ai/utils/schema";',
				"export const fn = zodToWireSchema;",
			].join("\n"),
		);

		const loaded = (await loadLegacyAirisModule(entry)) as { fn: unknown };
		expect(typeof loaded.fn).toBe("function");
	});

	it("exports getModel as getBundledModel", async () => {
		const loaded = (await loadLegacyAirisModule(
			await writeFixtureExtension(
				'import { getModel } from "@airis/airis-ai"; export const testGetModel = getModel;',
			),
	...
	it("exports getModels as getBundledModels", async () => {
		const loaded = (await loadLegacyAirisModule(
			await writeFixtureExtension(
				'import { getModels } from "@airis/airis-ai"; export const testGetModels = getModels;',
			),
		)) as { testGetModels: unknown };
		expect(loaded.testGetModels).toBe(getBundledModels);
	});

	it("re-exports calculateCost from @airis/airis-catalog/models (issue #4584)", async () => {
		// `calculateCost` was moved from the `@airis/airis-ai` barrel to
		// `@airis/airis-catalog/models` in the catalog split. Legacy extensions
		...
		// '.../legacy-airis-ai-shim.ts'` at extension-validation time.
		const loaded = (await loadLegacyAirisModule(
			await writeFixtureExtension(
				'import { calculateCost } from "@airis/airis-ai"; export const probe = calculateCost;',
			),
		)) as { probe: unknown };
		expect(loaded.probe).toBe(calculateCost);
	});

	it("re-exports modelsAreEqual and getBundledProviders from @airis/airis-catalog/models", async () => {
		const loaded = (await loadLegacyAirisModule(
			await writeFixtureExtension(
				[
					'import { modelsAreEqual, getBundledProviders } from "@airis/airis-ai";',
	...
	it("re-exports getBundledModel and getBundledModels from @airis/airis-catalog/models", async () => {
		const loaded = (await loadLegacyAirisModule(
			await writeFixtureExtension(
				[
					'import { getBundledModel, getBundledModels } from "@airis/airis-ai";',
	...
	it("exports StringEnum as a schema builder with options support", async () => {
		const loaded = (await loadLegacyAirisModule(
			await writeFixtureExtension(
				[
					'import { StringEnum } from "@airis/airis-ai";',
...
describe("legacy pi package root remaps (issue #1474)", () => {
	it("loads @earendil-works/pi-coding-agent root imports when host package resolution is unavailable", async () => {
		...
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@airis/airis-coding-agent" && from.endsWith(path.join("src", "extensibility", "plugins"))) {
		...
		const loaded = (await loadLegacyAirisModule(entry)) as { loadedVersion: string };
	...
	it("loads pi-vimmode's minified legacy imports", async () => {
		...
		const loaded = (await loadLegacyAirisModule(entry)) as { apiTypes: string[]; printable: string };
	...
	it("preserves legacy defineTool root imports and usable coding tools", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-legacy-coding-tools-"));
		tempRoots.push(dir);
	...
	it("falls back to legacy-scoped subpath peers for direct plugin imports", async () => {
		const realResolveSync = Bun.resolveSync.bind(Bun);
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@airis/airis-ai/oauth") {
				throw new Error(`canonical peer unavailable from ${from}`);
			}
			return realResolveSync(specifier, from);
		});

		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-legacy-direct-subpath-"));
		tempRoots.push(dir);
	...
	it("routes @earendil-works/pi-utils through canonical Bun.resolveSync in non-compiled mode", async () => {
		// Regression: when airis runs from a node_modules install (not the monorepo
		// and not a compiled binary), the bundled packages live at
		// `node_modules/@airis/airis-*`, not next to the source tree. Hardcoding
		...
		vi.spyOn(Bun, "resolveSync").mockImplementation((specifier: string, from: string) => {
			if (specifier === "@airis/airis-utils") {
		...
		const loaded = (await loadLegacyAirisModule(entry)) as { probe: () => boolean };
