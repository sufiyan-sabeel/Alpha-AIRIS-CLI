...
import {
	...
	parseStreamingJson,
	repairJson,
} from "@airis/airis-coding-agent/extensibility/legacy-airis-ai-shim";

// Issue #6859: pi extensions import runtime helpers from the `@earendil-works/pi-ai`
// (aliased to `@airis/airis-ai`) package root that airis's barrel no longer forwards.
// `isContextOverflow` moved under `@airis/airis-ai/error` and the JSON-repair
// helpers moved to `@airis/airis-utils`, so `export * from "@airis/airis-ai"` left
// them off the shim surface and a named import tripped Bun's static
// "No matching export" check during plugin validation (e.g.
// `airis plugin install pi-blackhole`). This pins the bridged root surface so it
