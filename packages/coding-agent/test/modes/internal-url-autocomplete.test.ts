...
import { resetActiveRulesForTests, setActiveRules } from "@airis/airis-coding-agent/capability/rule";
...
import {
	...
	isInternalUrlPrefix,
} from "@airis/airis-coding-agent/modes/internal-url-autocomplete";
import { PromptActionAutocompleteProvider } from "@airis/airis-coding-agent/modes/prompt-action-autocomplete";
...
describe("internal-url-autocomplete", () => {
	beforeEach(() => {
		...
		setActiveRules([rule("python", "roboairis rules"), rule("style")]);
	});
	...
	describe("getInternalUrlSuggestions", () => {
		...
		it("carries the candidate description through", async () => {
			...
			expect(result!.items[0]).toMatchObject({ value: "rule://python", description: "roboairis rules" });
		});
