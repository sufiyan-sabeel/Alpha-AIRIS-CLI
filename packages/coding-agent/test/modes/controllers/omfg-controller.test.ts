...
import type { AssistantMessage, Usage } from "@airis/airis-ai";
import type { Rule } from "@airis/airis-coding-agent/capability/rule";
import { OmfgController } from "@airis/airis-coding-agent/modes/controllers/omfg-controller";
...
import { Container, type TUI } from "@airis/airis-tui";
import { removeWithRetries } from "@airis/airis-utils";

const PROJECT_OPTION = "This project (.airis/rules)";
const GLOBAL_OPTION = "Global — all projects (~/.airis/agent/rules)";
...
describe("OmfgController", () => {
	it("saves a matching generated rule under project rules and registers it live", async () => {
		...
		const savedPath = path.join(harness.projectDir, ".airis", "rules", "ts-no-any.md");
		...
		expect(rendered).toContain(path.join(".airis", "rules", "ts-no-any.md"));
	...
	it("reiterates when the first valid rule does not match history", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".airis", "rules", "ts-no-any.md")).exists()).toBe(true);
	});
	...
	it("asks before saving when validation never confirms a match", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".airis", "rules", "no-match.md")).exists()).toBe(false);
	});
	...
	it("lets the user amend from the save selector before writing the rule", async () => {
		...
		expect(await Bun.file(path.join(harness.projectDir, ".airis", "rules", "ts-any-broad.md")).exists()).toBe(false);
		expect(await Bun.file(path.join(harness.projectDir, ".airis", "rules", "ts-no-explicit-any.md")).exists()).toBe(
			true,
		);
	});
	...
	it("clears the panel and aborts the inner request on Escape", async () => {
		...
		expect(controller.hasActiveRequest()).toBe(false);
		expect(await Bun.file(path.join(harness.projectDir, ".airis", "rules", "ts-no-any.md")).exists()).toBe(false);
	});
});
