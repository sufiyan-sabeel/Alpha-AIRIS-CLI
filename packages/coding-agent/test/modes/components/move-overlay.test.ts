...
describe("resolveExistingDirectory", () => {
	...
	beforeEach(async () => {
		tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-move-resolve-"));
	});
	afterEach(async () => {
...
describe("MoveOverlay", () => {
	...
	beforeEach(async () => {
		tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-move-overlay-"));
		cwd = tmp;
	...
	it("does not stat every directory entry per keystroke", async () => {
		...
		const bulk = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-move-overlay-bulk-"));
		try {
			for (let i = 0; i < 60; i++) {
	...
	it("classifies unknown-type Dirents via statSync fallback", async () => {
		...
		const unknownFs = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-move-overlay-unknown-"));
		try {
