...
import { PluginManager } from "@airis/airis-coding-agent/extensibility/plugins";
import {
...
} from "@airis/airis-coding-agent/modes/components/plugin-settings";
...
describe("PluginListComponent", () => {
	...
	it("empty-state mentions both npm and marketplace install commands", () => {
		...
		expect(text).toContain("airis plugin install <package>");
		expect(text).toContain("airis plugin install <name>@<marketplace>");
	});
...
describe("MarketplacePluginDetailComponent", () => {
	...
	it("shortens home-relative install paths to ~ before rendering", () => {
		...
		const installPath = `${home}/.airis/cache/plugins/sample@mkt`;
		...
		expect(text).toContain("~/.airis/cache/plugins/sample@mkt");
