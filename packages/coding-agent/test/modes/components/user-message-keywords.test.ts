...
import { CustomEditor } from "@airis/airis-coding-agent/modes/components/custom-editor";
import { UserMessageComponent } from "@airis/airis-coding-agent/modes/components/user-message";
import { getEditorTheme, initTheme } from "@airis/airis-coding-agent/modes/theme/theme";
...
describe("UserMessageComponent magic-keyword highlighting", () => {
	...
	it("wraps image references in file hyperlinks when a blob path is available", () => {
		const imagePath = path.resolve("/tmp/airis-image.png");
