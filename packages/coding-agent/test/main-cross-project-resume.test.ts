...
import * as modelResolverModule from "@airis/airis-coding-agent/config/model-resolver";
import { Settings } from "@airis/airis-coding-agent/config/settings";
import * as pluginHelpers from "@airis/airis-coding-agent/discovery/helpers";
import { createSessionManager, runRootCommand } from "@airis/airis-coding-agent/main";
...
import * as sessionListingModule from "@airis/airis-coding-agent/session/session-listing";
...
describe("createSessionManager — cross-project --resume", () => {
	...
	beforeEach(async () => {
		existingProject = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-xproj-"));
...
describe("runRootCommand — cross-project --resume", () => {
	...
	beforeEach(async () => {
		...
		root = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-xproj-root-"));
...
describe("createSessionManager — cross-project --resume relocation (moved worktree)", () => {
	...
	beforeEach(async () => {
		missingRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-moved-xproj-"));
