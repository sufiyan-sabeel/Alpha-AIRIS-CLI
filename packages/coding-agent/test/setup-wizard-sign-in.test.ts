...
import type { OAuthLoginCallbacks, OAuthProviderId } from "@airis/airis-ai/oauth/types";
import { SignInTab } from "@airis/airis-coding-agent/modes/setup-wizard/scenes/sign-in";
import type { SetupSceneHost } from "@airis/airis-coding-agent/modes/setup-wizard/scenes/types";
...
describe("SignInTab", () => {
	it("keeps the OSC8 login link and manual-code prompt above clipped wizard rows", async () => {
		const url = `https://example.com/oauth/authorize?client_id=airis&redirect_uri=http%3A%2F%2Flocalhost%3A45454%2Fcallback&state=${"a".repeat(96)}`;
	...
	it("copies the active login URL from the keyboard while the setup TUI owns selection", async () => {
		const url = "https://example.com/oauth/authorize?client_id=airis&state=copy";
		const loginGate = Promise.withResolvers<void>();
		const copySpy = vi.spyOn(clipboard, "copyToClipboard").mockResolvedValue(undefined);
