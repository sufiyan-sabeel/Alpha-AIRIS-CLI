...
import {
	...
	type SessionEvent,
} from "@airis/airis-coding-agent/extensibility/hooks";
import { theme } from "@airis/airis-coding-agent/modes/theme/theme";
...
describe.skipIf(!e2eApiKey("ANTHROPIC_API_KEY"))("Compaction hooks", () => {
	...
	beforeEach(() => {
		tempDir = path.join(os.tmpdir(), `airis-compaction-hooks-test-${Snowflake.next()}`);
