...
import { runRootCommand } from "@airis/airis-coding-agent/main";
...
describe("parseArgs — --max-time flag", () => {
	...
	it("reports invalid --max-time values as CLI usage errors", async () => {
		...
		expect(stderr).toContain("Run `airis --help` for available flags.");
	...
	it("converts maxTime to an absolute session deadline", async () => {
		using tempDir = TempDir.createSync("@airis-max-time-");
