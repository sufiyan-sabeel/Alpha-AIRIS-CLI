 ...
 * `@airis/airis-utils` logger and metric instruments through the agent
 * telemetry hooks, flushes, and exits 0 only if the receiver got a non-empty
 * protobuf POST at both /v1/logs and /v1/metrics.
 */

import type { AgentRunCoverage, AgentRunSummary, ChatUsageEvent } from "@airis/airis-agent-core";
import { emptyAgentRunCoverage, emptyAgentRunSummary } from "@airis/airis-agent-core";
import {
	createTelemetryExportConfig,
	flushTelemetryExport,
	initTelemetryExport,
	isTelemetryExportEnabled,
} from "@airis/airis-coding-agent/telemetry-export";
...
process.env.OTEL_SERVICE_NAME = "alpha-airis-cli-signals-probe";
...
await Bun.sleep(700);
await flushTelemetryExport();
assertSingleMetricPoint("airs.airis.agent.chat.calls");
assertSingleMetricPoint("airs.airis.agent.tool.calls");
assertSingleMetricPoint("airs.airis.agent.tool.duration");
await server.stop(true);
