 ...
 *
 * One-shot CLIs (`airis bench`, dry-balance) build a bare `ModelRegistry` that
 ...
 * `~/.airis/agent/extensions/`) was invisible to model resolution and
 * `airis bench <provider>/<model>` failed with "Model not found".
 *
 ...
 * passed explicitly so the test never touches the developer's real `~/.airis`.
 */
...
import { getModelMatchPreferences, resolveCliModel } from "@airis/airis-coding-agent/config/model-resolver";
import { resetSettingsForTest, Settings } from "@airis/airis-coding-agent/config/settings";
import { loadCliExtensionProviders } from "@airis/airis-coding-agent/sdk";
