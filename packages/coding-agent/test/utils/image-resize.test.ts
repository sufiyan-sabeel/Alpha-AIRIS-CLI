...
import { resizeImage } from "@airis/airis-coding-agent/utils/image-resize";
...
describe("resizeImage env wiring", () => {
	const prior = Bun.env.AIRIS_NO_WEBP;

	beforeEach(() => {
		delete (Bun.env as Record<string, string | undefined>).AIRIS_NO_WEBP;
	});

	afterEach(() => {
		if (prior === undefined) delete (Bun.env as Record<string, string | undefined>).AIRIS_NO_WEBP;
		else Bun.env.AIRIS_NO_WEBP = prior;
	});

	it("treats AIRIS_NO_WEBP=1 set at call time as exclusion (not baked at module load)", async () => {
		Bun.env.AIRIS_NO_WEBP = "1";
