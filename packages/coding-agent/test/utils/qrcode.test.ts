...
import { QrCode, type QrEcLevel, renderQrHalfBlocks } from "@airis/airis-coding-agent/utils/qrcode";
...
describe("QR encoder", () => {
	...
	const vectors: ReadonlyArray<{
		...
		{
			text: "https://my.airis.sh/#mgAYTZwEnpRQtca0CTgn-Q.gdJU",
			ecl: "M",
			mask: 4,
			version: 4,
			size: 33,
	...
	it("deterministically selects a penalty-minimizing mask when none is forced", () => {
		...
		const qr = QrCode.encodeText("https://my.airis.sh/#demo", "M");
...
describe("renderQrHalfBlocks", () => {
	it("frames the symbol in a light quiet zone wide enough for the margin", () => {
		const qr = QrCode.encodeText("https://airis.sh/#demo", "M");
		const margin = 3;
