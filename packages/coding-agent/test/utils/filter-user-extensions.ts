...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@airis/airis-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.airis/`), plus
//    the legacy non-XDG `~/.airis/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.airis/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/airis/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.airis` when
...
// symlinks `~/.airis/agent/extensions/foo.ts` to a dotfiles repo would see the
