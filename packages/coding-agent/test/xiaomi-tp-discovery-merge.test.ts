...
import { mergeDiscoveredModel } from "@airis/airis-coding-agent/config/model-registry";
...
describe("mergeDiscoveredModel", () => {
	...
	test("preserves provider override transport on rediscovery (#2555 openrouter gateway regression)", () => {
		// Bundled openrouter entry carries transport=airis-native after
		...
		expect(merged.transport).toBe("airis-native");
