...
import { applyDirenvPreflight, executeBash } from "@airis/airis-coding-agent/exec/bash-executor";
import { findEnvrc, loadDirenvEnv, parseDirenvExport } from "@airis/airis-coding-agent/exec/direnv";
...
function tmp(): string {
	const dir = TempDir.createSync("@airs-direnv-");
	tmpDirs.push(dir);
	return dir.path();
}
...
describe.skipIf(!hasDirenv)("bash executor direnv wiring (end-to-end)", () => {
	...
	it("removes variables the .envrc unsets from the command environment", async () => {
		...
		await allowEnvrc(root);
		// Inherited from the process env (as an AIRIS-provided var would be); the
