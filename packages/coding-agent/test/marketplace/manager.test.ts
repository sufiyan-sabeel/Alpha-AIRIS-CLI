...
import { listAirisExtensionRoots } from "@airis/airis-coding-agent/discovery/airis-extension-roots";
...
function buildMinimalFixture(): string {
	const root = fs.mkdtempSync(path.join(os.tmpdir(), "airis-mgr-fixture-"));
...
function createTestContext(): TestContext {
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "airis-mgr-test-"));
...
function mockPluginManagerPaths(root: string) {
	return [
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(root, "airis-plugins.lock.json")),
...
describe("MarketplaceManager", () => {
	...
	it("installPlugin → plugin in cache + in registry", async () => {
		...
		const runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "airis-plugins.lock.json")).json();
	...
	it("installPlugin exposes marketplace package to the runtime loader", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "airis-mgr-home-"));
		try {
			const pluginsDir = path.join(tmpHome, ".airis", "plugins");
			const manager = new MarketplaceManager({
				marketplacesRegistryPath: path.join(tmpHome, ".airis", "marketplaces.json"),
	...
	it("installPlugin keeps marketplace packages out of AIRIS extension roots", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "airis-mgr-home-"));
		try {
			const pluginsDir = path.join(tmpHome, ".airis", "plugins");
			const manager = new MarketplaceManager({
			...
			const roots = await listAirisExtensionRoots({ cwd: tmpHome, home: tmpHome, repoRoot: null });
	...
	it("installPlugin with scope:project exposes the marketplace package to the runtime loader", async () => {
		const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "airis-mgr-home-"));
		const projectAnchor = fs.mkdtempSync(path.join(os.tmpdir(), "airis-mgr-project-"));
		try {
			const userPluginsDir = path.join(tmpHome, ".airis", "plugins");
			const projectPluginsDir = path.join(projectAnchor, ".airis", "plugins");
			...
			const projectLock = await Bun.file(path.join(projectPluginsDir, "airis-plugins.lock.json")).json();
			...
			expect(fs.existsSync(path.join(userPluginsDir, "airis-plugins.lock.json"))).toBe(false);
		} finally {
			fs.rmSync(tmpHome, { recursive: true, force: true });
	...
	it("setPluginEnabled → persisted in registry", async () => {
		...
		let runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "airis-plugins.lock.json")).json();
		expect(runtimeConfig.plugins["hello-plugin"].enabled).toBe(false);
		...
		runtimeConfig = await Bun.file(path.join(ctx.tmpDir, "airis-plugins.lock.json")).json();
	...
	it("installPlugin scope:project when no projectInstalledRegistryPath → throws", async () => {
		const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "airis-mgr-noproj-"));
		try {
