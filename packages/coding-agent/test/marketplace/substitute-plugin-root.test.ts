...
import { substitutePluginRoot } from "@airis/airis-coding-agent/discovery/substitute-plugin-root";
...
const AIRIS_VAR = "$" + "{AIRIS_PLUGIN_ROOT}";
...
describe("substitutePluginRoot", () => {
	...
	it("replaces AIRIS_PLUGIN_ROOT in strings", () => {
		expect(substitutePluginRoot(`${AIRIS_VAR}/bin/server`, ROOT)).toBe("/plugins/my-plugin/bin/server");
	});

	it("replaces both variables in same string", () => {
		expect(substitutePluginRoot(`${CLAUDE_VAR}:${AIRIS_VAR}`, ROOT)).toBe("/plugins/my-plugin:/plugins/my-plugin");
	});
