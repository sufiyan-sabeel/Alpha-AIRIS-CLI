...
describe("fetchMarketplace", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "airis-fetcher-test-"));
	});
	...
	it("loads catalog from .airis-plugin/marketplace.json when present", async () => {
		const root = path.join(tmpDir, "airis-only");
		fs.mkdirSync(path.join(root, ".airis-plugin"), { recursive: true });
		const catalog = {
			name: "airis-only-marketplace",
			owner: { name: "Test" },
			plugins: [{ name: "airis-plugin", source: "./plugins/airis-plugin", description: "x" }],
		};
		fs.writeFileSync(path.join(root, ".airis-plugin", "marketplace.json"), JSON.stringify(catalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("airis-only-marketplace");
		expect(result.catalog.plugins[0].name).toBe("airis-plugin");
	});

	it("prefers .airis-plugin/marketplace.json over .claude-plugin/marketplace.json when both exist", async () => {
		const root = path.join(tmpDir, "both-catalogs");
		fs.mkdirSync(path.join(root, ".airis-plugin"), { recursive: true });
		fs.mkdirSync(path.join(root, ".claude-plugin"), { recursive: true });
		const ompCatalog = {
			name: "from-airis-plugin",
			owner: { name: "Test" },
		...
		fs.writeFileSync(path.join(root, ".airis-plugin", "marketplace.json"), JSON.stringify(ompCatalog));
		fs.writeFileSync(path.join(root, ".claude-plugin", "marketplace.json"), JSON.stringify(claudeCatalog));

		const result = await fetchMarketplace(root, tmpDir);
		expect(result.catalog.name).toBe("from-airis-plugin");
	});

	it("falls back to .claude-plugin/marketplace.json when .airis-plugin is absent", async () => {
	...
	it("error message names both candidate paths when neither exists", async () => {
		...
		await expect(fetchMarketplace(empty, tmpDir)).rejects.toThrow(
			/\.airis-plugin[\\/]marketplace\.json.*\.claude-plugin[\\/]marketplace\.json/,
		);
	});
