...
import type { ObservableSession } from "@airis/airis-coding-agent/modes/session-observer-registry";
...
function makeSubagentRegistry(sessions: ObservableSession[]) {
	...
	} as unknown as import("@airis/airis-coding-agent/modes/session-observer-registry").SessionObserverRegistry;
}
...
describe("Agent hub silent-abort regression", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "airis-overlay-test-"));
	});
