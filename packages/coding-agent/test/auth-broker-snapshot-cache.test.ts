...
import { discoverAuthStorage } from "@airis/airis-coding-agent/sdk";
import { removeWithRetries } from "@airis/airis-utils";

const ENV_KEYS = [
	"AIRIS_AUTH_BROKER_URL",
	"AIRIS_AUTH_BROKER_TOKEN",
	"AIRIS_AUTH_BROKER_SNAPSHOT_CACHE",
	"AIRIS_AUTH_BROKER_SNAPSHOT_TTL_MS",
] as const;
...
describe("discoverAuthStorage auth-broker snapshot cache", () => {
	...
	test("boots from a fresh encrypted cache when the broker is down", async () => {
		...
		process.env.AIRIS_AUTH_BROKER_URL = downUrl;
		process.env.AIRIS_AUTH_BROKER_TOKEN = TOKEN;
		process.env.AIRIS_AUTH_BROKER_SNAPSHOT_CACHE = cachePath;
		process.env.AIRIS_AUTH_BROKER_SNAPSHOT_TTL_MS = "3600000";
	...
	test("boots from a fresh cache when revalidation returns a server error", async () => {
		...
		let storage: AuthStorage | undefined;
		try {
			process.env.AIRIS_AUTH_BROKER_URL = url;
			process.env.AIRIS_AUTH_BROKER_TOKEN = TOKEN;
			process.env.AIRIS_AUTH_BROKER_SNAPSHOT_CACHE = cachePath;
			process.env.AIRIS_AUTH_BROKER_SNAPSHOT_TTL_MS = "3600000";
			await writeAuthBrokerSnapshotCache({
