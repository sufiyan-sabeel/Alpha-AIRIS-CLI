...
import { getOrCreateSnapshot, sanitizeSnapshotForBrush } from "@airis/airis-coding-agent/utils/shell-snapshot";
...
describe("sanitizeSnapshotForBrush", () => {
	...
	it("leaves non-alias lines (functions, exports, unalias) untouched", () => {
		const snapshot = [
			"# Shell snapshot - generated by airis agent",
			"unalias -a 2>/dev/null || true",
			"my_fn () { echo hi; }",
...
// `__airis_emit_referenced_exports` (shell-snapshot-fn-env.sh) re-exports env
...
describe("getOrCreateSnapshot", () => {
	it("re-exports env vars referenced by snapshotted functions (issue #3470)", async () => {
		const home = await fs.mkdtemp(path.join(os.tmpdir(), "airis-snap-3470-"));
		await fs.writeFile(
		...
		const shellLink = path.join(home, "bash-airis-3470");
		...
		// reason — UUID filenames shouldn't leak via `ls /tmp/airis.shell-snapshots`.
	...
	it("keeps the snapshot file at 0600 even when the rc file resets umask to 022", async () => {
		...
		const home = await fs.mkdtemp(path.join(os.tmpdir(), "airis-snap-umask-"));
		await fs.writeFile(
			path.join(home, ".bashrc"),
		...
		const realBash = REAL_BASH;
		if (!existsSync(realBash)) return;
		const shellLink = path.join(home, "bash-airis-umask");
		await fs.symlink(realBash, shellLink);
	...
	it("cleans up the empty snapshot file when the shell exits with a non-zero code", async () => {
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airis-snap-fail-"));
		...
		try {
			...
			const snapshotDir = path.join(testRoot, "airis.shell-snapshots");
	...
	it("cleans up the empty snapshot file when the shell fails to spawn", async () => {
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airis-snap-spawn-fail-"));
		const originalTmpDir = process.env.TMPDIR;
		process.env.TMPDIR = testRoot;
		try {
	...
	it("cleans up the empty snapshot file when the shell execution times out", async () => {
		const testRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airis-snap-timeout-"));
		const originalTmpDir = process.env.TMPDIR;
		process.env.TMPDIR = testRoot;
		try {
