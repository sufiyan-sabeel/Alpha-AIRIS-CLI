/**
 * Routing tests for `airis plugin install <local-path>` (#1945).
 *
...
import { runPluginCommand } from "@airis/airis-coding-agent/cli/plugin-cli";
import { PluginManager } from "@airis/airis-coding-agent/extensibility/plugins/manager";
import { MarketplaceManager } from "@airis/airis-coding-agent/extensibility/plugins/marketplace";
import type { InstalledPlugin } from "@airis/airis-coding-agent/extensibility/plugins/types";
...
async function createLocalPlugin(root: string): Promise<string> {
	...
	await Bun.write(
		...
		JSON.stringify({
			name: "kimi-datasource",
			version: "1.0.0",
			airis: { extensions: ["./src/extension.ts"] },
		}),
	);
	return localPlugin;
}
...
describe("runPluginCommand({ action: 'install', args: [<local>] })", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airis-plugin-install-local-"));
		...
		spyOn(piUtils, "getPluginsLockfile").mockReturnValue(path.join(tmpRoot, "airis-plugins.lock.json"));
	...
	test("real local plugin directory: install symlinks it like link would", async () => {
		...
		// symlink rather than copy-install, matching `airis plugin link`.
	...
	test("doctor --fix preserves linked local plugin state without package dependencies", async () => {
		await Bun.write(
			...
			JSON.stringify({ name: "airis-plugins", private: true, dependencies: {} }),
		);
		const localPlugin = await createLocalPlugin(tmpRoot);
