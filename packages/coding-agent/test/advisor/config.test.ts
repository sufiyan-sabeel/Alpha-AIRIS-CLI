...
describe("discoverAdvisorConfigs", () => {
	...
	beforeEach(async () => {
		tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-advisor-config-"));
		// Empty agent dir so the user-level search path can't pick up a real ~/.airis/WATCHDOG.yml.
		agentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-advisor-agentdir-"));
	});
...
describe("WATCHDOG.yml file round-trip", () => {
	let tmp: string;
	beforeEach(async () => {
		tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-advisor-file-"));
	});
	afterEach(async () => {
		await fsp.rm(tmp, { recursive: true, force: true });
	});
	...
	it("resolves project and user scope paths", () => {
		expect(advisorConfigFilePath("project", { projectDir: "/repo", agentDir: "/home/.airis" })).toBe(
			path.join("/repo", "WATCHDOG.yml"),
		);
		expect(advisorConfigFilePath("user", { projectDir: "/repo", agentDir: "/home/.airis" })).toBe(
			path.join("/home/.airis", "WATCHDOG.yml"),
		);
	});
});
...
describe("resolveAdvisorConfigEditPath", () => {
	let tmp: string;
	beforeEach(async () => {
		tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-advisor-resolve-"));
	});
	afterEach(async () => {
		await fsp.rm(tmp, { recursive: true, force: true });
	});
...
describe("per-advisor enabled field", () => {
	it("preserves explicit true, explicit false, and absence through save and discovery", async () => {
		const tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-advisor-enabled-"));
		try {
			const doc: WatchdogConfigDoc = {
				advisors: [
