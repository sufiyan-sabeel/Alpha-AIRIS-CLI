...
import { resolveUpdateMethodForTest } from "@airis/airis-coding-agent/cli/update-cli";
import { removeSyncWithRetries } from "@airis/airis-utils";

// Issue #845: on Windows with Bun installed via Scoop, ~/.bun is a junction
// to scoop\persist\Oven-sh.Bun\.bun. `bun pm bin -g` and the airis path that
...
// does not follow filesystem links, so it misclassified Bun-installed airis
// as "binary" and tried to swap airis.exe in place – which fails on Windows
...
describe("issue-845: resolveUpdateMethod follows symlinks/junctions", () => {
	...
	it("classifies airis reached through a symlinked bin dir as bun-managed", () => {
	...
	it("classifies airis at the real bin dir as bun-managed when bunBinDir is symlinked", () => {
		const ompAtReal = path.join(realBinDir, "airis");
