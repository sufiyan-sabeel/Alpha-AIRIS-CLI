...
import type { CustomToolContext } from "@airis/airis-coding-agent/extensibility/custom-tools";
import { DeferredMCPTool, MCPTool, type MCPToolDefinition } from "@airis/airis-coding-agent/mcp";
import type { MCPServerConnection } from "@airis/airis-coding-agent/mcp/types";
...
describe("MCP tool arguments", () => {
	...
	it("resolves local image arguments before forwarding tools/call", async () => {
		using tempDir = TempDir.createSync("@airs-mcp-local-image-");
		const calls: CapturedRequest[] = [];
