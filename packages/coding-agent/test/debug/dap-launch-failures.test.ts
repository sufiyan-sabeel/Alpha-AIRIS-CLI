...
import * as dapModule from "@airis/airis-coding-agent/dap";
import { connectSocket, DapClient, waitForTcpServerListening } from "@airis/airis-coding-agent/dap/client";
import { DapSessionManager } from "@airis/airis-coding-agent/dap/session";
import type {
	DapCapabilities,
	DapClientState,
	DapEventMessage,
	DapResolvedAdapter,
} from "@airis/airis-coding-agent/dap/types";
import type { ToolSession } from "@airis/airis-coding-agent/tools";
import { DebugTool } from "@airis/airis-coding-agent/tools/debug";
...
describe("DAP launch failure handling", () => {
	...
	it("waits for delayed Unix socket adapters before connecting on Linux", async () => {
		...
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-dlv-socket-"));
	...
	it("kills the detached adapter process when the Unix socket never appears (Linux)", async () => {
		if (process.platform !== "linux") return;
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-unix-leak-"));
		try {
	...
	it("kills the detached adapter process when it never dials back on the TCP client-addr path", async () => {
		...
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-tcp-leak-"));
			try {
...
describe("connectSocket unix transport", () => {
	it("rejects instead of hanging when the unix socket cannot be connected", async () => {
		...
		const deadSocket = path.join(
			os.tmpdir(),
			`airis-dap-dead-${Date.now()}-${Math.random().toString(36).slice(2)}.sock`,
		);
		const start = Date.now();
...
`;
	...
	async function withTcpAdapter(
		...
		const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-tcp-"));
...
describe("DebugTool launch validation", () => {
	it("rejects directory programs when the selected adapter cannot debug a directory", async () => {
		...
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-program-"));
			try {
	...
	it("allows directory programs when the selected adapter accepts them (dlv on a Go package)", async () => {
		...
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-dlv-dir-"));
			try {
	...
	it("prefers directory-capable dlv over native adapters for extensionless Go package directories", async () => {
		...
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-dlv-mixed-roots-"));
			try {
	...
	it("dlv launch with a compiled binary switches mode from debug to exec", async () => {
		...
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-dlv-exec-"));
			try {
	...
	it("throws targeted 'python not found in PATH' when adapter:'debugpy' is unresolvable for launch", async () => {
		...
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-debugpy-"));
			try {
	...
	it("throws targeted 'python not found in PATH' when adapter:'debugpy' is unresolvable for attach", async () => {
		...
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-debugpy-attach-"));
			try {
				const session: ToolSession = {
					cwd,
					hasUI: false,
	...
	it("shows the Delve install command when the canonical dlv adapter is unavailable", async () => {
		...
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-dlv-hint-"));
			try {
	...
	it("points to DAP configuration when a custom adapter command is unavailable", async () => {
		...
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-dlv-config-"));
			try {
				await fs.writeFile(path.join(cwd, "main.go"), "package main\n\nfunc main() {}\n");
				const session: ToolSession = {
					cwd,
	...
	it("shows the rdbg install command for explicit Ruby attach", async () => {
		const attachSpy = spyOn(dapModule, "selectAttachAdapter").mockReturnValue(null);
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-rdbg-attach-"));
			try {
				const session: ToolSession = {
					cwd,
					hasUI: false,
	...
	it("falls back to the generic 'No debugger adapter' error when adapter is unspecified", async () => {
		...
		try {
			const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-debug-noadapter-"));
			try {
				await fs.writeFile(path.join(cwd, "main.py"), "print('hi')");
				const session: ToolSession = {
					cwd,
