...
import { RawSseDebugBuffer } from "@airis/airis-coding-agent/debug/raw-sse-buffer";
...
describe("raw SSE report bundle", () => {
	it("includes captured raw SSE text and dropped-record disclosure", async () => {
		cleanupRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airis-raw-sse-report-"));
		...
		expect(rawSseText).toContain(": airis-debug-dropped records=");
