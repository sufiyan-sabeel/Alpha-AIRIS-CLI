...
import type { SessionManager } from "@airis/airis-coding-agent/session/session-manager";
import { executeAcpBuiltinSlashCommand } from "@airis/airis-coding-agent/slash-commands/acp-builtins";
import { removeWithRetries, setProjectDir } from "@airis/airis-utils";
...
describe("ACP builtin slash commands", () => {
	...
	it("dump: outputs transcript with LLM request JSON path when sidecar succeeds", async () => {
		...
		runtime.session.dumpLlmRequestToTmpDir = async () => "/tmp/airis-llm-request-test.json";

		const result = await executeAcpBuiltinSlashCommand("/dump", runtime);

		expect(result).toEqual({ consumed: true });
		expect(output[0]).toContain("Session content here");
		expect(output[0]).toContain("LLM request JSON: /tmp/airis-llm-request-test.json");
...
describe("wave 5 — adapters and polish", () => {
	...
	it("/mcp add foo --url https://example.com --token X --scope project: outputs success or propagates write error", async () => {
		// Uses project scope so it writes to /tmp/project/.airis/mcp.json which test infra controls.
		// We verify the command either reports success or a meaningful error (not a parse error).
		const mcpModule = await import("@airis/airis-coding-agent/mcp/config-writer");
	...
	it("/ssh add foo --host x --user y --scope user: calls addSSHHost", async () => {
		const sshModule = await import("@airis/airis-coding-agent/ssh/config-writer");
	...
	it("/marketplace discover: output is bulleted with '  - ' token", async () => {
		const { MarketplaceManager } = await import("@airis/airis-coding-agent/extensibility/plugins/marketplace");
...
describe("/move preflight flush", () => {
	it("aborts text-mode /move when pending settings flush fails", async () => {
		const targetDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-acp-move-"));
		try {
			const { output, fakeSessionManager, runtime } = createRuntime();
	...
	it("completes text-mode /move when flush succeeds", async () => {
		const targetDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-acp-move-ok-"));
		const originalProjectDir = process.cwd();
		try {
			const { output, fakeSessionManager, runtime } = createRuntime();
			let flushed = false;
