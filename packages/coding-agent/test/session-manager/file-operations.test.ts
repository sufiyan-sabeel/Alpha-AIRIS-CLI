...
import type { FileEntry, SessionHeader } from "@airis/airis-coding-agent/session/session-entries";
import { findMostRecentSession, resolveResumableSession } from "@airis/airis-coding-agent/session/session-listing";
import { loadEntriesFromFile } from "@airis/airis-coding-agent/session/session-loader";
import { SessionManager } from "@airis/airis-coding-agent/session/session-manager";
import { getConfigRootDir, getSessionsDir, removeSyncWithRetries, Snowflake, setAgentDir } from "@airis/airis-utils";
...
describe("SessionManager temp cwd session dirs", () => {
	...
	beforeEach(() => {
		testAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "airis-session-dir-test-"));
...
describe("SessionManager legacy session migration persistence", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "airis-session-manager-legacy-"));
	});
