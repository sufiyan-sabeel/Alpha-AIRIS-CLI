...
import { readTerminalBreadcrumbEntry } from "@airis/airis-coding-agent/session/session-paths";
...
describe("SessionManager subagent breadcrumb isolation", () => {
	...
	beforeEach(async () => {
		...
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-subagent-crumb-"));
