...
import { cleanupEmptyMoveSession, SessionManager } from "@airis/airis-coding-agent/session/session-manager";
...
describe("move-session cleanup tracking", () => {
	...
	beforeEach(async () => {
		testAgentDir = await fsp.mkdtemp(path.join(os.tmpdir(), "airis-move-cleanup-"));
