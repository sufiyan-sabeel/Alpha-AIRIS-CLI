 ...
 * `airis list` / `airis remove` / `airis marketplace <sub>` / `airis uninstall …` etc.
 * as top-level commands, but only `airis install` is registered. Before the fix,
 * `resolveCliArgv(["list"])` rewrote the bare verb to `["launch", "list"]`, so
 * `airis list` silently started an interactive agent session with "list" as the
 * initial LLM prompt instead of managing plugins (the real command is
 * `airis plugin list`). #4845 extended the same footgun to the multi-word
 * documented grammar: `airis marketplace add xyz` leaked the whole argv to the
 * model as a prompt.
 *
 ...
 * `airis plugin <action>` command rather than leaking to the model — while
 * genuine prose prompts that merely begin with one of these words still fall
 * through to `launch`.
 *
 * Imported via a relative path (not the `@airis/airis-coding-agent` alias) so the
...
describe("documented-but-unregistered plugin verbs do not leak to launch (#2935)", () => {
	test("bare `airis list` hints at `airis plugin list` instead of launching with 'list' as the prompt", () => {
		...
		expect("error" in result && result.error).toContain("airis plugin list");
	});

	test("bare `airis remove` hints at `airis plugin uninstall` instead of launching with 'remove' as the prompt", () => {
		const result = resolveCliArgv(["remove"]);
		expect(result).not.toEqual({ argv: ["launch", "remove"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("airis plugin uninstall");
	});
	...
	test("multi-word `airis marketplace add xyz` hints at `airis plugin marketplace` instead of leaking to the prompt (#4845)", () => {
		const result = resolveCliArgv(["marketplace", "add", "xyz"]);
		expect(result).not.toEqual({ argv: ["launch", "marketplace", "add", "xyz"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("airis plugin marketplace");
	});

	test("bare marketplace-family verbs hint at their `airis plugin` command (#4845)", () => {
		for (const [verb, hint] of [
			["marketplace", "airis plugin marketplace"],
			["discover", "airis plugin discover"],
			["upgrade", "airis plugin upgrade"],
			["uninstall", "airis plugin uninstall"],
			["enable", "airis plugin enable"],
			["disable", "airis plugin disable"],
		] as const) {
