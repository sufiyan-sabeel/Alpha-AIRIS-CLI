 ...
 *
 * A named profile relocates the agent directory to ~/.airis/profiles/<name>/agent.
 * The native config provider used to read user-scope mcp.json from the literal
 * home (~/.airis/agent/mcp.json) via `ctx.home`, so a profile never saw its own
 ...
 *
 * `os.homedir()` is mocked so the *old* code path (ctx.home + ".airis/agent")
...
describe("native user-level MCP discovery follows the active profile", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "airis-mcp-profile-home-"));
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-mcp-profile-project-"));
	...
	test("active profile loads its own user server, not the default profile's", async () => {
		// Active profile's agent dir (stand-in for ~/.airis/profiles/<name>/agent).
		const profileAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-mcp-profile-agent-"));
		...
		await writeMcpJson(path.join(tempHome, ".airis", "agent"), {
	...
	test("default profile loads the user server from ~/.airis/agent", async () => {
