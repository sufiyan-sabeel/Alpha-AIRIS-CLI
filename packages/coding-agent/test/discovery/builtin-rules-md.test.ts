 ...
 * from both `~/.airis/agent/RULES.md` (user) and the nearest `.airis/RULES.md`
...
import { type Rule, ruleCapability } from "@airis/airis-coding-agent/capability/rule";
...
test("user ~/.airis/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".airis", "agent", "RULES.md"),
...
test("project .airis/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".airis", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".airis", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".airis", "RULES.md"));
});

test("user and project sticky RULES.md both survive public capability dedup", async () => {
	const userRulesPath = path.join(home, ".airis", "agent", "RULES.md");
	const projectRulesPath = path.join(project, ".airis", "RULES.md");
...
test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".airis", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	// No RULES.md anywhere — only a sibling .airis/rules/ to make sure the directory exists.
	writeFile(path.join(home, ".airis", "agent", "rules", "other.md"), "# Unrelated rule\n");
