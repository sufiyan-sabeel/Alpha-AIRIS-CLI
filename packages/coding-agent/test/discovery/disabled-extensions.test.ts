...
import { initializeWithSettings, loadCapability } from "@airis/airis-coding-agent/discovery";
import { __resetDirsFromEnvForTests, removeWithRetries, setAgentDir } from "@airis/airis-utils";
...
describe("disabledExtensions runtime filtering", () => {
	...
	beforeEach(async () => {
		...
		tempHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-disabled-ext-home-"));
		process.env.HOME = tempHomeDir;
		vi.spyOn(os, "homedir").mockReturnValue(tempHomeDir);
		setAgentDir(path.join(tempHomeDir, ".airis", "agent"));
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-disabled-ext-"));
		await fs.mkdir(path.join(tempDir, ".airis"), { recursive: true });
		await fs.writeFile(path.join(tempDir, ".airis", "AGENTS.md"), "# project instructions\n");
	...
	afterEach(async () => {
		...
		restoreEnvValue("AIRIS_PROFILE", originalOmpProfileEnv);
