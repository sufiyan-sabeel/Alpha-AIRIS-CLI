 ...
 * in any stranger script (Codex hook scripts, scratch files, …) killed AIRIS
...
import { type Hook, hookCapability } from "@airis/airis-coding-agent/capability/hook";
...
describe("codex hook discovery", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "airis-codex-hooks-home-"));
		tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "airis-codex-hooks-cwd-"));
	...
	test("skips untyped Codex hook scripts so they never reach the extension loader (#3680)", async () => {
		...
		// AIRIS-shaped pre-*/post-* files but do not match the prefix.
