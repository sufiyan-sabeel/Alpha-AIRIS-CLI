...
import type { LoadContext, LoadResult } from "@airis/airis-coding-agent/capability/types";
import { scanSkillsFromDir } from "@airis/airis-coding-agent/discovery/helpers";
...
describe("monorepo skill discovery", () => {
	...
	test("finds skills in ancestor .airis/skills/ directories", async () => {
		// Root has a skill
		writeSkill(path.join(repoRoot, ".airis", "skills"), "root-skill", "From repo root");
		// Sub-project has a skill
		writeSkill(path.join(subProject, ".airis", "skills"), "local-skill", "From sub-project");
		...
		while (true) {
			const result = await scanSkillsFromDir(ctx, {
				dir: path.join(current, ".airis", "skills"),
				providerId: "native",
				level: "project",
			});
			results.push(result);
	...
	test("closest skill wins when same name exists at multiple levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".airis", "skills"), "shared-skill", "Root version");
		writeSkill(path.join(subProject, ".airis", "skills"), "shared-skill", "Local version");
	...
	test("finds skills across multiple ancestor levels", async () => {
		...
		writeSkill(path.join(repoRoot, ".airis", "skills"), "root-skill", "Root");
		writeSkill(path.join(packagesDir, ".airis", "skills"), "packages-skill", "Packages");
		writeSkill(path.join(subProject, ".airis", "skills"), "app-skill", "App");
	...
	test("walk-up stops at repo root and does not find skills above it", async () => {
		...
		writeSkill(path.join(tempDir, ".airis", "skills"), "above-repo-skill", "Above repo");
		// Skill AT the repo root (should be found)
		writeSkill(path.join(repoRoot, ".airis", "skills"), "root-skill", "At repo root");
