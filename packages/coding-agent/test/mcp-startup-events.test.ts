...
} from "@airis/airis-coding-agent/mcp/startup-events";
...
describe("mcp/startup-events — connection-status cross-module contract", () => {
	...
	it("sanitizes failure errors before rendering them in status text", () => {
		const homePath = `${os.homedir()}/.airis/mcp.log`;
		...
		expect(message).not.toContain("\t");
		expect(message).toContain("broken: failed at   ~/.airis/mcp.log");
	});

	it("sanitizes server names before rendering them in status text", () => {
		const homePath = `${os.homedir()}/.airis`;
		const message = formatMCPConnectionStatusMessage({
		...
		expect(message).not.toContain("\n");
		expect(message).not.toContain("\t");
		expect(message).toContain("Connected: ~/.airis/connected   server.");
		expect(message).toContain("Failed: ~/.airis/broken server: missing command.");
		expect(message).toContain("Still connecting: ~/.airis/pending");
	});
