...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "airis-html-template-"));
...
const bundledDependencyStubs: Record<string, string> = {
	"@airis/airis-utils": 'export const APP_NAME = "airis"; export const isEnoent = () => false;',
	"../../modes/theme/theme":
...
describe("HTML export template", () => {
	...
	test("composes the exact source template once and returns the cached bytes", async () => {
		...
		expect(first).toContain("const THEME_STORAGE_KEY = 'airis-export-theme';");
