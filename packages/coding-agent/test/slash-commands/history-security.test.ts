...
import { shouldSkipHistory } from "@airis/airis-coding-agent/modes/controllers/input-controller";
...
describe("shouldSkipHistory — security filter for slash command history", () => {
	...
	it("skips /join with a link argument (carries 32-byte room key and write token)", () => {
		expect(shouldSkipHistory("/join airis://share/abc123def456...")).toBe(true);
		expect(shouldSkipHistory("/join airis:abc123def456...")).toBe(true);
	});
