...
import { resolveResumableSession } from "@airis/airis-coding-agent/session/session-listing";
import { computeDefaultSessionDir } from "@airis/airis-coding-agent/session/session-paths";
...
beforeEach(async () => {
	tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-resume-command-"));
