...
import type { AgentDefinition, TaskParams } from "@airis/airis-coding-agent/task/types";
import type { IsolationHandle, WorktreeBaseline } from "@airis/airis-coding-agent/task/worktree";
...
import "@airis/airis-coding-agent/tools/yield";
...
describe("subagent LSP availability", () => {
	...
	it("opens isolated persisted subagent sessions with the worktree cwd", async () => {
		...
		mockIsolation();
		const { getOptions } = mockCreateAgentSession();
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-isolated-session-cwd-"));
		try {
