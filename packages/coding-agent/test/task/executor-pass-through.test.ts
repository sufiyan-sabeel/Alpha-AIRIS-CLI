...
import type { ToolPathWithSource } from "@airis/airis-coding-agent/extensibility/custom-tools";
import type { LoadExtensionsResult } from "@airis/airis-coding-agent/extensibility/extensions/types";
import type { MCPManager } from "@airis/airis-coding-agent/mcp/manager";
...
describe("runSubprocess parent-discovery pass-through (issue #2190)", () => {
	...
	it("forwards rules, preloadedExtensionPaths, and preloadedCustomToolPaths to createAgentSession", async () => {
		...
		const preloadedExtensionPaths = ["/abs/parent/.airis/extensions/foo.ts"];
