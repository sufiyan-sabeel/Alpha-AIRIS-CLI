...
} from "@airis/airis-coding-agent/task/isolation-runner";
import type { SingleResult } from "@airis/airis-coding-agent/task/types";
import * as worktreeModule from "@airis/airis-coding-agent/task/worktree";
import * as gitModule from "@airis/airis-coding-agent/utils/git";
...
async function seedFooRepo(finalContent: string): Promise<{ repoRoot: string; patchPath: string }> {
	const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airis-isolation-merge-"));
...
describe("runIsolatedSubprocess", () => {
	...
	it("preserves branch-mode output as a patch when branch transfer fails", async () => {
		const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airis-isolation-run-"));
		tempRoots.push(repoRoot);
		...
		expect(deleteSpy).toHaveBeenCalledWith(repoRoot, "airis/task/PreserveBranchFailure");
