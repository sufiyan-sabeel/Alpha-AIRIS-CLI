...
import {
	...
	type TtsrScanArgs,
	type TtsrTestArgs,
} from "@airis/airis-coding-agent/cli/ttsr-cli";
import { resetSettingsForTest } from "@airis/airis-coding-agent/config/settings";
import { getProjectAgentDir, getProjectDir, removeSyncWithRetries, setProjectDir } from "@airis/airis-utils";

let testTmpDir: string;

beforeAll(() => {
	testTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "airis-ttsr-tests-"));
});
...
describe("airis ttsr", () => {
	...
	describe("test — context inference and matching", () => {
		it("infers tool/edit context when a positional resolves to a .ts file and --source is omitted", async () => {
			...
			// Simulate `airis ttsr test --rule <rule> src/foo.ts`: the command layer
