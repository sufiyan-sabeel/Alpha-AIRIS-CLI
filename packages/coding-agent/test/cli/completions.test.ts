...
import { buildSpec, type CompletionSpec, generateCompletion } from "@airis/airis-coding-agent/cli/completion-gen";
import type { CliConfig, CommandCtor } from "@airis/airis-utils/cli";
...
describe("generateCompletion — bash", () => {
	...
	it("registers the dispatcher and resolves alias arms to the canonical handler", () => {
		expect(out).toContain("complete -F _omp airis");
		expect(out).toContain("_airis_cmd_commit");
	...
	it("completes enum, dynamic, and comma-list flag values by previous flag", () => {
		...
		expect(out).toContain('--model)\n\t\t\tCOMPREPLY=( $(compgen -W "$(command airis __complete models -- "$cur"');
		expect(out).toContain("--resume|-r)");
		expect(out).toContain("command airis __complete sessions");
		// static comma list routes through the comma-aware helper
		expect(out).toContain('--tools)\n\t\t\t_airis_comma "read bash"');
	...
	it("completes a subcommand's positional enum and its own flags", () => {
		expect(out).toContain("_airis_cmd_worktree()");
...
describe("generateCompletion — zsh", () => {
	...
	it("emits the compdef header and dual-mode (autoload + eval) tail", () => {
		expect(out.startsWith("#compdef airis")).toBe(true);
		expect(out).toContain('if [ "$funcstack[1]" = "_omp" ]; then');
		expect(out).toContain("compdef _omp airis");
	});

	it("maps value sources to the right _arguments actions", () => {
		expect(out).toContain("'--model[Model to use]:model:_airis_call models'");
		expect(out).toContain("'--models[Model list]:models:_airis_models_list'");
		expect(out).toContain("'--thinking[Effort]:value:(low high)'");
		expect(out).toContain("'--tools[Tools]:value:_airis_tools'");
		expect(out).toContain("'(-r --resume)'{-r,--resume}'[Resume]:session:_airis_call sessions'");
		...
		expect(out).toContain("_airis_tools() { _values -s , 'tools' read bash }");
	});

	it("dispatches aliased subcommands and completes positional enums", () => {
		expect(out).toContain("worktree|wt) _airis_cmd_worktree ;;");
...
describe("generateCompletion — fish", () => {
	...
	it("declares the no-subcommand predicate over every command token", () => {
		expect(out).toContain("function __fish_airis_no_subcommand");
	...
	it("maps value sources to fish completion args", () => {
		expect(out).toContain("-l model -d 'Model to use' -x -a '(command airis __complete models -- (commandline -ct))'");
		expect(out).toContain("-l thinking -d 'Effort' -x -a 'low high'");
		expect(out).toContain("-l tools -d 'Tools' -x -a 'read bash'");
		expect(out).toContain("-s r -l resume -d 'Resume' -x -a '(command airis __complete sessions");
...
describe("airis completions (integration / drift)", () => {
	it("emits a zsh script reflecting the live command + flag surface", async () => {
		...
		expect(stdout).toContain("_airis_cmd_commit");
		expect(stdout).toContain("'completions:");
		// zsh routes single-value dynamic flags through the _airis_call action, which
		// itself shells out to `airis __complete $kind`.
		expect(stdout).toContain("_airis_call models");
		expect(stdout).toContain("_airis_call sessions");
		expect(stdout).toContain("command airis __complete $kind");
		// Hidden/default commands must NOT surface as completable subcommands.
		expect(stdout).not.toContain("_airis_cmd_launch");
		expect(stdout).not.toContain("_airis_cmd___complete");
	});
});
