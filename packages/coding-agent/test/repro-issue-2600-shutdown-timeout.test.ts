 ...
 * `airis-discord-presence` waiting on a Discord IPC pipe that never replied)
...
describe("issue #2600 - session_shutdown handler timeout", () => {
	...
	beforeAll(async () => {
		sharedTempDir = TempDir.createSync("@airs-issue-2600-shared-");
	...
	async function buildRunnerWithHangingShutdown(count = 1): Promise<{
		...
		const tempDir = TempDir.createSync("@airs-issue-2600-test-");
