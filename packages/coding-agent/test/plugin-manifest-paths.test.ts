...
} from "@airis/airis-coding-agent/extensibility/plugins/loader";
import type { InstalledPlugin, PluginManifest } from "@airis/airis-coding-agent/extensibility/plugins/types";
...
describe("plugin manifest path resolution", () => {
	it("resolves a directory tools entry to its index, not the airis.extensions modules", () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "airis-manifest-paths-"));
		try {
			// The package declares both extensions and a directory-based tool entry.
			// `airis.extensions` and the sub-extension scan are extensions-specific and
			...
			fs.writeFileSync(
				...
				JSON.stringify({ name: "fixture-plugin", version: "1.0.0", airis: { extensions: ["./ext.ts"], tools: "." } }),
			);
