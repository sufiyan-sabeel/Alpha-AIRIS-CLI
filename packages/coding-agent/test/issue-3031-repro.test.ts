/**
 * Regression for https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI/issues/3031
 *
 * Mnemosyne's local embedding provider used to `import("onnxruntime-node")` and
 * `import("fastembed")` directly inside `fastembed-runtime.ts`. With
 * `memory.backend: mnemosyne` enabled on Windows that crashed Bun in two ways:
 ...
 * agent's main process hands `mnemosyne.setLocalModelInitializer` a wrapper that
 * round-trips through `__airis_worker_mnemosyne_embed`, and `SIGKILL`s the child
...
import {
	createMnemosyneEmbedSubprocess,
	MnemosyneEmbedClient,
	type MnemosyneEmbedWorkerHandle,
} from "@airis/airis-coding-agent/mnemosyne/embed-client";
import type {
	MnemosyneEmbedWorkerInbound,
	MnemosyneEmbedWorkerOutbound,
} from "@airis/airis-coding-agent/mnemosyne/embed-protocol";

describe("issue #3031 — mnemosyne embeddings live in an isolated subprocess", () => {
	it("ping/pongs through the spawned worker subprocess and tears it down cleanly", async () => {
		// `smokeTestMnemosyneEmbedWorker` is the runtime probe wired into
		...
		const script =
			'const { smokeTestMnemosyneEmbedWorker } = await import("@airis/airis-coding-agent/mnemosyne/embed-client"); await smokeTestMnemosyneEmbedWorker({ timeoutMs: 15000 });';
	...
	it("surfaces unexpected signal exits so in-flight callers don't await forever", async () => {
		...
		const sub = createMnemosyneEmbedSubprocess();
		try {
	...
	it("carries (model, cacheDir) on every embed so a respawned worker can self-init", async () => {
		// Without this contract: after `shutdownMnemosyneEmbedClient()` runs on
		// session dispose, mnemosyne still holds the cached `LocalEmbeddingModel`
		...
		const sentMessages: MnemosyneEmbedWorkerInbound[] = [];
		let messageHandler: ((message: MnemosyneEmbedWorkerOutbound) => void) | undefined;
		const spawnCount = { value: 0 };
		const spawn = (): MnemosyneEmbedWorkerHandle => {
			spawnCount.value += 1;
			return {
				send(message) {
		...
		const client = new MnemosyneEmbedClient(spawn);
		...
		const embeds = sentMessages.filter(
			(m): m is Extract<MnemosyneEmbedWorkerInbound, { type: "embed" }> => m.type === "embed",
		);
