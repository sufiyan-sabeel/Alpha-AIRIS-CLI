...
import { rewriteImports, wrapCode } from "@airis/airis-coding-agent/eval/js/context-manager";
import { indirectEval } from "@airis/airis-coding-agent/eval/js/shared/indirect-eval";
...
describe("rewriteImports", () => {
	it("rewrites a top-level default import", async () => {
		...
		expect(out).toContain('await __airis_import__("bar")');
	...
	it("rewrites destructured named imports with renames", async () => {
		...
		expect(out).toContain('await __airis_import__("pkg")');
	...
	it("rewrites namespace imports", async () => {
		...
		expect(out).toContain('const ns = await __airis_import__("pkg")');
	});
	...
	it("rewrites side-effect-only imports", async () => {
		...
		expect(out).toContain('await __airis_import__("polyfill")');
	});
	...
	it("preserves import attributes via the dynamic import options bag", async () => {
		...
		expect(out).toContain('await __airis_import__("./d.json", { with: { type: "json" } })');
		expect(out).toContain("const data =");
	});

	// Dynamic `import(...)` callees are swapped for a shim that prefers the worker-injected
	// `__airis_import__` helper but falls back to native dynamic import. The fallback matters:
	...
	const SHIM = '(typeof __airis_import__ === "function" ? __airis_import__ : (s, o) => import(s, o))';
	...
	it("routes dynamic import through the helper when present and native import when serialized into a foreign realm", async () => {
		...
		const hasOriginal = "__airis_import__" in globals;
		const originalOmpImport = globals.__airis_import__;
		if (hasOriginal) {
			delete globals.__airis_import__;
		}

		expect("__airis_import__" in globals).toBe(false);

		// Worker realm: helper global exists, call must route through it.
		const seen: string[] = [];
		globals.__airis_import__ = async (source: string) => {
			seen.push(source);
			return { stubbed: true };
		};
		try {
			...
			if (hasOriginal) {
				globals.__airis_import__ = originalOmpImport;
			} else {
				delete globals.__airis_import__;
			}
		}
	...
	it("does not rewrite import statements embedded in template literals (the bug)", async () => {
		...
		expect(out).not.toContain("await __airis_import__(");
	});
	...
	it("does not rewrite import statements inside block comments", async () => {
		...
		expect(out).not.toContain('await __airis_import__("bar")');
	});
	...
	it("rewrites real top-level imports while leaving template-embedded look-alikes alone", async () => {
		...
		const out = await rewriteImports(code);
		expect(out).toContain('await __airis_import__("alpha")');
		expect(out).toContain('await __airis_import__("gamma")');
		expect(out).not.toContain('await __airis_import__("beta")');
	...
	it("captures the final expression even when trailing empty statements follow", async () => {
		...
		expect(wrapped.source).toContain("__airis_set_final_expr__((await Promise.resolve(1)))");
	});
	...
	it("strips type-only imports before rewriting imports and top-level return", async () => {
		...
		expect(wrapped.finalExpressionReturned).toBe(true);
		expect(wrapped.source).toContain("__airis_set_final_expr__(42)");
