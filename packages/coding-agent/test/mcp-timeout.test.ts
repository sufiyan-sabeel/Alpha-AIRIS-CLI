...
import { isMCPTimeoutEnabled, resolveMCPTimeoutMs } from "@airis/airis-coding-agent/mcp/timeout";
import { logger } from "@airis/airis-utils";

const ORIGINAL_TIMEOUT = process.env.AIRIS_MCP_TIMEOUT_MS;

afterEach(() => {
	if (ORIGINAL_TIMEOUT === undefined) {
		delete process.env.AIRIS_MCP_TIMEOUT_MS;
	} else {
		process.env.AIRIS_MCP_TIMEOUT_MS = ORIGINAL_TIMEOUT;
	}
});
...
describe("MCP timeout configuration", () => {
	...
	test("allows the env override to disable MCP client-side timeouts", () => {
		process.env.AIRIS_MCP_TIMEOUT_MS = "0";
	...
	test("rejects non-numeric env values and falls back to the default", () => {
		process.env.AIRIS_MCP_TIMEOUT_MS = "not-a-number";
		const warn = spyOn(logger, "warn").mockImplementation(() => {});
