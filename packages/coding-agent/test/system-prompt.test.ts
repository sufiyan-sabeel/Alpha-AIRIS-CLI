...
async function runProbeScenario(options: {
	...
	const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "airis-gpu-probe-"));
	try {
		const binDir = path.join(tempRoot, "bin");
		const cacheRoot = path.join(tempRoot, "cache");
		const probeCountPath = path.join(tempRoot, "probe-count");
		await fs.mkdir(binDir, { recursive: true });
		await fs.mkdir(path.join(cacheRoot, "airis"), { recursive: true });
		const lspciPath = path.join(binDir, "lspci");
		await Bun.write(
			lspciPath,
			'#!/usr/bin/env sh\nprintf x >> "$AIRIS_GPU_PROBE_COUNT"\nif [ -n "$AIRIS_GPU_PROBE_VALID_OUTPUT" ]; then printf "%s\\n" "$AIRIS_GPU_PROBE_VALID_OUTPUT"; fi\nif [ "$AIRIS_GPU_PROBE_DESCENDANT_HOLDS_STDOUT" = "true" ]; then sleep "$AIRIS_GPU_PROBE_SLEEP" & exit 0; fi\nif [ "$AIRIS_GPU_PROBE_HOLD_STDOUT_OPEN" = "true" ]; then sleep "$AIRIS_GPU_PROBE_SLEEP" & wait "$!"; fi\nif [ -n "$AIRIS_GPU_PROBE_SLEEP" ]; then exec sleep "$AIRIS_GPU_PROBE_SLEEP"; fi\nexit 0\n',
		);
...
for (let index = 0; index < Number(process.env.AIRIS_GPU_PROBE_RUNS ?? "1"); index += 1) {
...
const countFile = Bun.file(process.env.AIRIS_GPU_PROBE_COUNT ?? "");
...
`,
		...
		const env: Record<string, string | undefined> = {
			...
			AIRIS_GPU_PROBE_COUNT: probeCountPath,
			AIRIS_GPU_PROBE_RUNS: String(options.runs),
		};
		...
		for (const key of ["PI_CODING_AGENT_DIR", "AIRIS_PROFILE", "PI_PROFILE", "AIRIS_CONFIG_DIR"]) {
			delete env[key];
		}
		if (options.sleepSeconds === undefined) {
			delete env.AIRIS_GPU_PROBE_SLEEP;
		} else {
			env.AIRIS_GPU_PROBE_SLEEP = String(options.sleepSeconds);
		}
		if (options.holdStdoutOpen) {
			env.AIRIS_GPU_PROBE_HOLD_STDOUT_OPEN = "true";
		} else {
			delete env.AIRIS_GPU_PROBE_HOLD_STDOUT_OPEN;
		}
		if (options.descendantHoldsStdout) {
			env.AIRIS_GPU_PROBE_DESCENDANT_HOLDS_STDOUT = "true";
		} else {
			delete env.AIRIS_GPU_PROBE_DESCENDANT_HOLDS_STDOUT;
		}
		if (options.validOutput !== undefined) {
			env.AIRIS_GPU_PROBE_VALID_OUTPUT = options.validOutput;
		} else {
			delete env.AIRIS_GPU_PROBE_VALID_OUTPUT;
		}
