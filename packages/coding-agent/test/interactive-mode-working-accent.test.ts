...
import * as sessionColor from "@airis/airis-coding-agent/utils/session-color";
import type { Container, NativeScrollbackLiveRegion } from "@airis/airis-tui";
...
async function createHarness(sessionName: string): Promise<Harness> {
	const tempDir = TempDir.createSync("@airs-working-accent-");
