...
import { buildSkillPromptMessage, type Skill } from "@airis/airis-coding-agent/extensibility/skills";
...
async function createSkill(body: string): Promise<{ dir: string; skill: Skill }> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), `airis-skill-prompt-${Snowflake.next()}-`));
