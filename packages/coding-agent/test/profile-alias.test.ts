...
describe("profile alias installer", () => {
	it("writes a bash-compatible function that forwards subcommands through airis", async () => {
		...
		expect(result.command).toBe("airis --profile=work");
		expect(files.get("/home/me/.bashrc")).toContain("airis-work() {");
		expect(files.get("/home/me/.bashrc")).toContain('command airis --profile=work "$@"');
	});
	...
	it("can target the current source invocation instead of the installed airis binary", async () => {
		const files = new Map<string, string>();
		...
		expect(files.get("/Users/me/.zshrc")).toContain("airis-work() {");
	...
	it("installs the zsh alias under ZDOTDIR when set", async () => {
		...
		expect(files.get(result.configPath)).toContain("airis-work() {");
	});
	...
	it("writes a fish function that forwards argv", async () => {
		...
		const content = files.get("/Users/me/.config/fish/conf.d/airis-profiles.fish") ?? "";
		expect(content).toContain("function airis-work --wraps airis");
		expect(content).toContain("command airis --profile=work $argv");
	});
	...
	it("installs the fish alias under XDG_CONFIG_HOME when set", async () => {
		...
		expect(result.configPath).toBe("/home/me/.dotfiles/config/fish/conf.d/airis-profiles.fish");
		expect(files.get(result.configPath)).toContain("function airis-work --wraps airis");
	});
	...
	it("writes a PowerShell function because aliases cannot carry arguments", async () => {
		...
		expect(content).toContain("function airis-work");
		expect(content).toContain("& airis --profile=work @args");
	});
	...
	it("detects pwsh from PSModulePath when SHELL is unset on Windows", async () => {
		...
		expect(files.get(result.configPath)).toContain("& airis --profile=work @args");
	});
	...
	it("replaces a previous block for the same alias", async () => {
		const files = new Map<string, string>([
			[
				...
				[
					"before",
					"# >>> airis profile alias: airis-work >>>",
					"alias airis-work='command airis --profile=old'",
					"# <<< airis profile alias: airis-work <<<",
					"after",
				].join("\n"),
			],
		...
		expect(content).toContain('command airis --profile=work "$@"');
	...
	it("refuses to rewrite a malformed managed block missing its end marker", async () => {
		...
		const original = ["# >>> airis profile alias: airis-work >>>", "airis-work() {", "export SECRET=keepme"].join("\n");
	...
	it("refuses to shadow the base airis command case-insensitively", async () => {
		for (const aliasName of ["airis", "AIRIS"]) {
			await expect(
				installProfileAlias({
					profile: "work",
					aliasName,
	...
	it("validates profile names before rendering shell code", async () => {
		...
		).rejects.toThrow("Invalid AIRIS profile");
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/airis-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/airis-profiles.fish'");
	});
