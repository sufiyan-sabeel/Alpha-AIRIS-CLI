...
import { initializeWithSettings, reset as resetDiscoveryCache } from "@airis/airis-coding-agent/discovery";
import { readMCPConfigFile, setMcpServerEnabled, setServerDisabled } from "@airis/airis-coding-agent/mcp/config-writer";
import { loadAllExtensions } from "@airis/airis-coding-agent/modes/components/extensions/state-manager";
import { __resetDirsFromEnvForTests, getMCPConfigPath, removeWithRetries, setAgentDir } from "@airis/airis-utils";
...
describe("loadAllExtensions MCP parity with /mcp list (issue #3827)", () => {
	...
	beforeEach(async () => {
		...
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-3827-project-"));
		userAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-3827-user-"));
		...
		await fs.writeFile(
			path.join(projectDir, ".airis", "mcp.json"),
			JSON.stringify({
				mcpServers: {
	...
	test("dashboard re-enable flips enabled:false in mcp.json (PR #3829 review)", async () => {
		...
		const projectMcpPath = path.join(projectDir, ".airis", "mcp.json");
	...
	test("dashboard disable on a config-resident server writes enabled:false (not denylist)", async () => {
		...
		const projectConfig = await readMCPConfigFile(path.join(projectDir, ".airis", "mcp.json"));
	...
	test("dashboard re-enable updates the row's non-primary source mcp.json before denylisting", async () => {
		const alternatePath = path.join(projectDir, ".airis", ".mcp.json");
		await Bun.write(
			alternatePath,
			JSON.stringify({
				mcpServers: {
