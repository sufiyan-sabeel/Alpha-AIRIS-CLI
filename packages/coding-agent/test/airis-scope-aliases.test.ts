 ...
 * `@mariozechner` (original), `@earendil-works` (fork), and `@alpha-airis-cli`
 * (canonical). The shim in `legacy-airis-compat.ts` remaps all three to the same
 ...
 *   - `@airis/airis-swarm-extension`         ⇒ `@airis/airis-utils`
 *   - `@plannotator/pi-extension`         ⇒ `@airis/airis-agent-core`
 *   - `@runfusion/fusion`                 ⇒ `@airis/airis-coding-agent/...`
 *
...
const canonicalCodingAgent = Bun.resolveSync("@airis/airis-coding-agent", import.meta.dir);
const canonicalCodingAgentExtensions = Bun.resolveSync(
	"@airis/airis-coding-agent/extensibility/extensions",
	import.meta.dir,
);
const canonicalUtils = Bun.resolveSync("@airis/airis-utils", import.meta.dir);
const canonicalTui = Bun.resolveSync("@airis/airis-tui", import.meta.dir);
// Subpath: upstream `pi-ai/oauth` re-exported `utils/oauth/index`; our pi-ai now
// exposes the same surface at the real `@airis/airis-ai/oauth` export, so the
// legacy `@mariozechner/pi-ai/oauth` specifier canonicalizes straight to it.
const canonicalAiOauth = Bun.resolveSync("@airis/airis-ai/oauth", import.meta.dir);
...
const CASES: readonly AliasCase[] = [
	...
	// @alpha-airis-cli self-import — canonical scope must still flow through the shim
	// so a duplicate copy is never dragged in from a plugin's own node_modules.
	{ id: "ohmypi-utils", aliasSpecifier: "@airis/airis-utils", canonicalPath: canonicalUtils, symbol: "logger" },
	{
		id: "ohmypi-coding-agent",
		aliasSpecifier: "@airis/airis-coding-agent",
	...
	// Subpath: legacy `pi-ai/oauth` resolves to the real `@airis/airis-ai/oauth`.
	{
...
describe("pi-* scope aliases", () => {
	...
	beforeEach(() => {
		projectDir = TempDir.createSync("@airis-scope-aliases-");
		...
		fs.writeFileSync(
			...
			JSON.stringify({
				...
				version: "1.0.0",
				airs: { extensions: ["./dist/extension.ts"] },
			}),
		);
