...
import { getProjectAgentDir, removeSyncWithRetries, Snowflake } from "@airis/airis-utils";
	...
	describe("project layer (on disk)", () => {
		...
		it("keeps failed project writes bound to their original cwd", async () => {
			...
			try {
				...
				expect(await Bun.file(path.join(bareProject, ".airis", "config.yml")).exists()).toBe(false);
			} finally {
				mkdirSpy.mockRestore();
			}

			await settings.flush();
			expect(YAML.parse(await Bun.file(path.join(startDir, ".airis", "config.yml")).text())).toEqual({
		...
		it("writes project model roles only to the project YAML", async () => {
			const settings = await Settings.init({ cwd: startDir, agentDir });
			const projectConfigPath = path.join(startDir, ".airis", "config.yml");
		...
		it("does not copy unedited roles from other project settings providers", async () => {
			await Bun.write(
				path.join(scopedProject, ".airis", "settings.json"),
			...
			expect(YAML.parse(await Bun.file(path.join(scopedProject, ".airis", "config.yml")).text())).toEqual({
		...
		it("reapplies only native model roles over normal project-provider precedence", async () => {
			...
			await Bun.write(
				path.join(scopedProject, ".airis", "config.yml"),
		...
		it("reports project roles over global role fallbacks", async () => {
			...
			await Bun.write(path.join(scopedProject, ".airis", "config.yml"), "modelRoles:\n  default: anthropic/project\n");
