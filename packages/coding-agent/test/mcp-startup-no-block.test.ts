/**
 * Regression test for issue #2100: airis startup blocked >25s while connecting
 * to MCP servers.
 *
 ...
 * (`AIRIS_MCP_TIMEOUT_MS`, default 30 000 ms) gated the entire UI.
 *
...
describe("MCP startup (issue #2100)", () => {
	...
	beforeEach(() => {
		workDir = fs.mkdtempSync(path.join(os.tmpdir(), "airis-mcp-startup-"));
	});
