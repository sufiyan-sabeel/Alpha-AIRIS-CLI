...
import { type Skill as CapabilitySkill, skillCapability } from "@airis/airis-coding-agent/capability/skill";
import { getCapability } from "@airis/airis-coding-agent/discovery";
import {
	loadSkills,
	loadSkillsFromDir,
	parseSkillInvocation,
	type Skill,
} from "@airis/airis-coding-agent/extensibility/skills";
...
describe("skills", () => {
	...
	describe("loadSkills with options", () => {
		...
		// canonical AIRIS-native `~/.agent[s]/skills` (the `agents` provider).
		...
		it("should still load ~/.agents/skills when codex/claude/native toggles are off (#2401)", async () => {
			const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "airs-agents-home-"));
			const tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "airs-agents-cwd-"));
		...
		it("respects an explicit enableAgentsUser: false (#2401)", async () => {
			const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "airs-agents-home-off-"));
			const tempCwd = await fs.mkdtemp(path.join(os.tmpdir(), "airs-agents-cwd-off-"));
		...
		// MUST NOT consider the AIRIS-native `enableAgentsUser`/`...Project`
		...
		it("should skip skills disabled via frontmatter", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-disabled-skill-"));
...
`,
		...
		it("should hide skills with disable-model-invocation frontmatter (Agent Skills spec)", async () => {
			const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-dmi-skill-"));
