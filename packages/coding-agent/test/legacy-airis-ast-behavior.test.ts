...
beforeAll(async () => {
	...
	rewriteRoot = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "airis-legacy-ast-rewrite-")));
...
async function loadCommonJsCase(testCase: CommonJsCase): Promise<{ keys: string[]; named: Record<string, unknown> }> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-legacy-ast-exports-"));
	tempRoots.push(dir);
	...
	return (await loadLegacyAirisModule(path.join(dir, "index.ts"))) as {
		keys: string[];
