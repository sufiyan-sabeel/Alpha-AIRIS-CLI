...
import type { SourceMeta } from "@airis/airis-coding-agent/capability/types";
import type { MCPServerConfig } from "@airis/airis-coding-agent/mcp/types";
import { collectMcpServerNames } from "@airis/airis-coding-agent/modes/controllers/mcp-command-controller";
import type { InteractiveModeContext } from "@airis/airis-coding-agent/modes/types";
import { buildTuiBuiltinSlashCommands } from "@airis/airis-coding-agent/slash-commands/builtin-registry";
import type { TuiSlashCommandRuntime } from "@airis/airis-coding-agent/slash-commands/types";
import {
	getConfigRootDir,
	getMCPConfigPath,
	getProjectDir,
...
describe("MCP server-name autocomplete", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-mcp-autocomplete-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-mcp-autocomplete-agent-"));
