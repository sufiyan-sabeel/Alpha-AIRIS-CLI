...
describe("normalizeShareServerUrl", () => {
	test("strips trailing slashes and falls back to the default", () => {
		expect(normalizeShareServerUrl("https://my.airis.sh/s/")).toBe("https://my.airis.sh/s");
		expect(normalizeShareServerUrl("https://example.com/s///")).toBe("https://example.com/s");
		expect(normalizeShareServerUrl(undefined)).toBe("https://my.airis.sh/s");
		expect(normalizeShareServerUrl("   ")).toBe("https://my.airis.sh/s");
	});
});
