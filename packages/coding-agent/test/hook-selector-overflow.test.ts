...
import { HookSelectorComponent } from "@airis/airis-coding-agent/modes/components/hook-selector";
...
describe("HookSelectorComponent", () => {
	...
	it("wraps outlined option text without omitting the tail", () => {
		const options = [
			"Option A: Move to AIRIS-native only by migrating reusable shared AI instructions into .airis/AGENTS.md, .airis/rules, .airis/skills, and .airis/agents while deliberately not creating a root .github directory.",
			"Option B: Keep dual support by migrating canonical instructions into .airis while also maintaining a root .github/copilot-instructions.md compatibility bridge for editors that do not understand AIRIS resources yet.",
		];
		const component = new HookSelectorComponent(
		...
		expect(normalizedPlain).toContain("do not understand AIRIS resources yet");
		for (const line of lines) {
			expect(visibleWidth(Bun.stripANSI(line))).toBeLessThanOrEqual(width);
		}
	});
		...
		const options = [
			{
				...
				description:
					"Authenticate via the provider keys and OAuth state already configured under ~/.airis without opening a new browser-based setup flow.",
			},
			{
				label: "Set up Alpha AIRIS-CLI in terminal",
				description:
