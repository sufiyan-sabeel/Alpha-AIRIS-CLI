...
import { getActiveSkills } from "@airis/airis-coding-agent/extensibility/skills";
import type { Skill } from "@airis/airis-coding-agent/sdk";
...
describe("createAgentSession skills option", () => {
	...
	beforeEach(() => {
		...
		// Create skill in .airis/skills/ for native project-level discovery
		skillsDir = path.join(tempDir, ".airis", "skills", "test-skill");
		fs.mkdirSync(skillsDir, { recursive: true });
		originalHome = process.env.HOME;
		tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-sdk-home-"));
		process.env.HOME = tempHomeDir;
		const nativeUserSkillsDir = path.join(tempHomeDir, ".airis", "agent", "skills");
...
`,
	...
	it("should still discover project skills when user skills directory is missing", async () => {
		const userAgentDir = path.join(tempHomeDir, ".airis", "agent");
	...
	it("refreshSkills reloads project skills on an existing session", async () => {
		...
		const runtimeSkillDir = path.join(tempDir, ".airis", "skills", "runtime-added-skill");
...
`,
	...
	it("manage_skill hot-registers managed skills in the active session", async () => {
		...
		const managedAgentDir = path.join(tempHomeDir, ".airis", "agent");
