...
import { JsRuntime, type RuntimeHooks } from "@airis/airis-coding-agent/eval/js/shared/runtime";

const GLOBAL_KEYS = ["__airis_import__", "read"] as const;
...
function snapshotGlobals(): Record<GlobalKey, GlobalSnapshot> {
	...
	return {
		__airis_import__: { exists: "__airis_import__" in globals, value: globals.__airis_import__ },
...
describe("JsRuntime global disposal", () => {
	it("keeps newer same-realm runtime globals after disposing an older runtime", () => {
		...
		try {
			...
			expect(globals.__airis_import__).toBe(secondImport);
			expect(globals.__airis_helpers__).toBe(second.helpers);
			expect(typeof globals.read).toBe("function");
	...
	it("reactivates older same-realm runtime globals when no other run is active", async () => {
		...
		try {
			...
			expect(globals.__airis_helpers__).toBe(first.helpers);
	...
	it("defers cross-runtime setCwd while another same-realm runtime is running", async () => {
		...
		try {
			...
			// setCwd: the saved __airis_session__ stack entry carries the new value.
			expect(await first.run("__airis_session__.cwd", undefined, hooks)).toBe(pendingCwd);
			expect(first.cwd).toBe(pendingCwd);
			expect(globals.__airis_helpers__).toBe(first.helpers);
			expect(globals.__airis_session__).toMatchObject({ cwd: pendingCwd });
		} finally {
			gate.resolve();
