...
import { computeBankScope, deriveBankId, ensureBankExists } from "@airis/airis-coding-agent/hindsight/bank";
...
describe("computeBankScope", () => {
	describe("scoping=global", () => {
		...
		it("falls back to the default bank name when bankId is unset", () => {
			expect(computeBankScope(baseConfig(), "/whatever")).toEqual({ bankId: "airis" });
		});
...
describe("deriveBankId (legacy wrapper)", () => {
	it("returns the bankId field of the resolved scope", () => {
		...
		expect(deriveBankId(baseConfig({ scoping: "per-project" }), "/work/proj")).toBe("airis-proj");
		expect(deriveBankId(baseConfig({ scoping: "per-project-tagged" }), "/work/proj")).toBe("airis");
	});
});
