...
import type { SSHHost } from "@airis/airis-coding-agent/capability/ssh";
...
import { SshProtocolHandler } from "@airis/airis-coding-agent/internal-urls/ssh-protocol";
import type { ToolSession } from "@airis/airis-coding-agent/tools";
import { GrepTool } from "@airis/airis-coding-agent/tools/grep";
...
describe.skipIf(!SSH_OK)("ssh:// handler against a real localhost ssh", () => {
	...
	const TMP = `/tmp/airis-ssh-e2e-${process.pid}`;
	...
	it("writes byte-exact, leaves no temp, and the read path round-trips", async () => {
		...
		const leftovers = await Bun.$`ssh -o BatchMode=yes localhost ls ${TMP} | grep -c airis-tmp || true`.text();
	...
	it("refuses to write to a directory and cleans up its temp", async () => {
		...
		const leftovers =
			await Bun.$`ssh -o BatchMode=yes localhost ls -A ${TMP} | grep -c "wdir.airis-tmp" || true`.text();
		expect(leftovers.trim()).toBe("0");
	});
...
describe.skipIf(!SSH_OK)("ssh:// through the real read/grep/write tools (localhost)", () => {
	const TMP = `/tmp/airis-ssh-tools-e2e-${process.pid}`;
	...
	it("GrepTool reports matches under the ssh:// URL with no scratch-temp leak", async () => {
		...
		expect(out).not.toContain("airis-search-probe");
		expect(detailsJson).not.toContain("airis-search-probe");
	});
