 ...
 *
 * Each test isolates `AIRIS_GITHUB_CACHE_DB` to a temp file and clears
...
import { getOrFetchIssue, getOrFetchPr } from "@airis/airis-coding-agent/tools/gh";
import {
	clearAll,
	getCached,
	getOrFetchView,
...
import { ToolAbortError, throwIfAborted } from "@airis/airis-coding-agent/tools/tool-errors";
...
describe("github-cache db layer", () => {
	...
	it("does not chmod an existing cache parent directory", async () => {
		...
		process.env.AIRIS_GITHUB_CACHE_DB = path.join(parent, "github-cache.db");
		resetCacheForTests();
