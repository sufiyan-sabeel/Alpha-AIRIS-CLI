...
import { BrowserTool } from "@airis/airis-coding-agent/tools/browser";
import { CmuxSocketClient } from "@airis/airis-coding-agent/tools/browser/cmux/socket-client";
import { getBrowsersMapForTest } from "@airis/airis-coding-agent/tools/browser/registry";
import { getTabsMapForTest, releaseTab } from "@airis/airis-coding-agent/tools/browser/tab-supervisor";
import type { ToolSession } from "@airis/airis-coding-agent/tools/index";
import { ToolAbortError, ToolError } from "@airis/airis-coding-agent/tools/tool-errors";
...
beforeEach(() => {
	...
	process.env.CMUX_SOCKET_PATH = `/tmp/airis-open-lease-${process.pid}-${Math.random().toString(36).slice(2)}.sock`;
});
