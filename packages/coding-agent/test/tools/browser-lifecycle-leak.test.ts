...
import { acquireBrowser, getBrowsersMapForTest } from "@airis/airis-coding-agent/tools/browser/registry";
import {
	acquireTab,
	getTabsMapForTest,
	releaseTab,
...
function makeKind(socketSuffix: string): CmuxKind {
	return { kind: "cmux", socketPath: `/tmp/airis-test-${socketSuffix}.sock`, surface: `surface-${socketSuffix}` };
}
...
describe("browser lifecycle — close deadlines", () => {
	...
	it("rejects a stuck close with the backend, tab, and pending resource", async () => {
		...
		const kind: CmuxKind = { kind: "cmux", socketPath: "/tmp/airis-close-deadline.sock" };
		const browser = await acquireBrowser(kind, { cwd: "/tmp" });
