...
import { buildBrowserNavigationHeaders } from "@airis/airis-coding-agent/web/search/providers/browser-headers";
...
describe("browser navigation headers", () => {
	...
	it("imports cleanly and falls back when header-generator data files are absent", async () => {
		const script = [
			...
			'const { buildBrowserNavigationHeaders } = await import("@airis/airis-coding-agent/web/search/providers/browser-headers");',
