...
import { tryRunRpcSkillCommand } from "@airis/airis-coding-agent/modes/rpc/rpc-mode";
import { type CustomMessage, SKILL_PROMPT_MESSAGE_TYPE } from "@airis/airis-coding-agent/session/messages";
import { removeWithRetries, Snowflake } from "@airis/airis-utils";

describe("tryRunRpcSkillCommand", () => {
	test("dispatches registered /skill commands as skill prompt messages", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), `airis-rpc-skill-${Snowflake.next()}-`));
