...
} from "@airis/airis-coding-agent/system-prompt";
...
describe("SYSTEM.md prompt assembly", () => {
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".airis");
	...
	it("suppresses discovered SYSTEM.md while preserving the project footer", async () => {
		...
		fs.writeFileSync(path.join(projectDir, ".airis", "SYSTEM.md"), "Discovered project SYSTEM prompt");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(path.join(projectDir, ".airis"), { recursive: true });
		fs.mkdirSync(path.join(tempHomeDir, ".airis", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".airis", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".airis", "SYSTEM.md"), "Project SYSTEM prompt");
