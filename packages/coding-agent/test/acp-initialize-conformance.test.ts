...
import { AcpAgent } from "@airis/airis-coding-agent/modes/acp/acp-agent";
import { ACP_TERMINAL_AUTH_FLAG, prepareAcpTerminalAuthArgs } from "@airis/airis-coding-agent/modes/acp/terminal-auth";
import type { AgentSession } from "@airis/airis-coding-agent/session/agent-session";
import { SessionManager } from "@airis/airis-coding-agent/session/session-manager";
import { getConfigRootDir, setAgentDir, VERSION } from "@airis/airis-utils";
...
async function createAgent(): Promise<AcpAgent> {
	const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "airis-acp-init-"));
