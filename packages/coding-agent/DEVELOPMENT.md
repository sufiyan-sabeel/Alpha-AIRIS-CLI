# Developing `@airis/airis-coding-agent`

This package is the `airis` CLI. This file is a **developer's map**: where things live
...
| Build the `dist/airis` binary | `bun run build` |
...
and dispatches the hidden `__airis_worker_*` argv selectors before loading the command
...
| `memories/`, `memory-backend/`, `mnemosyne/`, `hindsight/` | Memory subsystems and backends | [memory.md](../../docs/memory.md), [mnemosyne-memory-backend.md](../../docs/mnemosyne-memory-backend.md) |
...
- [toolconv/](../../docs/toolconv/) — per-family tool-call conversion (anthropic, harmony, qwen3, deepseek, glm-4.5, kimi-k2, gemini, gemma, airis-native); see also [ERRATA-GPT5-HARMONY.md](../../docs/ERRATA-GPT5-HARMONY.md)
...
### Natives (`crates/airis-natives`, `packages/natives`)
