...
Core implementation package for the `airis` coding agent in the `alpha-airis-cli` monorepo.

For installation, setup, provider configuration, model roles, slash commands, and full CLI reference, see:
- [Monorepo README (local)](../../README.md)
- [Monorepo README (GitHub)](https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI#readme)
...
The agent supports three mutually-exclusive memory backends, selected via the `memory.backend` setting (Settings → Memory tab, or `~/.airis/config.yml`):
