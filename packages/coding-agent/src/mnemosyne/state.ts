...
import type * as MnemosyneNs from "@airis/airis-mnemosyne";
import type { Mnemosyne, RecallResult } from "@airis/airis-mnemosyne";
import type * as MnemosyneCoreNs from "@airis/airis-mnemosyne/core";
import type { LocalModelInitializer } from "@airis/airis-mnemosyne/core";
...
import { mnemosyneEmbedClient } from "./embed-client";

// The mnemosyne package pulls the embeddings stack; keep it off the CLI startup
// module graph by loading it lazily at the async boundaries that need it.
let mnemosyneMod: typeof MnemosyneNs | undefined;
let mnemosyneCoreMod: typeof MnemosyneCoreNs | undefined;
...
function installLocalModelInitializer(setInitializer: (initializer: LocalModelInitializer) => void): void {
	...
	setInitializer(({ model, cacheDir }) =>
		mnemosyneEmbedClient.initialize(model, cacheDir).then(handle => {
			if (handle) return handle;
			throw new Error("mnemosyne embed subprocess unavailable");
		}),
	);
}

/**
 * Lazily load `@airis/airis-mnemosyne` (memoized) and route fastembed loads
 ...
 */
export async function loadMnemosyne(): Promise<typeof MnemosyneNs> {
	if (!mnemosyneMod) {
		mnemosyneMod = await import("@airis/airis-mnemosyne");
		installLocalModelInitializer(mnemosyneMod.setLocalModelInitializer);
	}
	return mnemosyneMod;
}

/** Lazily load `@airis/airis-mnemosyne/core` (memoized). */
export async function loadMnemosyneCore(): Promise<typeof MnemosyneCoreNs> {
	if (!mnemosyneCoreMod) {
		mnemosyneCoreMod = await import("@airis/airis-mnemosyne/core");
		installLocalModelInitializer(mnemosyneCoreMod.setLocalModelInitializer);
	}
	return mnemosyneCoreMod;
}

/** Sync access for code below an async boundary that already awaited {@link loadMnemosyne}. */
export function requireMnemosyne(): typeof MnemosyneNs {
	if (!mnemosyneMod) throw new Error("Mnemosyne module not loaded; await loadMnemosyne() first.");
	return mnemosyneMod;
}

/** Sync access for code below an async boundary that already awaited {@link loadMnemosyneCore}. */
export function requireMnemosyneCore(): typeof MnemosyneCoreNs {
	if (!mnemosyneCoreMod) throw new Error("Mnemosyne core module not loaded; await loadMnemosyneCore() first.");
	return mnemosyneCoreMod;
}
...
export interface MnemosyneMemoryEditResult {
	...
	bank?: string;
	store?: MnemosyneMemoryStore;
}

/** Which mnemosyne table a resolved memory id lives in. `fact` rows are
 * read-only projections of fact extraction (issue #4725): resolvable for
 * reads, never editable. */
export type MnemosyneMemoryStore = "working" | "episodic" | "fact";
...
/**
 * Full-row lookup result produced by {@link MnemosyneSessionState.getScopedMemory}.
 * Mirrors the shape stored in mnemosyne's working/episodic tables, tagged with
 * the scoped bank that actually held the row so callers can render it with
 * meaningful context.
 */
export interface MnemosyneScopedMemoryHit {
	bank: string;
	store: MnemosyneMemoryStore;
	row: {
		id: string;
...
type MnemosyneRetentionMessage = { role: string; content: string };

interface MnemosyneRetentionCursorRow {
	content: string;
...
function deriveRetainedTurnCursor(rows: readonly MnemosyneRetentionCursorRow[], sessionId: string): number {
	let cursor = 0;
...
function sliceUnretainedMessages(
	messages: MnemosyneRetentionMessage[],
	lastRetainedTurn: number,
): MnemosyneRetentionMessage[] {
...
export class MnemosyneSessionState {
	 ...
	 * (issue #4443).
	 */
	getScopedMemory(id: string): MnemosyneScopedMemoryHit | null {
		...
		for (const target of targets) {
			const raw = target.memory.get(id) as MnemosyneStoredMemoryRow | null;
			if (!raw) continue;
			const store: MnemosyneMemoryStore =
	...
	#restoreRetainedTurnCursor(): void {
		...
		const rows = this.memory.beam.db
			.prepare<MnemosyneRetentionCursorRow, [string]>(`
				SELECT
					content,
	 ...
	 * delete the DB files — e.g. `mnemosyneBackend.clear` — pass
	...
	async dispose(options: { consolidate?: boolean; timeoutMs?: number } = {}): Promise<void> {
		...
		const consolidatePromise = this.consolidate({ full: false, extract: false, sleep: false }).catch(
			(error: unknown) => {
				logger.warn("Mnemosyne: consolidation on dispose failed.", { error: String(error) });
			},
		);
		const { timeoutMs } = options;
		if (timeoutMs !== undefined && timeoutMs > 0) {
			const TIMED_OUT = Symbol("mnemosyne.dispose.timedOut");
			const winner = await Promise.race([
				consolidatePromise.then(() => undefined as unknown),
				Bun.sleep(timeoutMs).then(() => TIMED_OUT as unknown),
			]);
			if (winner === TIMED_OUT) {
				logger.warn("Mnemosyne: consolidate-on-dispose exceeded shutdown budget; detaching to background.", {
					timeoutMs,
				});
...
function createScopedResources(config: MnemosyneBackendConfig): MnemosyneScopedResources {
	// Env vars (MNEMOSYNE_POLYPHONIC_RECALL / MNEMOSYNE_ENHANCED_RECALL) still override
	...
	requireMnemosyne().configureRecallFeatures({
