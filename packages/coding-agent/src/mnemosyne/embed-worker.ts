/**
 * Mnemosyne local-embeddings worker. Loaded inside the dedicated subprocess
 * spawned by `embed-client.ts` (re-entered through the agent CLI's hidden
 * `__airis_worker_mnemosyne_embed` selector). The whole point of this module is
 ...
 * Windows shutdown (issue #3031, mnemosyne sibling of #1606/#1607) never runs
 * in either process.
 */

import { defaultLocalModelInitializer, type StandardEmbeddingModel } from "@airis/airis-mnemosyne/core";
import type { MnemosyneEmbedModelId, MnemosyneEmbedTransport, MnemosyneEmbedWorkerInbound } from "./embed-protocol";
...
async function loadModel(model: MnemosyneEmbedModelId, cacheDir: string | undefined): Promise<LoadedModel> {
	// Route through mnemosyne's shared initializer so the worker inherits BOTH
	...
	// mnemosyne's `fastembedModelName`); the parent only ever passes pre-vetted
	// fast-* identifiers.
...
function ensureLoaded(model: MnemosyneEmbedModelId, cacheDir: string | undefined): Promise<LoadedModel> {
...
async function handleEmbed(
	transport: MnemosyneEmbedTransport,
	message: Extract<MnemosyneEmbedWorkerInbound, { type: "embed" }>,
): Promise<void> {
	try {
		...
		// the parent SIGKILLed the previous subprocess but mnemosyne still
...
async function handleInit(
	transport: MnemosyneEmbedTransport,
	message: Extract<MnemosyneEmbedWorkerInbound, { type: "init" }>,
): Promise<void> {
	try {
...
export function startMnemosyneEmbedWorker(transport: MnemosyneEmbedTransport): void {
