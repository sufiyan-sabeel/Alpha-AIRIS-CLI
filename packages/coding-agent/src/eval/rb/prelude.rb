...
#
# `__airis_*` primitives (emit/present/status/scrub/run_id) are provided by
# runner.rb; this file only consumes them.

unless defined?($__airis_prelude_loaded) && $__airis_prelude_loaded
  $__airis_prelude_loaded = true
  ...
  def __airis_resolve_path(path)
    ...
    relative = __airis_url_decode(m[2].tr("\\", "/"))
  ...
  def display_image(base64, mime_type: "image/png")
    __airis_emit_display({ mime_type.to_s => base64.to_s })
    nil
  end
  ...
  def env(key = nil, value = nil)
    if key.nil?
      ...
      __airis_emit_status("env", "count" => items.size, "keys" => items.keys.first(20))
      return items
    end
    unless value.nil?
      ENV[key.to_s] = value.to_s
      __airis_emit_status("env", "key" => key.to_s, "value" => value.to_s, "action" => "set")
      return value
    end
    val = ENV[key.to_s]
    __airis_emit_status("env", "key" => key.to_s, "value" => val, "action" => "get")
    val
  end
  ...
  def read(path, offset = 1, limit = nil)
    resolved = __airis_resolve_path(path)
    ...
    __airis_emit_status("read", "path" => resolved.to_s, "chars" => data.length, "preview" => __airis_scrub(data[0, 500].to_s))
    data
  end
  ...
  def write(path, content)
    ...
    __airis_emit_status("write", "path" => resolved.to_s, "chars" => content.to_s.length)
    resolved.to_s
  end
  ...
  def __airis_apply_query(data, query)
  ...
  def output(*ids, format: "raw", query: nil, offset: nil, limit: nil)
    ...
    if artifacts_dir.nil? || artifacts_dir.empty?
      ...
      if session_file.nil? || session_file.empty?
        __airis_emit_status("output", "error" => "No session file available")
        raise "No session - output artifacts unavailable"
      end
      artifacts_dir = session_file.sub(/\.[^.]*\z/, "")
    end
    unless File.directory?(artifacts_dir)
      __airis_emit_status("output", "error" => "Artifacts directory not found", "path" => artifacts_dir)
    ...
    if query && (!offset.nil? || !limit.nil?)
      __airis_emit_status("output", "error" => "query cannot be combined with offset/limit")
      ...
      if query
        json_value =
          begin
            ...
            __airis_emit_status("output", "id" => output_id, "error" => "Not valid JSON: #{e.message}")
        ...
        if start_line > total_lines
          __airis_emit_status("output", "id" => output_id, "error" => "Offset #{start_line} beyond end (#{total_lines} lines)")
    ...
    unless not_found.empty?
      ...
      __airis_emit_status("output", "not_found" => not_found, "available_count" => available.length)
      raise msg
    end

    if ids.length == 1
      if format == "json"
        __airis_emit_status("output", "id" => ids[0], "chars" => results[0]["char_count"])
        return results[0]
      end
      __airis_emit_status("output", "id" => ids[0], "chars" => results[0]["content"].length)
      return results[0]["content"]
    end

    if format == "json"
      __airis_emit_status("output", "count" => results.length, "total_chars" => results.sum { |r| r["char_count"] })
      return results
    end
    combined = results.map { |r| { "id" => r["id"], "content" => r["content"] } }
    __airis_emit_status("output", "count" => combined.length, "total_chars" => combined.sum { |r| r["content"].length })
    combined
  end
  ...
  module OmpBridge
    ...
    def call(name, args)
      ...
      payload = JSON.generate("session" => session, "run" => $__airis_current_rid, "name" => name, "args" => args)
  ...
  def tool
    $__airis_tool_proxy ||= OmpToolProxy.new
  end
  ...
  def __airis_pool_map(items)
    arr = items.to_a
    return [] if arr.empty?
    limit = __airis_concurrency_limit
  ...
  def parallel(thunks)
    ...
    __airis_pool_map(list) { |t| t.call }
  end
  ...
  def pipeline(items, *stages)
    ...
    stages.each do |stage|
      ...
      current = __airis_pool_map(current) { |item| stage.call(item) }
    end
    current
  end
  ...
  def log(message)
    __airis_emit_status("log", "message" => message.to_s)
    nil
  end

  def phase(title)
    $__airis_current_phase = title.to_s
    __airis_emit_status("phase", "title" => title.to_s)
    nil
  end
