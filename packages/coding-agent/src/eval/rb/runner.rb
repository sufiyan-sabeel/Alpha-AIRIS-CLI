...
$__airis_out_mutex = Mutex.new
$__airis_raw_stderr = (STDERR.dup rescue STDERR)
$__airis_current_rid = nil
$__airis_capture_rid = nil
$__airis_exec_count = 0
$__airis_active_exec = 0
$__airis_silent = false

begin
  $__airis_frame_io = STDOUT.dup
  $__airis_frame_io.sync = true
  __airis_stdout_cap_r, __airis_stdout_cap_w = IO.pipe
  STDOUT.reopen(__airis_stdout_cap_w)
  STDOUT.sync = true
  __airis_stdout_cap_w.close
  $__airis_stdout_capture_read = __airis_stdout_cap_r
rescue StandardError
  $__airis_frame_io = STDOUT
  ($__airis_frame_io.sync = true) rescue nil
  $__airis_stdout_capture_read = nil
end

begin
  __airis_stderr_cap_r, __airis_stderr_cap_w = IO.pipe
  STDERR.reopen(__airis_stderr_cap_w)
  STDERR.sync = true
  __airis_stderr_cap_w.close
  $__airis_stderr_capture_read = __airis_stderr_cap_r
rescue StandardError
  $__airis_stderr_capture_read = nil
end
...
begin
  $__airis_proto_stdin = STDIN.dup
  STDIN.reopen(File.open(File::NULL, "r"))
rescue StandardError
  $__airis_proto_stdin = STDIN
end
...
def __airis_emit(frame)
  ...
  $__airis_out_mutex.synchronize do
    $__airis_frame_io.write(line)
    $__airis_frame_io.write("\n")
    $__airis_frame_io.flush
  end
rescue StandardError
  nil
end
...
def __airis_emit_stream(kind, text)
  rid = $__airis_current_rid
  if rid.nil?
    ($__airis_raw_stderr.write(text) rescue nil)
    return
  end
  __airis_emit("type" => kind, "id" => rid, "data" => __airis_scrub(text))
end

def __airis_emit_display(bundle, kind = "display")
  rid = $__airis_current_rid
  return if rid.nil?
  __airis_emit("type" => kind, "id" => rid, "bundle" => bundle)
end

def __airis_emit_status(op, data = {})
  status = { "op" => op.to_s }
  data.each { |k, v| status[k.to_s] = v }
  __airis_emit_display({ "application/x-airis-status" => status }, "display")
end

AIRIS_IMAGE_MIMES = %w[image/png image/jpeg].freeze
...
def __airis_image_payload(content)
  require "base64"
  s = content.to_s
  return s.gsub(/\s+/, "") if __airis_base64?(s)
...
def __airis_sniff_image_mime(bytes)
  b = bytes.to_s.b
...
# bundle is always JSON-safe before it reaches __airis_emit.
def __airis_normalize_bundle(hash)
  bundle = {}
  hash.each do |key, val|
    k = key.to_s
    bundle[k] =
      if AIRIS_IMAGE_MIMES.include?(k)
        __airis_image_payload(val)
...
def __airis_finalize_bundle(bundle, value)
  bundle["text/plain"] ||= __airis_scrub((value.inspect rescue value.class.name))
  bundle
end

# Rich-display resolution for non-collection objects. Honors the repo
# `to_airis_mime` convention first, then the IRuby protocol
...
def __airis_rich_mime_bundle(value)
  if value.respond_to?(:to_airis_mime)
    mime = (value.to_airis_mime rescue nil)
    return __airis_finalize_bundle(__airis_normalize_bundle(mime), value) if mime.is_a?(Hash) && !mime.empty?
  end
  if value.respond_to?(:to_iruby_mimebundle)
    ...
    return __airis_finalize_bundle(__airis_normalize_bundle(data), value) if data.is_a?(Hash) && !data.empty?
  end
  if value.respond_to?(:to_iruby)
    pair = (value.to_iruby rescue nil)
    if pair.is_a?(Array) && pair.size == 2 && !pair[0].nil?
      return __airis_finalize_bundle(__airis_normalize_bundle({ pair[0].to_s => pair[1] }), value)
    end
  end
  ...
  if value.respond_to?(:to_png)
    ...
    return __airis_finalize_bundle({ "image/png" => __airis_image_payload(png) }, value) if png
  end
  jpeg_method = %i[to_jpeg to_jpg].find { |m| value.respond_to?(m) }
  if jpeg_method
    jpg = (value.public_send(jpeg_method) rescue nil)
    return __airis_finalize_bundle({ "image/jpeg" => __airis_image_payload(jpg) }, value) if jpg
  end
  if value.respond_to?(:to_blob)
    blob = (value.to_blob rescue nil)
    if blob.is_a?(String) && (mime = __airis_sniff_image_mime(blob))
      return __airis_finalize_bundle({ mime => __airis_image_payload(blob) }, value)
    end
  end
...
# Other objects may expose a rich representation via `to_airis_mime` or the IRuby
# protocol (`to_iruby`/`to_iruby_mimebundle`); otherwise they fall back to inspect.
def __airis_mime_bundle(value)
  case value
  when String
    { "text/plain" => __airis_scrub(value) }
  when Hash, Array
    safe = begin
    ...
    if safe.nil?
      { "text/plain" => __airis_scrub(value.inspect) }
    else
      { "application/json" => safe, "text/plain" => __airis_scrub(value.inspect) }
    end
  when nil
    { "text/plain" => "nil" }
  else
    __airis_rich_mime_bundle(value) || { "text/plain" => __airis_scrub(value.inspect) }
  end
end

def __airis_present(value, kind = "display")
  __airis_emit_display(__airis_mime_bundle(value), kind)
end
...
def __airis_start_capture_drain(io, kind)
  return if io.nil?
  Thread.new do
    loop do
      chunk =
      ...
      rid = $__airis_capture_rid
      if rid.nil?
        ($__airis_raw_stderr.write(chunk) rescue nil)
      else
        __airis_emit("type" => kind, "id" => rid, "data" => __airis_scrub(chunk))
      end
    end
  end
end

def __airis_start_parent_watchdog
...
def __airis_install_idle_sigint
  Signal.trap("INT", "IGNORE") rescue nil
end

def __airis_install_exec_sigint
  Signal.trap("INT", "DEFAULT") rescue nil
end

def __airis_begin_exec
  $__airis_active_exec += 1
  __airis_install_exec_sigint
end

def __airis_end_exec
  $__airis_active_exec -= 1 if $__airis_active_exec > 0
  __airis_install_idle_sigint if $__airis_active_exec.zero?
end
...
def __airis_apply_request_runtime(req)
  cwd = req["cwd"]
  ...
  env = req["env"]
  if env.is_a?(Hash)
    AIRIS_MANAGED_ENV_KEYS.each do |key|
...
def __airis_should_display_result?(src)
  ...
  last = __airis_ast_last(node)
  return true if last.nil?
  !AIRIS_NON_DISPLAY_NODES.include?(last.type)
end
...
def __airis_emit_error(rid, exc, name_override = nil)
  ...
  __airis_emit(
    ...
    "id" => rid,
    "ename" => ename,
    "evalue" => __airis_scrub(evalue),
    "traceback" => traceback.map { |l| __airis_scrub(l) },
  )
end

def __airis_handle_request(req)
  rid = req["id"].to_s
  $__airis_current_rid = rid
  $__airis_capture_rid = rid
  $__airis_silent = req["silent"] == true
  $__airis_exec_count += 1
  count = $__airis_exec_count
  __airis_emit("type" => "started", "id" => rid)

  status = "ok"
  cancelled = false
  begin
    begin
      __airis_apply_request_runtime(req)
      src = req["code"].to_s
    rescue Exception => e # rubocop:disable Lint/RescueException
      __airis_emit_error(rid, e)
      __airis_emit("type" => "done", "id" => rid, "status" => "error", "executionCount" => count, "cancelled" => false)
      return
    end
    ...
    begin
      ...
      unless $__airis_silent || value.nil? || !__airis_should_display_result?(src)
        __airis_present(value, "result")
      end
    rescue Interrupt => e
      cancelled = true
      status = "error"
      __airis_emit_error(rid, e, "Interrupt")
    rescue SystemExit => e
      status = "error"
      __airis_emit_error(rid, e)
    rescue Exception => e # rubocop:disable Lint/RescueException
    ...
    __airis_emit("type" => "done", "id" => rid, "status" => status, "executionCount" => count, "cancelled" => cancelled)
  ensure
    $__airis_capture_rid = nil if $__airis_capture_rid == rid
    $__airis_current_rid = nil
  end
end
...
def __airis_main
  ...
  __airis_install_idle_sigint
  __airis_start_parent_watchdog
  __airis_start_capture_drain($__airis_stdout_capture_read, "stdout")
  __airis_start_capture_drain($__airis_stderr_capture_read, "stderr")

  $__airis_proto_stdin.each_line do |raw|
    line = raw.strip
    next if line.empty?
    req =
      begin
    ...
    __airis_handle_request(req) if req.is_a?(Hash)
  end
end
