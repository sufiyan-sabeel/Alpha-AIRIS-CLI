import { logger, postmortem, Snowflake, workerHostEntry } from "@airis/airis-utils";
import {
	createWorkerHandle,
 ...
 * `airis --smoke-test` so binary / source / tarball installs all exercise it.
 */
...
function spawnBunWorker(): WorkerHandle {
	try {
		...
		const worker = hostEntry
			? new Worker(hostEntry, { type: "module", argv: ["__airis_worker_js_eval"] })
