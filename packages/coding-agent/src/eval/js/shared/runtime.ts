...
export interface RuntimeOptions {
	...
	sessionId: string;
	/**
	 * Extra globals installed alongside `__airis_helpers__` / prelude. Use for stable, lifetime-
...
export class JsRuntime {
	...
	#install(extraGlobals: Record<string, unknown> | undefined): void {
		...
		const injected: Record<string, unknown> = {
			__airis_session__: this.#session,
			__airis_helpers__: this.helpers,
			__airis_call_tool__: async (name: string, args: unknown) => {
				const hooks = this.#activeHooks("tool");
				if (!hooks) return undefined;
				return await hooks.callTool(name, args);
			},
			__airis_import__: async (source: string, options?: ImportCallOptions) => {
			...
			__airis_import_from__: async (moduleUrl: string, source: string, options?: ImportCallOptions) => {
				const resolved = await this.#moduleLoader.resolveForModule(moduleUrl, source, this.#activeCwd());
				if (resolved.mode === "local") return resolved.value;
				const target = resolved.target;
				return options !== undefined ? await import(target, options) : await import(target);
			},
			__airis_get_require__: (moduleUrl?: string) => this.#activeRequire(moduleUrl),
			__airis_get_filename__: (moduleUrl?: string) => this.#moduleFilename(moduleUrl),
			__airis_get_dirname__: (moduleUrl?: string) => this.#moduleDirname(moduleUrl),
			__airis_emit_status__: (op: string, data: Record<string, unknown> = {}) => {
				const event: JsStatusEvent = { op, ...data };
				this.#activeHooks("emitStatus")?.onDisplay({ type: "status", event });
			},
			__airis_log__: (level: string, ...args: unknown[]) => {
				const prefix = level === "error" ? "[error] " : level === "warn" ? "[warn] " : "";
				const text = `${prefix}${formatConsoleArgs(args)}`;
				this.#activeHooks("log")?.onText(text.endsWith("\n") ? text : `${text}\n`);
			},
			__airis_table__: (...args: unknown[]) => {
			...
			__airis_display__: (value: unknown) => this.displayValue(value),
			__airis_set_final_expr__: (value: unknown) => {
