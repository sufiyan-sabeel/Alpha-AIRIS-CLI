...
// `__airis_import__` helper, which resolves the specifier against the active session cwd. A real
...
// worker-injected `__airis_import__` global does not exist. The swap therefore guards on the
...
const DYNAMIC_IMPORT_CALLEE = '(typeof __airis_import__ === "function" ? __airis_import__ : (s, o) => import(s, o))';

function buildOmpImportCall(sourceLiteral: string, optionsLiteral: string | undefined): string {
	// Route every static import through the worker-injected `__airis_import__` helper so the
	// specifier resolves against the session cwd (and `with`-attribute imports keep working).
	return optionsLiteral ? `__airis_import__(${sourceLiteral}, ${optionsLiteral})` : `__airis_import__(${sourceLiteral})`;
}
...
function buildOptionsLiteral(node: BabelImportDeclaration): string | undefined {
	...
	// Native dynamic import takes options as `{ with: { ... } }`. `__airis_import__` forwards the
...
export async function rewriteImports(code: string): Promise<string> {
	...
	// Top-level static `import` declarations become `await __airis_import__(...)` calls.
	...
	// Dynamic `import(...)` expressions (anywhere) get their callee swapped for `__airis_import__`
...
export async function rewriteDynamicImports(code: string, callee = "__airis_import__"): Promise<string> {
	if (!code.includes("import")) return code;
	const ast = await parseProgram(code);
	if (!ast) return code;
...
async function returnFinalExpression(code: string): Promise<{ source: string; returned: boolean }> {
	...
	if (last?.type === "ExpressionStatement") {
		...
		return { source: `${prefix}__airis_set_final_expr__((${trimmedStatement}));${suffix}`, returned: true };
	}
	if (last?.type === "ReturnStatement") {
		// Top-level `return value;` is otherwise swallowed: it forces the cell into an async IIFE
		// wrapper that discards the returned value. Rewrite into `__airis_set_final_expr__((expr))`
		...
		return { source: `${prefix}__airis_set_final_expr__((${expr}));${suffix}`, returned: true };
	}
