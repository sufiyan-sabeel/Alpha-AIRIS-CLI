...
if !isdefined(Main, :__airis_prelude_loaded)
    global __airis_prelude_loaded = true
end
...
function __airis_url_decode(s::String)
    res = IOBuffer()
    i = 1
    len = ncodeunits(s)
    while i <= len
...
function __airis_resolve_path(p::AbstractString)
    ...
    relative = __airis_url_decode(replace(string(m.captures[2]), '\\' => '/'))
...
function __airis_emit_status(op::String, fields::AbstractDict=Dict{String, Any}())
    ...
    Main.emit_frame(Dict(
        "type" => "display",
        "id" => Main.current_rid,
        "bundle" => Dict("application/x-airis-status" => status)
    ))
    return nothing
end
...
function Base.read(path::AbstractString, offset::Integer=1, limit::Union{Integer, Nothing}=nothing)
    resolved = __airis_resolve_path(string(path))
    ...
    Main.emit_frame(Dict(
        ...
        "id" => Main.current_rid,
        "bundle" => Dict(
            "application/x-airis-status" => Dict(
                "op" => "read",
                "path" => resolved,
...
function __airis_apply_query(data, query)
...
function __airis_json_text(value)
    if value === nothing
        return "null"
    end
    try
...
function __airis_optional_int(value, default::Int)
    if value === nothing
        return default
...
function output(ids...; format="raw", query=nothing, offset=nothing, limit=nothing)
    ...
    if isempty(artifacts_dir)
        ...
        if isempty(session_file)
            __airis_emit_status("output", Dict{String, Any}("error" => "No session file available"))
            error("No session - output artifacts unavailable")
        end
        artifacts_dir = replace(session_file, r"\.[^.]*$" => "")
    end
    if !isdir(artifacts_dir)
        __airis_emit_status("output", Dict{String, Any}("error" => "Artifacts directory not found", "path" => artifacts_dir))
        error("No artifacts directory found: $artifacts_dir")
    end
    if isempty(ids)
        __airis_emit_status("output", Dict{String, Any}("error" => "No IDs provided"))
        error("At least one output ID is required")
    end
    if query !== nothing && (offset !== nothing || limit !== nothing)
        __airis_emit_status("output", Dict{String, Any}("error" => "query cannot be combined with offset/limit"))
    ...
    for output_id_value in ids
        ...
        if query !== nothing
            json_value = try
                ...
                __airis_emit_status("output", Dict{String, Any}("id" => output_id, "error" => "Not valid JSON: $(err)"))
                error("Output $output_id is not valid JSON: $(err)")
            end
            result_value = __airis_apply_query(json_value, query)
            selected = __airis_json_text(result_value)
        elseif offset !== nothing || limit !== nothing
            start_line = max(1, __airis_optional_int(offset, 1))
            if start_line > total_lines
                __airis_emit_status("output", Dict{String, Any}("id" => output_id, "error" => "Offset $start_line beyond end ($total_lines lines)"))
                error("Offset $start_line is beyond end of output ($total_lines lines) for $output_id")
            end
            effective_limit = limit === nothing ? total_lines - start_line + 1 : __airis_optional_int(limit, total_lines - start_line + 1)
    ...
    if !isempty(not_found)
        ...
        __airis_emit_status("output", Dict{String, Any}("not_found" => not_found, "available_count" => length(available)))
        error(msg)
    end

    if length(ids) == 1
        if format == "json"
            __airis_emit_status("output", Dict{String, Any}("id" => string(ids[1]), "chars" => results[1]["char_count"]))
            return results[1]
        end
        __airis_emit_status("output", Dict{String, Any}("id" => string(ids[1]), "chars" => length(results[1]["content"])))
        return results[1]["content"]
    end

    if format == "json"
        __airis_emit_status("output", Dict{String, Any}("count" => length(results), "total_chars" => sum(r["char_count"] for r in results)))
        return results
    end

    __airis_emit_status("output", Dict{String, Any}("count" => length(results), "total_chars" => sum(length(r["content"]) for r in results)))
    return results
end
...
function __airis_call_bridge(name::String, args::Dict{String, Any})
...
function (tc::OmpToolCallable)(args...; kwargs...)
    ...
    return __airis_call_bridge("tool:" * tc.name, args_dict)
end
...
function completion(prompt::String; model="default", system=nothing, schema=nothing, kwargs...)
    ...
    res = __airis_call_bridge("__completion__", args_dict)
...
function agent(prompt::String; agent="task", model=nothing, label=nothing, schema=nothing, schema_mode=nothing, isolated=nothing, apply=nothing, merge=nothing, handle=false, kwargs...)
    ...
    res = __airis_call_bridge("__agent__", args_dict)
    text = res isa AbstractDict ? get(res, "text", res) : res
...
function _concurrency_limit()
    try
        snap = __airis_call_bridge("__concurrency__", Dict{String, Any}())
...
function __airis_budget_snapshot()
    try
        snap = __airis_call_bridge("__budget__", Dict{String, Any}())
...
function __airis_budget_int(value, default::Int=0)
...
function Base.getproperty(::OmpBudgetProxy, sym::Symbol)
    if sym === :total
        snap = __airis_budget_snapshot()
        return get(snap, "total", nothing)
    elseif sym === :hard
        snap = __airis_budget_snapshot()
        return get(snap, "hard", false) == true
    elseif sym === :spent
        return () -> __airis_budget_int(get(__airis_budget_snapshot(), "spent", 0), 0)
    elseif sym === :remaining
        return () -> begin
            ...
            return max(0, __airis_budget_int(total, 0) - __airis_budget_int(get(snap, "spent", 0), 0))
        end
    end
