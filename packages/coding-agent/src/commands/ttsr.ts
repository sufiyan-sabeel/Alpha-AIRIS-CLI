...
/**
 * `airis ttsr` — inspect and test Time-Traveling Stream Rules.
 *
 * `airis ttsr test` feeds a snippet (inline, --file, or stdin) through the real
 * TTSR matching pipeline and reports which rules would trigger. `airis ttsr list`
...
export default class Ttsr extends Command {
	...
	static examples = [
		"airis ttsr list",
		"airis ttsr test 'const x: any = 1'",
		"airis ttsr test src/foo.ts",
		"airis ttsr test --file src/foo.ts",
		"airis ttsr test --file src/foo.ts --source text",
		"airis ttsr test --rule .airis/rules/no-any.md --source tool --path src/foo.ts 'const x: any = 1'",
		"echo 'Box::leak(&mut v)' | airis ttsr test --file - --path src/lib.rs",
		"airis ttsr test --source tool --tool edit --path src/foo.ts 'const x: any = 1'",
		"airis ttsr scan",
		"airis ttsr scan src/",
		"airis ttsr scan -r .airis/rules/no-any.md src/",
	];
	...
	async run(): Promise<void> {
		...
		// inline text — so `airis ttsr test src/foo.ts` works without --file.
