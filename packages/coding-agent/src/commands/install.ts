/**
 * `airis install <target>` — top-level convenience over `airis plugin install` /
 * `airis plugin link`.
 *
 * The docs (airis.sh/docs/extension-authoring) advertise
 *
 *   airis install ./my-extension
 *
...
/**
 * Heuristic used to decide whether `airis install <target>` should `link` a
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: airis install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}
