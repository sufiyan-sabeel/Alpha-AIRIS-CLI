...
export default class Agents extends Command {
	...
	static flags = {
		...
		user: Flags.boolean({ description: "Write to ~/.airis/agent/agents (default)" }),
		project: Flags.boolean({ description: "Write to ./.airis/agents" }),
	};

	static examples = [
		"# Export bundled agents into user config (default)\n  airis agents unpack",
		"# Export bundled agents into project config\n  airis agents unpack --project",
		"# Overwrite existing local agent files\n  airis agents unpack --project --force",
		"# Export into a custom directory\n  airis agents unpack --dir ./tmp/agents --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Agents);
		if (!args.action) {
			renderCommandHelp("airis", "agents", Agents);
			return;
		}
