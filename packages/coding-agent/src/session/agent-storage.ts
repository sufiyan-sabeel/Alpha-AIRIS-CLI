...
import { AsyncDrain, getAgentDbPath, getStatsDbPath, isRecord, logger } from "@airis/airis-utils";
...
/** Row shape read from an `airis stats` messages table during backfill. */
 ...
 * Delegates auth credential operations to AuthCredentialStore from @airis/airis-ai.
...
`);
	 ...
	 * `airis stats` database (`~/.airis/stats.db`) into model_perf. Fire-and-forget:
	 ...
	 * A missing stats.db leaves the marker unset so a later `airis stats` run
	...
	/**
	 * Imports recent measurable request rows from an `airis stats` database
