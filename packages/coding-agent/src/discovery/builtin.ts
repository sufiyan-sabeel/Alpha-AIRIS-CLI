/**
 * Builtin Provider (.airis)
 *
 * Primary provider for AIRIS native configs. Supports all capabilities.
 */
import * as path from "node:path";
import { getAgentDir, logger, parseFrontmatter, tryParseJson } from "@airis/airis-utils";
...
const DISPLAY_NAME = "AIRIS";
const DESCRIPTION = "Native AIRIS configuration from ~/.airis and .airis/";
const PRIORITY = 100;
...
async function getConfigDirs(ctx: LoadContext): Promise<Array<{ dir: string; level: "user" | "project" }>> {
	...
	// profile's agent dir (~/.airis/profiles/<name>/agent), like sessions and MCP.
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	// Walk up from cwd finding .airis/skills/ in ancestors (closest first)
	...
	// User-level scan from ~/.airis/agent/skills/
...
registerProvider<Skill>(skillCapability.id, {
	...
	description: "Auto-generated managed skills from ~/.airis/agent/managed-skills",
...
async function loadRules(ctx: LoadContext): Promise<LoadResult<Rule>> {
	...
	// https://airis.sh/docs/context-files as the file that gets "re-injected near
	// the current turn so they keep hold across long conversations".
	// User scope:    ~/.airis/agent/RULES.md
	// Project scope: nearest .airis/RULES.md walking up from cwd to repoRoot
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load AGENTS.md from .airis/ directories",
	priority: PRIORITY,
