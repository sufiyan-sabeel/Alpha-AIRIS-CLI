...
import { FileType, glob } from "@airis/airis-natives";
import {
	CONFIG_DIR_NAME,
	getAgentDir,
	getConfigDirName,
...
async function readExtensionModuleManifest(
	...
	const pkg = tryParseJson<{ airis?: ExtensionModuleManifest; pi?: ExtensionModuleManifest }>(content);
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.airis/`.
 *    The first match returns `<dir>/.airis/plugins/installed_plugins.json`.
 * 2. If no `.airis/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.airis/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .airis/ directory (nearest wins).
	// Stop before os.homedir() — ~/.airis/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.airis/plugins/installed_plugins.json`
 * when no project anchor (.airis/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .airis/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * Reads ~/.claude/plugins/installed_plugins.json and ~/.airis/plugins/installed_plugins.json,
...
export async function listClaudePluginRoots(
	...
	// ── AIRIS installed plugins registry ───────────────────────────────────────
	// AIRIS registry is authoritative: its entries replace Claude's entries for the same plugin ID.
	...
	if (ompContent) {
		...
		if (ompRegistry) {
			for (const [pluginId, entries] of Object.entries(ompRegistry.plugins)) {
				...
				// AIRIS is authoritative: drop all Claude-sourced entries for this plugin ID
			...
			warnings.push(`Failed to parse AIRIS plugin registry: ${ompRegistryPath}`);
		}
	}

	// ── Project-scoped AIRIS registry ────────────────────────────────────────
	// Loaded from the nearest .airis/plugins/installed_plugins.json relative to cwd.
 ...
 * These are prepended to the cache with highest precedence (before AIRIS/Claude entries).
