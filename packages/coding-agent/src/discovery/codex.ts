...
import { logger, parseFrontmatter } from "@airis/airis-utils";
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	...
	// AIRIS hooks must be named `pre-<tool>.<ts|js>` or `post-<tool>.<ts|js>`.
	// Files without that prefix are not AIRIS hooks (e.g. the standalone Codex
