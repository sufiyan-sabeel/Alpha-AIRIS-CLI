 ...
 * `prompts/`, `.mcp.json`) are wired into discovery by `airis-plugins.ts`.
 *
 * CLI-provided paths are injected via {@link injectAirisExtensionCliRoots}
 * before discovery runs; settings paths are read lazily from
 * `<scope>/settings.json` in {@link listAirisExtensionRoots} to mirror what
 * `loadExtensionModules` already does.
 *
 * @see ./airis-plugins.ts
 * @see ./builtin.ts `loadExtensionModules`
 */
import * as fs from "node:fs/promises";
import * as path from "node:path";
import { getAgentDir, isEnoent, logger, tryParseJson } from "@airis/airis-utils";
...
export interface AirisExtensionRoot {
 ...
 * the registered set; {@link clearAirisExtensionCliRoots} resets for tests.
 */
export function injectAirisExtensionCliRoots(paths: readonly string[], home: string, cwd: string): void {
...
export function clearAirisExtensionCliRoots(): void {
	injectedCliRoots = [];
}

/** Inspect currently-injected CLI roots (read-only). Exposed for diagnostics + tests. */
export function getInjectedAirisExtensionCliRoots(): readonly AirisExtensionRoot[] {
...
function scopeDirs(ctx: LoadContext): ScopeDirs {
	return {
		project: path.join(ctx.cwd, ".airis"),
		user: getAgentDir(),
	};
}
 ...
 * are dropped):
 *
 * 1. CLI roots injected via {@link injectAirisExtensionCliRoots}
 * 2. Project `<cwd>/.airis/settings.json#extensions`
 * 3. User `~/.airis/agent/settings.json#extensions`
 * 4. Enabled npm/link plugins installed under `<plugins>/node_modules/` (for
 *    `airis install <pkg>` / `airis plugin install` / `airis plugin link`). Marketplace
 ...
 */
export async function listAirisExtensionRoots(ctx: LoadContext): Promise<AirisExtensionRoot[]> {
	...
	const roots: AirisExtensionRoot[] = [];
 ...
 * "Claude Code Marketplace" and "AIRIS Extension Packages".
 */
