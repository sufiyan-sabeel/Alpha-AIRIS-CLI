/**
 * AIRIS extension-package sub-discovery provider.
 *
 ...
 * `rules/`, `prompts/`, and `.mcp.json` — are picked up by airis's standard
 * discovery surfaces. The native `airis` provider in `builtin.ts` only walks
 * `.airis/` and `~/.airis/agent/`, so without this provider those sub-trees are
 * silently ignored.
 *
 * Provider priority is set below the native `airis` provider (100) so an
 * extension package never shadows the user's own `.airis/` configuration on
 * dedup.
 *
 * @see ./airis-extension-roots.ts
 * @see ../../docs/extension-loading.md
 */
import * as path from "node:path";
import { logger, parseFrontmatter, tryParseJson } from "@airis/airis-utils";
...
import { listAirisExtensionRoots, type AirisExtensionRoot } from "./airis-extension-roots";
import { resolvePluginStdioPaths } from "./substitute-plugin-root";

const PROVIDER_ID = "airis-plugins";
const DISPLAY_NAME = "AIRIS Extension Packages";
const DESCRIPTION =
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	const roots = await listAirisExtensionRoots(ctx);
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	const roots = await listAirisExtensionRoots(ctx);
	const tasks: Array<{ root: AirisExtensionRoot; hookType: "pre" | "post" }> = [];
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	const tasks: Array<{ root: AirisExtensionRoot; mcpPath: string }> = [];
	for (const root of roots) {
	...
	for (let i = 0; i < tasks.length; i++) {
		...
		if (!parsed) {
			warnings.push(`[airis-plugins] Invalid JSON in ${mcpPath}`);
			logger.warn(`[airis-plugins] Invalid JSON in ${mcpPath}`);
			continue;
		}
		...
		for (const [serverName, serverCfg] of Object.entries(servers)) {
			...
			if (typeof cfg.command !== "string" && typeof cfg.url !== "string") {
				warnings.push(`[airis-plugins] Skipping MCP server "${serverName}" in ${mcpPath}: missing command or url`);
				continue;
			}
