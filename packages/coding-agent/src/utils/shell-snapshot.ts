...
echo "# Shell snapshot - generated by airis agent" >| "$SNAPSHOT_FILE"
...
${fnEnvHelper}
printf '%s\\n' "$__airis_funcs" | __airis_emit_referenced_exports >> "$SNAPSHOT_FILE"
unset -f __airis_sq_quote __airis_emit_export_for __airis_emit_referenced_exports 2>/dev/null
unset __airis_funcs __airis_qbuf __airis_qout __airis_sq __airis_xv __airis_name 2>/dev/null
...
export async function getOrCreateSnapshot(
	...
	const snapshotDir = path.join(os.tmpdir(), "airis.shell-snapshots");
