...
import type { ImageContent, Model } from "@airis/airis-ai";
import { formatBytes, readImageMetadata, SUPPORTED_IMAGE_MIME_TYPES } from "@airis/airis-utils";
 ...
 * to PNG/JPEG instead — the automatic equivalent of `AIRIS_NO_WEBP=1`.
 */
 ...
 * `AIRIS_NO_WEBP` env fallback in {@link resizeImage} still applies. Feed straight
...
export interface LoadImageInputOptions {
	...
	/** Force non-WebP output (e.g. for Ollama). Leave unset to honor `AIRIS_NO_WEBP`. */
	excludeWebP?: boolean;
}
