...
/**
 * Read `AIRIS_NO_WEBP` per-call so runtime toggles take effect.
...
function isWebPExcluded(): boolean {
	const raw = Bun.env.AIRIS_NO_WEBP;
 ...
 *
 * Set AIRIS_NO_WEBP to exclude WebP from encoding (llama.cpp STB doesn't decode it).
 *
...
export async function resizeImage(img: ImageContent, options?: ImageResizeOptions): Promise<ResizedImage> {
	...
	try {
		...
		// WebP usually beats JPEG by 25–35% but is disabled when AIRIS_NO_WEBP is set
		...
		// unless AIRIS_NO_WEBP is set (llama.cpp STB incompatibility).
