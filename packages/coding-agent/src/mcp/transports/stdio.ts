...
import { getProjectDir, readJsonl, Snowflake } from "@airis/airis-utils";
...
export interface StdioSpawnCommand {
	 ...
	 *
	 * Windows uses this only when the AIRIS host has no console to share. When
	 ...
	 * wrapper chains must stay in the AIRIS console session so nested console
...
export async function resolveStdioSpawnCommand(
	...
	// same console session. Only hide the child when AIRIS itself has no console
