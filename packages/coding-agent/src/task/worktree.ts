...
import { getWorktreeDir, logger, Snowflake } from "@airis/airis-utils";
...
async function writeSyntheticTree(
	...
	const tempIndex = path.join(os.tmpdir(), `airis-task-index-${Snowflake.next()}`);
	try {
	...
	for (const [relativePath, repoPatches] of byRepo) {
		...
		const stashed =
			(await git.status(nestedDir)).trim().length > 0
				? await git.stash.push(nestedDir, `airis-isolation-${Snowflake.next()}`)
				: false;
		try {
...
export async function ensureIsolation(
	...
	for (const candidate of candidates) {
		...
		// `airis worktree clear` never sees this sandbox without a live owner,
...
async function replayFilteredAgentCommits(opts: FilteredAgentReplayOptions): Promise<void> {
	...
	const tmpDir = path.join(os.tmpdir(), `airis-branch-${Snowflake.next()}`);
	try {
 ...
 * branch named `airis/task/${taskId}`. Only root-repo changes go on the branch;
...
export async function commitToBranch(
	...
	const branchName = `airis/task/${taskId}`;
...
export async function mergeTaskBranches(
	...
	return git.withRepoLock(repoRoot, async () => {
		...
		const didStash = await git.stash.push(repoRoot, "airis-task-merge");
