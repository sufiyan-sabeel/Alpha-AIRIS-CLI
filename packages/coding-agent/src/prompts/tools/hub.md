...
Project-scoped long-running processes shared by every airis instance in the same directory. A long-running service, watcher, debugger, REPL, or process needing later input MUST use `op:"start"`, not `bash`.
...
- **`start`** launches `application` + `args` directly. `cwd` defaults to the session directory; `pty` defaults true.
  ...
  - `persist: true` opts out of last-airis teardown; `detached: true` survives broker shutdown and all airis exits (implies persist, disables PTY input). Omit both unless their survival guarantees are required.
