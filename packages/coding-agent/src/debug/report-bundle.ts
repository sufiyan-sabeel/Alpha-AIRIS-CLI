...
import type { WorkProfile } from "@airis/airis-natives";
import { APP_NAME, getLogPath, getLogsDir, getReportsDir, isEnoent } from "@airis/airis-utils";
...
export async function createReportBundle(options: ReportBundleOptions): Promise<ReportBundleResult> {
	...
	const outputPath = path.join(reportsDir, `airis-report-${timestamp}.tar.gz`);
 ...
 * after a crash still captures the fatal PID's `airis.<date>.<pid>.log`. Files
