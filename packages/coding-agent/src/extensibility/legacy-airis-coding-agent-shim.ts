 ...
 * `@airis/airis-coding-agent` (or one of its aliased scopes like
 ...
 * the same module identity as a direct `@airis/airis-coding-agent` import.
 */

import { Database } from "bun:sqlite";
import * as fs from "node:fs";
import * as path from "node:path";
import type { AgentToolResult, AgentToolUpdateCallback } from "@airis/airis-agent-core";
import { type AuthCredential, SqliteAuthCredentialStore, type TSchema } from "@airis/airis-ai";
import { piEscapeRegexLiteral, piJoinPath } from "@airis/airis-ai/providers/cursor-airis-args";
import { getKeybindings, type Keybinding, Text } from "@airis/airis-tui";
import {
	getAgentDbPath,
	getAgentDir,
	getProjectDir,
...
const LEGACY_BUILTIN_TOOL_MARKER = "__airisLegacyBuiltinTool";
...
export function createEditToolDefinition(cwd: string, options?: EditToolOptions): ToolDefinition {
	if (options?.operations) {
		throw new Error(
			"Legacy EditToolOptions.operations is not supported: AIRIS's built-in edit tool writes the local " +
...
export function createWriteToolDefinition(cwd: string, options?: WriteToolOptions): ToolDefinition {
	if (options?.operations) {
		throw new Error(
			"Legacy WriteToolOptions.operations is not supported: AIRIS's built-in write tool writes the local " +
...
/**
 * Enumerates the extensions AIRIS would load through the historical package
...
export class DefaultPackageManager {
	...
	/** Resolve enabled extension paths with their AIRIS plugin provenance. */
 ...
 *
 * AIRIS does the same discovery inline inside `createAgentSession()`, so this
 ...
 * override below translates them into AIRIS's native session options
 ...
 * extensions in the wild — themes are silently dropped (AIRIS has no
...
export interface ResourceLoader {
	...
	readonly __airisLegacyAirisLoader?: true;
}
...
export class DefaultResourceLoader implements ResourceLoader {
	readonly __airisLegacyAirisLoader = true as const;
...
/**
 * Legacy pi extensions call `createAgentSession({ resourceLoader })`. AIRIS's
 ...
 * `noExtensions`/`noSkills` opt-outs), re-run AIRIS's own discovery, and
 ...
 *
 * Translate the loader's captured state into AIRIS's option fields, then
 ...
 *
 * Modern AIRIS auth storage is asynchronous, while older provider extensions
...
// `getAgentDir`; `getProjectDir` (a `@airis/airis-utils` helper) and
...
export { getProjectDir } from "@airis/airis-utils";
 ...
 *
 * airis's canonical `getPackageDir()` (`../config`) returns `undefined` inside a
...
// from its package root (via `./core/compaction/index.ts`). In airis it lives in
// `@airis/airis-agent-core/compaction`, and the coding-agent barrel below does
// not forward it, so legacy extensions importing it fail Bun's static export
// check during validation (issue #6583).
export { estimateTokens } from "@airis/airis-agent-core/compaction";

// Same barrel gap for two more legacy package-root exports: pi re-exported the
// `CONFIG_DIR_NAME` constant and the CLI parser `parseArgs`. In airis
// `CONFIG_DIR_NAME` lives in `@airis/airis-utils` and `parseArgs` in
...
export { CONFIG_DIR_NAME } from "@airis/airis-utils";
