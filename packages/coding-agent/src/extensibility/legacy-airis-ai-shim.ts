 ...
 * `@airis/airis-ai` (or one of its aliased scopes like `@earendil-works/pi-ai`
 ...
 * `Type.Object({ ... })`, so this file is served by `legacy-airis-compat.ts` in
 * place of the real pi-ai entrypoint whenever a legacy extension imports the
 * bare package root. Subpath imports (`@airis/airis-ai/oauth`, etc.)
...
/** Clamp a historical Pi thinking level against AIRIS's model metadata. */
 ...
 * were authored against rather than AIRIS's own `Error`-based classifiers.
 */
 ...
 * `@airis/airis-catalog/models`. `getModel`/`getModels` are the historical
 ...
 * `@earendil-works/pi-ai` exposed from its package root but airis's
 * `@airis/airis-ai` barrel no longer forwards. Each symbol still exists in the
 ...
 * static named-export check (e.g. `airis plugin install pi-blackhole`).
 *
 ...
 * `@airis/airis-ai/error`) and the JSON-repair helpers that airis relocated to
 * `@airis/airis-utils`. Upstream root symbols with no airis equivalent are
 * intentionally not shimmed — the package has diverged and there is nothing to
 * forward.
 */
export { isContextOverflow } from "@airis/airis-ai/error";
export { parseJsonWithRepair, parseStreamingJson, repairJson } from "@airis/airis-utils";
