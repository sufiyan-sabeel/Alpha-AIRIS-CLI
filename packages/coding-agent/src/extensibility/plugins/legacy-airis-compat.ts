/// <reference path="./legacy-airis-virtual-modules.d.ts" />
...
import { isCompiledBinary, stripWindowsExtendedLengthPathPrefix } from "@airis/airis-utils";
...
// surfaces through `airis-legacy-airis-bundled:<key>` synthetic modules.
// `scripts/legacy-airis-virtual-module.ts` derives literal dynamic-import edges
...
const BUNDLED_VIRTUAL_SCHEME = "airis-legacy-airis-bundled:";
const BUNDLED_VIRTUAL_NAMESPACE = "airis-legacy-airis-bundled";
const BUNDLED_MODULES_GLOBAL = "__airisLegacyAirisBundledModules";
...
function ensureBundledModuleLoadersLoaded(): Promise<BundledModuleLoaders> {
	if (!IS_COMPILED_BINARY) {
		return Promise.reject(new Error("airis:legacy-airis-shim: bundled modules are only available in compiled mode"));
	}
	if (!bundledModuleLoadersPromise) {
		bundledModuleLoadersPromise = import("airis-legacy-airis-modules").then(module => {
...
async function loadBundledModule(moduleKey: string): Promise<void> {
	...
	if (!loader) {
		throw new Error(`airis:legacy-airis-shim: no bundled module registered for ${moduleKey}`);
	}
...
export function resolveBundledVirtualSpecifier(specifier: string): BundledVirtualResolveResult {
	const registryKey = isBundledVirtualSpecifier(specifier)
		...
		: specifier;
	if (!registryKey) {
		throw new Error("airis:legacy-airis-shim: bundled virtual specifier has no registry key");
	}
	return { path: registryKey, namespace: BUNDLED_VIRTUAL_NAMESPACE };
}
...
function synthesizeBundledModuleSourceFromModules(moduleKey: string, modules: BundledModules): string {
	...
	const lines: string[] = [
		`const __airis_bundled = globalThis[${JSON.stringify(BUNDLED_MODULES_GLOBAL)}][${JSON.stringify(moduleKey)}];`,
	];
	...
	for (const exportName in mod) {
		...
		lines.push(`export const ${exportName} = __airis_bundled[${JSON.stringify(exportName)}];`);
	}
	if (hasDefault) {
		lines.push("export default __airis_bundled.default;");
	}
	lines.push("");
	return lines.join("\n");
}
 ...
 * `airis-legacy-airis-bundled:<key>` import.
 */
...
export function __getLegacyAirisBundledModulesGlobal(): string {
	return BUNDLED_MODULES_GLOBAL;
}

// Canonical scope for in-process pi packages. Plugins published against any of
// the aliased scopes below (mariozechner's original publish, earendil-works'
// fork, or the canonical @alpha-airis-cli scope itself) are remapped to this scope and
// resolved against the bundled copy that ships inside the airis binary. This
...
const CANONICAL_PI_SCOPE = "@alpha-airis-cli";

// Scopes that have historically been used to publish (or alias) the same set
// of internal pi-* packages. `@alpha-airis-cli` is intentionally included so direct
...
const PI_SCOPE_ALIASES = ["alpha-airis-cli", "mariozechner", "earendil-works"] as const;

// Internal pi-* package basenames bundled inside the airis binary.
const PI_PACKAGE_NAMES = ["pi-agent-core", "pi-ai", "pi-coding-agent", "airis-natives", "pi-tui", "pi-utils"] as const;
...
const LEGACY_AIRIS_SPECIFIER_FILTER = new RegExp(`^@(?:${PI_SCOPE_ALTERNATION})/(?:${PI_PACKAGE_ALTERNATION})(?:/.*)?$`);
...
// is served through the `airis-legacy-airis-bundled:` virtual namespace (see the
 ...
 * bare `@airis/airis-coding-agent` here: from a global install Bun can pick an
 ...
 * `airis-legacy-airis-bundled:` virtual namespace (issue #3423) — bunfs paths are
...
// imports such as `@airis/airis-ai/oauth` continue to resolve directly
// against the bundled pi-ai package.
const LEGACY_AIRIS_AI_SHIM_PATH = IS_COMPILED_BINARY
	? bundledModuleVirtualSpecifier(`${CANONICAL_PI_SCOPE}/pi-ai`)
	: sourceShimPath("legacy-airis-ai-shim.ts");
...
const LEGACY_AIRIS_CODING_AGENT_SHIM_PATH = IS_COMPILED_BINARY
	? bundledModuleVirtualSpecifier(`${CANONICAL_PI_SCOPE}/pi-coding-agent`)
	: sourceShimPath("legacy-airis-coding-agent-shim.ts");
...
const LEGACY_AIRIS_TUI_SHIM_PATH = IS_COMPILED_BINARY
	? bundledModuleVirtualSpecifier(`${CANONICAL_PI_SCOPE}/pi-tui`)
	: sourceShimPath("legacy-airis-tui-shim.ts");
...
// other bundled host packages (`pi-agent-core`, `airis-natives`, `pi-utils`) are
...
// `node_modules/@airis/airis-*`.
//
// Compiled-binary entries are `airis-legacy-airis-bundled:<key>` specifiers handed
// to the synthetic onLoad in `installLegacyAirisSpecifierShim()` — bunfs paths
 ...
 * through to the canonical-resolution path. Virtual `airis-legacy-airis-bundled:`
...
export function __buildLegacyPiPackageRootOverrides(
	...
	const candidates: Record<string, string> = {
		[`${CANONICAL_PI_SCOPE}/pi-ai`]: LEGACY_AIRIS_AI_SHIM_PATH,
		[`${CANONICAL_PI_SCOPE}/pi-coding-agent`]: LEGACY_AIRIS_CODING_AGENT_SHIM_PATH,
		[`${CANONICAL_PI_SCOPE}/pi-tui`]: LEGACY_AIRIS_TUI_SHIM_PATH,
	};
	if (isCompiled) {
...
function remapLegacyPiSpecifier(specifier: string): string | null {
	if (!LEGACY_AIRIS_SPECIFIER_FILTER.test(specifier)) {
		return null;
	}
...
/**
 * Resolve a canonical `@airis/airis-*` specifier to a filesystem path, preferring
...
function toImportSpecifier(resolvedPath: string): string {
	// Virtual `airis-legacy-airis-bundled:` specifiers are served by the synthetic
	// onLoad in `installLegacyAirisSpecifierShim()`; wrapping them as `file://`
...
/**
 * Rewrite the extension-owned specifiers AIRIS must host-resolve — legacy
...
const COMMONJS_REQUIRE_GLOBAL = "__airisLegacyAirisRequireGraphModule";
 ...
 * realpath of every reachable source module AIRIS must rewrite at load time.
...
function synthesizeCommonJsDefaultModule(modulePath: string, source: string, targetPath = modulePath): string {
	...
	commonJsModuleCache.delete(modulePath);
	const exportsBinding = "__airisLegacyAirisCommonJsExports";
	const namedExports = collectCommonJsNamedExports(executableSource, targetPath)
		.map(
			(name, index) =>
				`const __airisLegacyPiCommonJsExport${index} = ${exportsBinding}[${JSON.stringify(name)}]; export { __airisLegacyPiCommonJsExport${index} as ${name} };`,
		)
		.join("\n");
...
/**
 * Linkedom's canvas bridge uses its bundled fallback because AIRIS does not ship
 * native canvas.
 */
...
async function installExtensionGraphHook(
	...
	if (asyncModules.size > 0) {
		...
		Bun.plugin({
			name: `airis:legacy-airis-ext:${hookId}`,
			setup(build) {
 ...
 */
export async function loadLegacyAirisModule(resolvedPath: string): Promise<unknown> {
...
function resolveLegacyPiSpecifier(args: { path: string; importer: string }): LegacyPiResolveResult | undefined {
	...
	// Primary: resolve the canonical @airis/airis-* specifier from the host binary
	...
	try {
		...
		// plugin installed @alpha-airis-cli peer deps, then try the original legacy
...
export function installLegacyAirisSpecifierShim(): void {
	...
	Bun.plugin({
		name: "airis:legacy-airis-shim",
		setup(build) {
			build.onResolve({ filter: LEGACY_AIRIS_SPECIFIER_FILTER, namespace: "file" }, resolveLegacyPiSpecifier);
			...
			// Compiled mode serves `airis-legacy-airis-bundled:<key>` imports from
