...
import { logger, sanitizeText } from "@airis/airis-utils";
	...
	if (mode === "text") {
		...
		if (assistantMsg) {
			...
			if (
				...
				// AIRIS-owned Chromium survives this exit (issue #5643). `dispose()`
				...
				await session.dispose({ mnemosyneConsolidateTimeoutMs: SHUTDOWN_CONSOLIDATE_BUDGET_MS });
