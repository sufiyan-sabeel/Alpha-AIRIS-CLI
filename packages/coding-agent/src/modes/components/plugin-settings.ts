...
export class PluginListComponent extends Container {
	...
	constructor(
		...
		if (entries.length === 0) {
			...
			this.addChild(new Spacer(1));
			this.addChild(new Text(theme.fg("dim", "  Install npm plugins:        airis plugin install <package>"), 0, 0));
			this.addChild(
				new Text(theme.fg("dim", "  Install marketplace plugins: airis plugin install <name>@<marketplace>"), 0, 0),
			);
			this.addChild(new Spacer(1));
			this.addChild(new DynamicBorder());
