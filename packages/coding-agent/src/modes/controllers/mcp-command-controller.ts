...
import { type Component, replaceTabs, Spacer, Text } from "@airis/airis-tui";
import { getMCPConfigPath, getProjectDir } from "@airis/airis-utils";
 ...
 * SSH/WSL/headless sessions where the AIRIS-hosted `/launch` loopback URL would
...
export class MCPCommandController {
			...
			const flow = new MCPOAuthFlow(
				...
				},
				{
					onAuth: (info: { url: string; launchUrl?: string; instructions?: string }) => {
						...
						// only resolves against the AIRIS host), and OSC 52 is a
						// wire-level protocol — the terminal writes it to the user's
						// LOCAL clipboard even when AIRIS is on a remote SSH box.
	 ...
	 *
	 * Unlike {@link #findConfiguredServer} (which only reads writable AIRIS config
	...
	async #resolveOAuthEndpointsFromServer(
		...
		// Stdio servers manage credentials inside the child process; AIRIS's OAuth
		...
		if (config.type !== "http" && config.type !== "sse") {
			...
			throw new Error(
				usesMcpRemote
					? `this server proxies OAuth through mcp-remote, which caches tokens machine-wide in ~/.mcp-auth (shared across every AIRIS profile). Clear ~/.mcp-auth to force a fresh login, or replace the proxy with ${httpHint} so AIRIS manages OAuth per profile.`
					: `stdio servers manage their own credentials, so AIRIS has no OAuth to reauthorize. If the service supports OAuth over HTTP, configure it as ${httpHint} instead.`,
			);
		}
