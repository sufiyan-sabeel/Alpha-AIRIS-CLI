 ...
 */
import { getProjectDir, getSSHConfigPath } from "@airis/airis-utils";
	...
	async #handleList(): Promise<void> {
		try {
			...
			if (userHosts.length > 0) {
				lines.push(theme.fg("accent", "User level") + theme.fg("muted", ` (~/.airis/agent/ssh.json):`));
			...
			if (projectHosts.length > 0) {
				lines.push(theme.fg("accent", "Project level") + theme.fg("muted", ` (.airis/ssh.json):`));
