...
import { Process, type PtyRunResult, PtySession } from "@airis/airis-natives";
import { isEexist, isEnoent, logger, postmortem, procmgr, sanitizeText, setProcessName } from "@airis/airis-utils";
...
export async function startDaemonBrokerFromEnvironment(): Promise<void> {
	...
	if (!lease) return;
	setProcessName("airis daemon broker");
