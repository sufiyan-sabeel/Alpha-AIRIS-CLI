...
import { CONFIG_DIR_NAME, getConfigAgentDirName, getProjectDir } from "@airis/airis-utils";
 ...
 * project with airis's own assets and was the source of issue #1423 (the host
 * project's `CHANGELOG.md` rendered as airis's startup changelog).
 */
...
/**
 * Path to airis's own `CHANGELOG.md`, or `undefined` when the package directory
 ...
 * User-level: ~/.airis/agent, ~/.claude, ~/.codex, ~/.gemini
 * Project-level: .airis, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".airis", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.airis/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.airis/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.airis/agent/commands", source: ".airis", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.airis, .claude) - the nearest one found.
