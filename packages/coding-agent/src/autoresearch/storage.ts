...
import { getAutoresearchDbPath, getAutoresearchProjectDir, logger } from "@airis/airis-utils";
 ...
 *
 * Used to key per-project autoresearch state under `~/.airis/autoresearch/`.
...
async function resolveAutoresearchPaths(cwd: string): Promise<{ dbPath: string; projectDir: string }> {
	const override = process.env.AIRIS_AUTORESEARCH_DB_DIR;
