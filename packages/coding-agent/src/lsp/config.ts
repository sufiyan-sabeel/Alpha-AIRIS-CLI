...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@airis/airis-utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.airis/, .pi/, .claude/)
	...
	// User config directories (~/.airis/agent/, ~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .airis/lsp.*, .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.airis/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
