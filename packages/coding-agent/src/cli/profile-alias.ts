...
import { normalizeProfileName } from "@airis/airis-utils/dirs";
...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "airis") {
		throw new Error('Invalid alias "airis". Refusing to shadow the base airis command.');
	}
...
function resolveShellConfigPath(
	...
	switch (shell) {
		...
		case "fish": {
			...
			return posixJoinUnc(configHome, "fish", "conf.d", "airis-profiles.fish");
		}
		case "pwsh":
...
function renderAliasBlock(
	...
	const start = `# >>> airis profile alias: ${aliasName} >>>`;
	const end = `# <<< airis profile alias: ${aliasName} <<<`;
	let body: string;
	switch (shell) {
		case "fish":
			body = [
				`function ${aliasName} --wraps airis --description 'AIRIS profile ${profile}'`,
