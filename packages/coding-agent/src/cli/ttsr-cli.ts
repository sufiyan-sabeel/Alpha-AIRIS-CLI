 ...
 *
 * `airis ttsr test` — feed a snippet (inline text, `--file`, or stdin) through the
 ...
 *
 * `airis ttsr list` — show every TTSR-registered rule the current project/user
 * config would load, with its conditions, scope, and source.
 */
import * as fs from "node:fs";
import * as path from "node:path";
import { AstMatchStrictness, astMatch, FileType, type GlobMatch, glob } from "@airis/airis-natives";
import { getProjectDir } from "@airis/airis-utils/dirs";
