/**
 * `airis auth-gateway` command handlers.
 *
 * Boots a forward-proxy server that lets less-trusted clients (the macOS
 * usage widget, roboairis containers, …) make provider API calls without ever
 ...
 * `AIRIS_AUTH_BROKER_URL` / `auth.broker.url` precedence used elsewhere).
 *
 * Sub-verbs:
...
import { DEFAULT_AUTH_GATEWAY_BIND, startAuthGateway } from "@airis/airis-ai/auth-gateway";
import { type GeneratedProvider, getBundledModels } from "@airis/airis-catalog/models";
import { getConfigRootDir, isEnoent, logger, VERSION } from "@airis/airis-utils";
...
async function runServe(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`airis auth-gateway serve` requires AIRIS_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). The gateway is itself a broker client.",
		);
	}
	...
	// cached + broker-discovered), so the discovery-only models airis itself
	...
	// a client-requested `model` field into a airis-ai `Model<Api>` before dispatch;
...
async function runStatus(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		...
		if (flags.json) {
			...
			process.stdout.write(`${chalk.yellow("No broker configured.")} Set AIRIS_AUTH_BROKER_URL.\n`);
			process.stdout.write(
	...
	try {
		...
		if (flags.json) {
			...
			if (!tokenPresent) {
				process.stdout.write(
					"Run `airis auth-gateway token` or `airis auth-gateway serve` to create a bearer token.\n",
				);
			}
		}
 ...
 * - `cursor-agent` and `airis-native` (gateway forwarding) have transport quirks
 ...
 * airis-native transport (would loop through the gateway), and placeholder /
...
function pickProbeCandidates(provider: string): Model<Api>[] {
	...
	const candidates = bundled.filter(model => {
		if (model.transport === "airis-native") return false;
...
/**
 * `airis auth-gateway check` — probe each broker-supplied credential and print
...
async function runCheck(flags: AuthGatewayCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"`airis auth-gateway check` requires AIRIS_AUTH_BROKER_URL (or `auth.broker.url`/`auth.broker.token` in config.yml). It probes the same credentials the gateway would serve.",
		);
	}
