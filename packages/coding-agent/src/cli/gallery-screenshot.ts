/**
 * Render `airis gallery` output to PNG screenshots via VHS.
 *
...
export async function captureGalleryScreenshots(
	...
	if (!vhs) {
		throw new Error(
			"`airis gallery --screenshot` requires VHS, which is not installed. " +
	...
	const baseDir = options.out
		...
		: fs.mkdtempSync(path.join(os.tmpdir(), "airis-gallery-"));
