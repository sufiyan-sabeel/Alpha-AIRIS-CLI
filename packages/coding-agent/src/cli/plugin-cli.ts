 ...
 *
 * Handles `airis plugin <command>` subcommands for plugin lifecycle management.
 */
	...
	try {
		if (pluginId) {
			...
			if (flags.scope) {
				console.error(
					chalk.yellow(
						`Warning: --scope is ignored when upgrading all plugins. Use 'airis plugin upgrade <id> --scope ${flags.scope}' to target a specific plugin and scope.`,
					),
				);
			}
...
async function handleInstall(
	...
	if (packages.length === 0) {
		...
		console.error(chalk.dim(`  ${APP_NAME} plugin install @airis/airis-exa`));
	...
	for (const spec of packages) {
		...
		if (target.type === "local") {
			...
			// `airis plugin link <path>` so users can use either verb interchangeably.
			if (flags.scope) {
				console.error(
					chalk.yellow(
...
${chalk.bold("Examples:")}
  ${APP_NAME} plugin install @airis/airis-exa[search]
