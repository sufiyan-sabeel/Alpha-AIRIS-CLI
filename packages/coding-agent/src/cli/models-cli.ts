/**
 * `airis models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.airis/models.db`
 ...
 */
import type { Api, Effort, Model } from "@airis/airis-ai";
import { getSupportedEfforts } from "@airis/airis-catalog/model-thinking";
import { formatNumber, getProjectDir } from "@airis/airis-utils";
 ...
 * name doubles as an `airis models <provider>` shortcut.
 */
...
/** `airis models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/**
 * Entry point for the standalone `airis models` command: bootstraps auth storage,
...
export async function runModelsCommand(command: ModelsCommandArgs): Promise<void> {
	...
	if (action === "find" && (!pattern || pattern.trim().length === 0)) {
		process.stderr.write("`airis models find` requires a search substring, e.g. `airis models find minimax`\n");
		process.exitCode = 1;
		return;
	}
