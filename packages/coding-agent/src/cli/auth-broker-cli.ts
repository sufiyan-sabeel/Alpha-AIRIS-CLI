/**
 * `airis auth-broker` command handlers.
 *
 * Sub-verbs:
...
import { AuthBrokerClient, DEFAULT_AUTH_BROKER_BIND, startAuthBroker } from "@airis/airis-ai/auth-broker";
import { $which, APP_NAME, getAgentDbPath, getConfigRootDir, isEnoent, logger, VERSION } from "@airis/airis-utils";
import { setTransports as setLoggerTransports } from "@airis/airis-utils/logger";
import { $ } from "bun";
...
async function runServe(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	// skip the rotating ~/.airis/logs/ file the TUI default would have used.
...
async function runLogin(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!providerArg) {
		if (flags.via) {
			throw new Error(
				"Usage: airis auth-broker login <provider> --via=user@host (provider required for remote login)",
			);
		}
	...
	try {
		...
		await storage.login(provider, {
			onAuth({ url, launchUrl, instructions }) {
				...
				// sessions where a `launchUrl` (loopback `/launch` on the AIRIS
				...
				if (launchUrl && launchUrl !== url) {
					// Local shortcut for the machine running AIRIS. Terminals or
...
/**
 * Maps the `type` field of a CLIProxyAPI credential JSON to the airis provider id.
...
async function loadImportPlan(
	...
	for (const file of files) {
		...
		if (!provider) {
			skipped.push({
				file,
				reason: `cannot determine airis provider from type=${json.type ?? "?"} (pass --provider to override)`,
			});
			continue;
		}
...
async function runImport(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!target) {
		throw new Error("Usage: airis auth-broker import <file|dir> [--provider=<id>] [--include-disabled] [--dry-run]");
	}
...
async function runMigrate(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"AIRIS_AUTH_BROKER_URL must be set (or `auth.broker.url` in config.yml). `migrate` uploads local credentials to a configured broker.",
		);
	}
	if (flags.fromLocal !== true) {
		throw new Error(
			"`airis auth-broker migrate` requires an explicit source. Pass `--from-local` to migrate from the local SQLite store and env vars.",
		);
	}
	...
	try {
		for (const row of localStore.listAuthCredentials()) {
			// Skip placeholder sentinels that airis-ai treats as "authenticated via
...
async function runStatus(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!cfg) {
		const message = "No auth-broker configured (set AIRIS_AUTH_BROKER_URL to enable).";
