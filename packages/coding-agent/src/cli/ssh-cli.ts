 ...
 *
 * Handles `airis ssh <command>` subcommands for SSH host configuration management.
 */
...
async function handleAdd(cmd: SSHCommandArgs): Promise<void> {
	...
	if (!name) {
		...
		process.stdout.write(
			chalk.dim("Usage: airis ssh add <name> --host <address> [--user <user>] [--port <port>] [--key <path>]\n"),
		);
		process.exitCode = 1;
		return;
	}
	...
	if (!host) {
		...
		process.stdout.write(chalk.dim("Usage: airis ssh add <name> --host <address>\n"));
		process.exitCode = 1;
		return;
	}
...
async function handleRemove(cmd: SSHCommandArgs): Promise<void> {
	...
	if (!name) {
		process.stdout.write(chalk.red("Error: Host name required\n"));
		process.stdout.write(chalk.dim("Usage: airis ssh remove <name> [--scope project|user]\n"));
		process.exitCode = 1;
		return;
	}
...
async function handleList(cmd: SSHCommandArgs): Promise<void> {
	...
	if (!hasProject && !hasUser) {
		...
		process.stdout.write(chalk.dim("Add one with: airis ssh add <name> --host <address>\n"));
		return;
	}

	if (hasProject) {
		process.stdout.write(chalk.bold("Project SSH Hosts (.airis/ssh.json):\n"));
	...
	if (hasUser) {
		process.stdout.write(chalk.bold("User SSH Hosts (~/.airis/agent/ssh.json):\n"));
