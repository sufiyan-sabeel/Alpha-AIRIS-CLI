/**
 * Protocol handler for airis:// URLs.
 *
 * Serves statically embedded documentation files bundled at build time.
 *
 * URL forms:
 * - airis:// - Lists all available documentation files
 * - airis://<file>.md - Reads a specific documentation file
 */
...
export class OmpProtocolHandler implements ProtocolHandler {
	...
	async #listDocs(url: InternalUrl): Promise<InternalResource> {
		...
		const listing = filenames.map(f => `- [${f}](airis://${f})`).join("\n");
	...
	async #readDoc(filename: string, url: InternalUrl): Promise<InternalResource> {
		...
		if (path.isAbsolute(filename)) {
			throw new Error("Absolute paths are not allowed in airis:// URLs");
		}

		const normalized = path.posix.normalize(filename.replaceAll("\\", "/"));
		if (normalized === ".." || normalized.startsWith("../") || normalized.includes("/../")) {
			throw new Error("Path traversal (..) is not allowed in airis:// URLs");
		}
		...
		if (content === undefined) {
			...
			const suffix =
				suggestions.length > 0
					...
					: "\nUse airis:// to list available files.";
