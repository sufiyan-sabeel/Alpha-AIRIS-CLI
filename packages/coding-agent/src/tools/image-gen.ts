...
import { type ApiKey, type FetchImpl, getEnvApiKey, type Model, withAuth } from "@airis/airis-ai";
...
import { getAntigravityUserAgent } from "@airis/airis-catalog/wire/gemini-headers";
import {
	$env,
	isEnoent,
	parseImageMetadata,
...
async function saveImageToTemp(image: InlineImageData): Promise<string> {
	...
	const filename = `airis-image-${Snowflake.next()}.${ext}`;
