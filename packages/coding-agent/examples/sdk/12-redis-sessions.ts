 ...
 * `BlobStore` keep writing to `~/.airis/agent/...`. Reach for an object store
 * (S3, R2, GCS) if you need those off-host too.
 */

import { createAgentSession, RedisSessionStorage, SessionManager } from "@airis/airis-coding-agent";
...
const storage = await RedisSessionStorage.create({
	client: redis,
	prefix: "airis:sessions:", // optional, this is the default
});
