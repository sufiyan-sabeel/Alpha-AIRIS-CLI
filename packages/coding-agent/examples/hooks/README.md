...
Example hooks for airis-coding-agent.

## Usage

```bash
# Load a hook with --hook flag
airis --hook examples/hooks/permission-gate.ts

# Or copy to hooks directory for auto-discovery
cp permission-gate.ts ~/.airis/agent/hooks/
```
...
```typescript
import type { HookAPI } from "@airis/airis-coding-agent/hooks";
