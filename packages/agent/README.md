...
Stateful agent with tool execution and event streaming. Built on `@airis/airis-ai`.

## Installation

```bash
npm install @airis/airis-agent
```

## Quick Start

```typescript
import { Agent } from "@airis/airis-agent";
...
```typescript
declare module "@airis/airis-agent" {
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@airis/airis-ai`).
...
```typescript
import { Agent, streamProxy } from "@airis/airis-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@airis/airis-agent";
