...
} from "@airis/airis-agent-core/agent-loop";
import type {
	AgentContext,
	AgentEvent,
	AgentLoopConfig,
...
import type { AssistantMessage, AssistantMessageEvent, Context, Message, ToolResultMessage } from "@airis/airis-ai";
...
declare module "@airis/airis-agent-core/types" {
...
describe("agentLoop kCursorExecResolved (issue #4348)", () => {
	it("skips execute for a toolCall block marked as already run by Cursor's exec channel", async () => {
		const { kCursorExecResolved } = await import("@airis/airis-ai/utils/block-symbols");
