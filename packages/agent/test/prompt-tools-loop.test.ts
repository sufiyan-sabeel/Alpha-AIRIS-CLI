...
import type { AgentContext, AgentLoopConfig, AgentMessage, AgentTool } from "@airis/airis-agent-core/types";
import type { AssistantMessage, Context, Message, TextContent, ToolResultMessage } from "@airis/airis-ai";
...
describe("agentLoop with owned in-band tool calls", () => {
	...
	it("uses AIRIS_DIALECT=minimax when config.dialect is unset", async () => {
		const before = Bun.env.AIRIS_DIALECT;
		Bun.env.AIRIS_DIALECT = "minimax";
		try {
			const echoArgs: Array<{ msg: string }> = [];
			const toolSchema = type({ msg: "string" });
			const echoTool: AgentTool<typeof toolSchema, { msg: string }> = {
			...
			if (before === undefined) delete Bun.env.AIRIS_DIALECT;
			else Bun.env.AIRIS_DIALECT = before;
		}
	});
});
