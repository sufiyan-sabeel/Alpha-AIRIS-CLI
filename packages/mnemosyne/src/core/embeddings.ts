...
import { type ApiKey, getOpenRouterHeaders, withAuth } from "@airis/airis-ai";
...
// (configured via `withMnemosyneRuntimeOptions`) gets a stable integer id so the cache
// scope reflects the runtime's actual embedding source. Two Mnemosyne instances in the
...
export async function quarantineCorruptModelFile(message: string, cacheDir?: string): Promise<boolean> {
	...
	try {
		...
		logger.warn("mnemosyne: quarantined corrupt local embedding model; retrying init", { modelFile });
	} catch {
 ...
 * resolved model name, and the API base URL so two `Mnemosyne` instances in the same
 ...
 * `MNEMOSYNE_EMBEDDING_MAX_INPUT_CHARS`, then the bundled `8192` default. `0`
...
function effectiveMaxInputChars(): number {
	...
	const envValue = Number.parseInt($env.MNEMOSYNE_EMBEDDING_MAX_INPUT_CHARS ?? "", 10);
...
function capInputs(texts: readonly string[]): readonly string[] {
	...
	logger[mnemosyneDebugEnabled() ? "warn" : "debug"]("mnemosyne: embedding input truncated", {
 ...
 *
 * Reads (in order): the active provider's `model` from `withMnemosyneRuntimeOptions`,
 * the `MNEMOSYNE_EMBEDDING_MODEL` env var, then the bundled fastembed default. Stored
...
async function getLocalModel(): Promise<LocalEmbeddingModel | null> {
	...
	try {
		...
		logger[mnemosyneDebugEnabled() ? "warn" : "debug"]("mnemosyne: local embedding model failed to load", {
			model: modelName,
			error: String(error),
		});
...
async function embedApi(texts: readonly string[]): Promise<EmbeddingMatrix | null> {
	...
	try {
		...
		const response = await withAuth(apiKey, async key => {
			...
			if (res.status === 401) {
				throw new ProviderHttpError("mnemosyne embedding request unauthorized (401)", 401, { headers: res.headers });
			}
			return res;
		});
		if (!response.ok) {
...
export async function embed(texts: readonly string[]): Promise<EmbeddingMatrix | null> {
	...
	try {
		...
		logger[mnemosyneDebugEnabled() ? "warn" : "debug"]("mnemosyne: local embedding failed", {
