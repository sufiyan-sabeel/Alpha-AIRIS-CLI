...
function activeAirisAiModel(): Model<Api> | undefined {
	const model = activeLlmOptions()?.model;
	return isAirisAiModel(model) ? model : undefined;
}
...
function llmEnabled(): boolean {
	...
	if (activeCustomCompletion() !== undefined || activeAirisAiModel() !== undefined) {
		return true;
	}
...
export async function callConfiguredCompletion(
	...
	const model = activeAirisAiModel();
	if (model === undefined) {
		return null;
	}
	try {
...
export function configuredLlmWillHandleCall(): boolean {
	return llmEnabled() && (activeCustomCompletion() !== undefined || activeAirisAiModel() !== undefined);
}
...
export async function callRemoteLlm(
	...
	try {
		...
		const response = await withAuth(llmApiKey(), async key => {
			...
			if (res.status === 401) {
				throw new ProviderHttpError("mnemosyne remote LLM request unauthorized (401)", 401, { headers: res.headers });
			}
			return res;
		});
		if (!response.ok) {
