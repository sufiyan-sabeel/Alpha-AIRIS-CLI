...
import { getMnemosyneRuntimeOptions, mnemosyneDebugEnabled, withMnemosyneRuntimeOptions } from "../runtime-options";
...
async function runEmbedding(beam: BeamMemoryState, items: readonly EmbedItem[]): Promise<void> {
	try {
		...
		logger[mnemosyneDebugEnabled() ? "warn" : "debug"]("mnemosyne: background embedding failed", {
 ...
 * set by `Mnemosyne.#withRuntimeOptions` has already exited by the time the task runs.
 */
export function scheduleEmbedding(beam: BeamMemoryState, items: readonly EmbedItem[]): void {
	...
	const task = withMnemosyneRuntimeOptions(runtimeOptions, () => runEmbedding(beam, cleaned));
