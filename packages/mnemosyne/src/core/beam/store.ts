...
function proactiveLinkingAllowed(beam: BeamMemoryState): boolean {
	const override = process.env.MNEMOSYNE_PROACTIVE_LINKING;
 ...
 * `MNEMOSYNE_NO_EMBEDDINGS` env, or an unresolved (empty) active model — so a
...
export function reconcileEmbeddingModel(beam: BeamMemoryState): void {
	...
	if (mismatch) {
		...
		logger.info("mnemosyne: embedding model changed, rebuilding", {
	...
	logger.info("mnemosyne: resuming interrupted embedding rebuild", { to: active, count: missing.length });
	rebuild(missing);
}
