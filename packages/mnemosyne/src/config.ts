...
export function embeddingApiUrl(env: Env = process.env): string {
	return envString("MNEMOSYNE_EMBEDDING_API_URL", envString("OPENROUTER_BASE_URL", DEFAULT_EMBEDDING_API_URL, env), env);
}
...
export function embeddingMaxInputChars(env: Env = process.env): number {
	return Math.max(0, envInt("MNEMOSYNE_EMBEDDING_MAX_INPUT_CHARS", 8192, env));
}
...
export function maxEpisodeChars(env: Env = process.env): number {
	return Math.max(1, envInt("MNEMOSYNE_MAX_EPISODE_CHARS", 100000, env));
}
 ...
 * (e.g. the coding-agent `mnemosyne.polyphonicRecall` / `mnemosyne.enhancedRecall` /
 * `mnemosyne.proactiveLinking` settings) lands here; the `MNEMOSYNE_POLYPHONIC_RECALL` /
 * `MNEMOSYNE_ENHANCED_RECALL` / `MNEMOSYNE_PROACTIVE_LINKING` environment variables still
...
export function polyphonicRecallEnabled(env: Env = process.env): boolean {
	const value = envOptionalString("MNEMOSYNE_POLYPHONIC_RECALL", env);
...
export function enhancedRecallEnabled(env: Env = process.env): boolean {
	const value = envOptionalString("MNEMOSYNE_ENHANCED_RECALL", env);
	return value === undefined ? enhancedRecallDefault : value === "1";
}

export function proactiveLinkingEnabled(env: Env = process.env): boolean {
	const value = envOptionalString("MNEMOSYNE_PROACTIVE_LINKING", env);
