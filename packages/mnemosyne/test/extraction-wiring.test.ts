...
import type { MnemosyneLlmCompletion } from "@airis/airis-mnemosyne/core/runtime-options";
...
describe("remember(extract) wires the LLM fact extractor", () => {
		...
		const memory = makeMemory({
			complete: () => {
				...
				return JSON.stringify({
					...
					kg: [{ subject: "Mnemosyne", predicate: "uses", object: "SQLite" }],
				});
			},
		});
	...
	it("uses a runtime-configured remote LLM in background extraction", async () => {
		const previousEnabled = process.env.MNEMOSYNE_LLM_ENABLED;
		const previousBaseUrl = process.env.MNEMOSYNE_LLM_BASE_URL;
		let requestedUrl = "";
		let requestedBody = "";
		...
		try {
			...
			delete process.env.MNEMOSYNE_LLM_BASE_URL;
			const memory = new Mnemosyne({
			...
			if (previousEnabled === undefined) delete process.env.MNEMOSYNE_LLM_ENABLED;
			else process.env.MNEMOSYNE_LLM_ENABLED = previousEnabled;
			if (previousBaseUrl === undefined) delete process.env.MNEMOSYNE_LLM_BASE_URL;
			else process.env.MNEMOSYNE_LLM_BASE_URL = previousBaseUrl;
		}
	});
