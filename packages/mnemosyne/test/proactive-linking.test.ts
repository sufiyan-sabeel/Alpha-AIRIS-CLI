...
import "./setup";
import { configureRecallFeatures } from "@airis/airis-mnemosyne/config";
import { BeamMemory } from "@airis/airis-mnemosyne/core/beam";
import type { EpisodicGraph, RelatedMemory } from "@airis/airis-mnemosyne/core/episodic-graph";
...
describe("proactive memory linking", () => {
	...
	it("keeps host configuration scoped to each Mnemosyne instance", () => {
		delete process.env.MNEMOSYNE_PROACTIVE_LINKING;
		const enabled = new Mnemosyne({
			sessionId: "proactive-mnemosyne-on",
			dbPath: ":memory:",
			proactiveLinking: true,
		});
		configureRecallFeatures({ proactiveLinking: false });
		const disabled = new Mnemosyne({
			sessionId: "proactive-mnemosyne-off",
			dbPath: ":memory:",
			proactiveLinking: false,
		});
		try {
	...
	it("lets the environment variable override instance configuration", () => {
		process.env.MNEMOSYNE_PROACTIVE_LINKING = "0";
