...
import { factRecall, formatContext, recall, recallEnhanced } from "@airis/airis-mnemosyne/core/beam/recall";
import { initBeam } from "@airis/airis-mnemosyne/core/beam/schema";
import type { BeamMemoryState } from "@airis/airis-mnemosyne/core/beam/types";
...
describe("beam recall free functions", () => {
	...
	it("keeps exact working-memory hits above weak matching facts in enhanced recall", async () => {
		...
		insertWorking(
			beam,
			"wm-quasar",
			"MNEMOSYNE FULL PIPELINE TEST 20260613: The user Verge prefers AIRIS memory to run at full power. Unique entity QuasarOtter owns SignalPineapple and uses RecallEngine-Seven.",
		);
		beam.db.run(
			"INSERT INTO facts (fact_id, session_id, subject, predicate, object, timestamp, confidence) VALUES (?, ?, ?, ?, ?, ?, ?)",
			[
				...
				"Instruction",
				"states",
				"always trigger the user stop sequence in mnemosyne's callRemoteLlm test",
		...
		const results = await recallEnhanced(
			beam,
			"QuasarOtter SignalPineapple RecallEngine-Seven MNEMOSYNE FULL PIPELINE TEST 20260613",
			3,
