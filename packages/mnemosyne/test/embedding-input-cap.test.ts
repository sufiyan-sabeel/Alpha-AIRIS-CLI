...
/**
 * Regression coverage for issue #3126: `MnemosyneSessionState.retainMessages`
 ...
 * input to `MNEMOSYNE_EMBEDDING_MAX_INPUT_CHARS` (default 8192) before the
 * provider sees it.
 */
...
const ENV_KEY = "MNEMOSYNE_EMBEDDING_MAX_INPUT_CHARS";
...
describe("embed() input cap (#3126)", () => {
	...
	it("honors MNEMOSYNE_EMBEDDING_MAX_INPUT_CHARS env override", async () => {
		const provider = captureProvider();
		setEmbeddingProviderForTests(provider);
	...
	it("respects a constructor-scoped maxInputChars override", async () => {
		...
		await withEnvValue(undefined, () =>
			withMnemosyneRuntimeOptions({ embeddings: { maxInputChars: 256 } }, () => embed(["w".repeat(10_000)])),
		);
	...
	it("preserves both ends of a chronological transcript via the head/tail clip", async () => {
		...
		// `MnemosyneSessionState.retainMessages` hands `embed()` the full
