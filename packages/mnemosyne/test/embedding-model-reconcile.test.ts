 ...
 * the `Mnemosyne` constructor) wipes every stale vector and enqueues all live
...
import "./setup";
import { initBeam } from "@airis/airis-mnemosyne/core/beam";
...
function countEmbeddings(memory: Mnemosyne): number {
...
describe("reconcileEmbeddingModel on store open", () => {
	it("wipes stale embeddings + binary vectors and re-embeds when the model changed", async () => {
		...
		const memory = new Mnemosyne({ db, embeddings: { model: NEW_MODEL, provider: fakeEmbed() } });
		try {
	...
	it("does not wipe when embeddings are disabled via the MNEMOSYNE_NO_EMBEDDINGS env", () => {
		const { db } = seedDb(OLD_MODEL);
		const previous = process.env.MNEMOSYNE_NO_EMBEDDINGS;
		process.env.MNEMOSYNE_NO_EMBEDDINGS = "1";
		let memory: Mnemosyne | undefined;
		try {
			...
			memory = new Mnemosyne({ db, embeddings: { model: NEW_MODEL, provider: fakeEmbed() } });
			expect(countEmbeddings(memory)).toBe(2);
			expect(memory.beam.pendingExtractions.size).toBe(0);
			const ep = memory.conn.query("SELECT binary_vector AS v FROM episodic_memory WHERE id = 'ep-1'").get() as {
				v: Uint8Array | null;
			...
			if (previous === undefined) {
				...
				process.env.MNEMOSYNE_NO_EMBEDDINGS = previous;
			}
			memory?.close();
			db.close();
		}
	...
	it("does not wipe when the active embedding model is empty", () => {
		...
		try {
			...
			memory = new Mnemosyne({ db, embeddings: { model: "", provider: fakeEmbed() } });
			expect(countEmbeddings(memory)).toBe(2);
			expect(memory.beam.pendingExtractions.size).toBe(0);
		} finally {
			memory?.close();
	...
	it("does not reconcile a read-only open (reconcile: false), even on a model change", () => {
		...
		try {
			...
			memory = new Mnemosyne({ db, embeddings: { model: NEW_MODEL, provider: fakeEmbed() }, reconcile: false });
			expect(countEmbeddings(memory)).toBe(2);
			expect(memory.beam.pendingExtractions.size).toBe(0);
			const ep = memory.conn.query("SELECT binary_vector AS v FROM episodic_memory WHERE id = 'ep-1'").get() as {
				v: Uint8Array | null;
