/**
 * Regression for https://github.com/sufiyan-sabeel/Alpha-AIRIS-CLI/issues/1832
 *
 * Before the fix:
...
import "./setup";
import { cmdRemember } from "@airis/airis-mnemosyne/cli";
import { BeamMemory } from "@airis/airis-mnemosyne/core/beam";
import { Mnemosyne } from "@airis/airis-mnemosyne/core/memory";
import {
	type ResolvedMnemosyneRuntimeOptions,
	withMnemosyneRuntimeOptions,
} from "@airis/airis-mnemosyne/core/runtime-options";
...
function withFakeMemory<T>(fn: (memory: Mnemosyne, calls: () => number) => Promise<T>): Promise<T> {
...
/**
 * Re-enter the per-Mnemosyne runtime-options scope when reaching into `memory.beam`
 * directly (only `Mnemosyne.remember`/`recall`/etc. enter it automatically).
 */
function inScope<T>(memory: Mnemosyne, fn: () => T): T {
	return withMnemosyneRuntimeOptions(memory.runtimeOptions, fn);
}

function readEmbeddings(memory: Mnemosyne): EmbeddingRow[] {
	return memory.conn
...
describe("issue #1832 — embedding write/read coverage", () => {
	...
	it("flushes pending embeddings before close so short-lived `mnemosyne store` owners persist them", async () => {
		...
		const { provider, calls } = fakeProvider();
		const dbPath = `${tmpdir()}/mnemosyne-1833-${randomBytes(6).toString("hex")}.db`;
		const captured: string[] = [];
		const stdout = { write: (s: string) => captured.push(s) };
		const stderr = { write: (s: string) => captured.push(s) };
		const runtimeOptions: ResolvedMnemosyneRuntimeOptions = {
			embeddings: { provider },
		};

		await withMnemosyneRuntimeOptions(runtimeOptions, async () => {
	...
	it("scopes embedQuery() cache per provider so two Mnemosyne runtimes do not cross-contaminate", async () => {
		// Repro for #1833 review comment: `embedQuery()` caches by query text only, so a
		// second `Mnemosyne` in the same process with a different provider would read back
		// the first runtime's vector and score against it. Each Mnemosyne here exposes a
		...
		const alpha = new Mnemosyne({
			db: new Database(":memory:"),
			embeddings: { provider: alphaProvider },
		});
		const beta = new Mnemosyne({
			db: new Database(":memory:"),
