/**
 * `<airis-tool-view>` — web-component wrapper around ToolView for non-React
 * hosts (the exported-session HTML page).
 *
 * Payload sources, in priority order:
 * 1. `el.data = {...}` property assignment.
 * 2. `data-key` attribute → lookup in `globalThis.__AIRIS_TOOL_VIEW_DATA`
...
export class OmpToolViewElement extends HTMLElement {
	...
	#resolveProps(): ToolViewProps | null {
		...
		if (key) {
			const store = (globalThis as { __AIRIS_TOOL_VIEW_DATA?: PayloadStore }).__AIRIS_TOOL_VIEW_DATA;
...
export function defineToolViewElement(tag = "airis-tool-view"): void {
