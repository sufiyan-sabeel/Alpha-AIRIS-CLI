...
import { AuthStorage, REMOTE_REFRESH_SENTINEL, SqliteAuthCredentialStore } from "@airis/airis-ai";
import {
	AuthBrokerClient,
...
describe("RemoteAuthCredentialStore SSE integration", () => {
	...
	test("loads the account pool once for broker-backed discovery", async () => {
		...
		await withEnv(
			{
				AIRIS_AUTH_BROKER_URL: handle!.url,
				AIRIS_AUTH_BROKER_TOKEN: token,
		...
		await withEnv(
			{
				AIRIS_AUTH_BROKER_URL: handle!.url,
				AIRIS_AUTH_BROKER_TOKEN: token,
				AIRIS_AUTH_BROKER_ACCOUNT_POOL_FILE: path.join(tempDir, "missing-account-pool.json"),
			},
			async () => {
				const discovered = await discoverAuthStorage({
					agentDir: tempDir,
