 ...
 *
 * Concurrent airis startups can race against WAL recovery so the auth-store init
...
import { isSqliteBusyError, SqliteAuthCredentialStore } from "@airis/airis-ai/auth-storage";
...
describe("SqliteAuthCredentialStore.open SQLITE_BUSY handling", () => {
	...
	beforeEach(async () => {
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-ai-sqlite-busy-"));
	});
