...
import { __resetVertexTokenCache } from "@airis/airis-ai/providers/google-auth";
import { complete, getEnvApiKey, stream } from "@airis/airis-ai/stream";
import type { Api, Context, ImageContent, Model, OptionsForApi, Tool, ToolResultMessage } from "@airis/airis-ai/types";
...
describe("Generate E2E Tests", () => {
	...
	describe("google-vertex env auth", () => {
		...
		it("routes impersonated_service_account ADC through IAM to the Vertex request", async () => {
			...
			const originalApiKey = Bun.env.GOOGLE_CLOUD_API_KEY;
			const originalGac = Bun.env.GOOGLE_APPLICATION_CREDENTIALS;
			const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "airis-vertex-impersonation-"));
