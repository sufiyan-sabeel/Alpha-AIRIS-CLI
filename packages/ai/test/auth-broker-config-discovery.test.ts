...
import { discoverAuthStorage, resolveAuthBrokerConfig } from "@airis/airis-ai/auth-broker";
...
const SUPPRESS_AUTH_BROKER_ENV = {
	...
	AIRIS_AUTH_BROKER_ACCOUNT_POOL_FILE: undefined,
} as const;
...
describe("resolveAuthBrokerConfig config discovery", () => {
	...
	test("rejects unreadable or malformed account-pool files before connecting", async () => {
		...
		const brokerEnv = {
			...
			AIRIS_AUTH_BROKER_URL: "http://127.0.0.1:1",
			AIRIS_AUTH_BROKER_TOKEN: "test-token",
			AIRIS_AUTH_BROKER_ACCOUNT_POOL_FILE: poolPath,
		} as const;

		await withEnv(brokerEnv, async () => {
			await expect(discoverAuthStorage({ agentDir })).rejects.toThrow(
				"Unable to read AIRIS_AUTH_BROKER_ACCOUNT_POOL_FILE",
			);
