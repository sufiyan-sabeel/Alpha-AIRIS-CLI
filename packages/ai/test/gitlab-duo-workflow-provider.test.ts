...
import { isContextOverflow } from "@airis/airis-ai/error";
import {
...
} from "@airis/airis-ai/providers/gitlab-duo-workflow";
...
describe("GitLab Duo Workflow provider protocol", () => {
	...
	it("advertises AIRIS tools under their bare names with the official GitLab MCP schema", () => {
		...
		// bare name AIRIS's own tool docs use.
	...
	it("puts the AIRIS system prompt in the inline flow system slot with reasoning events", () => {
		const systemContext: Context = {
			systemPrompt: ["AIRIS authoritative operating rules. Bridge the local tools."],
		...
		// The system slot carries AIRIS's real system prompt verbatim — no gateway preamble.
		expect(prompt?.prompt_template.system).toContain("AIRIS authoritative operating rules.");
	...
	it("builds startRequest goal as a bare ChatML transcript with tool-run linkage", () => {
		...
		const replayContext: Context = {
			systemPrompt: [`AIRIS system instructions: preserve the local tool bridge. token ${patToken}`],
			messages: [
				{
					role: "user",
		...
		// no <instructions>. The AIRIS system prompt rides the flow config's system slot.
		...
		expect(payload.goal).not.toContain("AIRIS system instructions: preserve the local tool bridge.");
		...
		// rides the very next turn), so the AIRIS-internal call id is omitted from the
		...
		// The AIRIS system prompt lives in the flow config system slot, not the goal.
		const flowPrompt = payload.flowConfig?.prompts[0];
		expect(flowPrompt?.prompt_template.system).toContain("AIRIS system instructions: preserve the local tool bridge.");
	...
	it("strips the AIRIS-internal intent (i) field from replayed tool-call args", () => {
		const replayContext: Context = {
			systemPrompt: ["system"],
			messages: [
...
describe("GitLab Duo Workflow WebSocket state machine", () => {
			...
			channel_values: {
				ui_chat_log: [
					{ message_type: "user", content: "Question" },
					{
						...
						tool_info: { name: "mcp__airis__read", args: { path: "src/index.ts" } },
					},
			...
			channel_values: {
				ui_chat_log: [
					...
					{ message_type: "agent", content: "Found the target. Reading it now." },
					{
						...
						tool_info: { name: "mcp__airis__read", args: { path: "README.md" } },
					},
					{
						message_type: "tool",
						content: "README text",
	...
	it("emits standard tool calls instead of executing GitLab actions in the provider", async () => {
		...
		socket.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					requestID: "req-mcp-1",
					runMCPTool: { name: "mcp__airis__read", args: JSON.stringify({ path: "src/index.ts" }) },
				}),
			}),
		);
	...
	it("finalizes one assistant message per tool-call action (serial MCP dispatch)", async () => {
		...
		socket.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					requestID: "req-a",
					runMCPTool: { name: "mcp__airis__read", args: JSON.stringify({ path: "a.ts" }) },
				}),
			}),
		);
		...
		socket?.onmessage?.(
			new MessageEvent("message", {
				data: JSON.stringify({
					...
					runMCPTool: { name: "mcp__airis__read", args: JSON.stringify({ path: "README.md" }) },
				}),
			}),
		);
	...
	it("re-seeds a fresh workflow goal with the entire conversation history including prior tool results", async () => {
		...
		// by its tool-result turn); the AIRIS-internal id is omitted from the transcript.
