 ...
 * with the gateway live (`airis auth-gateway serve` or pm2).
 */
...
const MODEL = Bun.env.AIRIS_E2E_OPENAI_RESPONSES_MODEL ?? "gpt-5.3-codex";
...
the airis auth-gateway's OpenAI Responses prompt-caching pipeline. The same
...
describe.skipIf(!gateway.ok)("auth-gateway: openai-responses prompt caching e2e", () => {
	...
	it("automatically caches the instructions prefix across two turns", async () => {
		...
		const cacheKey = `airis-e2e-${nonce}`;
