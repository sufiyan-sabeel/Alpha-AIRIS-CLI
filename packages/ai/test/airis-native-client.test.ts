...
import { streamairisNative } from "@airis/airis-ai/providers/airis-native-client";
import type {
	AssistantMessage,
...
describe("streamairisNative request shape", () => {
	it("POSTs `{modelId, context, options, stream:true}` to `<baseUrl>/v1/airis/stream`", async () => {
		...
		const stream = streamairisNative(fakeModel(), baseContext, {
			apiKey: "gw-bearer",
			fetch: fetchImpl,
			temperature: 0.7,
		});
		...
		expect(captured.url).toBe("http://llm-gateway.internal:4000/v1/airis/stream");
	...
	it("normalizes trailing slashes on `baseUrl` so the endpoint never double-slashes", async () => {
		...
		await streamairisNative(fakeModel({ baseUrl: "http://llm-gateway.internal:4000///" }), baseContext, {
			apiKey: "k",
			fetch: fetchImpl,
		}).result();
		expect(captured.url).toBe("http://llm-gateway.internal:4000/v1/airis/stream");
	...
	it("forwards `model.headers` and lets a caller-supplied Authorization win", async () => {
		...
		await streamairisNative(
			fakeModel({ headers: { "x-airis-slot": "roboairis-1", Authorization: "Bearer model-wins" } }),
			baseContext,
			{ apiKey: "options-loses", fetch: fetchImpl },
		).result();

		const headers = captured.init?.headers as Record<string, string>;
		expect(headers["x-airis-slot"]).toBe("roboairis-1");
	...
	it("throws synchronously when `baseUrl` is missing", async () => {
		...
		const stream = streamairisNative(broken, baseContext, { apiKey: "k" });
		await expect(stream.result()).rejects.toThrow(/baseUrl/);
	});
});

describe("streamairisNative event flow", () => {
	it("pushes parsed events verbatim and resolves `.result()` on terminal `done`", async () => {
		...
		const stream = streamairisNative(fakeModel(), baseContext, { apiKey: "k", fetch: fetchImpl });
	...
	it("uses PI_STREAM_FIRST_EVENT_TIMEOUT_MS for silent airis-native streams", async () => {
	...
	it("rejects when a airis-native stream stalls after semantic progress", async () => {
		const partial = baseAssistant({ content: [{ type: "text", text: "hi" }] });
		const chunks = [
	...
	it("does not time out a healthy airis-native stream that keeps making semantic progress", async () => {
