...
import { encodeStream, formatError, parseRequest } from "@airis/airis-ai/providers/airis-native-server";
import type {
	AssistantMessage,
...
describe("airis-native parseRequest", () => {
...
describe("airis-native gateway cache controls", () => {
	it("delivers statefulResponses false to the provider stream", async () => {
		registerMockApi();
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "gw-airis-native-cache-"));
		const storage = await AuthStorage.create(path.join(dir, "auth.db"));
		storage.setRuntimeApiKey("openrouter", "test-key");
		const mock = createMockModel({ provider: "openrouter", id: "airis-native-cache" });
		...
		try {
			...
			const response = await fetch(`${handle.url}/v1/airis/stream`, {
				method: "POST",
				headers: { Authorization: "Bearer test-token", "Content-Type": "application/json" },
				body: JSON.stringify({
					modelId: "airis-native-cache",
					context: baseContext,
					options: { promptCacheKey: "bench-cache-pair", statefulResponses: false },
					stream: false,
				}),
...
describe("airis-native encodeStream", () => {
	it("ships every AssistantMessageEvent verbatim, terminated by [DONE]", async () => {
		// Pi-native is airis-talks-to-airis: the client feeds parsed events directly
...
describe("airis-native formatError", () => {
