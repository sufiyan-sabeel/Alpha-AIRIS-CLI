...
```bash
npm install @airis/airis-ai
```

## Quick Start

```typescript
import { z, getModel, stream, complete, Context, Tool } from "@airis/airis-ai";
...
```typescript
import { z, Tool } from "@airis/airis-ai";
...
```typescript
import { stream, validateToolCall, Tool } from "@airis/airis-ai";
...
```typescript
import * as fs from "node:fs";
import { getModel, complete } from "@airis/airis-ai";
...
```typescript
import { getModel, streamSimple, completeSimple } from "@airis/airis-ai";
...
```typescript
import { getModel, stream } from "@airis/airis-ai";
...
```typescript
import { getProviders, getModels, getModel } from "@airis/airis-ai";
...
```typescript
import { Model, stream } from "@airis/airis-ai";
...
```typescript
import { getModel, complete, Context } from "@airis/airis-ai";
...
```typescript
import { Context, getModel, complete } from "@airis/airis-ai";
...
bundles. Bun's `fetch` does not consume the env var natively, so airis injects
...
```typescript
import { getEnvApiKey } from "@airis/airis-ai";
...
Authenticate via the [`airis`](https://airis.sh) coding-agent CLI, which drives this library's OAuth/API-key flows in-process and persists into `agent.db`:

```bash
airis auth-broker login              # interactive provider selection
airis auth-broker login anthropic    # login to a specific provider
airis auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
airis auth-broker list               # list supported providers
airis auth-broker logout             # interactive — pick a stored credential to remove
```
...
```typescript
import { loginGitHubCopilot } from "@airis/airis-ai";
import * as fs from "node:fs";
...
```typescript
import { getModel, complete, getOAuthApiKey } from "@airis/airis-ai";
import * as fs from "node:fs";
