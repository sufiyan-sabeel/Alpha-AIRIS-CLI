...
/**
 * Anthropic Messages API (https://docs.anthropic.com/en/api/messages) ↔ airis-ai
 * gateway translation. Inbound: foreign HTTP body → airis Context. Outbound:
 * airis AssistantMessage[Stream] → Anthropic-shaped JSON / SSE.
 */
...
// airis-ai's `ImageContent` only carries base64 + mimeType. When the inbound
...
function warnNonBase64ImageSource(sourceType: string): void {
	...
	logger.warn("anthropic-messages: image source surfaced as text placeholder (airis-ai ImageContent lacks URL channel)", {
		sourceType,
	});
}
...
function walkUserContent(
	...
	for (const block of blocks) {
		if (block.type === "text") {
			...
			// siblings in the same user message. airis-ai's history is a flat
		...
		switch (block.type) {
			...
			case "web_search_tool_result":
				if (isAnthropicWebSearchHistoryBlock(block)) {
					...
					// Other server tools use distinct result block types that airis
 ...
 * `cache_control: { type: "ephemeral", ttl?: "1h"|"5m" }`. airis-ai's
...
export function parseRequest(body: unknown, headers?: Headers): ParsedRequest {
	...
	// emit at most one tool call per turn; map to airis-ai's negated boolean.
...
export function encodeResponse(message: AssistantMessage, requestedModelId: string): Record<string, unknown> {
	...
	return {
		...
		// TODO: surface the matched stop sequence once airis-ai's
						...
						case "done": {
							...
							controller.enqueue(
								sseFrame("message_delta", {
									type: "message_delta",
									// TODO: surface matched stop sequence once airis-ai
