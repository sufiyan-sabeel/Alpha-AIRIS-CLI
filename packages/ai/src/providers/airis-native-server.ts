/**
 * Airis-native wire format for the auth-gateway.
 *
 * Where the OpenAI / Anthropic / Responses route modules translate foreign
 * wire shapes through airis-ai's canonical {@link Context}, this module accepts
 * the canonical shape *directly* — for clients that already speak airis-ai
 * (containerized airis, the swarm extension, roboairis's sidecar auth-gateway).
 ...
 * translations impose on first-class airis-ai fields (service tier, cache
 * markers, thinking budgets, tool-choice variants, …).
 *
 * The streaming wire is {@link AssistantMessageEvent} serialized verbatim and
 * SSE-framed. Same type airis-ai already produces internally; the client feeds
...
export interface airisNativeParsedRequest {
	modelId: string;
	context: Context;
 ...
 * `SimpleStreamOptions` from older / newer airis builds without per-version
 * conditionals.
 */
...
/**
 * Parse a airis-native request body. Validation is intentionally minimal — only
 * the shape the gateway itself reads is checked (`modelId`, `context.messages`
 * array, options is an object). Everything downstream is the canonical airis-ai
 ...
 */
export function parseRequest(body: unknown, _headers?: Headers): airisNativeParsedRequest {
	...
	// `stream` defaults to true — airis-native clients overwhelmingly stream, and
 ...
 *
 * No per-event re-shaping: the airis-native client is airis-ai itself, so the
 ...
 * targets (containerized airis → host gateway, roboairis slot → airis-auth-gateway
...
/**
 * Airis-native error envelope:
 *   `{ error: { type, message } }`
 *
 * Mirrors OpenAI's outer shape (which clients/SDKs already parse) without the
 * provider-specific status taxonomy — airis-native callers consume `type`
 * directly.
 */
