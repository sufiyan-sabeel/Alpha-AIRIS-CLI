 ...
 * airis-agent-core / streamSimple-shaped consumers without an HTTP client.
 *
 * Usage:
 *
...
export class MockModel implements Model<MockApi> {
	...
	/** A streamFn-compatible callable. Forward to `agentLoop` or airs `stream()`. */
...
/** Stream function for `Model<"mock">`. Matches the airis-ai per-provider stream signature. */
...
export function registerMockApi(sourceId = "airis-ai/mock"): void {
