/**
 * Client half of the airis-native auth-gateway protocol.
 *
 * Dispatches a {@link streamSimple}-shaped request to an `airis auth-gateway`
 * via `POST /v1/airis/stream`, reads the SSE event stream back, and pushes the
 ...
 * way.
 *
 * Activated when a {@link Model} has `transport: "airis-native"` set; the
 * dispatch hook lives in `streamSimple()` (see `../stream.ts`). Used by
 * containerized airis deployments (roboairis slots, the swarm extension) that
...
const PI_NATIVE_STREAM_IDLE_TIMEOUT_ERROR = "airis-native stream stalled while waiting for the next event";
const PI_NATIVE_STREAM_FIRST_EVENT_TIMEOUT_ERROR = "airis-native stream timed out while waiting for the first event";

function isairisNativeProgressEvent(event: unknown): boolean {
...
/**
 * Resolve the `/v1/airis/stream` endpoint URL from the model's `baseUrl`.
 * Trims a trailing slash so concatenation can't double-slash; throws when
 * the baseUrl is missing (transport=airis-native without a gateway target is
 * a configuration error, not a runtime recoverable one).
 */
function resolveStreamUrl(model: Model<Api>): string {
	if (!model.baseUrl) {
		throw new AIError.ConfigurationError(
			`airis-native transport requires \`baseUrl\` on model ${model.id} (set it on the provider config in models.yml)`,
		);
	}
	return `${model.baseUrl.replace(/\/+$/, "")}/v1/airis/stream`;
}
...
/**
 * Stream a turn through an `airis auth-gateway` over the airis-native protocol.
 *
 ...
 * non-streaming path.
 */
export function streamairisNative<TApi extends Api>(
	model: Model<TApi>,
	context: Context,
	...
	void (async () => {
		...
		try {
			...
			const watchedSource = iterateWithIdleTimeout(source, {
				...
				isProgressItem: isairisNativeProgressEvent,
			});
			...
			if (!sawTerminal) {
				...
				// `airis-native-server.encodeStream`.
