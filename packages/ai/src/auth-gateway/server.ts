/**
 * airis auth-gateway HTTP server.
 *
 * Accepts any provider-format request (OpenAI chat-completions, Anthropic
 * messages, OpenAI Responses) and dispatches through airis-ai's `streamSimple()`
 ...
 * pure protocol translation: foreign wire → airis Context → airis-ai stream() →
 * airis events → foreign wire.
 *
 * Endpoints:
...
import * as airisNative from "../providers/airis-native-server";
...
export interface AuthGatewayBootOptions extends AuthGatewayServerOptions {
	...
	storage: AuthStorage;
	/**
	 * Resolve a client-requested model id to a airis-ai Model. Caller supplies
	 * this from a ModelRegistry (lives in `coding-agent` to avoid an inverse
	 * dependency in `airis-ai`).
	 */
...
// (passthrough fast-path removed — it bypassed airis-ai provider logic, in
...
function deriveSessionId(modelId: string, context: Context): string {
	...
	if (first) {
		...
		// of the same conversation (airis re-stamps every parsed Message). role +
...
function buildStreamOptions(parsed: ParsedFormatRequest, api: Api, signal: AbortSignal): SimpleStreamOptions {
	...
	// Fields that don't yet have a matching airis-ai `SimpleStreamOptions` slot.
	...
	// TODO(airis-ai): land first-class fields and replace these blocks.
	if (
...
async function handleFormatEndpoint(
	...
	// airis-ai's stream() does NOT consult AuthStorage — the caller (us) is
...
/**
 * Airis-native fast path: `POST /v1/airis/stream`. Accepts the canonical airis-ai
 * `Context` directly (no wire-format round-trip) and emits a bandwidth-shrunk
 * event stream matching `airis-agent`'s `streamProxy`. Skips the OpenAI /
 * Anthropic / Responses translation layers — those exist to bridge foreign
 * SDKs (llm-git, anthropic-sdk, openai-sdk), and bridging back to airis-native
 ...
 * path.
 */
async function handleairisNative(bootOpts: AuthGatewayBootOptions, req: Request, peer: string): Promise<Response> {
	const startedAt = performance.now();
	const requestId = crypto.randomUUID();
	const controller = mirrorRequestAbort(req);
	const aborted = (): Response => airisNative.formatError(499, "request_aborted", "client closed request");
	...
	try {
		...
		if (controller.signal.aborted) return aborted();
		return airisNative.formatError(400, "invalid_request_error", `Invalid JSON body: ${String(error)}`);
	}
	if (controller.signal.aborted) return aborted();

	let parsed: airisNative.airisNativeParsedRequest;
	try {
		parsed = airisNative.parseRequest(body, req.headers);
	} catch (error) {
		if (controller.signal.aborted) return aborted();
		const message = error instanceof Error ? error.message : String(error);
		return airisNative.formatError(400, "invalid_request_error", message);
	}

	const model = bootOpts.resolveModel(parsed.modelId);
	if (!model) {
		return airisNative.formatError(404, "invalid_request_error", `Unknown model: ${parsed.modelId}`);
	}
	// Airis-native already parsed `streamOpts.sessionId` (when set by the
	...
	try {
		...
		const classified = classifyGatewayError(error);
		logger.warn("auth-gateway getApiKey threw", { provider: model.provider, peer, error: classified.message });
		return airisNative.formatError(classified.status, classified.type, classified.message);
	}
	if (controller.signal.aborted) return aborted();
	if (!apiKey) {
		return airisNative.formatError(
			401,
			"authentication_error",
			`No credential available for provider ${model.provider}`,
		);
	...
	if (!parsed.stream) {
		try {
			...
			if (message.stopReason === "aborted" || message.stopReason === "error") {
				...
				if (message.stopReason === "aborted") {
					return airisNative.formatError(499, "request_aborted", errorMessage);
				}
				const classified = classifyGatewayError(errorMessage);
				return airisNative.formatError(classified.status, classified.type, errorMessage);
			}
			return json(200, { message }, gatewayResponseHeaders(model, { requestId, message, startedAt }));
		} catch (error) {
			if (controller.signal.aborted) return aborted();
			const classified = classifyGatewayError(error);
			logger.warn("auth-gateway non-streaming aborted", { format: "airis-native", error: classified.message, peer });
			return airisNative.formatError(classified.status, classified.type, classified.message);
		}
	}
	...
	try {
		...
		const classified = classifyGatewayError(error);
		logger.warn("auth-gateway streamSimple threw", { format: "airis-native", error: classified.message, peer });
		return airisNative.formatError(classified.status, classified.type, classified.message);
	}
	if (controller.signal.aborted) return aborted();

	const sseStream = airisNative.encodeStream(events, parsed.modelId, parsed.options, {
		signal: controller.signal,
		onCancel: reason => {
			if (!controller.signal.aborted) {
				controller.abort(reason instanceof Error ? reason : new Error("client closed request"));
	...
	const server = Bun.serve({
		...
		fetch: async (req): Promise<Response> => {
			...
			try {
				...
				// Airis-native fast path. Same auth + provider plumbing as the
				// foreign-wire routes, just without the wire-format translation.
				if (req.method === "POST" && pathname === "/v1/airis/stream") {
					return withCors(await handleairisNative(opts, req, peer), req);
				}
