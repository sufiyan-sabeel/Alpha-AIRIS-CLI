...
import { discoverAuthStorage } from "@airis/airis-ai/auth-broker/discover";
import type { OAuthAccess } from "@airis/airis-ai/auth-storage";
import type { OAuthProvider } from "@airis/airis-ai/oauth/types";
import { getGitLabDuoModels } from "@airis/airis-ai/providers/gitlab-duo";
...
async function fetchAntigravityModels(): Promise<ModelSpec<"google-gemini-cli">[]> {
	...
	if (!access) {
		...
		console.log("Tip: If you are logged in under a specific profile, run with AIRIS_PROFILE=<name>.");
		return [];
	}
	try {
