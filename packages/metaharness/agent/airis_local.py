"""Harbor agent that runs the LOCAL alpha-airis-cli (`airis`) build inside task containers.

Unlike Harbor's built-in `pi` agent (which `npm i -g @mariozechner/pi-coding-agent`),
this runs the working tree at `/work/airis`. Install modes (`AIRIS_BENCH_INSTALL`):

  * `source` (default): the runner bind-mounts the repo read-only plus a
    prebuilt linux `node_modules` tree and a linux `bun` binary; airis runs
  ...
  * binary (`--binary`): a self-contained compiled airis binary is uploaded.

Auth never enters the container: a generated `~/.airis/agent/models.yml` routes the
...
(see `AIRIS_BENCH_*` below); the agent reads them from `os.environ` directly.

Selected via `harbor run --agent-import-path airis_local:OmpLocal` with the
...
def _patch_harbor_cleanup_cancellation() -> None:
    ...
    if getattr(Trial, "_airis_cleanup_cancellation_patch", False):
        return
    ...
    Trial._airis_cleanup_cancellation_patch = True
...
def _patch_apple_container_dns() -> None:
    ...
    sets AIRIS_BENCH_CONTAINER_DNS for apple-container jobs; absent, no-op.
    """
    dns = os.environ.get("AIRIS_BENCH_CONTAINER_DNS")
    if not dns:
        return
    from harbor.environments.apple_container import AppleContainerEnvironment

    if getattr(AppleContainerEnvironment, "_airis_dns_patch", False):
        return
    ...
    AppleContainerEnvironment._airis_dns_patch = True
...
_TARBALL_DST = "/tmp/airis-local.tgz"
_MODELS_DST = "/tmp/airis-models.yml"
_CONFIG_DST = "/tmp/airis-config.yml"
_OUTPUT_FILENAME = "airis.txt"
...
class OmpLocal(BaseInstalledAgent):
    ...
    def __init__(self, *args, **kwargs) -> None:  # noqa: D401 - thin wrapper
        ...
        self._install_mode = _env("AIRIS_BENCH_INSTALL", "source")
        self._tarball = _env("AIRIS_BENCH_TARBALL")
        self._pkg_version = _env("AIRIS_BENCH_VERSION", "latest")
        self._models_yaml_path = _env("AIRIS_BENCH_MODELS_YAML")
        self._gateway_url = _env(
            "AIRIS_BENCH_GATEWAY_URL", "http://host.docker.internal:4000"
        )
        self._gateway_token = _env("AIRIS_BENCH_GATEWAY_TOKEN", "no-auth-dummy")
        self._gateway_providers = [
            p.strip()
            for p in _env(
                "AIRIS_BENCH_GATEWAY_PROVIDERS", "anthropic,openai-codex"
            ).split(",")
            if p.strip()
        ]
        self._thinking = _env("AIRIS_BENCH_THINKING")
        self._auto_approve = _truthy(_env("AIRIS_BENCH_AUTO_APPROVE", "1"))
        # Extra CLI args forwarded verbatim to the in-container airis invocation,
        # JSON-array-encoded by the runner (AIRIS_BENCH_AGENT_ARGS) so multi-word
        # values survive without a second layer of shell quoting.
        self._agent_args = self._parse_agent_args()
        self._bun_version = _env("AIRIS_BENCH_BUN_VERSION", "1.3.14")
        self._gateway_on = _env("AIRIS_BENCH_GATEWAY", "1") != "0"
        ...
        self._web_search = _truthy(_env("AIRIS_BENCH_WEB_SEARCH", "0"))
        # Extra env (PI_* dialect knobs, explicit --env) the runner forwards into
        # the in-container airis run, JSON-encoded in AIRIS_BENCH_FORWARD_ENV.
        self._forward_env = self._parse_forward_env()
        # Source-mount paths (defaults must match the runner's compose overlay).
        self._source_dir = _env("AIRIS_BENCH_SOURCE_DIR", "/opt/airis/src")
        self._source_bun = _env("AIRIS_BENCH_SOURCE_BUN", "/opt/airis/bin/bun")
        self._source_arch = _env("AIRIS_BENCH_SOURCE_ARCH")
        # Resolved during install(); reused by version + run commands.
        self._home = "/root"
        self._bun = "/root/.bun/bin/bun"
        self._cli = "/root/.airis-bench/app/dist/cli.js"
        self._binary_arm64 = _env("AIRIS_BENCH_BINARY_ARM64")
        self._binary_x64 = _env("AIRIS_BENCH_BINARY_X64")
    ...
    def _wrap(self, command: str) -> str:
        ...
        airis spawns Bun worker subprocesses at runtime, so `bun` must resolve on
    ...
    async def install(self, environment: BaseEnvironment) -> None:
        ...
        if self._binary:
            ...
            await self.exec_as_root(
                ...
                command=(
                    ...
                    "  echo 'ERROR: Alpine/musl base image; @airis/airis-natives ships no musl prebuilt' >&2; exit 3; "
        ...
        # 3) Auth + model config under $HOME/.airis/agent.
        if self._gateway_on:
    ...
    async def _install_source(self, environment: BaseEnvironment) -> str:
        """Verify the read-only repo + linux deps mounts and run airis from TS source.
        ...
        await self.exec_as_agent(
            ...
            command=(
                "set -e; "
                f"test -x {q(self._source_bun)} || {{ echo 'airis source mode: bun mount missing' >&2; exit 5; }}; "
                f"test -f {q(cli)} || {{ echo 'airis source mode: repo mount missing' >&2; exit 5; }}; "
                f"test -d {q(self._source_dir + '/node_modules/@alpha-airis-cli')} || "
                "{ echo 'airis source mode: linux deps mount missing' >&2; exit 5; }; "
    ...
    async def _install_local(self, environment: BaseEnvironment) -> str:
        if not self._tarball:
            raise RuntimeError(
                "AIRIS_BENCH_INSTALL=local requires AIRIS_BENCH_TARBALL (host tarball path)"
            )
        await environment.upload_file(self._tarball, _TARBALL_DST)
        app = f"{self._home}/.airis-bench/app"
        await self.exec_as_agent(
            environment,
            command=self._wrap(
                "set -e; "
                ...
                'echo "pinning native @airis/airis-natives-linux-$na@$ver"; '
                'bun add --production "@airis/airis-natives-linux-$na@$ver"'
            ),
            timeout_sec=900,
        )
    ...
    async def _install_binary(self, environment: BaseEnvironment) -> str:
        """Probe container arch, upload only the matching self-contained airis binary."""
        arch = (
            await self.exec_as_agent(environment, command="uname -m")
        ).stdout.strip()
        ...
        if not hostbin:
            raise RuntimeError(
                f"binary mode: no airis binary provided for container arch {arch}"
            )
        app_dir = f"{self._home}/.airis-bench"
        dst = f"{app_dir}/airis"
        staging = "/tmp/airis-bin"
        await self.exec_as_agent(
    ...
    async def _install_published(self, environment: BaseEnvironment) -> str:
        app = f"{self._home}/.airis-bench/app"
        spec = f"@airis/airis-coding-agent@{self._pkg_version}"
        await self.exec_as_agent(
            environment,
            command=self._wrap(
                "set -e; "
        ...
        return f"{app}/node_modules/@airis/airis-coding-agent/dist/cli.js"
    ...
    async def _write_models_yaml(self, environment: BaseEnvironment) -> None:
        if self._models_yaml_path and os.path.isfile(self._models_yaml_path):
            ...
            staged = _MODELS_DST
            heredoc = (
                f"cat > {_MODELS_DST} <<'AIRIS_MODELS_EOF'\n{content}\nAIRIS_MODELS_EOF"
            )
            await self.exec_as_agent(environment, command=heredoc)
        await self.exec_as_agent(
            environment,
            command=(
                f'mkdir -p "$HOME/.airis/agent"; '
                f'cp {shlex.quote(staged)} "$HOME/.airis/agent/models.yml"'
            ),
        )
    ...
    async def _write_config(self, environment: BaseEnvironment) -> None:
        """Write $HOME/.airis/agent/config.yml: the web_search toggle.
        ...
        content = "\n".join(lines)
        heredoc = f"cat > {_CONFIG_DST} <<'AIRIS_CONFIG_EOF'\n{content}\nAIRIS_CONFIG_EOF"
        await self.exec_as_agent(environment, command=heredoc)
        await self.exec_as_agent(
            environment,
            command=(
                f'mkdir -p "$HOME/.airis/agent"; '
                f'cp {shlex.quote(_CONFIG_DST)} "$HOME/.airis/agent/config.yml"'
            ),
        )

    @staticmethod
    def _parse_forward_env() -> dict[str, str]:
        """Extra run-time env from the runner (AIRIS_BENCH_FORWARD_ENV = JSON object)."""
        raw = _env("AIRIS_BENCH_FORWARD_ENV")
        if not raw:
            return {}
        try:
    ...
    def _parse_agent_args() -> list[str]:
        """Extra CLI args from the runner (AIRIS_BENCH_AGENT_ARGS = JSON array)."""
        raw = _env("AIRIS_BENCH_AGENT_ARGS")
        if not raw:
            return []
        try:
            parsed = json.loads(raw)
    ...
    async def run(
        ...
        # markdown bullet, as in pytorch-model-recovery). Without this, airis parses
        ...
        # Exec env for the airis run. Direct-auth (no-gateway) mode contributes the
    ...
    def _sum_main(self, path: Path, acc: "_Usage") -> None:
        """Sum assistant `message_end` usage from airis's stdout JSONL.
