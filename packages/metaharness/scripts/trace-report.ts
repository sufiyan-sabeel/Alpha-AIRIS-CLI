 ...
 *
 * Auth: provider API keys resolve through airis's auth storage
 * (~/.airis/agent/agent.db: stored key, OAuth, or env var fallback).
 */

import { parseArgs } from "node:util";
import { type Api, AuthStorage, completeSimple, type Model, SqliteAuthCredentialStore } from "@airis/airis-ai";
...
async function openModel(modelSpec: string, storage: AuthStorage): Promise<OpenedModel> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (run \`airis login\` or set the provider env var)`);
	}
