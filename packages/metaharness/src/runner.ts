...
/**
 * Harbor benchmark runner for the local `airis` build.
 *
 * Orchestrates Harbor (`harbor run`) against any Harbor dataset (default
 * terminal-bench-2) using a custom agent (`agent/airis_local.py`) that installs
...
const AGENT_IMPORT_PATH = "airis_local:OmpLocal";

/** Container-side mount points for `--install source` (must match airis_local.py defaults). */
const SOURCE_SRC_MOUNT = "/opt/airis/src";
const SOURCE_BIN_MOUNT = "/opt/airis/bin";
...
/**
 * Resolver injected into Apple Container runs (AIRIS_BENCH_CONTAINER_DNS overrides).
 ...
 */
const CONTAINER_DNS = process.env.AIRIS_BENCH_CONTAINER_DNS || "1.1.1.1";
...
export interface Config {
	...
	/** Extra args forwarded verbatim to the in-container airis CLI invocation (repeatable). */
	agentArgs: string[];
...
const HELP = `metaharness runner (local airis)
...
Model / agent:
  -m, --model <provider/model>   Model (repeatable). Default anthropic/claude-sonnet-4-6
      --agent <name>             airis (default) | oracle | nop | any harbor agent
      --install <source|local|published> airis install mode (default: source).
      ...
      --version <v>              airis version for published install (default: latest)
      --thinking <level>         off|minimal|low|medium|high|xhigh|max

      --tarball <path>           Reuse a prebuilt airis tarball (implies --install local, --no-build)
      ...
      --env <KEY[=VALUE]>        Forward env into airis container (repeatable).
...
Gateway (auth, no keys in container):
      ...
      --web-search               Enable airis web_search (off by default; can't auth via gateway)
 ...
 * `agent/airis.txt` JSONL. Only bytes appended since the previous call are read
...
function parseTrial(dir: string, name: string): Trial | null {
	...
	if (!fs.existsSync(resultPath)) {
		...
		// Realtime cost from the live agent airis.txt log, parsed incrementally.
		const probe = probeTrialCost(path.join(dir, "agent", "airis.txt"));
	...
	costProbes.delete(path.join(dir, "agent", "airis.txt"));
...
function writeReport(st: RenderState, benchDir: string, exitCode: number): string {
	...
	const successPct = tot.done > 0 ? (tot.pass / tot.done) * 100 : 0;
	const lines: string[] = [];
	const isOmp = st.cfg.agent === "airis";
	const argsLabel = agentArgsLabel(st.cfg);
...
function buildTarball(benchDir: string): string {
	process.stdout.write(dim("packing local airis (bun pm pack)…\n"));
...
/** Linux deps tree + mount plan for running airis straight from the mounted repo. */
...
function writeComposeOverlay(benchDir: string, cfg: Config, source: SourceMount | null): string | null {
	...
	const file = path.join(benchDir, "airis-compose-overlay.yaml");
...
function writeModelsYaml(benchDir: string, cfg: Config): string {
	...
	for (const p of providers) {
		...
		lines.push("    transport: airis-native");
...
function buildHarborArgs(
	...
	if (cfg.agent === "airis") {
		// Config + secrets travel via env (AIRIS_BENCH_*); the agent reads os.environ.
...
/**
 * Env vars injected into the in-container airis run: every host `PI_*` knob (minus
...
export function buildHarborEnv(
	...
	// Drop any stale AIRIS_BENCH_FORWARD_ENV inherited from the caller's shell before
	// the agent-type early return, so it never leaks (incl. into the dry-run dump).
	delete env.AIRIS_BENCH_FORWARD_ENV;
	if (cfg.agent !== "airis") return env;
	const prepend = (k: string, v: string): void => {
		env[k] = env[k] ? `${v}:${env[k]}` : v;
	};
	prepend("PYTHONPATH", AGENT_DIR);
	env.AIRIS_BENCH_INSTALL = cfg.install;
	env.AIRIS_BENCH_VERSION = cfg.version ?? version;
	if (tarball) env.AIRIS_BENCH_TARBALL = tarball;
	if (source) {
		env.AIRIS_BENCH_SOURCE_DIR = SOURCE_SRC_MOUNT;
		env.AIRIS_BENCH_SOURCE_BUN = `${SOURCE_BIN_MOUNT}/bun`;
		env.AIRIS_BENCH_SOURCE_ARCH = source.arch;
	}
	if (cfg.binaryArm64) env.AIRIS_BENCH_BINARY_ARM64 = cfg.binaryArm64;
	if (cfg.binaryX64) env.AIRIS_BENCH_BINARY_X64 = cfg.binaryX64;
	if (cfg.thinking) env.AIRIS_BENCH_THINKING = cfg.thinking;
	if (cfg.agentArgs.length > 0) env.AIRIS_BENCH_AGENT_ARGS = JSON.stringify(cfg.agentArgs);
	if (cfg.webSearch) env.AIRIS_BENCH_WEB_SEARCH = "1";
	env.AIRIS_BENCH_GATEWAY = cfg.gateway ? "1" : "0";
	if (cfg.gateway) {
		env.AIRIS_BENCH_MODELS_YAML = modelsYaml;
		env.AIRIS_BENCH_GATEWAY_URL = cfg.gatewayUrl;
		env.AIRIS_BENCH_GATEWAY_TOKEN = cfg.gatewayToken;
		env.AIRIS_BENCH_GATEWAY_PROVIDERS = deriveProviders(cfg).join(",");
	}
	if (cfg.envType === "apple-container") env.AIRIS_BENCH_CONTAINER_DNS = CONTAINER_DNS;
	const forward = collectForwardEnv(cfg);
	if (Object.keys(forward).length > 0) env.AIRIS_BENCH_FORWARD_ENV = JSON.stringify(forward);
	return env;
}
...
async function runBenchmark(cfg: Config): Promise<BenchmarkRun> {
	...
	if (cfg.agent === "airis" && cfg.envType === "docker" && !which("docker")) {
	...
	if (cfg.agent === "airis" && cfg.install === "local" && !cfg.binaryArm64 && !cfg.binaryX64) {
		if (tarball) {
	...
	if (cfg.agent === "airis" && cfg.install === "source" && !cfg.binaryArm64 && !cfg.binaryX64) {
		source = prepareSourceDeps(cfg);
	}

	// models.yml (gateway)
	let modelsYaml = "";
	if (cfg.agent === "airis" && cfg.gateway) {
		modelsYaml = writeModelsYaml(benchDir, cfg);
		if (!gatewayHealthOk(cfg.gatewayUrl)) {
			process.stderr.write(
				yellow(
					`warning: gateway ${cfg.gatewayUrl} health check failed (continuing). Is the pm2 'airis-auth-gateway' running?\n`,
				),
			);
		}
	}
	...
	if (cfg.dryRun) {
		...
		process.stdout.write(bold("airis env:\n"));
		for (const key in harborEnv) {
			if (key === "AIRIS_BENCH_FORWARD_ENV") continue;
			if (key.startsWith("AIRIS_BENCH_") || key === "PYTHONPATH") process.stdout.write(`  ${key}=${harborEnv[key]}\n`);
		}
		if (harborEnv.AIRIS_BENCH_FORWARD_ENV) {
			const parsedForwardEnv: unknown = JSON.parse(harborEnv.AIRIS_BENCH_FORWARD_ENV);
			if (parsedForwardEnv !== null && typeof parsedForwardEnv === "object" && !Array.isArray(parsedForwardEnv)) {
				...
				process.stdout.write(`  AIRIS_BENCH_FORWARD_ENV=${keys.join(",")} (values hidden)\n`);
			}
		}
		process.stdout.write(`\njob dir: ${jobDir}\nbench dir: ${benchDir}\n`);
